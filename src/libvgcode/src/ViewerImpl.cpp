...
void ViewerImpl::TextureData::set_positions(const std::vector<Vec3> &positions)
{
    ...
    for (size_t i = 0; i < m_count; ++i)
    {
        ...
        {
            glsafe(glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB32F, static_cast<GLsizei>(w), static_cast<GLsizei>(h), 0,
                                GL_RGB, GL_FLOAT, &positions[offset]));
        ...
        else
        {
            ...
            glsafe(glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, static_cast<GLsizei>(w), static_cast<GLsizei>(h - 1), GL_RGB,
                                   GL_FLOAT, &positions[offset]));
            glsafe(glTexSubImage2D(GL_TEXTURE_2D, 0, 0, static_cast<GLsizei>(h - 1),
                                   static_cast<GLsizei>(remaining % w), 1, GL_RGB, GL_FLOAT,
                                   &positions[offset + w * (h - 1)]));
...
void ViewerImpl::TextureData::set_heights_widths_angles(const std::vector<Vec3> &heights_widths_angles)
{
    ...
    for (size_t i = 0; i < m_count; ++i)
    {
        ...
        {
            glsafe(glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB32F, static_cast<GLsizei>(w), static_cast<GLsizei>(h), 0,
                                GL_RGB, GL_FLOAT, &heights_widths_angles[offset]));
        ...
        else
        {
            ...
            glsafe(glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB32F, static_cast<GLsizei>(w), static_cast<GLsizei>(h), 0,
                                GL_RGB, GL_FLOAT, nullptr));
            glsafe(glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, static_cast<GLsizei>(w), static_cast<GLsizei>(h - 1), GL_RGB,
                                   GL_FLOAT, &heights_widths_angles[offset]));
            glsafe(glTexSubImage2D(GL_TEXTURE_2D, 0, 0, static_cast<GLsizei>(h - 1),
                                   static_cast<GLsizei>(remaining % w), 1, GL_RGB, GL_FLOAT,
                                   &heights_widths_angles[offset + w * (h - 1)]));
...
void ViewerImpl::TextureData::set_colors(const std::vector<float> &colors)
{
    ...
    for (size_t i = 0; i < m_count; ++i)
    {
        ...
        if (remaining >= tex_capacity)
        {
            glsafe(glTexImage2D(GL_TEXTURE_2D, 0, GL_R32F, static_cast<GLsizei>(w), static_cast<GLsizei>(h), 0, GL_RED,
                                GL_FLOAT, &colors[offset]));
        ...
        else
        {
            ...
            glsafe(glTexImage2D(GL_TEXTURE_2D, 0, GL_R32F, static_cast<GLsizei>(w), static_cast<GLsizei>(h), 0, GL_RED,
                                GL_FLOAT, nullptr));
            glsafe(glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, static_cast<GLsizei>(w), static_cast<GLsizei>(h - 1), GL_RED,
                                   GL_FLOAT, &colors[offset]));
            glsafe(glTexSubImage2D(GL_TEXTURE_2D, 0, 0, static_cast<GLsizei>(h - 1),
                                   static_cast<GLsizei>(remaining % w), 1, GL_RED, GL_FLOAT,
                                   &colors[offset + w * (h - 1)]));
...
void ViewerImpl::TextureData::set_enabled_segments(const std::vector<uint32_t> &enabled_segments)
{
    ...
    for (size_t i = 0; i < enabled_segments.size(); ++i)
    {
        ...
        if (i + 1 == enabled_segments.size() || new_tex)
        {
            ...
            {
                glsafe(glTexImage2D(GL_TEXTURE_2D, 0, GL_R32UI, static_cast<GLsizei>(w), static_cast<GLsizei>(h), 0,
                                    GL_RED_INTEGER, GL_UNSIGNED_INT, curr_segments.data()));
            ...
            else
            {
                ...
                if (h == 1)
                {
                    glsafe(glTexImage2D(GL_TEXTURE_2D, 0, GL_R32UI, static_cast<GLsizei>(w), 1, 0, GL_RED_INTEGER,
                ...
                else
                {
                    ...
                    glsafe(glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, static_cast<GLsizei>(w), static_cast<GLsizei>(h - 1),
                                           GL_RED_INTEGER, GL_UNSIGNED_INT, curr_segments.data()));
                    glsafe(glTexSubImage2D(GL_TEXTURE_2D, 0, 0, static_cast<GLsizei>(h - 1),
                                           static_cast<GLsizei>(curr_segments.size() % w), 1, GL_RED_INTEGER,
                                           GL_UNSIGNED_INT, &curr_segments[w * (h - 1)]));
...
void ViewerImpl::TextureData::set_enabled_options(const std::vector<uint32_t> &enabled_options)
{
    ...
    for (size_t i = 0; i < enabled_options.size(); ++i)
    {
        ...
        if (i + 1 == enabled_options.size() || new_tex)
        {
            ...
            {
                glsafe(glTexImage2D(GL_TEXTURE_2D, 0, GL_R32UI, static_cast<GLsizei>(w), static_cast<GLsizei>(h), 0,
                                    GL_RED_INTEGER, GL_UNSIGNED_INT, curr_options.data()));
            ...
            else
            {
                ...
                else
                {
                    ...
                    glsafe(glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, static_cast<GLsizei>(w), static_cast<GLsizei>(h - 1),
                                           GL_RED_INTEGER, GL_UNSIGNED_INT, curr_options.data()));
                    glsafe(glTexSubImage2D(GL_TEXTURE_2D, 0, 0, static_cast<GLsizei>(h - 1),
                                           static_cast<GLsizei>(curr_options.size() % w), 1, GL_RED_INTEGER,
                                           GL_UNSIGNED_INT, &curr_options[w * (h - 1)]));
...
void ViewerImpl::init(const std::string &opengl_context_version)
{
    ...
    glcheck();
    assert(m_uni_segments_view_matrix_id != -1 && m_uni_segments_projection_matrix_id != -1 &&
           m_uni_segments_camera_position_id != -1 && m_uni_segments_positions_tex_id != -1 &&
           m_uni_segments_height_width_angle_tex_id != -1 && m_uni_segments_colors_tex_id != -1 &&
           m_uni_segments_segment_index_tex_id != -1 && m_uni_segments_clipping_plane_id != -1);
...
// Fix for G-code viewer color map issue
...
static void extract_pos_and_or_hwa(const std::vector<PathVertex> &vertices, float travels_radius, float wipes_radius,
...
void ViewerImpl::load(GCodeInputData &&gcode_data)
{
    ...
    auto progress_callback = gcode_data.progress_callback;
    ...
    for (size_t i = 0; i < m_vertices.size(); ++i)
    {
        ...
        // Yield to UI thread every 50000 vertices with progress update (and always on first iteration)
        if (progress_callback && (i == 0 || i % 50000 == 0))
        {
            // Vertex processing is 0-40% of total rendering progress
            float vertex_progress = (float) i / (float) m_vertices.size() * 0.4f;
            progress_callback(vertex_progress);
        }
    }
    ...
    {
        progress_callback(0.4f); // Vertex processing complete (40%)
    }
    ...
    extract_pos_and_or_hwa(m_vertices, m_travels_radius, m_wipes_radius, m_valid_lines_bitset, &positions,
                           &heights_widths_angles, true);
    ...
    if (!positions.empty())
    {
        ...
        if (progress_callback)
        {
            progress_callback(0.50f);
        }
        // create and fill positions buffer (HEAVY BLOCKING OPERATION)
        ...
        if (progress_callback)
        {
            progress_callback(0.65f);
        }
        // create and fill height, width and angles buffer (HEAVY BLOCKING OPERATION)
...
void ViewerImpl::update_enabled_entities()
{
    ...
    glsafe(glBindBuffer(GL_TEXTURE_BUFFER, m_enabled_segments_buf_id));
    if (!enabled_segments.empty())
        glsafe(glBufferData(GL_TEXTURE_BUFFER, enabled_segments.size() * sizeof(uint32_t), enabled_segments.data(),
                            GL_STATIC_DRAW));
    else
    ...
    glsafe(glBindBuffer(GL_TEXTURE_BUFFER, m_enabled_options_buf_id));
    if (!enabled_options.empty())
        glsafe(glBufferData(GL_TEXTURE_BUFFER, enabled_options.size() * sizeof(uint32_t), enabled_options.data(),
                            GL_STATIC_DRAW));
    else
        glsafe(glBufferData(GL_TEXTURE_BUFFER, 0, nullptr, GL_STATIC_DRAW));
...
Color ViewerImpl::get_vertex_color(const PathVertex &v) const
{
    ...
    if ((v.is_wipe() && (m_settings.view_type != EViewType::Speed && m_settings.view_type != EViewType::ActualSpeed)) ||
        v.is_option())
    ...
    case EViewType::FanSpeed:
    {
        return v.is_travel() ? get_option_color(move_type_to_option(v.type))
                             : m_fan_speed_range.get_color_at(v.fan_speed);
    }
    case EViewType::Temperature:
    {
        return v.is_travel() ? get_option_color(move_type_to_option(v.type))
                             : m_temperature_range.get_color_at(v.temperature);
    }
    case EViewType::VolumetricFlowRate:
    {
        return v.is_travel() ? get_option_color(move_type_to_option(v.type))
                             : m_volumetric_rate_range.get_color_at(v.volumetric_rate());
    }
    ...
    {
        return v.is_travel() ? get_option_color(move_type_to_option(v.type))
                             : m_layer_time_range[0].get_color_at(
                                   m_layers.get_layer_time(m_settings.time_mode, static_cast<size_t>(v.layer_id)));
    }
    case EViewType::LayerTimeLogarithmic:
    {
        return v.is_travel() ? get_option_color(move_type_to_option(v.type))
                             : m_layer_time_range[1].get_color_at(
                                   m_layers.get_layer_time(m_settings.time_mode, static_cast<size_t>(v.layer_id)));
    }
    case EViewType::Tool:
    {
...
void ViewerImpl::update_view_full_range()
{
    ...
    else
    {
        ...
        if (first_it != last_it)
            m_view_range.set_full(std::distance(m_vertices.begin(), first_it),
                                  std::distance(m_vertices.begin(), last_it));
        else
            m_view_range.set_full(Range());
...
void ViewerImpl::update_heights_widths()
{
    ...
    heights_widths_angles.reserve(m_vertices.size());
    extract_pos_and_or_hwa(m_vertices, m_travels_radius, m_wipes_radius, m_valid_lines_bitset, nullptr,
                           &heights_widths_angles);
    m_texture_data.set_heights_widths_angles(heights_widths_angles);
#else
