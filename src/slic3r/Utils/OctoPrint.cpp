            ...
            if (rc == CURLUE_OK)
            {
                ...
                if (rc == CURLUE_OK)
                {
                    ...
                    BOOST_LOG_TRIVIAL(error)
                        << "OctoPrint substitute_host: failed to extract the URL after substitution";
            }
            else
                BOOST_LOG_TRIVIAL(error) << "OctoPrint substitute_host: failed to substitute host " << sub_addr
                                         << " in URL " << orig_addr;
        }
        else
...
const char *OctoPrint::get_name() const
{
    return "OctoPrint";
}
    ...
    http.on_error(
            [&](std::string body, std::string error, unsigned status)
            {
                BOOST_LOG_TRIVIAL(error) << boost::format(
                                                "%1%: Error getting version at %2% : %3%, HTTP %4%, body: `%5%`") %
...
bool OctoPrint::test(wxString &msg) const
{
    ...
    http.on_error(
            [&](std::string body, std::string error, unsigned status)
            {
                BOOST_LOG_TRIVIAL(error) << boost::format("%1%: Error getting version: %2%, HTTP %3%, body: `%4%`") %
...
bool OctoPrint::upload(PrintHostUpload upload_data, ProgressFn prorgess_fn, ErrorFn error_fn, InfoFn info_fn) const
{
    ...
    else if (GUI::get_app_config()->get_bool("allow_ip_resolve") && boost::algorithm::ends_with(host, ".local"))
    {
        Bonjour("octoprint")
            ...
            .on_resolve(
                [&ra = resolved_addr](const std::vector<BonjourReply> &replies)
                {
    ...
    if (resolved_addr.empty())
    {
        ...
        BOOST_LOG_TRIVIAL(error) << "preFlight failed to resolve hostname " << m_host
    ...
    else if (resolved_addr.size() == 1)
    {
        ...
        return upload_inner_with_resolved_ip(std::move(upload_data), prorgess_fn, error_fn, info_fn,
                                             resolved_addr.front());
    }
    else if (resolved_addr.size() == 2 && resolved_addr[0].is_v4() != resolved_addr[1].is_v4())
    {
        ...
        if (!upload_inner_with_resolved_ip(
                std::move(upload_data), prorgess_fn, [&msg = error_message, resolved_addr](wxString error)
                { msg = GUI::format_wxstr("%1%: %2%", resolved_addr.front(), error); }, info_fn,
                resolved_addr.front()) &&
            !upload_inner_with_resolved_ip(
                std::move(upload_data), prorgess_fn, [&msg = error_message, resolved_addr](wxString error)
                { msg += GUI::format_wxstr("\n%1%: %2%", resolved_addr.back(), error); }, info_fn,
                resolved_addr.back()))
        {
...
#ifdef WIN32
bool OctoPrint::upload_inner_with_resolved_ip(PrintHostUpload upload_data, ProgressFn prorgess_fn, ErrorFn error_fn,
                                              InfoFn info_fn, const boost::asio::ip::address &resolved_addr) const
{
    ...
    BOOST_LOG_TRIVIAL(info) << boost::format("%1%: Uploading file %2% at %3%, filename: %4%, path: %5%, print: %6%") %
                                   ...
                                   (upload_data.post_action == PrintHostPostUploadAction::StartPrint ? "true"
                                                                                                     : "false");
    ...
    http.form_add("print", upload_data.post_action == PrintHostPostUploadAction::StartPrint ? "true" : "false")
        ...
        .on_error(
            ...
            {
                BOOST_LOG_TRIVIAL(error) << boost::format(
                                                "%1%: Error uploading file to %2%: %3%, HTTP %4%, body: `%5%`") %
                                                name % url % error % status % body;
...
bool OctoPrint::upload_inner_with_host(PrintHostUpload upload_data, ProgressFn prorgess_fn, ErrorFn error_fn,
                                       InfoFn info_fn) const
{
    ...
    http.form_add("print", upload_data.post_action == PrintHostPostUploadAction::StartPrint ? "true" : "false")
        ...
        .on_error(
            [&](std::string body, std::string error, unsigned status)
            {
                BOOST_LOG_TRIVIAL(error) << boost::format("%1%: Error uploading file: %2%, HTTP %3%, body: `%4%`") %
                                                name % error % status % body;
                error_fn(format_error(body, error, status));
                res = false;
            })
...
bool LocalLink::get_storage(wxArrayString &storage_path, wxArrayString &storage_name) const
{
    ...
    http.on_error(
            [&](std::string body, std::string error, unsigned status)
            {
                BOOST_LOG_TRIVIAL(error) << boost::format("%1%: Error getting storage: %2%, HTTP %3%, body: `%4%`") %
                                                name % error % status % body;
                ...
                try
                {
                    ...
                    for (const auto &section : ptree.front().second)
                    {
                        ...
                        if (path && (!available || *available))
                        {
                            ...
                            si.free_space = space ? std::stoll(*space)
                                                  : 1; // If free_space is missing, assume there is free space.
    ...
    {
        if (!storage.empty())
        { // otherwise error_msg is already filled
            error_msg = L"\n\n" + _L("Storages found") + L": \n";
            for (const auto &si : storage)
            {
                                              ...
                                              InfoFn info_fn, const boost::asio::ip::address &resolved_addr) const
{
    ...
    BOOST_LOG_TRIVIAL(info)
        << boost::format("%1%: Uploading file %2% at %3%, filename: %4%, path: %5%, print: %6%, method: %7%") % name %
    ...
    http.set_put_body(upload_data.source_path)
        ...
        .on_complete(
            [&](std::string body, unsigned status)
            {
                ...
                BOOST_LOG_TRIVIAL(debug) << boost::format("%1%: File uploaded: HTTP %2%: %3%") % name % status %
                                                widebody;
        ...
        .on_complete(
            [&](std::string body, unsigned status)
            {
                if (m_show_after_message)
                {
                    ...
                    std::string message = m_show_after_message ? (boost::format("%1%") % widebody).str()
                                                               : std::string();
                    if (status == 202)
