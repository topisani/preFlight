...
const char *Moonraker::get_name() const
{
    return "Moonraker";
}
        ...
        .on_complete(
            [&](std::string body, unsigned)
            {
                ...
                try
                {
                    ...
                    BOOST_LOG_TRIVIAL(info) << boost::format("%1%: Got version: %2%") % name %
                                                   ptree.front().second.get_optional<std::string>("moonraker_version");
                }
