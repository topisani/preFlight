...
struct CurlGlobalInit
{
    ...
    CurlGlobalInit()
    {
        ...
        if (!ssl_cafile || !fs::exists(fs::path(ssl_cafile)))
        {
            ...
            if (!bundle)
                message = _u8L("Could not detect system SSL certificate store. "
                               "preFlight will be unable to establish secure "
                               "network connections.");
            else
                message = Slic3r::GUI::format(_L("preFlight detected system SSL certificate store in: %1%"), bundle);

            message += "\n" + Slic3r::GUI::format(_L("To specify the system certificate store manually, please "
        ...
        {
            message += _u8L("CURL init has failed. preFlight will be unable to establish "
...
void Http::priv::form_add_file(const char *name, const fs::path &path, const char *filename)
{
    ...
    if (filename != nullptr)
    {
        ::curl_formadd(&form, &form_end, CURLFORM_COPYNAME, name, CURLFORM_FILENAME, filename, CURLFORM_CONTENTTYPE,
                       "application/octet-stream", CURLFORM_STREAM, static_cast<void *>(&stream),
                       CURLFORM_CONTENTSLENGTH, static_cast<long>(size), CURLFORM_END);
    }
}
...
{
    return (boost::format("%1%:\n%2%\n[Error %3%]") % ::curl_easy_strerror(curlcode) % error_buffer.c_str() % curlcode)
        .str();
}
...
void Http::priv::http_perform(const HttpRetryOpt &retry_opts)
{
    ...
    std::uniform_int_distribution<std::chrono::milliseconds::rep> randomized_delay(
        retry_opts.initial_delay.count(), (retry_opts.initial_delay.count() * 3) / 2);
    ...
    do
    {
        ...
        {
            num_retries++;
            BOOST_LOG_TRIVIAL(error) << "HTTP Transient error (code=" << res << ", http_status=" << http_status
...
Http &Http::form_add(const std::string &name, const std::string &contents)
{
    if (p)
    {
        ::curl_formadd(&p->form, &p->form_end, CURLFORM_COPYNAME, name.c_str(), CURLFORM_COPYCONTENTS, contents.c_str(),
                       CURLFORM_END);
    }
...
Http &Http::form_add_file(const std::string &name, const fs::path &path)
{
    if (p)
    {
        p->form_add_file(name.c_str(), path.c_str(), nullptr);
    }
    return *this;
}
...
Http &Http::form_add_file(const std::string &name, const fs::path &path, const std::string &filename)
{
    if (p)
    {
        p->form_add_file(name.c_str(), path.c_str(), filename.c_str());
    }
    return *this;
}
...
// This option is only supported for Schannel (the native Windows SSL library). No-op on other platforms.
...
Http &Http::set_referer(const std::string &referer)
{
    if (p)
    {
        ::curl_easy_setopt(p->curl, CURLOPT_REFERER, referer.c_str());
    }
    return *this;
}
...
Http::Ptr Http::perform(const HttpRetryOpt &retry_opts)
{
    ...
    if (self->p)
    {
        auto io_thread = std::thread([self, &retry_opts]() { self->p->http_perform(retry_opts); });
...
{
    os << "Http::Progress("
       << "dltotal = " << progress.dltotal << ", dlnow = " << progress.dlnow << ", ultotal = " << progress.ultotal
