...
inline wxFont get_default_font(const wxWindow *window)
{
    return get_default_font_for_dpi(window, get_dpi_for_window(window));
}
...
public:
    DPIAware(wxWindow *parent, wxWindowID id, const wxString &title, const wxPoint &pos = wxDefaultPosition,
             ...
             const wxString &name = wxFrameNameStr, const int font_point_size = -1)
...
#ifndef __WXOSX__
#if wxVERSION_EQUAL_OR_GREATER_THAN(3, 1, 3)
        this->Bind(wxEVT_DPI_CHANGED,
                   [this](wxDPIChangedEvent &evt)
                   {
...
#else
        this->Bind(EVT_DPI_CHANGED_SLICER,
                   [this](const DpiChangedEvent &evt)
                   {
...
private:
    ...
    struct EventStorageFun : EventStorageBase
    {
        ...
        EventStorageFun(wxEvtHandler *emitter, const EvTag &tag, Fun fun)
            : emitter(emitter), tag(tag), fun(std::move(fun))
        {
...
public:
    CheckboxFileDialog(wxWindow *parent, const wxString &checkbox_label, bool checkbox_value,
                       const wxString &message = wxFileSelectorPromptStr, const wxString &default_dir = wxEmptyString,
                       const wxString &default_file = wxEmptyString,
                       const wxString &wildcard = wxFileSelectorDefaultWildcardStr, long style = wxFD_DEFAULT_STYLE,
