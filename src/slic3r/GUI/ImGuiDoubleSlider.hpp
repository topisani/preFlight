...
{
public:
    ImGuiControl(int lowerValue, int higherValue, int minValue, int maxValue,
                 ImGuiSliderFlags flags = ImGuiSliderFlags_None, std::string name = "d_slider",
...
private:
    ...
    std::string get_label_on_move(int pos) const
    {
        return m_cb_get_label_on_move ? m_cb_get_label_on_move(pos) : get_label(pos);
    }
    ...
    bool draw_slider(int *higher_pos, int *lower_pos, std::string &higher_label, std::string &lower_label,
...
public:
    ...
    {
        m_ctrl = ImGuiControl(lowerPos, higherPos, minPos, maxPos, is_horizontal ? 0 : ImGuiSliderFlags_Vertical, name,
                              !is_horizontal);
    ...
    ValType GetValueAtPos(int pos)
    {
        return (m_values.empty() || pos < 0 || pos >= static_cast<int>(m_values.size())) ? static_cast<ValType>(0)
                                                                                         : m_values[pos];
    }
    ...
    virtual void Render(const int canvas_width, const int canvas_height, float extra_scale = 1.f,
                        float offset = 0.f) = 0;
...
protected:
    ...
    virtual std::string get_label(int pos) const
    {
        ...
        if (pos >= int(m_values.size()))
            return "ErrVal";
        return to_string_with_precision(
            static_cast<ValType>(m_alternate_values.empty() ? m_values[pos] : m_alternate_values[pos]));
    }
