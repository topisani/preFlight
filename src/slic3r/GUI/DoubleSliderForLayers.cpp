...
DSForLayers::DSForLayers(int lowerValue, int higherValue, int minValue, int maxValue, bool allow_editing)
    ...
    if (!m_values.empty())
        for (const TickCode &tick : m_ticks.ticks)
        {
            if (tick.tick > val_size)
                break;
            values.emplace_back(
                CustomGCode::Item{m_values[tick.tick], tick.type, tick.extruder, tick.color, tick.extra});
        }
...
void DSForLayers::draw_ticks(const ImRect &slideable_region)
{
    ...
    const ImU32 tick_clr = ImGui::ColorConvertFloat4ToU32(m_show_ruler ? ImGuiPureWrap::COL_ORANGE_LIGHT
                                                                       : ImGuiPureWrap::COL_ORANGE_DARK);
    const ImU32 tick_hovered_clr = ImGui::ColorConvertFloat4ToU32(m_show_ruler ? ImGuiPureWrap::COL_ORANGE_DARK
                                                                               : ImGuiPureWrap::COL_WINDOW_BACKGROUND);
    ...
    while (tick_it != m_ticks.ticks.end())
    {
        ...
        ImRect tick_hover_box = ImRect(x_center - tick_border.x, tick_pos - tick_border.y, x_center + tick_border.x,
    ...
    while (tick_it != m_ticks.ticks.end())
    {
        ...
        {
            // delete tick
            if (render_button(ImGui::RemoveTick, ImGui::RemoveTickHovered, btn_label, icon_pos, fiActionIcon,
                              tick_it->tick))
            {
                ...
                break;
                 ...
                 dmRegular) // if we have non-regular draw mode, all ticks should be marked with error icon
            activate_this_tick = render_button(ImGui::ErrorTick, ImGui::ErrorTickHovered, btn_label, icon_pos, fiTick,
                                               tick_it->tick);
        else if (tick_it->type == ColorChange || tick_it->type == ToolChange)
        {
            if (m_ticks.is_conflict_tick(*tick_it, m_mode, m_values[tick_it->tick]))
                activate_this_tick = render_button(ImGui::ErrorTick, ImGui::ErrorTickHovered, btn_label, icon_pos,
                                                   fiTick, tick_it->tick);
        }
        else if (tick_it->type == CustomGCode::PausePrint)
            activate_this_tick = render_button(ImGui::PausePrint, ImGui::PausePrintHovered, btn_label, icon_pos, fiTick,
                                               tick_it->tick);
        else
            activate_this_tick = render_button(ImGui::EditGCode, ImGui::EditGCodeHovered, btn_label, icon_pos, fiTick,
                                               tick_it->tick);
...
void DSForLayers::draw_ruler(const ImRect &slideable_region)
{
    ...
    // Guard against degenerate slider (division by zero or single layer)
    if (m_ctrl.GetMaxPos() <= m_ctrl.GetMinPos())
        return;

    const double step = double(slideable_region.GetHeight()) / (m_ctrl.GetMaxPos() - m_ctrl.GetMinPos());

    // Guard against slider being too narrow for meaningful ruler
    ...
    // Layer 0 offset: slider position N shows m_values[N-1], so ruler marks
    // found at m_values index 'tick' should be drawn at position 'tick + 1'
    auto get_ruler_draw_pos = [this, slideable_region](int tick) -> float
    {
        return m_ctrl.GetPositionInRect(tick + 1, slideable_region);
    };
    ...
    auto draw_text = [max_val, x_center, label_height, long_outer_x, this](const int tick, const float tick_pos)
    {
        ...
        std::string label = (tick >= 0 && tick < (int) m_values.size())
                                ? format(max_val > 100.0 ? "%1$.1f" : "%1$.2f", m_values[tick])
                                : "";
    ...
    auto draw_short_ticks = [this, short_outer_x, draw_tick, get_ruler_draw_pos](double &current_tick, int max_tick)
    {
        if (m_ruler.short_step <= 0.0)
            return;
        while (current_tick < max_tick)
        {
            float pos = get_ruler_draw_pos(lround(current_tick));
    ...
    if (m_ruler.long_step < 0)
    {
        ...
        {
            float last_pos = get_ruler_draw_pos(m_ctrl.GetMaxPos() - 1);
            while (tick <= m_ctrl.GetMaxPos() && sequence < m_ruler.count())
            {
                ...
                float pos = get_ruler_draw_pos(tick);
                draw_tick(pos, long_outer_x);
                if (prev_y_pos < 0 || pos == last_pos ||
        ...
        // very short object or some non-trivial ruler with non-regular step
        else
        {
            if (step <
    ...
    else
    {
        ...
        if (m_ruler.count() == 1)
            // Use values_size - 1 (last valid tick index), not GetMaxPos() (slider position)
            // Tick indices are 0 to values_size-1, mapping to m_values[]
            last_positions.emplace_back(values_size - 1);
        else
        {
            ...
            double value = 0.0;
            size_t sequence = 0;
            int max_iterations = values_size + 1;
            int iterations = 0;
            ...
            while (tick <= m_ctrl.GetMaxPos())
            {
                ...
                else if (tick == prev_tick)
                {
                    // Force progress if tick didn't advance
                    tick++;
                }
            }
        }

        float last_pos = get_ruler_draw_pos(last_positions[sequence]);
        int max_iterations2 = values_size + 1;
        int iterations2 = 0;

        while (tick <= m_ctrl.GetMaxPos())
        {
            if (++iterations2 > max_iterations2)
                break; // Safety guard against infinite loop
            ...
            if (sequence < m_ruler.count() && value == m_ruler.max_values[sequence])
            {
                ...
                if (sequence < m_ruler.count())
                    last_pos = get_ruler_draw_pos(last_positions[sequence]);
            }
            else if (tick == prev_tick)
            {
                // Force progress if tick didn't advance
    ...
    if (m_pos_on_move > 0)
    {
        ...
        ImRect move_line = ImRect(x_center + 0.75f * inner_x, line_pos - tick_width, x_center + 1.5f * long_outer_x,
                                  line_pos);
        ImGui::RenderFrame(move_line.Min, move_line.Max,
                           ImGui::ColorConvertFloat4ToU32(ImGuiPureWrap::COL_ORANGE_LIGHT), false);
        m_pos_on_move = -1;
    }
}
...
void DSForLayers::draw_colored_band(const ImRect &groove, const ImRect &slideable_region)
{
    ...
    ImRect blank_rect = ImRect(groove.GetCenter().x - blank_width, groove.Min.y, groove.GetCenter().x + blank_width,
                               groove.Max.y);
                ...
                (m_mode == MultiAsSingle && (tick_it->type == ToolChange || tick_it->type == ColorChange)))
            {
                ...
                if (!clr_str.empty())
                {
                    ...
                    else
                    {
                        ...
                        if (ImGui::IsMouseHoveringRect(band_rect.Min, band_rect.Max) && g.IO.MouseClicked[1] &&
                            !m_ctrl.IsRClickOnThumb())
                        {
...
bool DSForLayers::render_multi_extruders_menu(bool switch_current_code /* = false*/)
{
    ...
    if (extruders_cnt > 1)
    {
        ...
        {
            const std::string menu_name = switch_current_code ? _u8L("Switch code to Change extruder")
            ...
            if (ImGuiPureWrap::begin_menu(menu_name.c_str()))
            {
                ...
                for (int i = 1; i <= extruders_cnt; i++)
                {
                    ...
                    if (ImGuiPureWrap::menu_item_with_icon(item_name.c_str(), "", ImVec2(14, 14) * m_scale, icon_clr,
                                                           false, !is_active_extruder))
                    {
        ...
        if (ImGuiPureWrap::begin_menu(menu_name.c_str()))
        {
            ...
            for (int i = 1; i <= extruders_cnt; i++)
            {
                const bool is_used_extruder =
                    used_extruders_for_tick.empty()
                        ? true
                        : // #ys_FIXME till used_extruders_for_tick doesn't filled correct for mmMultiExtruder
...
void DSForLayers::render_cog_menu()
{
    ...
    if (ImGui::BeginPopup("cog_menu_popup"))
    {
        ...
        if (ImGuiPureWrap::menu_item_with_icon(_u8L("Show estimated print time on hover").c_str(), "", icon_sz, 0,
        ...
        if (ImGuiPureWrap::menu_item_with_icon(_u8L("Sequential slider applied only to top layer").c_str(), "", icon_sz,
                                               0, m_seq_top_layer_only))
        {
        ...
        if (can_edit())
        {
            if (ImGuiPureWrap::menu_item_with_icon(_u8L("Use default colors").c_str(), "", icon_sz, 0,
                                                   m_ticks.used_default_colors()))
            {
...
bool DSForLayers::render_button(const wchar_t btn_icon, const wchar_t btn_icon_hovered, const std::string &label_id,
                                const ImVec2 &pos, FocusedItem focus, int tick /*= -1*/)
{
    ...
    int windows_flag = ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoMove |
    ...
    const bool ret = m_imgui->image_button(g.HoveredWindow == g.CurrentWindow ? btn_icon_hovered : btn_icon, tooltip,
                                           false);
...
bool DSForLayers::render_jump_to_window(const ImVec2 &pos, double *active_value, double min_z, double max_z)
{
    ...
    ImGui::InputDouble("##jump_to", active_value, 0.0, 0.0, "%.2f",
                         ...
                         float offset /*= 0.f*/)
{
    ...
    else if (m_ctrl.IsLClickOnThumb() && ImGui::GetIO().KeyShift && can_edit() &&
    ...
    if (render_button(is_one_layer ? ImGui::Lock : ImGui::Unlock,
                      is_one_layer ? ImGui::LockHovered : ImGui::UnlockHovered, "one_layer", btn_pos, fiOneLayerIcon))
        ChangeOneLayerLock();
...
std::string DSForLayers::get_label(int pos, LabelType label_type, const std::string &fmt /* = "%1$.2f"*/) const
{
    ...
    if (m_values.empty())
        return format("%1%", pos);
    // With Layer 0 added, position can be up to m_values.size() (not size()-1)
    // Since we use z_index = max(0, value-1), check that z_index is valid
    size_t z_index = std::max(0, static_cast<int>(value) - 1);
    if (z_index >= m_values.size())
        return "ErrVal";
    ...
    // Position 0 and 1 both show first layer height, position N shows layer N height
    // Layer 0 = beginning of layer 1, Layer 1 = end of layer 1, both at same z
    // z_index already calculated above for bounds check
    std::string str = format(fmt, m_values[z_index]);
...
std::string DSForLayers::get_tooltip(int tick /*=-1*/)
{
    ...
    if (tick_code_it == m_ticks.ticks.end() && m_focus == fiActionIcon) // tick doesn't exist
    {
        ...
        tooltip += (m_mode == MultiAsSingle    ? _u8L("Add extruder change - Left click")
                    : m_mode == SingleExtruder ? _u8L("Add color change - Left click for predefined color or "
    ...
    if (tick_code_it != m_ticks.ticks.end()) // tick exists
    {
        ...
        tooltip += tick_code_it->type == ColorChange ? (m_mode == SingleExtruder && tick_code_it->extruder == 1
                                                            ? format(_u8L("Color change (\"%1%\")"), gcode(ColorChange))
                                                            : format(_u8L("Color change (\"%1%\") for Extruder %2%"),
                                                                     gcode(ColorChange), tick_code_it->extruder))
                   : tick_code_it->type == CustomGCode::PausePrint
                       ? format(_u8L("Pause print (\"%1%\")"), gcode(CustomGCode::PausePrint))
                   : tick_code_it->type == Template ? format(_u8L("Custom template (\"%1%\")"), gcode(Template))
