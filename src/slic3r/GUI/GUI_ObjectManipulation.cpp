...
#else
    temp = new choice_ctrl(parent, wxID_ANY, wxString(""), wxDefaultPosition, size, 0, nullptr,
...
ObjectManipulation::ObjectManipulation(wxWindow *parent) : OG_Settings(parent, true)
{
    ...
    m_uniform_scale = !wxGetApp().app_config->has("uniform_scale_lock") ||
                      wxGetApp().app_config->get_bool("uniform_scale_lock");
    ...
    if (is_windows10())
        m_fix_by_winsdk_bitmap->Bind(wxEVT_CONTEXT_MENU,
                                     [this](wxCommandEvent &e)
                                     {
                                         // if object/sub-object has no errors
                                         if (m_fix_by_winsdk_bitmap->GetBitmap().GetRefData() ==
                                             wxNullBitmap.GetRefData())
                                             return;
    ...
    m_word_local_combo->Bind(wxEVT_COMBOBOX,
                             ([this](wxCommandEvent &evt) { this->set_coordinates_type(evt.GetString()); }),
    ...
    m_lock_bnt->Bind(wxEVT_BUTTON,
                     [this](wxCommandEvent &event)
                     {
                         // Don't call event.Skip() - we handle the toggle ourselves to avoid race
                         // condition with update_if_dirty() being called on idle events
                         bool new_lock_state = !m_lock_bnt->IsLocked(); // Invert current state
                         m_lock_bnt->SetLock(new_lock_state);           // Manually toggle the button
                         wxGetApp().app_config->set("uniform_scale_lock", new_lock_state ? "1" : "0");
                         wxGetApp().app_config->save();
                         set_uniform_scaling(new_lock_state);
                     });
    ...
    if (wxOSX)
        set_font_and_background_style(size_Label, wxGetApp().normal_font());
    ...
    for (size_t axis_idx = 0; axis_idx < sizeof(axes); axis_idx++)
    {
        ...
        auto btn = new ScalableButton(parent, wxID_ANY, "mirroring_off", wxEmptyString, wxDefaultSize,
    ...
    m_drop_to_bed_button->Bind(
        ...
        [=](wxCommandEvent &e)
        {
            ...
            if (selection.is_single_volume_or_modifier())
            {
                ...
                {
                    Vec3d diff = volume->get_instance_transformation().get_matrix_no_offset().inverse() *
                                 (min_z * Vec3d::UnitZ());
                    if (is_local_coordinates())
            ...
            else if (selection.is_single_full_instance())
            {
                ...
                {
                    const GLVolume *volume = selection.get_first_volume();
                    const Vec3d diff = m_cache.position -
                                       volume->get_instance_transformation().get_matrix_no_offset().inverse() *
                                           (min_z * Vec3d::UnitZ());
    ...
    m_reset_skew_button->Bind(wxEVT_BUTTON,
                              [this](wxCommandEvent &e)
                              {
                                  GLCanvas3D *canvas = wxGetApp().plater()->canvas3D();
                                  Selection &selection = canvas->get_selection();
...
void ObjectManipulation::Show(const bool show)
{
    ...
    if (show)
    {
        ...
        bool show_world_local_combo = wxGetApp().get_mode() != comSimple &&
...
{
    return dynamic_cast<const wxStaticBoxSizer *>(m_og->sizer)
        ->GetStaticBox()
        ->IsShown(); //  m_og->get_grid_sizer()->IsShown(2);
}
...
void ObjectManipulation::Enable(const bool enable)
{
    ...
    for (wxWindow *win :
         std::initializer_list<wxWindow *>{m_reset_scale_button, m_reset_rotation_button, m_drop_to_bed_button,
                                           m_check_inch, m_lock_bnt, m_reset_skew_button})
        win->Enable(enable);
}
...
void ObjectManipulation::update_ui_from_settings()
{
    if (m_imperial_units != wxGetApp().app_config->get_bool("use_inches"))
    {
        ...
        auto update_unit_text = [](const wxString &new_unit_text, wxStaticText *widget)
        {
            ...
            if (wxOSX)
                set_font_and_background_style(widget, wxGetApp().normal_font());
        };
        ...
        for (int i = 0; i < 3; ++i)
        {
            auto update = [this, i](/*ManipulationEditorKey*/ int key_id, const Vec3d &new_value)
            {
                ...
                if (id >= 0)
                    m_editors[id]->set_value(new_text);
            };
...
void ObjectManipulation::update_settings_value(const Selection &selection)
{
    ...
    if (selection.is_single_full_instance())
    {
        ...
        else
        {
            ...
            m_new_rotation = Vec3d::Zero();
            m_new_size = selection.get_bounding_box_in_current_reference_system().first.size();
            m_new_scale = m_new_size.cwiseQuotient(selection.get_full_unscaled_instance_local_bounding_box().size()) *
                          100.0;
        }
    ...
    else if (obj_list->is_connectors_item_selected() || obj_list->multiple_selection() ||
             obj_list->is_selected(itInstanceRoot))
    {
        reset_settings_value();
...
void ObjectManipulation::update_if_dirty()
{
    ...
    {
        auto update =
            [this, i](Vec3d &cached, Vec3d &cached_rounded, ManipulationEditorKey key_id, const Vec3d &new_value)
        {
...
void ObjectManipulation::update_reset_buttons_visibility()
{
    ...
    if (selection.is_single_full_instance() || selection.is_single_volume_or_modifier())
    {
        ...
        const GLVolume *volume = selection.get_first_volume();
        const Geometry::Transformation trafo = selection.is_single_full_instance()
                                                   ? volume->get_instance_transformation()
                                                   : volume->get_volume_transformation();
    ...
    wxGetApp().CallAfter(
        [this, show_drop_to_bed, show_rotation, show_scale, show_mirror, show_skew]
        {
...
{
    if (const std::string &warning_icon_name = warning.warning_icon_name; !warning_icon_name.empty())
...
void ObjectManipulation::change_rotation_value(int axis, double value)
{
    ...
    selection.setup_cache();
    selection.rotate((M_PI / 180.0) * (transformation_type.absolute() ? rotation : rotation - m_cache.rotation),
                     transformation_type);
...
static const char axes[] = {'x', 'y', 'z'};
ManipulationEditor::ManipulationEditor(ObjectManipulation *parent, const std::string &opt_key, int axis)
    ...
    , m_axis(axis)
{
    ...
    this->GetTextCtrl()->Bind(wxEVT_SET_FOCUS,
                              [this, parent](wxFocusEvent &e)
                              {
