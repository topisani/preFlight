...
wxString double_to_string(double const value, const int max_precision /*= 4*/)
{
    ...
    wxString s = wxNumberFormatter::ToString(value, std::abs(value) < 0.0001 ? 10 : max_precision,
...
int Field::def_width_wider()
{
    return 16;
}
int Field::def_width_thinner()
{
    return 4;
}
...
bool Field::is_matched(const std::string &string, const std::string &pattern)
{
    std::regex regex_pattern(
        pattern, std::regex_constants::icase); // use ::icase to make the matching case insensitive like /i in perl
...
void Field::get_value_by_opt_type(wxString &str, const bool check_value /* = true*/)
{
    ...
    {
        if (m_opt.type == coFloatOrPercent && m_opt.opt_key == "first_layer_height" && !str.IsEmpty() &&
            str.Last() == '%')
        {
            // Workaroud to avoid of using of the % for first layer height
            // Fix for field selection issue
            wxString label = m_opt.full_label.empty() ? _(m_opt.label) : _(m_opt.full_label);
            show_error(m_parent, format_wxstr(_L("%s doesn't support percentage"), label));
        ...
        if ((m_opt.type == coFloatOrPercent || m_opt.type == coFloatsOrPercents) && !str.IsEmpty() && str.Last() != '%')
        {
            ...
            else if (((m_opt.sidetext.rfind("mm/s") != std::string::npos && std::abs(val) > m_opt.max) ||
                      (m_opt.sidetext.rfind("mm ") != std::string::npos && std::abs(val) > /*1*/ m_opt.max_literal)) &&
                     (m_value.empty() || into_u8(str) != boost::any_cast<std::string>(m_value)))
            {
                ...
                if ((!infill_anchors || val > 100) && dialog.ShowModal() == wxID_YES)
                {
                    ...
                    set_value(
                        stVal,
                        false); // it's no needed but can be helpful, when inputted value contained "," instead of "."
            }
        }
        ...
        if (m_opt.opt_key == "thumbnails")
        {
            ...
            if (errors != enum_bitmask<ThumbnailError>())
            {
                ...
                if (errors.has(ThumbnailError::InvalidVal))
                    error_str += format_wxstr(
                        _L("Invalid input format. Expected vector of dimensions in the following format: \"%1%\""),
                        "XxY/EXT, XxY/EXT, ...");
...
{
    if (!win ||
        (static_cast<T *>(win)->GetValue().empty() && type != coString && type != coStrings && type != coPoints))
        return false;
    return true;
}
...
void TextCtrl::BUILD()
{
    ...
    if (m_opt.readonly && temp->GetTextCtrl())
        temp->GetTextCtrl()->SetEditable(false);

    temp->SetToolTip(get_tooltip_text(text_value));

    if (style & wxTE_PROCESS_ENTER)
    {
        temp->Bind(wxEVT_TEXT_ENTER,
                   (
                       [this, temp](wxEvent &e)
                       {
...
void SpinCtrl::sys_color_changed()
{
    Field::sys_color_changed();
...
{
    // SpinInputBase::Enable() handles all color setting directly
...
void Choice::BUILD()
{
    ...
    {
        m_is_editable = true;
        temp = new choice_ctrl(m_parent, wxID_ANY, wxString(""), wxDefaultPosition, size, 0, nullptr,
                               wxTE_PROCESS_ENTER | DD_NO_CHECK_ICON);
    }
    else
    {
...
void Choice::set_value(const boost::any &value, bool change_event)
{
    ...
    case coStrings:
    {
        wxString text_value = m_opt.type == coInt ? wxString::Format(_T("%i"), int(boost::any_cast<int>(value)))
...
boost::any &ColourPicker::get_value()
{
    ...
    m_value = (colour == wxTransparentColour) ? std::string("")
                                              : encode_color(ColorRGB(colour.Red(), colour.Green(), colour.Blue()));
    return m_value;
}
...
void PointCtrl::BUILD()
{
    ...
    x_textctrl->Bind(wxEVT_TEXT_ENTER, ([this](wxCommandEvent e) { propagate_value(x_textctrl); }),
                     x_textctrl->GetId());
    y_textctrl->Bind(wxEVT_TEXT_ENTER, ([this](wxCommandEvent e) { propagate_value(y_textctrl); }),
                     y_textctrl->GetId());

    x_textctrl->Bind(wxEVT_KILL_FOCUS,
                     (
                         [this](wxEvent &e)
                         {
                             e.Skip();
                     ...
                     x_textctrl->GetId());
    y_textctrl->Bind(wxEVT_KILL_FOCUS,
                     (
                         [this](wxEvent &e)
                         {
                             e.Skip();
...
void SliderCtrl::BUILD()
{
    ...
    m_slider = new wxSlider(m_parent, wxID_ANY, def_val * m_scale, min * m_scale, max * m_scale, wxDefaultPosition,
                            size);
