...
struct Plater::priv
{
    ...
    wxPanel *view_panel{nullptr}; // Panel to hold view3D and preview
    ...
    static const std::regex pattern_preflight;
        ...
        if (dirty_state.is_dirty())
        {
            ...
            if (mainframe->can_save_as())
            {
                ...
                if (suggested_project_name.IsEmpty())
                {
                    ...
                    suggested_project_name = output_file.empty() ? _L("Untitled")
                                                                 : from_u8(output_file.stem().string());
                }
                ...
                if (act.empty())
                {
                    RichMessageDialog dialog(mainframe,
                                             reason + "\n" +
                                                 format_wxstr(_L("Do you want to save the changes to \"%1%\"?"),
                                                              suggested_project_name),
                                             wxString(SLIC3R_APP_NAME), wxYES_NO | wxCANCEL);
                    ...
                    if (res != wxID_CANCEL)
                        if (dialog.IsCheckBoxChecked())
                        {
                            ...
                            wxString msg =
                                _L("preFlight will remember your choice.") + "\n\n" +
                            ...
                            MessageDialog msg_dlg(mainframe, msg, _L("preFlight: Don't ask me again"),
    ...
    void show_legend(bool show)
    {
        if (current_panel == preview)
            preview->get_canvas3d()->show_legend(show);
    }
    ...
    void update_preview_moves_slider(std::optional<int> visible_range_min = std::nullopt,
                                     std::optional<int> visible_range_max = std::nullopt);
    ...
    void take_snapshot(const std::string &snapshot_name,
                       UndoRedo::SnapshotType snapshot_type = UndoRedo::SnapshotType::Action);
    ...
    bool replace_volume_with_stl(int object_idx, int volume_idx, const fs::path &new_path,
                                 const wxString &snapshot = "");
    ...
    void set_bed_shape(const Pointfs &shape, const double max_print_height, const std::string &custom_texture,
                       const std::string &custom_model, bool force_as_custom = false);
    ...
    void generate_thumbnail(ThumbnailData &data, unsigned int w, unsigned int h,
...
Plater::priv::priv(Plater *q, MainFrame *main_frame)
    ...
    , config(Slic3r::DynamicPrintConfig::new_from_defaults_keys(
          {"bed_shape", "bed_custom_texture", "bed_custom_model", "complete_objects", "duplicate_distance",
           "extruder_clearance_radius", "extruder_clearance_height", "skirts", "skirt_distance", "brim_width",
           ...
           "support_material_extruder", "support_material_interface_extruder", "support_material_contact_distance",
           "support_material_bottom_contact_distance", "raft_layers"}))
...
void Plater::priv::init()
{
    ...
    preview = new Preview(q, bed, &model, config, &background_process, &gcode_results,
    ...
    if (wxGetApp().is_editor())
    {
        ...
        this->q->Bind(EVT_FORCE_INVALIDATE_SLICE, [this](SimpleEvent &) { this->q->force_invalidate_slice(); });
    }
    ...
    if (wxGetApp().is_editor())
    {
        ...
        view3D_canvas->Bind(EVT_GLCANVAS_INCREASE_INSTANCES,
                            [this](Event<int> &evt)
                            {
                                if (evt.data == 1)
        ...
        view3D_canvas->Bind(EVT_GLCANVAS_ENABLE_ACTION_BUTTONS,
                            [this](Event<bool> &evt) { this->sidebar->enable_buttons(evt.data); });
    ...
    if (wxGetApp().is_editor())
    {
        ...
        preview->get_wxglcanvas()->Bind(EVT_GLCANVAS_COLLAPSE_SIDEBAR, [this](SimpleEvent &)
                                        { this->q->collapse_sidebar(!this->q->is_sidebar_collapsed()); });
    }
    ...
    // Drop target:
    main_frame->SetDropTarget(
        new PlaterDropTarget(*main_frame, *q)); // if my understanding is right, wxWindow takes the owenership
    q->Layout();
    ...
    this->q->Bind(EVT_HID_DEVICE_ATTACHED,
                  [this](HIDDeviceAttachedEvent &evt) { mouse3d_controller.device_attached(evt.data); });
    this->q->Bind(EVT_HID_DEVICE_DETACHED,
                  [this](HIDDeviceAttachedEvent &evt) { mouse3d_controller.device_detached(evt.data); });
#endif /* _WIN32 */
    ...
    if (wxGetApp().is_editor())
    {
        this->q->Bind(EVT_EJECT_DRIVE_NOTIFICAION_CLICKED,
            ...
            [this](RemovableDriveEjectEvent &evt)
            {
                if (evt.data.second)
                {
                    ...
                    notification_manager->push_notification(
                        ...
                        format(
                            _L("Successfully unmounted. The device %s(%s) can now be safely removed from the computer."),
        ...
        this->q->Bind(EVT_REMOVABLE_DRIVES_CHANGED,
                      [this](RemovableDrivesChangedEvent &)
                      {
                          q->show_action_buttons();
        ...
        this->q->Bind(
            EVT_REMOVABLE_DRIVE_ADDED,
            [this](wxCommandEvent &evt)
            {
                boost::system::error_code ec;
                if (!fs::exists(fs::path(evt.GetString().utf8_string()) / "preflight_printer_settings.ini", ec) || ec)
                    return;
                if (evt.GetInt() == 0)
                { // not at startup, show dialog
                    wxGetApp().open_wifi_config_dialog(false, evt.GetString());
                }
                else
                { // at startup, show only notification
                    notification_manager->push_notification(
                        NotificationType::WifiConfigFileDetected,
                        ...
                        ,
                        _u8L("Write Wi-Fi credentials."),
                        [evt /*, CONFIG_FILE_NAME*/](wxEvtHandler *evt_hndlr)
                        {
        ...
        this->q->Bind(EVT_VOLUME_ATTACHED,
                      [this](VolumeAttachedEvent &evt) { wxGetApp().removable_drive_manager()->volumes_changed(); });
        this->q->Bind(EVT_VOLUME_DETACHED,
                      [this](VolumeDetachedEvent &evt) { wxGetApp().removable_drive_manager()->volumes_changed(); });
#endif /* _WIN32 */
    }
    ...
    this->q->Bind(EVT_LOAD_MODEL_OTHER_INSTANCE,
                  [this](LoadFromOtherInstanceEvent &evt)
                  {
    ...
    if (wxGetApp().is_editor())
    {
        this->q->Bind(EVT_START_DOWNLOAD_OTHER_INSTANCE,
                      [](StartDownloadOtherInstanceEvent &evt)
                      {
...
Plater::priv::~priv()
{
    // Must shut down the Mouse3DController thread before destroying the object,
    // otherwise std::thread::~thread() calls std::terminate() if thread is joinable
...
void Plater::priv::update(unsigned int flags)
{
    ...
    const bool force_background_processing_restart = this->printer_technology == ptSLA ||
                                                     (flags &
                                                      (unsigned int) UpdateParams::FORCE_BACKGROUND_PROCESSING_UPDATE);
    if (force_background_processing_restart)
        // Update the SLAPrint from the current Model, so that the reload_scene()
        // pulls the correct data.
        update_status = this->update_background_process(
            false, flags & (unsigned int) UpdateParams::POSTPONE_VALIDATION_ERROR_MESSAGE);
...
{
    // Defer the layout update to avoid immediate resize and "jumpy" behavior
    sidebar->collapse(collapse);

    // Request a layout update on the next event loop iteration
    // This allows the UI to update smoothly without freezing
    // Note: mainframe is nullptr in GCodeViewer standalone mode
    ...
    // Only update the old collapse toolbar if the user has explicitly enabled it in preferences
    if (wxGetApp().app_config->get_bool("show_collapse_button"))
    {
    ...
    {
        // Keep the old toolbar disabled when using the new hamburger menu
        collapse_toolbar.set_enabled(false);
    }
...
void Plater::notify_about_installed_presets()
{
    ...
    if (!names.empty())
    {
        std::string notif_text =
            into_u8(_L_PLURAL("The preset below was temporarily installed on the active instance of preFlight",
                              "The presets below were temporarily installed on the active instance of preFlight",
                              names.size())) +
            ":";
                                             ...
                                             bool load_config, bool imperial_units /* = false*/)
{
    ...
    for (size_t i = 0; i < input_files_size; ++i)
    {
            ...
            wxGetApp().app_config->get_bool("show_step_import_parameters"))
        {
            ...
            {
                wxGetApp().app_config->set("linear_precision",
                                           float_to_string_decimal_point(dlg.get_linear_precision()));
        ...
        const bool type_3mf = std::regex_match(path.string(), pattern_3mf) ||
                              std::regex_match(path.string(), pattern_zip);
        ...
        {
            Slic3r::GUI::show_info(nullptr, _L("PrintRequest can only be loaded if an SLA printer is selected."),
                                   _L("Error!"));
            continue;
        }
        ...
        try
        {
            if (load_config)
            {
                model = FileReader::load_model_with_config(path.string(), &config_loaded, &config_substitutions,
                                                           generator_version, FileReader::LoadAttribute::CheckVersion,
                                                           &load_stats);
            }
            else if (load_model)
            {
        ...
        {
            std::string message = GUI::format(_L("Failed loading file \"%1%\" due to an invalid configuration."),
                                              filename.string()) +
                                  "\n\n" + e.what();
        ...
        if (!load_stats.step_import_warning.empty())
        {
            notification_manager->push_notification(NotificationType::CustomNotification,
                                                    NotificationManager::NotificationLevel::WarningNotificationLevel,
                                                    _u8L("STEP Import Warning") + "\n" +
                                                        load_stats.step_import_warning);
        }

        if (load_config)
        {
            const bool had_config = !config_loaded.empty();
            ...
            else
            {
                ...
                if (generator_version && // set only when loaded with configuration
                    *generator_version > Semver(SLIC3R_VERSION))
                {
                    ...
                    wxString message = format_wxstr(
                        _L("<b>Unable to load configuration from project\n\nFile: </b>%1%\n\n"
                           "This project was created in a newer version of preFlight. Only the geometry was loaded.\n"
                           "Update to the latest version for full compatibility.\nFor more info: <a href=%2%>%2%</a>"),
                        from_path(filename), url);
                    HtmlCapableRichMessageDialog dialog(q, message, title, wxOK, [&url](const std::string &)
                                                        { wxGetApp().open_browser_with_warning_dialog(url); });
                    dialog.ShowModal();
                }
            }

            // Warn when loading a 3mf from another slicer (not preFlight).
            // PrusaSlicer-family files have partial config; other slicers have none.
            if (type_3mf && !generator_version.has_value() && !load_stats.generator_application.empty())
            {
                if (had_config)
                {
                    MessageDialog dlg(q,
                                      format_wxstr(_L("This file was saved by %1%.\n\n"
                                                      "Some settings were imported but may be incomplete.\n"
                                                      "Please review your settings carefully before slicing."),
                                                   load_stats.generator_application),
                                      _L("Import Notice"), wxICON_WARNING | wxOK);
                    dlg.ShowModal();
                }
                else
                {
                    MessageDialog dlg(q,
                                      format_wxstr(_L("This file was saved by %1%.\n\n"
                                                      "No settings could be imported - only geometry was loaded.\n"
                                                      "Please configure your print settings before slicing."),
                                                   load_stats.generator_application),
                                      _L("Import Notice"), wxICON_WARNING | wxOK);
                    dlg.ShowModal();
                }
            ...
            if (!config.empty())
            {
                ...
                // 1. The project uses a different printer profile name, OR
                // 2. The project uses the same name but has different printer settings
                ...
                if (!project_printer.empty())
                {
                    ...
                    {
                        // Different printer profile name - ask to switch
                        MessageDialog dlg(q,
                                          format_wxstr(_L("This project was saved with printer profile:\n\n  %1%\n\n"
                                                          "Your current printer profile is:\n\n  %2%\n\n"
                                                          "Would you like to switch to the project's printer profile?"),
                                                       project_printer, current_printer),
                                          _L("Different Printer Profile"), wxICON_QUESTION | wxYES_NO);

                        should_apply_printer_from_project = (dlg.ShowModal() == wxID_YES);
                    }
                    else
                    {
                        // Same printer name - check if settings differ
                        std::vector<std::string> differing_options;

                        for (const std::string &key : Preset::printer_options())
                        {
                            const ConfigOption *current_opt = current_config.option(key);
                            const ConfigOption *project_opt = config.option(key);

                            if (current_opt && project_opt && *current_opt != *project_opt)
                            {
                                differing_options.push_back(key);
                            }
                        }

                        if (!differing_options.empty())
                        {
                            // Settings differ - warn user
                            wxString diff_list;
                            size_t max_to_show = std::min(differing_options.size(), size_t(5));
                            for (size_t i = 0; i < max_to_show; ++i)
                            {
                                diff_list += "  - " + differing_options[i] + "\n";
                            }
                            if (differing_options.size() > max_to_show)
                            {
                                diff_list += format_wxstr(_L("  ...and %1% more"),
                                                          differing_options.size() - max_to_show);
                            }

                            MessageDialog dlg(
                                q,
                                format_wxstr(
                                    _L("This project contains different printer settings than your current profile '%1%'.\n\n"
                                       "Modified settings:\n%2%\n"
                                       "Would you like to apply the project's printer settings?\n\n"
                                       "Selecting 'No' will keep your current printer settings."),
                                    current_printer, diff_list),
                                _L("Printer Settings Differ"), wxICON_QUESTION | wxYES_NO);

                            should_apply_printer_from_project = (dlg.ShowModal() == wxID_YES);
                        }
                    }

                    if (!should_apply_printer_from_project)
                    {
                        // User wants to keep current printer settings
                        // Instead of erasing (which breaks downstream code expecting these keys),
                        // copy current printer config values over the project's values
                        config.set_key_value("printer_settings_id", new ConfigOptionString(current_printer));
                        for (const std::string &key : Preset::printer_options())
                        {
                            const ConfigOption *current_opt = current_config.option(key);
                            if (current_opt)
                            {
                                config.set_key_value(key, current_opt->clone());
                            }
                        }
                    }
                }
        ...
        if (load_model)
        {
            if (!is_project_file)
            {
                ...
                {
                    MessageDialog(q,
                                  format_wxstr(_L_PLURAL("Object size from file %s appears to be zero.\n"
                                                         "This object has been removed from the model",
                                                         "Objects size from file %s appears to be zero.\n"
                                                         "These objects have been removed from the model",
                                                         load_stats.deleted_objects_cnt),
                                               from_path(filename)) +
                                      "\n",
                                  _L("The size of the object is zero"), wxICON_INFORMATION | wxOK)
                        .ShowModal();
                }
                        ...
                        RichMessageDialog dlg(
                            ...
                            format_wxstr(
                                _L_PLURAL(
                                    ...
                                    "The internal unit of preFlight is a millimeter. Do you want to recalculate the dimensions of the object?",
                                    "The dimensions of some objects from file %s seem to be defined in meters.\n"
                                    "The internal unit of preFlight is a millimeter. Do you want to recalculate the dimensions of these objects?",
                                    model.objects.size()),
                                from_path(filename)) +
                                "\n",
                ...
                if (load_stats.looks_like_multipart_object)
                {
                    ...
                    if (/*!type_printRequest && */ answer ==
                        wxID_YES) //! type_printRequest is no need here, SLA allow multipart object now
            ...
            if ((wxGetApp().get_mode() == comSimple) && type_3mf && model_has_advanced_features(model))
            {
                MessageDialog msg_dlg(
                    q,
                    _L("This file cannot be loaded in a simple mode. Do you want to switch to an advanced mode?") +
                        "\n",
            ...
            for (ModelObject *model_object : model.objects)
            {
                ...
                if (type_printRequest)
                {
                    ...
                    {
                        obj_instance->set_offset(
                            obj_instance->get_offset() +
                            Slic3r::to_3d(this->bed.build_volume().bed_center(), -model_object->origin_translation(2)));
                    }
                }
            }
            ...
            if (type_printRequest)
            {
                ...
                {
                    std::string preset_name = wxGetApp().preset_bundle->get_preset_name_by_alias_invisible(
                        Preset::Type::TYPE_SLA_MATERIAL, material.first);
                    Preset *prst = wxGetApp().preset_bundle->sla_materials.find_preset(preset_name, false);
                    if (!prst)
                    { //did not find compatible profile
                        ...
                        for (auto it = prints.begin(); it != prints.end(); ++it)
                        {
                            if (it->name != edited_print_name)
                            {
                                ...
                                preset_name = wxGetApp().preset_bundle->get_preset_name_by_alias_invisible(
                                    Preset::Type::TYPE_SLA_MATERIAL, material.first);
    ...
    if (extra_model != nullptr && extra_model->objects.size() > 1)
    {
        MessageDialog msg_dlg(q,
                              _L("Multiple objects were loaded for a multi-material printer.\n"
    ...
    {
        wxGetApp().app_config->update_skein_dir(
            input_files[input_files.size() - 1].parent_path().make_preferred().string());
    ...
    for (size_t idx : obj_idxs)
    {
        ...
        {
            const TriangleMeshStats stats = ModelProcessing::get_object_mesh_stats(model.objects[idx]);
            if (!stats.manifold())
            {
                std::string msg = GUI::format(_L("Model \"%1%\" has %2% open edges and may not slice correctly."),
                                              model.objects[idx]->name, stats.open_edges);
                notification_manager->push_notification(NotificationType::CustomNotification,
                                                        NotificationManager::NotificationLevel::WarningNotificationLevel,
                                                        msg);
            }
    ...
    if (generator_version && *generator_version < Semver("2.9.0-alpha1"))
    {
...
std::vector<size_t> Plater::priv::load_model_objects(const ModelObjectPtrs &model_objects, bool allow_negative_z,
                                                     bool call_selection_changed /*= true*/)
{
    const Vec3d bed_size = Slic3r::to_3d(this->bed.build_volume().bounding_volume2d().size(), 1.0) -
                           2.0 * Vec3d::Ones();
    ...
    for (ModelObject *model_object : model_objects)
    {
        ...
        std::string object_name = object->name.empty() ? fs::path(object->input_file).filename().string()
                                                       : object->name;
        ...
        for (size_t i = 0;
             i < object->instances.size() && !object->is_cut() // don't apply scaled_down functionality to cut objects
             ;
             ++i)
        {
            ...
            if (!s_multiple_beds.get_loading_project_flag())
                instance->set_offset(instance->get_offset() +
    ...
    {
        GUI::show_info(
            q, _L("Your object appears to be too large, so it was automatically scaled down to fit your print bed."),
            _L("Object too large?"));
    }
...
wxString Plater::priv::get_export_file(GUI::FileType file_type)
{
    ...
    wxFileDialog dlg(q, dlg_title,
                     out_dir == temp_dir ? from_u8(wxGetApp().app_config->get("last_output_path"))
...
void Plater::priv::object_list_changed()
{
    ...
    if (printer_technology == ptFFF)
    {
        for (std::size_t bed_index{}; bed_index < s_multiple_beds.get_number_of_beds(); ++bed_index)
        {
            ...
            else if (wxGetApp().plater()->get_fff_prints()[bed_index]->finished() &&
            ...
            for (const ModelObject *object : wxGetApp().model().objects)
            {
                for (const ModelInstance *instance : object->instances)
                {
                    ...
                    if (it != s_multiple_beds.get_inst_map().end() && it->second == bed_index && instance->printable &&
    ...
    sidebar->enable_buttons(s_multiple_beds.is_bed_occupied(s_multiple_beds.get_active_bed()) && !export_in_progress &&
        ...
        InfoDialog dialog(q, _L("Delete object which is a part of cut object"),
                          _L("You try to delete an object which is a part of a cut object.") + "\n" +
                              _L("This action will break a cut information.\n"
                                 "After that preFlight can't guarantee model consistency"),
...
void Plater::priv::split_object()
{
    ...
    if (new_objects.size() == 1)
        ...
        Slic3r::GUI::warning_catcher(
            q, _L("The selected object couldn't be split because it contains only one solid part."));
    else
    {
...
void Plater::priv::process_validation_warning(const std::vector<std::string> &warnings) const
{
    ...
    for (std::string text : warnings)
    {
        ...
        else if (text == "_BED_TEMPS_DIFFER" || text == "_BED_TEMPS_CHANGED")
        {
            ...
            {
                GUI::wxGetApp()
                    .get_tab(Preset::Type::TYPE_PRINT)
                    ->activate_option("bed_temperature_extruder", boost::nowide::widen("Multiple Extruders"),
                                      {"wipe_tower_extruder"});
                return true;
            };
        }
        ...
        else if (text == "_SUPPORT_NOZZLE_DIAMETER_DIFFER")
        {
            ...
            action_fn = [](wxEvtHandler *)
            {
                GUI::wxGetApp().jump_to_option("support_material_extruder", Preset::Type::TYPE_PRINT,
                                               boost::nowide::widen("Multiple Extruders"));
                return false;
            };
        }
        else if (text == "_OVERLAP_OUT_OF_BOUNDS")
        {
            text = _u8L("One or more overlap settings have absolute (mm) values that exceed "
                        "the valid range for the current extrusion widths. This can happen when "
                        "extrusion widths are changed after setting overlap values in mm. "
                        "Consider reviewing the ");
            hypertext = _u8L("overlap settings in Print Settings > Advanced.");
            multiline = true;
            notification_type = NotificationType::SlicingWarning;
            action_fn = [](wxEvtHandler *)
            {
                GUI::wxGetApp()
                    .get_tab(Preset::Type::TYPE_PRINT)
                    ->activate_option("infill_overlap", boost::nowide::widen("Advanced"));
                return true;
            };
        }

        notification_manager->push_notification(notification_type,
                                                NotificationManager::NotificationLevel::WarningNotificationLevel,
                                                _u8L("WARNING:") + "\n" + text, hypertext, action_fn, text_after, 0,
                                                multiline);
    }
}

std::vector<Print::ApplyStatus> apply_to_inactive_beds(Model &model, std::vector<std::unique_ptr<Print>> &prints,
                                                       const DynamicPrintConfig &config)
{
    ...
    for (std::size_t bed_index{0}; bed_index < prints.size(); ++bed_index)
    {
        ...
        with_single_bed_model_fff(model, bed_index, [&]() { result[bed_index] = print->apply(model, config); });
    }
    return result;
}
...
void Plater::priv::regenerate_thumbnails(SimpleEvent &)
{
    ...
    for (int i = 0; i < num; ++i)
    {
        ...
        glsafe(glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB32F, static_cast<GLsizei>(w), static_cast<GLsizei>(h), 0, GL_RGBA,
                            GL_UNSIGNED_BYTE, data.pixels.data()));
...
unsigned int Plater::priv::update_background_process(bool force_validation, bool postpone_error_messages)
{
    ...
    if (full_config.has("binary_gcode")) // needed for SLA
        full_config.set("binary_gcode", bool(full_config.opt_bool("binary_gcode") &
    ...
    const PresetWithVendorProfile &printer_with_vendor =
        wxGetApp().preset_bundle->printers.get_preset_with_vendor_profile(selected_printer);
    ...
    std::vector<Print::ApplyStatus> apply_statuses{
        printer_technology == ptFFF ? apply_to_inactive_beds(q->model(), q->p->fff_prints, full_config)
    ...
    const bool any_status_changed{std::any_of(apply_statuses.begin(), apply_statuses.end(),
    ...
    if (invalidated == Print::APPLY_STATUS_INVALIDATED)
    {
        ...
        // preFlight enhancement: Always return to 3D view when slicing becomes invalid
        if (current_panel == preview)
        {
            set_current_panel(view3D);
        ...
        (return_state & UPDATE_BACKGROUND_PROCESS_RESTART) == 0)
    {
        ...
        wxQueueEvent(GUI::wxGetApp().mainframe->m_plater,
                     new SlicingProcessCompletedEvent(EVT_PROCESS_COMPLETED, 0, SlicingProcessCompletedEvent::Cancelled,
                                                      std::exception_ptr{}));
    }
...
void Plater::priv::export_gcode(fs::path output_path, bool output_path_on_removable_media, PrintHostJob upload_job)
{
    ...
    if (!output_path.empty())
    {
        ...
        try
        {
            // Export directly from memory - no re-slicing needed!
            background_process.direct_export_gcode(output_path.string(), output_path_on_removable_media);

            // Close "Exporting..." notification
            notification_manager->stop_delayed_notifications_of_type(NotificationType::ExportOngoing);
            notification_manager->close_notification_of_type(NotificationType::ExportOngoing);

            // Show "Export finished" notification with open folder option
            notification_manager->push_exporting_finished_notification(
                output_path.string(), output_path.parent_path().string(),
                output_path_on_removable_media && platform_flavor() != PlatformFlavor::LinuxOnChromium);
        }
        catch (const std::exception &e)
        {
            notification_manager->stop_delayed_notifications_of_type(NotificationType::ExportOngoing);
...
void Plater::priv::update_fff_scene()
{
    ...
    // However, if user is viewing Preview tab, skip this expensive operation (~10 seconds)
    if (!is_preview_shown())
    {
    ...
    {
        // Mark that 3D scene needs refresh when user switches back to Prepare tab
...
bool Plater::priv::replace_volume_with_stl(int object_idx, int volume_idx, const fs::path &new_path,
                                           const wxString &snapshot)
{
...
void Plater::priv::reload_from_disk()
{
    ...
    missing_input_paths.erase(std::unique(missing_input_paths.begin(), missing_input_paths.end()),
                              missing_input_paths.end());
...
void Plater::priv::set_current_panel(wxPanel *panel)
{
    ...
    {
        // Only show sidebar if it's not collapsed
    ...
    // Force layout update of parent sizer to resize panels when sidebar is shown/hidden
    q->Layout();
    ...
    if (current_panel == view3D)
    {
        ...
        if (wxGetApp().mainframe && wxGetApp().mainframe->m_modern_tabbar)
        {
            // Only switch to Prepare tab if currently on Preview tab
            if (wxGetApp().mainframe->m_modern_tabbar->IsSelected(GUI::ModernTabBar::TAB_PREVIEW))
            {
                wxGetApp().mainframe->m_modern_tabbar->SelectTab(GUI::ModernTabBar::TAB_PREPARE);
            }
        }
    ...
    else if (current_panel == preview)
    {
        ...
        */
        ...
        if (wxGetApp().mainframe && wxGetApp().mainframe->m_modern_tabbar)
        {
            wxGetApp().mainframe->m_modern_tabbar->SelectTab(GUI::ModernTabBar::TAB_PREVIEW);
        }
    }
...
void Plater::priv::on_slicing_update(SlicingStatusEvent &evt)
{
    if (evt.status.percent >= -1)
    {
        ...
        // Don't call set_slicing_progress_rendering() as it resets progress to 0%
        // Just update the percentage and text normally - stays at 100% when rendering
        notification_manager->set_slicing_progress_percentage(evt.status.text, (float) evt.status.percent / 100.0f);

        if (preview && preview->IsShown() && evt.status.percent >= 0 && evt.status.percent <= 100)
        {
            // Get max Z height from the print
            double max_z = 0.0;
            ...
            if (print)
            {
                for (const auto *obj : print->objects())
                {
                    if (!obj->layers().empty())
                    {
                        max_z = std::max(max_z, obj->layers().back()->print_z);
                    }
                }
            }

            // Calculate current Z height based on progress percentage
            if (max_z > 0.0)
            {
                const double progress_z = (evt.status.percent / 100.0) * max_z;
                preview->get_canvas3d()->set_shell_progress_height(progress_z);
                preview->get_canvas3d()->set_as_dirty();
                preview->get_canvas3d()->request_extra_frame();
            }
        }
    }
    ...
    for (int flags : flagss)
    {
        ...
        for (int warning_step : warning_steps)
        {
            for (ObjectID object_id : object_ids)
            {
                ...
                else if (flags & (PrintBase::SlicingStatus::UPDATE_PRINT_STEP_WARNINGS |
                                  PrintBase::SlicingStatus::UPDATE_PRINT_OBJECT_STEP_WARNINGS))
                {
...
bool Plater::priv::warnings_dialog()
{
    ...
    InfoDialog msg_wingow(this->q, format_wxstr("%1% %2%", SLIC3R_APP_NAME, _L("generated warnings")), from_u8(text),
                          true);
...
void Plater::priv::on_process_completed(SlicingProcessCompletedEvent &evt)
{
    ...
    if (evt.success())
    {
        ...
        if (wxGetApp().mainframe && wxGetApp().mainframe->m_modern_tabbar)
        {
            wxGetApp().mainframe->m_modern_tabbar->EnableTab(GUI::ModernTabBar::TAB_PREVIEW, true);
            wxGetApp().mainframe->m_modern_tabbar->SelectTab(GUI::ModernTabBar::TAB_PREVIEW);
...
void Plater::priv::on_right_click(RBtnEvent &evt)
{
    ...
    if (obj_idx == -1)
    { // no one or several object are selected
        if (evt.data.second)
        {                                    // right button was clicked on empty space
...
void Plater::priv::generate_thumbnail(ThumbnailData &data, unsigned int w, unsigned int h,
                                      const ThumbnailsParams &thumbnail_params, Camera::EType camera_type)
{
...
{
    return (current_panel == view3D) ? view3D->get_canvas3d()
                                     : ((current_panel == preview) ? preview->get_canvas3d() : nullptr);
}
...
bool Plater::priv::init_view_toolbar()
{
    ...
    // The view toolbar needs to be explicitly set as vertical for right-side placement
    view_toolbar.set_layout_type(GLToolbar::Layout::Vertical);
    view_toolbar.set_horizontal_orientation(GLToolbar::Layout::HO_Center);
    view_toolbar.set_vertical_orientation(GLToolbar::Layout::VO_Bottom);
    // Force layout recalculation by getting dimensions
    view_toolbar.get_width();
    view_toolbar.get_height();
    ...
    {
        if (this->q != nullptr)
            wxPostEvent(this->q, SimpleEvent(EVT_GLVIEWTOOLBAR_3D));
    };
    ...
    {
        if (this->q != nullptr)
            wxPostEvent(this->q, SimpleEvent(EVT_GLVIEWTOOLBAR_PREVIEW));
    };
    if (!view_toolbar.add_item(item))
        return false;
...
void Plater::priv::update_preview_moves_slider(std::optional<int> visible_range_min,
                                               std::optional<int> visible_range_max)
{
...
bool Plater::priv::layers_height_allowed() const
{
    ...
    return 0 <= obj_idx && obj_idx < (int) model.objects.size() &&
           model.objects[obj_idx]->max_z() > SINKING_Z_THRESHOLD && config->opt_bool("variable_layer_height") &&
...
void Plater::priv::set_bed_shape(const Pointfs &shape, const double max_print_height, const std::string &custom_texture,
...
bool Plater::priv::can_increase_instances() const
{
    ...
    if (q->canvas3D()->get_gizmos_manager().get_current_type() == GLGizmosManager::Emboss)
        return false;
...
bool Plater::priv::can_decrease_instances(int obj_idx /*= -1*/) const
{
    ...
    return obj_idx < (int) model.objects.size() && (model.objects[obj_idx]->instances.size() > 1) &&
...
{
    if (model.objects.empty() || !m_worker.is_idle())
        return false;
...
void Plater::priv::show_action_buttons(const bool ready_to_slice_) const
{
    ...
    const auto print_host_opt = selected_printer_config
                                    ? selected_printer_config->option<ConfigOptionString>("print_host")
                                    : nullptr;
    ...
    else
    {
        ...
        if (sidebar->show_reslice(ready_to_slice) | sidebar->show_export(!ready_to_slice) |
...
void Plater::priv::take_snapshot(const std::string &snapshot_name, const UndoRedo::SnapshotType snapshot_type)
{
    ...
    if (snapshot_type == UndoRedo::SnapshotType::ProjectSeparator &&
        get_config_bool("clear_undo_redo_stack_on_new_project"))
        this->undo_redo_stack().clear();
    this->undo_redo_stack().take_snapshot(snapshot_name, model, view3D->get_canvas3d()->get_selection(), gizmos,
                                          snapshot_data);
    ...
    BOOST_LOG_TRIVIAL(info) << "Undo / Redo snapshot taken: " << snapshot_name << ", Undo / Redo stack memory: "
...
void Plater::priv::undo_redo_to(std::vector<UndoRedo::Snapshot>::const_iterator it_snapshot)
{
    ...
    if (!new_variable_layer_editing_active && view3D->is_layers_editing_enabled())
        view3D->get_canvas3d()->force_main_toolbar_left_action(
            view3D->get_canvas3d()->get_main_toolbar_item_id("layersediting"));

    // Make a copy of the snapshot, undo/redo could invalidate the iterator
    const UndoRedo::Snapshot snapshot_copy = *it_snapshot;
    // Do the jump in time.
    if (it_snapshot->timestamp < this->undo_redo_stack().active_snapshot_time()
            ? this->undo_redo_stack().undo(model, this->view3D->get_canvas3d()->get_selection(),
                                           this->view3D->get_canvas3d()->get_gizmos_manager(), top_snapshot_data,
                                           it_snapshot->timestamp)
            : this->undo_redo_stack().redo(model, this->view3D->get_canvas3d()->get_gizmos_manager(),
                                           it_snapshot->timestamp))
    {
        if (printer_technology_changed)
        {
        ...
        this->sidebar->obj_list()->set_selection_mode(
            new_selected_settings_on_sidebar    ? ObjectList::SELECTION_MODE::smSettings
...
void Plater::priv::update_after_undo_redo(const UndoRedo::Snapshot &snapshot, bool /* temp_snapshot_was_taken */)
{
    ...
    this->view3D->get_canvas3d()->get_selection().set_deserialized(
        GUI::Selection::EMode(this->undo_redo_stack().selection_deserialized().mode),
...
void Plater::priv::bring_instance_forward() const
{
    ...
    BOOST_LOG_TRIVIAL(debug) << "preFlight window going forward";
...
{
    return p->is_project_dirty();
}
bool Plater::is_presets_dirty() const
{
    return p->is_presets_dirty();
}
...
{
    return p->save_project_if_dirty(reason);
}
void Plater::reset_project_dirty_after_save()
{
    p->reset_project_dirty_after_save();
}
void Plater::reset_project_dirty_initial_presets()
{
    p->reset_project_dirty_initial_presets();
}
#if ENABLE_PROJECT_DIRTY_STATE_DEBUG_WINDOW
void Plater::render_project_state_debug_window() const
{
    p->render_project_state_debug_window();
}
#endif // ENABLE_PROJECT_DIRTY_STATE_DEBUG_WINDOW

Sidebar &Plater::sidebar()
{
    return *p->sidebar;
}
const Model &Plater::model() const
{
    return p->model;
}
Model &Plater::model()
...
{
    if (int saved_project = p->save_project_if_dirty(
            _L("Creating a new project while the current project is modified."));
        saved_project == wxID_CANCEL)
        return;
    else
    {
        wxString header =
            _L("Creating a new project while some presets are modified.") + "\n" +
            (saved_project == wxID_YES
                 ? _L("You can keep presets modifications to the new project or discard them")
...
void Plater::load_project(const wxString &filename)
{
    ...
    Plater::TakeSnapshot snapshot(this,
                                  _L("Load Project") + ": " +
                                      wxString::FromUTF8(into_path(filename).stem().string().c_str()),
...
void Plater::load_gcode(const wxString &filename)
{
    ...
    catch (const std::exception &)
    {
        ...
        MessageDialog(this, _L("The selected file") + ":\n" + filename + "\n" + _L("does not contain valid gcode."),
                      wxString(GCODEVIEWER_APP_NAME) + " - " + _L("Error while loading .gcode file"),
...
void Plater::convert_gcode_to_ascii()
{
    ...
    // Perform conversion
    {
        ...
        {
            MessageDialog msg_dlg(this, _L(std::string(translate_result(res))), _L("Error converting G-code file"),
    ...
    MessageDialog msg_dlg(this,
                          Slic3r::GUI::format_wxstr("%1%\n%2%", _L("Successfully created G-code ASCII file"),
                                                    output_file),
...
void Plater::convert_gcode_to_binary()
{
    ...
    MessageDialog msg_dlg(this,
                          Slic3r::GUI::format_wxstr("%1%\n%2%", _L("Successfully created G-code binary file"),
                                                    output_file),
...
std::vector<size_t> Plater::load_files(const std::vector<fs::path> &input_files, bool load_model, bool load_config,
                                       bool imperial_units /*= false*/)
{
    return p->load_files(input_files, load_model, load_config, imperial_units);
}
...
LoadProjectsDialog::LoadProjectsDialog(const std::vector<fs::path> &paths)
    : DPIDialog(static_cast<wxWindow *>(wxGetApp().mainframe), wxID_ANY,
                format_wxstr(_L("%1% - Multiple projects file"), SLIC3R_APP_NAME), wxDefaultPosition, wxDefaultSize,
                wxDEFAULT_DIALOG_STYLE)
{
    ...
    if (contains_projects)
    {
        ...
        m_combo_project = new wxComboBox(this, wxID_ANY, wxEmptyString, wxDefaultPosition, wxDefaultSize, filenames,
                                         wxCB_READONLY);
        m_combo_project->SetValue(filenames.front());
        m_combo_project->Enable(false);

        m_combo_config = new wxComboBox(this, wxID_ANY, wxEmptyString, wxDefaultPosition, wxDefaultSize, filenames,
                                        wxCB_READONLY);
    ...
    // all geometry
    wxRadioButton *btn = new wxRadioButton(this, wxID_ANY, _L("Import 3D models"), wxDefaultPosition, wxDefaultSize,
                                           id == 0 ? wxRB_GROUP : 0);
    btn->SetValue(id == m_action);
    btn->Bind(wxEVT_RADIOBUTTON,
              [this, id, contains_projects](wxCommandEvent &)
              {
                  m_action = id;
                  if (contains_projects)
                  {
    ...
    {
        btn = new wxRadioButton(this, wxID_ANY, _L("Start a new instance of preFlight"), wxDefaultPosition,
                                wxDefaultSize, id == 0 ? wxRB_GROUP : 0);
        btn->SetValue(id == m_action);
        btn->Bind(wxEVT_RADIOBUTTON,
                  [this, id, contains_projects](wxCommandEvent &)
                  {
    ...
    {
        // one project
        btn = new wxRadioButton(this, wxID_ANY, _L("Select one to load as project"), wxDefaultPosition, wxDefaultSize,
                                id == 0 ? wxRB_GROUP : 0);
        btn->SetValue(false);
        btn->Bind(wxEVT_RADIOBUTTON,
        ...
        btn = new wxRadioButton(this, wxID_ANY, _L("Select only one file to load the configuration."),
                                wxDefaultPosition, wxDefaultSize, id == 0 ? wxRB_GROUP : 0);
        btn->SetValue(id == m_action);
        btn->Bind(wxEVT_RADIOBUTTON,
                  [this, id, instances_allowed](wxCommandEvent &)
                  {
                      m_action = id;
                      if (instances_allowed)
                          m_combo_project->Enable(false);
...
bool Plater::preview_zip_archive(const boost::filesystem::path &archive_path)
{
    ...
    try
    {
        ...
        if (!open_zip_reader(&archive, archive_path.string()))
        {
            ...
            std::string err_msg = GUI::format(_u8L("Loading of a ZIP archive on path %1% has failed."),
                                              archive_path.string());
            ...
            for (auto &path_w_size : selected_paths)
            {
                ...
                for (mz_uint i = 0; i < num_entries; ++i)
                {
                    if (mz_zip_reader_file_stat(&archive, i, &stat))
                    {
                        ...
                        size_t extra_size = mz_zip_reader_get_filename_from_extra(&archive, i, extra.data(),
                                                                                  extra.size());
                        if (extra_size > 0)
                        {
                        ...
                        try
                        {
                            ...
                            if (res == 0)
                            {
                                ...
                                wxString error_log = GUI::format_wxstr(_L("Failed to unzip file to %1%: %2%"),
                                                                       final_path.string(),
                            ...
                            {
                                wxString error_log = GUI::format_wxstr(
                                    _L("Failed to find unzipped file at %1%. Unzipping of file has failed."),
                                    final_path.string());
                                BOOST_LOG_TRIVIAL(error) << error_log;
                                show_error(nullptr, error_log);
                                break;
                            ...
                            auto file_info = is_project_3mf(final_path.string());
                            if ((boost::algorithm::iends_with(filename, ".3mf") && !file_info.has_config) ||
                                (boost::algorithm::iends_with(filename, ".amf") &&
...
ProjectDropDialog::ProjectDropDialog(const std::string &filename)
    : DPIDialog(static_cast<wxWindow *>(wxGetApp().mainframe), wxID_ANY,
                format_wxstr("%1% - %2%", SLIC3R_APP_NAME, _L("Load project file")), wxDefaultPosition, wxDefaultSize,
                wxDEFAULT_DIALOG_STYLE)
{
    SetFont(wxGetApp().normal_font());
    int em = wxGetApp().em_unit();
    ...
    if (!single_instance_only)
        choices.Add(_L("Start new preFlight instance"));
    ...
    m_action = std::clamp(std::stoi(wxGetApp().app_config->get("drop_project_action")),
                          ...
                          single_instance_only ? static_cast<int>(LoadType::LoadConfig)
                                               : static_cast<int>(LoadType::OpenWindow)) -
               1;
    ...
    {
        wxRadioButton *btn = new wxRadioButton(this, wxID_ANY, label, wxDefaultPosition, wxDefaultSize,
                                               id == 0 ? wxRB_GROUP : 0);
        btn->SetValue(id == m_action);
...
{
    const std::regex pattern_drop(".*[.](stl|obj|amf|3mf|step|stp|zip|printRequest)", std::regex::icase);
    ...
    for (std::vector<fs::path>::const_reverse_iterator it = paths.rbegin(); it != paths.rend(); ++it)
    {
        ...
        auto file_info = is_project_3mf(it->string());

        if (boost::algorithm::iends_with(filename, ".zip") && file_info.has_config)
        {
            BOOST_LOG_TRIVIAL(warning)
                << "File with .zip extension is 3mf project, opening as it would have .3mf extension: " << *it;
        ...
        if (handle_as_project && load_just_one_file)
        {
            ...
            {
                if (boost::algorithm::iends_with(filename, ".3mf") &&
                    (!file_info.has_config && !file_info.generator_version.has_value()))
                {
                    // No config and not preFlight - load geometry only.
                    // Warn if the file came from another slicer (has Application tag).
                    if (!file_info.generator_application.empty())
                    {
                        MessageDialog dlg(static_cast<wxWindow *>(this),
                                          format_wxstr(_L("This file was saved by %1%.\n\n"
                                                          "No settings could be imported - only geometry was loaded.\n"
                                                          "Please configure your print settings before slicing."),
                                                       file_info.generator_application),
                                          _L("Import Notice"), wxICON_WARNING | wxOK);
                        dlg.ShowModal();
                    }
                ...
                else
                {
                    if (wxGetApp().app_config->get_bool("show_drop_project_dialog"))
                    {
                        ...
                        load_type = static_cast<ProjectDropDialog::LoadType>(
                            std::clamp(std::stoi(wxGetApp().app_config->get("drop_project_action")),
                                       static_cast<int>(ProjectDropDialog::LoadType::OpenProject),
                                       static_cast<int>(ProjectDropDialog::LoadType::LoadConfig)));
                }
            }
            ...
            if (!load_just_one_file)
            {
                WarningDialog dlg(static_cast<wxWindow *>(this),
                                  format_wxstr(_L("You have several files for loading and \"%1%\" is one of them.\n"
                                               ...
                                               filename),
                                  format_wxstr("%1% - %2%", SLIC3R_APP_NAME, _L("Drag and drop several files")),
                                  wxYES_NO);
    ...
    if (!load_config)
    {
        ...
        WarningDialog dlg(static_cast<wxWindow *>(this),
                          _L("You have several files for loading.\n"
...
void Plater::update(unsigned int flags)
{
    p->update(flags);
}
...
const Worker &Plater::get_ui_job_worker() const
{
    return p->m_worker;
}
...
void Plater::select_view_3D(const std::string &name)
{
    p->select_view_3D(name);
}

bool Plater::is_preview_shown() const
{
    return p->is_preview_shown();
}
bool Plater::is_preview_loaded() const
{
    return p->is_preview_loaded();
}
bool Plater::is_view3D_shown() const
{
...
{
    return p->are_view3D_labels_shown();
}
void Plater::show_view3D_labels(bool show)
{
    p->show_view3D_labels(show);
}

bool Plater::is_legend_shown() const
{
    return p->is_legend_shown();
}
void Plater::show_legend(bool show)
{
    p->show_legend(show);
}

bool Plater::is_sidebar_collapsed() const
{
    return p->is_sidebar_collapsed();
}
void Plater::collapse_sidebar(bool show)
{
    p->collapse_sidebar(show);
}

bool Plater::is_view3D_layers_editing_enabled() const
{
    return p->is_view3D_layers_editing_enabled();
}

void Plater::select_all()
{
    p->select_all();
}
void Plater::deselect_all()
{
    p->deselect_all();
}
...
void Plater::reset_with_confirm()
{
    if (p->model.objects.empty() ||
        ...
        MessageDialog(static_cast<wxWindow *>(this), _L("All objects will be removed, continue?"),
                      wxString(SLIC3R_APP_NAME) + " - " + _L("Delete all"),
                      wxYES_NO | wxCANCEL | wxYES_DEFAULT | wxCENTRE)
                .ShowModal() == wxID_YES)
        reset();
}

bool Plater::delete_object_from_model(size_t obj_idx)
{
    return p->delete_object_from_model(obj_idx);
}
...
void Plater::increase_instances(size_t num, int obj_idx, int inst_idx)
{
    ...
    ModelInstance *model_instance = (inst_idx >= 0) ? model_object->instances[inst_idx]
                                                    : model_object->instances.back();
...
{
    if (!can_decrease_instances(obj_idx))
    {
        return;
    }
...
static long GetNumberFromUser(const wxString &msg, const wxString &prompt, const wxString &title, long value, long min,
                              long max, wxWindow *parent)
{
#ifdef _WIN32
...
void Plater::set_number_of_copies()
{
    ...
    const int num = GetNumberFromUser(" ", _L("Enter the number of copies:"), _L("Copies of the selected object"),
                                      init_cnt, 0, 1000, this);
    if (num < 0)
        return;
...
void Plater::fill_bed_with_instances()
{
    ...
    if (w.is_idle())
    {
        ...
        auto scene = arr2::Scene{build_scene(*this, ArrangeSelectionMode::CurrentBedSelectionOnly)};
...
void Plater::convert_unit(ConversionType conv_type)
{
    ...
    TakeSnapshot snapshot(this, conv_type == ConversionType::CONV_FROM_INCH ? _L("Convert from imperial units")
                                : conv_type == ConversionType::CONV_TO_INCH
                                    ? _L("Revert conversion from imperial units")
                                : conv_type == ConversionType::CONV_FROM_METER ? _L("Convert from meters")
                                                                               : _L("Revert conversion from meters"));
    wxBusyCursor wait;
...
void Plater::apply_cut_object_to_model(size_t obj_idx, const ModelObjectPtrs &new_objects)
{
    ...
    const auto bed_index{instances.size() == 1 && first_instance_bed_index ? *first_instance_bed_index : active_bed};
...
std::optional<fs::path> Plater::get_default_output_file()
{
    try
    {
        ...
        const std::string path{
            this->p->background_process.output_filepath_for_project(into_path(get_project_filename(".3mf")))};
...
std::string get_output_start_dir(const bool prefer_removable, const fs::path &default_output_file)
{
    ...
    std::string last_output_dir{
        appconfig.get_last_output_dir(default_output_file.parent_path().string(), prefer_removable)};
    if (!prefer_removable)
    {
...
std::optional<wxString> Plater::check_output_path_has_error(const boost::filesystem::path &path) const
{
    ...
    {
        return {_L("The provided file name is not valid.") + "\n" +
    ...
    if (this->printer_technology() == ptFFF)
    {
        ...
        const wxString error{
            check_binary_vs_ascii_gcode_extension(printer_technology(), ext, supports_binary && uses_binary)};
        if (!error.IsEmpty())
        {
            return error;
        }
...
std::optional<fs::path> Plater::get_output_path(const std::string &start_dir, const fs::path &default_output_file)
{
    ...
    {
        const t_link_clicked on_link_clicked = [](const std::string &key) -> void
        {
            wxGetApp().jump_to_option(key);
        };
...
{
    wxDirDialog dlg(this, _L("Choose export directory:"), start_dir);
...
void Plater::export_gcode_to_path(const fs::path &output_path, const std::function<void(bool)> &export_callback)
{
    AppConfig &appconfig{*wxGetApp().app_config};
    RemovableDriveManager &removable_drive_manager{*wxGetApp().removable_drive_manager()};
    bool path_on_removable_media = removable_drive_manager.set_and_verify_last_save_path(output_path.string());
    p->notification_manager->new_export_began(path_on_removable_media);
    p->exporting_status = path_on_removable_media ? ExportingStatus::EXPORTING_TO_REMOVABLE
...
void Plater::with_mocked_fff_background_process(Print &print, GCodeProcessorResult &result, const int bed_index,
...
void Plater::export_all_gcodes(bool prefer_removable)
{
    ...
    for (int print_index{0}; print_index < s_multiple_beds.get_number_of_beds(); ++print_index)
    {
        ...
        this->with_mocked_fff_background_process(*print, this->p->gcode_results[print_index], print_index,
                                                 [&]()
                                                 {
        ...
        const fs::path filename{default_filename.stem().string() + "_bed" + std::to_string(print_index + 1) +
                                default_filename.extension().string()};
        const fs::path output_file{output_dir / filename};
        prints_to_export.insert({print_index, {*print, this->p->gcode_results[print_index], print_index}});
    ...
    for (auto &[bed_index, optional_path] : output_paths)
    {
        ...
        with_mocked_fff_background_process(
            print_to_export.print, print_to_export.processor_result, print_to_export.bed,
            [&]()
            {
                this->p->background_process.set_temp_output_path(print_to_export.bed);
                export_gcode_to_path(path,
                                     [&](const bool on_removable)
                                     {
                                         this->p->background_process.finalize_gcode(path.string(),
...
void Plater::export_stl_obj(bool extended, bool selection_only)
{
    ...
    std::function<TriangleMesh(const ModelObject &mo, int instance_id)> mesh_to_export = mesh_to_export_fff;
    ...
    if (selection_only)
    {
        ...
        if (selection.get_mode() == Selection::Instance)
            mesh = mesh_to_export(*model_object,
                                  (selection.is_single_full_object() && model_object->instances.size() > 1)
                                      ? -1
                                      : selection.get_instance_idx());
        else
        {
...
bool Plater::export_3mf(const boost::filesystem::path &output_path)
{
    ...
    try
    {
        ret = Slic3r::store_3mf(path_u8.c_str(), &p->model, export_config ? &cfg : nullptr, full_pathnames,
                                &thumbnail_data);
    }
...
void Plater::reslice_SLA_until_step(int /*step*/, const ModelObject & /*object*/, bool /*postpone_error_messages*/) {}
...
bool load_secret(const std::string &id, const std::string &opt, std::string &usr, std::string &psswd)
{
    ...
    {
        std::string msg = GUI::format("%1% (%2%).", _u8L("This system doesn't support storing passwords securely"),
                                      errmsg);
...
void Plater::send_gcode_inner(DynamicPrintConfig *physical_printer_config)
{
    ...
    for (;;)
    {
        ...
        if (printer_technology() == ptFFF)
        {
            ...
            const bool binary_output = wxGetApp().preset_bundle->printers.get_edited_preset().config.opt_bool(
                                           "binary_gcode") &&
                                       wxGetApp().app_config->get_bool("use_binary_gcode_when_supported");
            const wxString error_str = check_binary_vs_ascii_gcode_extension(printer_technology(), ext, binary_output);
            if (!error_str.IsEmpty())
            {
                ErrorDialog(this, error_str,
                            t_kill_focus([](const std::string &key) -> void { wxGetApp().jump_to_option(key); }))
                    .ShowModal();
                return;
            }
        }
...
void Plater::take_snapshot(const wxString &snapshot_name, UndoRedo::SnapshotType snapshot_type)
{
    p->take_snapshot(snapshot_name, snapshot_type);
}
void Plater::set_plater_dirty(bool is_dirty)
{
    p->dirty_state.update_from_undo_redo_stack(is_dirty);
}
void Plater::suppress_snapshots()
{
    p->suppress_snapshots();
}
...
bool Plater::update_filament_colors_in_full_config()
{
    ...
    for (const auto &extr_filaments : extruders_filaments)
        filament_colors.push_back(filaments.find_preset(extr_filaments.get_selected_preset_name(), true)
                                      ->config.opt_string("filament_colour", (unsigned) 0));
...
void Plater::set_bed_shape() const
{
    if (wxGetApp().is_gcode_viewer())
    {
        // In GCodeViewer mode, p->config has defaults, so use the main app's preset instead
        set_default_bed_shape();
        return;
    }
...
void Plater::set_bed_shape(const Pointfs &shape, const double max_print_height, const std::string &custom_texture,
                           const std::string &custom_model, bool force_as_custom) const
{
...
{
    // GCodeViewer uses a separate config file (preFlightGcodeViewer.ini), so we need to
    // load the main app's presets to get the correct bed shape from the user's printer selection.
    if (wxGetApp().is_gcode_viewer())
    {
        // Load a temporary AppConfig from the main editor's config file
        AppConfig editor_config(AppConfig::EAppMode::Editor);
        editor_config.load();

        // Create a temporary preset bundle and load presets using the editor's config
        PresetBundle temp_bundle;
        try
        {
            temp_bundle.setup_directories();
            temp_bundle.load_presets(editor_config, ForwardCompatibilitySubstitutionRule::EnableSilent);

            const DynamicPrintConfig full_config = temp_bundle.full_config();
            const auto *bed_shape_opt = full_config.option<ConfigOptionPoints>("bed_shape");
            if (bed_shape_opt && !bed_shape_opt->values.empty())
            {
                const auto *bed_model_opt = full_config.option<ConfigOptionString>("bed_custom_model");
                const auto *bed_texture_opt = full_config.option<ConfigOptionString>("bed_custom_texture");
                set_bed_shape(bed_shape_opt->values, 0.0, bed_model_opt ? bed_model_opt->value : "",
                              bed_texture_opt ? bed_texture_opt->value : "", true);
                return;
            }
        }
        catch (...)
    ...
    {
        // Normal editor mode - use existing preset bundle
    ...
    // Fallback to hardcoded default if preset not available
...
void Plater::force_filament_colors_update()
{
        ...
        p->config->option<ConfigOptionStrings>("filament_colour")->values.size() == extruders_filaments.size())
    {
        ...
        for (const auto &extr_filaments : extruders_filaments)
            filament_colors.push_back(
                extr_filaments.get_selected_preset()->config.opt_string("filament_colour", (unsigned) 0));
...
void Plater::force_filament_cb_update()
{
    ...
    tab->select_preset(
        wxGetApp().preset_bundle->extruders_filaments[tab->get_active_extruder()].get_selected_preset_name());
}
...
std::vector<std::string> Plater::get_extruder_color_strings_from_plater_config(
    const GCodeProcessorResult *const result) const
{
...
void Plater::arrange(bool current_bed_only)
{
    ...
    if (current_bed_only)
        mode = wxGetKeyState(WXK_SHIFT) ? ArrangeSelectionMode::CurrentBedSelectionOnly
    ...
    if (p->can_arrange())
    {
        if (sequential)
        {
            try
            {
                replace_job(this->get_ui_job_worker(),
                            std::make_unique<SeqArrangeJob>(this->model(), *p->config, current_bed_only));
            }
            catch (const ExceptionCannotAttemptSeqArrange &)
            {
                ErrorDialog dlg(
                    this,
                    _L("Sequential arrange for a single bed is only allowed when all instances of the affected objects are on the same bed."),
                    false);
                dlg.ShowModal();
            }
        }
...
const DynamicPrintConfig *Plater::config() const
{
    return p->config;
}
...
void Plater::force_invalidate_slice()
{
    // UNIFIED SOLUTION FOR PREFLIGHT FEATURES
    // This method invalidates slicing when preFlight settings change

    // Step 1: Stop any running background process
    p->background_process.stop();

    // Step 2: Check if we have sliced data before reset
    bool had_sliced_data = p->background_process.finished();

    // Step 3: Reset the background process completely
    // This is the nuclear option but it ensures everything is clean
    p->background_process.reset();

    // Step 4: Clear preview data only if we had sliced data
    if (had_sliced_data)
    {
        ...
        // Step 6: Disable the Preview tab and reset Slice button
        if (wxGetApp().mainframe && wxGetApp().mainframe->m_modern_tabbar)
        {
            wxGetApp().mainframe->m_modern_tabbar->EnableTab(GUI::ModernTabBar::TAB_PREVIEW, false);
            wxGetApp().mainframe->m_modern_tabbar->UpdateSliceButtonState(false);
        ...
        // Step 7: Switch to 3D view if in Preview
        if (p->is_preview_shown())
        {
            p->set_current_panel(p->view3D);
            p->view_toolbar.select_item("3D");
        }

        // Step 8: Update action buttons to show "Slice now"
        p->show_action_buttons(true);
    }

    // Step 9: Re-initialize the background process with current model and config
    // This is necessary after reset() to make slicing work again
    p->update_background_process(false);

    // Step 10: Reset print status to idle so slice button works
    s_print_statuses[s_multiple_beds.get_active_bed()] = PrintStatus::idle;

    // Step 11: Schedule background process
    p->schedule_background_process();
}

void Plater::mirror(Axis axis)
{
    p->mirror(axis);
}
...
void Plater::split_volume()
{
    p->split_volume();
}
void Plater::update_menus()
{
    p->menus.update();
}
void Plater::show_action_buttons(const bool ready_to_slice) const
{
    p->show_action_buttons(ready_to_slice);
}
void Plater::show_action_buttons() const
{
    p->show_action_buttons(p->ready_to_slice);
}

void Plater::show_autoslicing_action_buttons() const
{
    p->show_autoslicing_action_buttons();
};
...
void Plater::sys_color_changed()
{
    ...
    if (wxGetApp().mainframe && wxGetApp().mainframe->m_modern_tabbar)
    {
        wxGetApp().mainframe->m_modern_tabbar->sys_color_changed();
    }
...
void Plater::init_environment_texture()
{
    if (p->environment_texture.get_id() == 0)
        p->environment_texture.load_from_file(resources_dir() + "/icons/Pmetal_001.png", false,
                                              GLTexture::SingleThreaded, false);
}
...
bool Plater::can_delete() const
{
    return p->can_delete();
}
bool Plater::can_delete_all() const
{
    return p->can_delete_all();
}
bool Plater::can_increase_instances() const
{
    return p->can_increase_instances();
}
bool Plater::can_decrease_instances(int obj_idx /* = -1*/) const
{
    return p->can_decrease_instances(obj_idx);
}
bool Plater::can_set_instance_to_object() const
{
    return p->can_set_instance_to_object();
}
bool Plater::can_fix_through_winsdk() const
{
    return p->can_fix_through_winsdk();
}
bool Plater::can_simplify() const
{
    return p->can_simplify();
}
bool Plater::can_split_to_objects() const
{
    return p->can_split_to_objects();
}
bool Plater::can_split_to_volumes() const
{
    return p->can_split_to_volumes();
}
bool Plater::can_arrange() const
{
    return p->can_arrange();
}
bool Plater::can_layers_editing() const
{
    return p->can_layers_editing();
}
...
bool Plater::can_reload_from_disk() const
{
    return p->can_reload_from_disk();
}
bool Plater::can_replace_with_stl() const
{
    return p->can_replace_with_stl();
}
bool Plater::can_mirror() const
{
    return p->can_mirror();
}
bool Plater::can_split(bool to_objects) const
{
    return p->can_split(to_objects);
}
bool Plater::can_scale_to_print_volume() const
{
    return p->can_scale_to_print_volume();
}

const UndoRedo::Stack &Plater::undo_redo_stack_main() const
{
    return p->undo_redo_stack_main();
}
void Plater::clear_undo_redo_stack_main()
{
    p->undo_redo_stack_main().clear();
}
void Plater::enter_gizmos_stack()
{
    p->enter_gizmos_stack();
}
void Plater::leave_gizmos_stack()
{
    p->leave_gizmos_stack();
}
bool Plater::inside_snapshot_capture()
{
    return p->inside_snapshot_capture();
}
...
{
    return p->menus.object_menu();
}
wxMenu *Plater::part_menu()
{
    return p->menus.part_menu();
}
wxMenu *Plater::text_part_menu()
{
    return p->menus.text_part_menu();
}
wxMenu *Plater::svg_part_menu()
{
    return p->menus.svg_part_menu();
}
wxMenu *Plater::sla_object_menu()
{
    return p->menus.sla_object_menu();
}
wxMenu *Plater::default_menu()
{
    return p->menus.default_menu();
}
wxMenu *Plater::instance_menu()
{
    return p->menus.instance_menu();
}
wxMenu *Plater::layer_menu()
{
    return p->menus.layer_menu();
}
wxMenu *Plater::multi_selection_menu()
{
    return p->menus.multi_selection_menu();
}

Print &Plater::active_fff_print()
{
    return *p->fff_prints[s_multiple_beds.get_active_bed()];
}
...
PlaterAfterLoadAutoArrange::PlaterAfterLoadAutoArrange()
{
    ...
    m_enabled = plater->model().objects.empty() && plater->printer_technology() == ptFFF &&
