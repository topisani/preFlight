...
// Fix for combo box appearance issues
...
{
    // Fix for preset selection issues
...
void PresetComboBox::update(std::string select_preset_name)
{
    ...
    const ExtruderFilaments *extruder_filaments = m_preset_bundle->extruders_filaments.empty()
                                                      ? nullptr
                                                      : &m_preset_bundle->extruders_filaments[m_extruder_idx];
    ...
    for (size_t i = presets.front().is_visible ? 0 : m_collection->num_default_presets(); i < presets.size(); ++i)
    {
        ...
        const bool is_compatible = m_type == Preset::TYPE_FILAMENT && extruder_filaments
        ...
        std::string main_icon_name = m_type == Preset::TYPE_PRINTER && preset.printer_technology() == ptSLA
                                         ? "sla_printer"
                                         : m_main_bitmap_name;

        auto bmp = get_bmp(bitmap_key, main_icon_name, "lock_closed", is_enabled, is_compatible,
                           preset.is_system || preset.is_default);
        assert(bmp);
        ...
        if (!is_enabled)
        {
            ...
            if (preset.is_dirty && m_show_modif_preset_separately)
                incomp_presets.push_back(
                    {get_preset_name_with_suffix(preset), get_preset_name_with_suffix(preset).Lower(), bmp, false});
        }
...
wxBitmapBundle *PresetComboBox::get_bmp(std::string bitmap_key, bool wide_icons, const std::string &main_icon_name,
                                        ...
                                        bool is_single_bar /* = false*/, const std::string &filament_rgb /* = ""*/,
                                        ...
                                        const std::string &material_rgb /* = ""*/)
{
    ...
    if (bmp_bndl == nullptr)
    {
        ...
        if (wide_icons)
            ...
            bmps.emplace_back(is_compatible ? get_empty_bmp_bundle(norm_icon_width, icon_height)
        ...
        if (m_type == Preset::TYPE_FILAMENT && !filament_rgb.empty())
        {
            ...
            bmps.emplace_back(
                get_solid_bmp_bundle(is_single_bar ? wide_icon_width : norm_icon_width, icon_height, filament_rgb));
            if (!is_single_bar)
        ...
        bmps.emplace_back(is_system ? get_bmp_bundle("lock_closed")
...
wxBitmapBundle *PresetComboBox::get_bmp(std::string bitmap_key, const std::string &main_icon_name,
...
bool PresetComboBox::selection_is_changed_according_to_physical_printers()
{
    ...
    if (old_printer_preset == preset_name)
    {
        ...
        if (TabPrinter *printer_tab = dynamic_cast<TabPrinter *>(tab))
            printer_tab->update_rrf_retrieve_btn_state();
...
void PlaterPresetComboBox::OnSelect(wxCommandEvent &evt)
{
    ...
    else if (marker == LABEL_ITEM_PHYSICAL_PRINTER || m_last_selected != selected_item ||
...
void PlaterPresetComboBox::show_add_menu()
{
    ...
    append_menu_item(
        ...
        [](wxCommandEvent &) { wxTheApp->CallAfter([]() { run_wizard(ConfigWizard::SP_PRINTERS); }); }, "edit_uni",
...
void PlaterPresetComboBox::show_edit_menu()
{
    ...
    {
        append_menu_item(
            menu, wxID_ANY, _L("Edit physical printer"), "", [this](wxCommandEvent &)
            { this->edit_physical_printer(); }, "cog", menu, []() { return true; }, wxGetApp().plater());
        ...
        {
            append_menu_item(
                menu, wxID_ANY, _L("Open the physical printer URL"), "",
                [this](wxCommandEvent &) { this->open_physical_printer_url(); }, "open_browser", menu,
                []() { return true; }, wxGetApp().plater());
        }

        append_menu_item(
            menu, wxID_ANY, _L("Delete physical printer"), "", [this](wxCommandEvent &)
            { this->del_physical_printer(); }, "cross", menu, []() { return true; }, wxGetApp().plater());
    }
    else
        append_menu_item(
            menu, wxID_ANY, _L("Add/Remove presets"), "",
...
void PlaterPresetComboBox::update()
{
    ...
    const ExtruderFilaments &extruder_filaments =
    ...
    bool wide_icons = m_type == Preset::TYPE_FILAMENT ? extruder_filaments.get_selected_filament() &&
                                                            !extruder_filaments.get_selected_filament()->is_compatible
                                                      : m_collection->get_selected_idx() != size_t(-1) &&
                                                            !m_collection->get_selected_preset().is_compatible;
    ...
    for (size_t i = presets.front().is_visible ? 0 : m_collection->num_default_presets(); i < presets.size(); ++i)
    {
        ...
        std::string bitmap_type_name = bitmap_key = m_type == Preset::TYPE_PRINTER &&
                                                            preset.printer_technology() == ptSLA
                                                        ? "sla_printer"
                                                        : m_main_bitmap_name;
        ...
        {
            material_rgb = is_selected
                               ? m_preset_bundle->sla_materials.get_edited_preset().config.opt_string("material_colour")
                               : preset.config.opt_string("material_colour");
            if (material_rgb.empty())
                material_rgb = print_config_def.get("material_colour")->get_default_value<ConfigOptionString>()->value;
        }

        auto bmp = get_bmp(bitmap_key, wide_icons, bitmap_type_name, is_compatible,
                           preset.is_system || preset.is_default, single_bar, filament_rgb, extruder_rgb, material_rgb);
        assert(bmp);
            ...
            for (PhysicalPrinterCollection::ConstIterator it = ph_printers.begin(); it != ph_printers.end(); ++it)
            {
                for (const std::string &preset_name : it->get_preset_names())
                {
                    ...
                    preset_data.push_back({wxString::FromUTF8(it->get_full_name(preset_name)).Lower(), preset_name,
                                           it->get_full_name(preset_name), is_selected});
                }
            }
            std::sort(preset_data.begin(), preset_data.end(),
                      [](const PhysicalPrinterPresetData &a, const PhysicalPrinterPresetData &b)
