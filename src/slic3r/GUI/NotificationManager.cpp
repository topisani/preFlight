...
//------PopNotification--------
NotificationManager::PopNotification::PopNotification(const NotificationData &n, NotificationIDProvider &id_provider,
                                                      wxEvtHandler *evt_handler, const bool multiline)
    : m_data(n)
...
void NotificationManager::PopNotification::render(GLCanvas3D &canvas, float initial_y, bool move_from_overlay,
                                                  float overlay_width)
{
    ...
    // Set window position with center anchor for X (0.5f) and top anchor for Y (0.0f)
    ImGuiPureWrap::set_next_window_pos(win_pos.x, win_pos.y, ImGuiCond_Always, 0.5f, 0.0f);
    ...
    // Since window is now centered, check from center point
    float half_width = m_window_width * 0.5f;
    if (mouse_pos.x < win_pos.x + half_width && mouse_pos.x > win_pos.x - half_width && mouse_pos.y > win_pos.y &&
    ...
    if (m_state == EState::FadingOut)
    {
        ...
        push_style_color(ImGuiCol_ButtonHovered, ImGui::GetStyleColorVec4(ImGuiCol_ButtonHovered), true,
    ...
    int flags = ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoMove |
...
void NotificationManager::PopNotification::count_lines()
{
    ...
    while (last_end < text.size() - 1)
    {
        ...
        if (next_hard_end != std::string::npos &&
            ImGui::CalcTextSize(text.substr(last_end, next_hard_end - last_end).c_str()).x <
            ...
            if (ImGui::CalcTextSize(text.substr(last_end).c_str()).x >= m_window_width - m_window_width_offset)
            {
                        ...
                        (m_window_width - m_window_width_offset) / 4 * 3)
                {
                    ...
                    while (last_end + letter_count < text.size() &&
                           ImGui::CalcTextSize(text.substr(last_end, letter_count).c_str()).x <
                               m_window_width - m_window_width_offset)
                    {
	...
	*/
    ...
    if (!m_text2.empty())
    {
        ...
        if (size_of_last_line >=
            m_window_width - m_window_width_offset - ImGui::CalcTextSize(text.substr(0, first_end).c_str()).x)
        {
        ...
        while (last_end < text.size() - 1)
        {
            ...
            {
                // find next suitable endline
                if (ImGui::CalcTextSize(text.substr(last_end).c_str()).x >=
                    m_window_width - m_window_width_offset - size_of_last_line)
                {
                    // more than one line till end
                    size_t next_space = text.find_first_of(' ', last_end);
                    ...
                    {
                        float width_of_a = ImGui::CalcTextSize("a").x;
                        int letter_count = (int) ((m_window_width - m_window_width_offset - size_of_last_line) /
                                                  width_of_a);
                        while (last_end + letter_count < text.size() &&
                               ImGui::CalcTextSize(text.substr(last_end, letter_count).c_str()).x <
                                   m_window_width - m_window_width_offset - size_of_last_line)
...
{
    m_window_height = m_multiline ? std::max(m_lines_count, m_normal_lines_count) * m_line_height
                                  : m_normal_lines_count * m_line_height;
    m_window_height += 1 * m_line_height; // top and bottom
}

void NotificationManager::PopNotification::render_text(const float win_size_x, const float win_size_y,
                                                       const float win_pos_x, const float win_pos_y)
{
    float x_offset = m_left_indentation;
    int last_end = 0;
    float starting_y = (m_lines_count == 2
                            ? win_size_y / 2 - m_line_height
    ...
    for (size_t i = 0; i < (m_multiline ? m_endlines.size() : std::min(m_endlines.size(), m_normal_lines_count)); i++)
    {
        ...
        if (m_endlines.size() > i && m_text1.size() >= m_endlines[i])
        {
            if (i == m_normal_lines_count - 1 && m_endlines.size() > m_normal_lines_count && !m_multiline)
            {
                ...
                line = m_text1.substr(m_endlines[m_normal_lines_count - 2] +
                                          (m_text1[m_endlines[m_normal_lines_count - 2]] == '\n' ||
                                                   m_text1[m_endlines[m_normal_lines_count - 2]] == ' '
                                               ? 1
                                               : 0),
                                      m_endlines[m_normal_lines_count - 1] - m_endlines[m_normal_lines_count - 2] -
                                          (m_text1[m_endlines[m_normal_lines_count - 2]] == '\n' ||
                                                   m_text1[m_endlines[m_normal_lines_count - 2]] == ' '
                                               ? 1
                                               : 0));
                while (ImGui::CalcTextSize(line.c_str()).x >
                       m_window_width - m_window_width_offset -
                           ImGui::CalcTextSize(("  [" + _u8L("More") + "]").c_str()).x)
                {
    ...
    {
        render_hypertext(x_offset + ImGui::CalcTextSize((line + " ").c_str()).x,
                         starting_y + (m_normal_lines_count - 1) * shift_y, "[" + _u8L("More") + "]", true);
    }
    else if (!m_hypertext.empty())
    {
        render_hypertext(x_offset + ImGui::CalcTextSize((line + (line.empty() ? "" : " ")).c_str()).x,
    ...
    if (!m_text2.empty() && (m_multiline || m_lines_count <= m_normal_lines_count))
    {
        ...
        for (size_t i = 0; i < (m_multiline ? m_endlines2.size() : m_normal_lines_count); i++)
        {
            if (i == 0) //first line X is shifted by hypertext
                ImGui::SetCursorPosX(x_offset +
                                     ImGui::CalcTextSize((line + m_hypertext + (line.empty() ? " " : "  ")).c_str()).x);
            else
                ImGui::SetCursorPosX(x_offset);
...
void NotificationManager::PopNotification::render_hypertext(const float text_x, const float text_y,
                                                            const std::string text, bool more)
{
    //invisible button
    ...
    //hover color
    ImVec4 primary_color = Theme::Primary::GetImGuiColor();
    if (ImGui::IsItemHovered(ImGuiHoveredFlags_RectOnly))
        primary_color.y += 0.2f;

    //text
    push_style_color(ImGuiCol_Text, primary_color, m_state == EState::FadingOut, m_current_fade_opacity);
    ...
    ImGui::GetWindowDrawList()->AddLine(
        lineStart, lineEnd,
        IM_COL32((int) (primary_color.x * 255), (int) (primary_color.y * 255), (int) (primary_color.z * 255),
                 (int) (primary_color.w * 255.f * (m_state == EState::FadingOut ? m_current_fade_opacity : 1.f))));
}

void NotificationManager::PopNotification::render_close_button(const float win_size_x, const float win_size_y,
                                                               const float win_pos_x, const float win_pos_y)
{
    ...
    if (ImGui::IsMouseHoveringRect(ImVec2(win_pos.x - win_size.x / 10.f, win_pos.y),
                                   ImVec2(win_pos.x,
                                          win_pos.y + win_size.y - (m_minimize_b_visible ? 2 * m_line_height : 0)),
                                   true))
    {
...
void NotificationManager::PopNotification::render_minimize_button(const float win_pos_x, const float win_pos_y)
{
    ...
    if (ImGui::IsMouseHoveringRect(ImVec2(win_pos_x - m_window_width / 10.f,
                                          win_pos_y + m_window_height - 2 * m_line_height + 1),
...
bool NotificationManager::PopNotification::update_state(bool paused, const int64_t delta)
{
    ...
    if (m_state == EState::FadingOut && !paused)
    {
        ...
        m_current_fade_opacity = std::clamp(1.0f - 0.001f * static_cast<float>(curr_time) / FADING_OUT_DURATION, 0.0f,
                                            1.0f);
...
void NotificationManager::ExportFinishedNotification::render_text(const float win_size_x, const float win_size_y,
                                                                  const float win_pos_x, const float win_pos_y)
{
    if (m_eject_pending)
    {
    ...
    for (size_t i = 0; i < m_lines_count; i++)
    {
        ...
        if (m_text1.size() >= m_endlines[i])
        {
            ...
            {
                render_hypertext(x_offset + ImGui::CalcTextSize(line.c_str()).x + ImGui::CalcTextSize("   ").x,
                                 starting_y, _u8L("Open Folder."));
            }
        }
    }
}

void NotificationManager::ExportFinishedNotification::render_close_button(const float win_size_x,
                                                                          const float win_size_y, const float win_pos_x,
...
void NotificationManager::ExportFinishedNotification::render_eject_button(const float win_size_x,
                                                                          const float win_size_y, const float win_pos_x,
                                                                          const float win_pos_y)
{
    ImVec2 win_size(win_size_x, win_size_y);
...
void NotificationManager::ProgressBarNotification::render_text(const float win_size_x, const float win_size_y,
                                                               const float win_pos_x, const float win_pos_y)
{
    ...
    if (m_multiline)
    {
        ...
        std::string line = m_text1.substr(
            m_endlines[0] + (m_text1[m_endlines[0]] == '\n' || m_text1[m_endlines[0]] == ' ' ? 1 : 0),
            m_endlines[1] - m_endlines[0] - (m_text1[m_endlines[0]] == '\n' || m_text1[m_endlines[0]] == ' ' ? 1 : 0));
        ImGuiPureWrap::text(line.c_str());
...
void NotificationManager::ProgressBarNotification::render_bar(const float win_size_x, const float win_size_y,
                                                              const float win_pos_x, const float win_pos_y)
{
    ImVec4 primary_color = Theme::Primary::GetImGuiColor();
    ImVec4 gray_color = ImVec4(.34f, .34f, .34f, 1.0f);
    // win_pos_x is now the center of the window, so we need to calculate from center
    float half_width = win_size_x / 2.0f;
    ImVec2 lineEnd = ImVec2(win_pos_x + half_width - m_window_width_offset,
                            win_pos_y + win_size_y / 2 + (m_multiline ? m_line_height / 2 : 0));
    ImVec2 lineStart = ImVec2(win_pos_x - half_width + m_left_indentation,
                              win_pos_y + win_size_y / 2 + (m_multiline ? m_line_height / 2 : 0));
    ...
    ImGui::GetWindowDrawList()->AddLine(lineStart, lineEnd,
                                        IM_COL32((int) (gray_color.x * 255), (int) (gray_color.y * 255),
                                                 (int) (gray_color.z * 255), (m_current_fade_opacity * 255.f)),
                                        m_line_height * 0.2f);
    ImGui::GetWindowDrawList()->AddLine(lineStart, midPoint,
                                        IM_COL32((int) (primary_color.x * 255), (int) (primary_color.y * 255),
                                                 (int) (primary_color.z * 255), (m_current_fade_opacity * 255.f)),
                                        m_line_height * 0.2f);
...
void NotificationManager::ProgressBarWithCancelNotification::render_close_button(const float win_size_x,
                                                                                 const float win_size_y,
...
void NotificationManager::ProgressBarWithCancelNotification::render_close_button_inner(const float win_size_x,
                                                                                       const float win_size_y,
                                                                                       const float win_pos_x,
                                                                                       const float win_pos_y)
{
...
void NotificationManager::ProgressBarWithCancelNotification::render_cancel_button_inner(const float win_size_x,
                                                                                        const float win_size_y,
                                                                                        const float win_pos_x,
                                                                                        const float win_pos_y)
{
...
void NotificationManager::ProgressBarWithCancelNotification::render_bar(const float win_size_x, const float win_size_y,
                                                                        const float win_pos_x, const float win_pos_y)
{
...
void NotificationManager::URLDownloadWithPrintablesLinkNotification::render_text(const float win_size_x,
                                                                                 const float win_size_y,
                                                                                 const float win_pos_x,
                                                                                 const float win_pos_y)
{
...
void NotificationManager::URLDownloadNotification::render_close_button(const float win_size_x, const float win_size_y,
                                                                       const float win_pos_x, const float win_pos_y)
{
    if (m_percentage < 0.f || m_percentage >= 1.f)
    {
...
void NotificationManager::URLDownloadNotification::render_close_button_inner(const float win_size_x,
                                                                             const float win_size_y,
                                                                             const float win_pos_x,
                                                                             const float win_pos_y)
{
...
void NotificationManager::URLDownloadNotification::render_pause_cancel_buttons_inner(const float win_size_x,
                                                                                     const float win_size_y,
                                                                                     const float win_pos_x,
                                                                                     const float win_pos_y)
{
...
void NotificationManager::URLDownloadNotification::render_pause_button_inner(const float win_size_x,
                                                                             const float win_size_y,
                                                                             const float win_pos_x,
                                                                             const float win_pos_y)
{
...
void NotificationManager::URLDownloadNotification::render_open_button_inner(const float win_size_x,
                                                                            const float win_size_y,
                                                                            const float win_pos_x,
                                                                            const float win_pos_y)
{
...
void NotificationManager::URLDownloadNotification::render_cancel_button_inner(const float win_size_x,
                                                                              const float win_size_y,
                                                                              const float win_pos_x,
                                                                              const float win_pos_y)
{
...
void NotificationManager::URLDownloadNotification::render_bar(const float win_size_x, const float win_size_y,
                                                              const float win_pos_x, const float win_pos_y)
{
...
void NotificationManager::PrintHostUploadNotification::init()
{
    ...
    if (m_state == EState::NotFading &&
        (m_uj_state == UploadJobState::PB_COMPLETED || m_uj_state == UploadJobState::PB_COMPLETED_WITH_WARNING))
        m_state = EState::Shown;
}
...
void NotificationManager::PrintHostUploadNotification::render_text(const float win_size_x, const float win_size_y,
                                                                   const float win_pos_x, const float win_pos_y)
{
...
void NotificationManager::PrintHostUploadNotification::render_bar(const float win_size_x, const float win_size_y,
                                                                  const float win_pos_x, const float win_pos_y)
{
    std::string text;
    switch (m_uj_state)
    ...
    case Slic3r::GUI::NotificationManager::PrintHostUploadNotification::UploadJobState::PB_PROGRESS:
    {
        ...
        std::stringstream stream;
        stream << std::fixed << std::setprecision(2) << (int) (m_percentage * 100) << "% - " << uploaded << " of "
               << m_file_size << "MB uploaded";
        text = stream.str();
        ImGui::SetCursorPosX(m_left_indentation);
        ImGui::SetCursorPosY(win_size_y / 2 + win_size_y / 6 - (m_multiline ? 0 : m_line_height / 4));
        break;
...
void NotificationManager::PrintHostUploadNotification::render_close_button(const float win_size_x,
                                                                           const float win_size_y,
                                                                           const float win_pos_x, const float win_pos_y)
{
    if (m_has_cancel_button)
...
void NotificationManager::PrintHostUploadNotification::render_cancel_button(const float win_size_x,
                                                                            const float win_size_y,
                                                                            const float win_pos_x,
                                                                            const float win_pos_y)
{
...
void NotificationManager::UpdatedItemsInfoNotification::add_type(InfoItemType type)
{
    ...
    for (it = m_types_and_counts.begin(); it != m_types_and_counts.end(); ++it)
    {
        ...
        case InfoItemType::CustomSupports:
            text += format(_L_PLURAL("%1$d object was loaded with custom supports.",
                                     "%1$d objects were loaded with custom supports.", (*it).second),
                           (*it).second) +
                    "\n";
            break;
        case InfoItemType::CustomSeam:
            text += format(_L_PLURAL("%1$d object was loaded with custom seam.",
                                     "%1$d objects were loaded with custom seam.", (*it).second),
                           (*it).second) +
                    "\n";
            break;
        ...
        case InfoItemType::Sinking:
            text += format(_L_PLURAL("%1$d object was loaded with partial sinking.",
                                     "%1$d objects were loaded with partial sinking.", (*it).second),
                           (*it).second) +
                    "\n";
            break;
        case InfoItemType::CutConnectors:
            text += format(_L_PLURAL("%1$d object was loaded as a part of cut object.",
                                     "%1$d objects were loaded as parts of cut object", (*it).second),
                           (*it).second) +
                    "\n";
            break;
...
bool NotificationManager::SlicingProgressNotification::set_progress_state(float percent)
{
    ...
    else if (percent >= 1.f)
    {
        ...
        // Don't auto-switch to SP_COMPLETED - wait for explicit call
        return set_progress_state(m_sp_state, percent);
    }
    else
    ...
    NotificationManager::SlicingProgressNotification::SlicingProgressState state, float percent /* = 0.f*/)
{
    ...
    case Slic3r::GUI::NotificationManager::SlicingProgressNotification::SlicingProgressState::SP_PROGRESS:
        ...
        m_sp_state = state;
        return true;
    case Slic3r::GUI::NotificationManager::SlicingProgressNotification::SlicingProgressState::SP_RENDERING:
        // Rendering state allows transition from SP_PROGRESS or SP_RENDERING
        if (m_sp_state != SlicingProgressState::SP_PROGRESS && m_sp_state != SlicingProgressState::SP_RENDERING)
            return false;
        // Accept progress percentage for rendering (0-100%)
        set_percentage(percent);
        m_has_cancel_button = false; // Can't cancel during rendering
        m_sp_state = state;
        return true;
    ...
    case Slic3r::GUI::NotificationManager::SlicingProgressNotification::SlicingProgressState::SP_COMPLETED:
        if (m_sp_state != SlicingProgressState::SP_BEGAN && m_sp_state != SlicingProgressState::SP_PROGRESS &&
            m_sp_state != SlicingProgressState::SP_RENDERING)
            return false;
        set_percentage(1);
        m_has_cancel_button = false;
        m_has_print_info = false;
...
void NotificationManager::SlicingProgressNotification::set_status_text(const std::string &text)
{
    ...
    {
        NotificationData data{NotificationType::SlicingProgress, NotificationLevel::ProgressBarNotificationLevel, 0,
                              _u8L("Slicing finished."), m_is_fff ? _u8L("Export G-Code.") : _u8L("Export.")};
        update(data);
        m_state = EState::Shown;
    }
    break;
...
void NotificationManager::SlicingProgressNotification::set_print_info(const std::string &info)
{
    if (m_sp_state != SlicingProgressState::SP_COMPLETED)
    {
        ...
        set_status_text(_u8L("Slicing finished")); // Trigger the text update for SP_COMPLETED state
    }
    else
    {
...
void NotificationManager::SlicingProgressNotification::render_text(const float win_size_x, const float win_size_y,
                                                                   const float win_pos_x, const float win_pos_y)
{
    if (m_sp_state == SlicingProgressState::SP_PROGRESS ||
        m_sp_state ==
            SlicingProgressState::
                SP_RENDERING /*|| (m_sp_state == SlicingProgressState::SP_COMPLETED && !m_sidebar_collapsed)*/)
    {
...
void NotificationManager::SlicingProgressNotification::render_bar(const float win_size_x, const float win_size_y,
                                                                  const float win_pos_x, const float win_pos_y)
{
    if (m_sp_state != SlicingProgressState::SP_PROGRESS && m_sp_state != SlicingProgressState::SP_RENDERING)
    {
...
void NotificationManager::SlicingProgressNotification::render_hypertext(const float text_x, const float text_y,
                                                                        const std::string text, bool more)
{
...
void NotificationManager::SlicingProgressNotification::render_cancel_button(const float win_size_x,
                                                                            const float win_size_y,
                                                                            const float win_pos_x,
                                                                            const float win_pos_y)
{
...
void NotificationManager::SlicingProgressNotification::render_close_button(const float win_size_x,
                                                                           const float win_size_y,
                                                                           const float win_pos_x, const float win_pos_y)
{
...
{
    NotificationData data{NotificationType::ProgressIndicator, NotificationLevel::ProgressBarNotificationLevel, 0,
                          text};
    update(data);
}
...
void NotificationManager::ProgressIndicatorNotification::render_cancel_button(const float win_size_x,
                                                                              const float win_size_y,
                                                                              const float win_pos_x,
                                                                              const float win_pos_y)
{
...
void NotificationManager::ProgressIndicatorNotification::render_close_button(const float win_size_x,
                                                                             const float win_size_y,
                                                                             const float win_pos_x,
                                                                             const float win_pos_y)
{
...
{
    push_notification_data({NotificationType::CustomNotification, NotificationLevel::RegularNotificationLevel, 10,
                            text},
                           timestamp);
}

void NotificationManager::push_notification(NotificationType type, NotificationLevel level, const std::string &text,
                                            const std::string &hypertext, std::function<bool(wxEvtHandler *)> callback,
                                            const std::string &text_after, const int timestamp, const bool multiline)
{
...
void NotificationManager::push_delayed_notification(const NotificationType type,
                                                    std::function<bool(void)> condition_callback, int64_t initial_delay,
                                                    int64_t delay_interval)
{
    ...
    assert(it != std::end(basic_notifications));
    if (it != std::end(basic_notifications))
        push_delayed_notification_data(std::make_unique<PopNotification>(*it, m_id_provider, m_evt_handler),
                                       condition_callback, initial_delay, delay_interval);
}
...
{
    push_notification_data({NotificationType::PlaterError, NotificationLevel::ErrorNotificationLevel, 0,
                            _u8L("ERROR:") + "\n" + text},
                           0);
}
...
void NotificationManager::close_plater_error_notification(const std::string &text)
{
    for (std::unique_ptr<PopNotification> &notification : m_pop_notifications)
    {
        if (notification->get_type() == NotificationType::PlaterError &&
            notification->compare_text(_u8L("ERROR:") + "\n" + text))
        {
            notification->close();
        }
    }
...
void NotificationManager::push_plater_warning_notification(const std::string &text)
{
    ...
    NotificationData data{NotificationType::PlaterWarning, NotificationLevel::WarningNotificationLevel, 0,
                          _u8L("WARNING:") + "\n" + text};

    auto notification = std::make_unique<NotificationManager::PlaterWarningNotification>(data, m_id_provider,
                                                                                         m_evt_handler);
...
void NotificationManager::push_simplify_suggestion_notification(
    const std::string &text, ObjectID object_id, const std::string &hypertext /* = ""*/,
...
void NotificationManager::push_version_notification(NotificationType type, NotificationLevel level,
                                                    const std::string &text, const std::string &hypertext,
                                                    std::function<bool(wxEvtHandler *)> callback)
{
    assert(type == NotificationType::NewAlphaAvailable || type == NotificationType::NewBetaAvailable ||
    ...
    for (std::unique_ptr<PopNotification> &notification : m_pop_notifications)
    {
        ...
        if (notification->get_type() == NotificationType::NoNewReleaseAvailable &&
            (type == NotificationType::NewAlphaAvailable || type == NotificationType::NewBetaAvailable))
        {
            notification->close();
        }
    }
...
void NotificationManager::push_exporting_finished_notification(const std::string &path, const std::string &dir_path,
                                                               bool on_removable)
{
    close_notification_of_type(NotificationType::ExportFinished);
    NotificationData data{NotificationType::ExportFinished, NotificationLevel::RegularNotificationLevel,
                          on_removable ? 0 : 20, _u8L("Exporting finished.") + "\n" + path};
    push_notification_data(std::make_unique<NotificationManager::ExportFinishedNotification>(data, m_id_provider,
                                                                                             m_evt_handler,
                                                                                             on_removable, dir_path),
                           0);
    set_slicing_progress_hidden();
}
...
void NotificationManager::push_upload_job_notification(int id, float filesize, const std::string &filename,
                                                       const std::string &host, float percentage)
{
    ...
    push_notification_data(std::make_unique<NotificationManager::PrintHostUploadNotification>(data, m_id_provider,
                                                                                              m_evt_handler, 0, id,
                                                                                              filesize, filename, host),
                           0);
}
void NotificationManager::set_upload_job_notification_percentage(int id, const std::string &filename,
                                                                 const std::string &host, float percentage)
{
    for (std::unique_ptr<PopNotification> &notification : m_pop_notifications)
    {
...
void NotificationManager::upload_job_notification_show_canceled(int id, const std::string &filename,
...
void NotificationManager::upload_job_notification_show_error(int id, const std::string &filename,
                                                             const std::string &host)
{
    for (std::unique_ptr<PopNotification> &notification : m_pop_notifications)
    {
                                                              ...
                                                              std::function<bool()> cancel_callback)
{
    ...
    push_notification_data(std::make_unique<NotificationManager::ProgressBarWithCancelNotification>(data, m_id_provider,
                                                                                                    m_evt_handler,
                                                                                                    cancel_callback),
                           0);
}
...
void NotificationManager::push_download_URL_progress_notification(
    size_t id, const std::string &text, std::function<bool(DownloaderUserAction, int)> user_action_callback)
{
    // If already exists
    for (std::unique_ptr<PopNotification> &notification : m_pop_notifications)
    {
    ...
    // push new one
    NotificationData data{NotificationType::URLDownload, NotificationLevel::ProgressBarNotificationLevel, 5,
                          _u8L("Download") + ": " + text};
    push_notification_data(std::make_unique<NotificationManager::URLDownloadNotification>(data, m_id_provider,
                                                                                          m_evt_handler, id,
                                                                                          user_action_callback),
                           0);
}

void NotificationManager::push_download_URL_progress_notification_with_printables_link(
    size_t id, const std::string &text, const std::string &url,
    std::function<bool(DownloaderUserAction, int)> user_action_callback,
    std::function<void(std::string)> hypertext_callback)
{
    // If already exists
    for (std::unique_ptr<PopNotification> &notification : m_pop_notifications)
    {
    ...
    // push new one
    NotificationData data{NotificationType::URLDownload, NotificationLevel::ProgressBarNotificationLevel, 30,
                          _u8L("Download") + ": " + text, url};
    push_notification_data(std::make_unique<NotificationManager::URLDownloadWithPrintablesLinkNotification>(
                               data, m_id_provider, m_evt_handler, id, user_action_callback, hypertext_callback),
                           0);
}
    ...
    NotificationData data{NotificationType::SlicingProgress,
                          ...
                          [](wxEvtHandler *evnthndlr)
                          {
                              if (evnthndlr != nullptr)
                                  wxPostEvent(evnthndlr, ExportGcodeNotificationClickedEvent(
                                                             EVT_EXPORT_GCODE_NOTIFICAION_CLICKED));
                              return true;
                          }};
    push_notification_data(std::make_unique<NotificationManager::SlicingProgressNotification>(data, m_id_provider,
                                                                                              m_evt_handler,
                                                                                              cancel_callback),
                           0);
}
...
void NotificationManager::set_slicing_progress_rendering()
{
    for (std::unique_ptr<PopNotification> &notification : m_pop_notifications)
    {
        if (notification->get_type() == NotificationType::SlicingProgress)
        {
            SlicingProgressNotification *spn = dynamic_cast<SlicingProgressNotification *>(notification.get());
            spn->set_progress_state(SlicingProgressNotification::SlicingProgressState::SP_RENDERING, 0.0f);
            spn->set_status_text(_u8L("Rendering"));
            wxGetApp().plater()->get_current_canvas3D()->schedule_extra_frame(0);
            return;
        }
    }
...
void NotificationManager::set_rendering_progress(float percentage)
{
    for (std::unique_ptr<PopNotification> &notification : m_pop_notifications)
    {
        if (notification->get_type() == NotificationType::SlicingProgress)
        {
            SlicingProgressNotification *spn = dynamic_cast<SlicingProgressNotification *>(notification.get());
            spn->set_progress_state(SlicingProgressNotification::SlicingProgressState::SP_RENDERING, percentage);
...
void NotificationManager::init_progress_indicator()
{
    ...
    auto notification = std::make_unique<NotificationManager::ProgressIndicatorNotification>(data, m_id_provider,
                                                                                             m_evt_handler);
    push_notification_data(std::move(notification), 0);
}
...
void NotificationManager::push_hint_notification(bool open_next)
{
    ...
    if (!open_next)
    {
        push_notification_data(std::make_unique<NotificationManager::HintNotification>(data, m_id_provider,
                                                                                       m_evt_handler, open_next),
                               0);
    ...
    else
    {
        ...
        push_delayed_notification_data(std::make_unique<NotificationManager::HintNotification>(data, m_id_provider,
                                                                                               m_evt_handler,
                                                                                               open_next),
                                       condition, 500, 30000);
    }
...
void NotificationManager::push_updated_item_info_notification(InfoItemType type)
{
    ...
    auto notification = std::make_unique<NotificationManager::UpdatedItemsInfoNotification>(data, m_id_provider,
                                                                                            m_evt_handler, type);
    if (push_notification_data(std::move(notification), 0))
    {
...
{
    return push_notification_data(std::make_unique<PopNotification>(notification_data, m_id_provider, m_evt_handler,
                                                                    multiline),
                                  timestamp);
}
bool NotificationManager::push_notification_data(std::unique_ptr<NotificationManager::PopNotification> notification,
                                                 int timestamp)
{
    ...
    {
        if (m_initialized)
        { // ignore update action - it cant be initialized if canvas and imgui context is not ready
...
void NotificationManager::push_delayed_notification_data(
    std::unique_ptr<NotificationManager::PopNotification> notification, std::function<bool(void)> condition_callback,
    int64_t initial_delay, int64_t delay_interval)
{
    ...
    m_waiting_notifications.emplace_back(std::move(notification), condition_callback,
                                         initial_delay == 0 ? delay_interval : initial_delay, delay_interval);
    wxGetApp().plater()->get_current_canvas3D()->schedule_extra_frame(initial_delay == 0 ? delay_interval
                                                                                         : initial_delay);
}
...
bool NotificationManager::update_notifications(GLCanvas3D &canvas)
{
    ...
    for (auto it = m_waiting_notifications.begin(); it != m_waiting_notifications.end();)
    {
        ...
        if ((*it).remaining_time <= 0)
        {
            if ((*it).notification && (*it).condition_callback())
            { // push notification, erase it from waiting list (frame is scheduled by push)
