...
ObjectLayers::ObjectLayers(wxWindow *parent) : OG_Settings(parent, true)
{
    ...
    {
        auto temp = new wxStaticText(m_parent, wxID_ANY, _(col), wxDefaultPosition, wxDefaultSize,
...
wxSizer *ObjectLayers::create_layer(const t_layer_height_range &range, PlusMinusButton *delete_button,
                                    PlusMinusButton *add_button)
{
    ...
    auto editor = new LayerRangeEditor(this, double_to_string(range.first), etMinZ, set_focus_data,
                                       [range, update_focus_data, this, delete_button,
                                        add_button](coordf_t min_z, bool enter_pressed, bool dont_update_ui)
                                       {
    ...
    editor = new LayerRangeEditor(this, double_to_string(range.second), etMaxZ, set_focus_data,
                                  [range, update_focus_data, this, delete_button,
                                   add_button](coordf_t max_z, bool enter_pressed, bool dont_update_ui)
                                  {
    ...
    editor = new LayerRangeEditor(this,
                                  double_to_string(
                                      m_object->layer_config_ranges[range].option("layer_height")->getFloat()),
                                  etLayerHeight, set_focus_data, [range](coordf_t layer_height, bool, bool)
...
void ObjectLayers::create_layers_list()
{
    for (const auto &layer : m_object->layer_config_ranges)
    {
        ...
        add_btn->Bind(wxEVT_BUTTON,
                      [add_btn](wxEvent &) { wxGetApp().obj_list()->add_layer_range_after_current(add_btn->range); });
    }
}
...
void ObjectLayers::sys_color_changed()
{
    ...
    for (int i = 0; i < cells_cnt; ++i)
    {
        const wxSizerItem *item = m_grid_sizer->GetItem(i);
        if (item->IsSizer())
        { // case when we have editor with buttons
            for (size_t btn : {2, 3})
            { // del_btn, add_btn
                wxSizerItem *b_item = item->GetSizer()->GetItem(btn);
...
LayerRangeEditor::LayerRangeEditor(ObjectLayers *parent, const wxString &value, EditorType type,
    ...
    , wxTextCtrl(parent->m_parent, wxID_ANY, value, wxDefaultPosition,
                 wxSize(8 * em_unit(parent->m_parent), wxDefaultCoord),
                 wxTE_PROCESS_ENTER
#ifdef _WIN32
                     | wxBORDER_SIMPLE
#endif
             ...
             * */
                ...
                if (m_type & etLayerHeight)
                {
                    if (!edit_fn(get_value(), false,
                                 dynamic_cast<ObjectLayers::PlusMinusButton *>(e.GetWindow()) != nullptr))
                        SetValue(m_valid_value);
                    else
                        m_valid_value = double_to_string(get_value());
                    e.Skip();
