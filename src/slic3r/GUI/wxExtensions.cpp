...
wxMenuItem *append_menu_item(wxMenu *menu, int id, const wxString &string, const wxString &description,
                             std::function<void(wxCommandEvent &event)> cb, wxBitmapBundle *icon,
                             wxEvtHandler *event_handler, std::function<bool()> const cb_condition, wxWindow *parent,
                             int insert_pos /* = wxNOT_FOUND*/)
{
    ...
    if (wxAcceleratorEntry *entry = wxAcceleratorEntry::Create(string))
    {
        static const std::set<int> special_keys = {
            WXK_PAGEUP, WXK_PAGEDOWN, WXK_NUMPAD_PAGEDOWN, WXK_END,    WXK_HOME, WXK_LEFT, WXK_UP,
            WXK_RIGHT,  WXK_DOWN,     WXK_INSERT,          WXK_DELETE,
        };
    ...
    {
        parent->Bind(
            wxEVT_UPDATE_UI, [cb_condition, item, parent](wxUpdateUIEvent &evt)
...
wxMenuItem *append_submenu(wxMenu *menu, wxMenu *sub_menu, int id, const wxString &string, const wxString &description,
                           const std::string &icon, std::function<bool()> const cb_condition, wxWindow *parent)
{
    if (id == wxID_ANY)
        id = wxNewId();
...
wxMenuItem *append_menu_check_item(wxMenu *menu, int id, const wxString &string, const wxString &description,
                                   std::function<void(wxCommandEvent &event)> cb, wxEvtHandler *event_handler,
                                   std::function<bool()> const enable_condition,
                                   std::function<bool()> const check_condition, wxWindow *parent)
{
    if (id == wxID_ANY)
        id = wxNewId();
    ...
    if (parent)
        parent->Bind(
            wxEVT_UPDATE_UI,
            [enable_condition, check_condition](wxUpdateUIEvent &evt)
            {
...
void apply_extruder_selector(Slic3r::GUI::BitmapComboBox **ctrl, wxWindow *parent,
                             const std::string &first_item /* = ""*/, wxPoint pos /* = wxDefaultPosition*/,
                             wxSize size /* = wxDefaultSize*/, bool use_thin_icon /* = false*/)
{
    ...
    for (wxBitmapBundle *bmp : icons)
    {
        ...
        (*ctrl)->Append(use_full_item_name ? Slic3r::GUI::from_u8((boost::format("%1% %2%") % str % i).str())
                                           : wxString::Format("%d", i),
                        *bmp);
        ++i;
    }
...
LockButton::LockButton(wxWindow *parent, wxWindowID id, const wxPoint &pos /*= wxDefaultPosition*/,
...
// preFlightBitmap
// ----------------------------------------------------------------------------
ScalableBitmap::ScalableBitmap(wxWindow *parent, const std::string &icon_name, const int width /* = 16*/,
                               const int height /* = -1*/, const bool grayscale /* = false*/)
    : m_parent(parent), m_icon_name(icon_name), m_bmp_width(width), m_bmp_height(height)
{
    m_bmp = *get_bmp_bundle(icon_name, width, height);
    m_bitmap = m_bmp.GetBitmapFor(m_parent);
}

ScalableBitmap::ScalableBitmap(wxWindow *parent, const std::string &icon_name, const wxSize icon_size,
    ...
    : m_parent(parent), m_bmp_width(icon_size.x), m_bmp_height(icon_size.y)
{
    ...
    if (ext == ".png" || ext == ".jpg")
    {
        ...
        if (wxSize sz = bitmap.GetSize(); sz.x != sz.y)
        {
            ...
            wxRect rc = sz.GetWidth() > sz.GetHeight() ? wxRect(int(0.5 * (sz.x - sz.y)), 0, bmp_side, bmp_side)
...
ScalableButton::ScalableButton(wxWindow *parent, wxWindowID id, const std::string &icon_name /*= ""*/,
                               const wxString &label /* = wxEmptyString*/, const wxSize &size /* = wxDefaultSize*/,
                               const wxPoint &pos /* = wxDefaultPosition*/,
                               long style /*= wxBU_EXACTFIT | wxNO_BORDER*/, int width /* = 16*/, int height /* = -1*/)
    : m_parent(parent)
...
ScalableButton::ScalableButton(wxWindow *parent, wxWindowID id, const ScalableBitmap &bitmap,
...
BlinkingBitmap::BlinkingBitmap(wxWindow *parent, const std::string &icon_name)
    : wxStaticBitmap(parent, wxID_ANY, wxNullBitmap, wxDefaultPosition,
                     wxSize(int(1.6 * Slic3r::GUI::wxGetApp().em_unit()), -1))
{
...
{
    owner->Bind(wxEVT_TIMER, [this](wxTimerEvent &) { blink(); });
}
...
void HighlighterForWx::init(const std::vector<std::pair<OG_CustomCtrl *, bool *>> &blinking_custom_ctrls_params)
{
    ...
    const bool input_failed = blinking_custom_ctrls_params.empty() ||
                              std::any_of(blinking_custom_ctrls_params.cbegin(), blinking_custom_ctrls_params.cend(),
                                          [](auto &params)
                                          { return params.first == nullptr || params.second == nullptr; });
