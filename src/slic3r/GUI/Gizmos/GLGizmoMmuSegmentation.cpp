...
static inline void show_notification_extruders_limit_exceeded()
{
    wxGetApp().plater()->get_notification_manager()->push_notification(
        ...
        GUI::format(
            _L("Your printer has more extruders than the multi-material painting gizmo supports. For this reason, only the "
...
void GLGizmoMmuSegmentation::data_changed(bool is_serializing)
{
    ...
    if (int prev_extruders_count = int(m_original_extruders_colors.size());
        prev_extruders_count != wxGetApp().extruders_edited_cnt() ||
        wxGetApp().plater()->get_extruder_colors_from_plater_config() != m_original_extruders_colors)
    {
        if (wxGetApp().extruders_edited_cnt() > int(GLGizmoMmuSegmentation::EXTRUDERS_LIMIT))
            show_notification_extruders_limit_exceeded();
...
static void render_extruders_combo(const std::string &label, const std::vector<std::string> &extruders,
                                   const std::vector<ColorRGBA> &extruders_colors, size_t &selection_idx)
{
    ...
    if (ImGui::BeginCombo(label.c_str(), ""))
    {
        for (size_t extruder_idx = 0;
             extruder_idx < std::min(extruders.size(), GLGizmoMmuSegmentation::EXTRUDERS_LIMIT); ++extruder_idx)
        {
            ...
            ImGui::GetWindowDrawList()->AddRectFilled(start_position,
                                                      ImVec2(start_position.x + height + height / 2,
                                                             start_position.y + height),
                                                      ImGuiPSWrap::to_ImU32(extruders_colors[extruder_idx]));
            ImGui::GetWindowDrawList()->AddRect(start_position,
                                                ImVec2(start_position.x + height + height / 2,
                                                       start_position.y + height),
                                                IM_COL32_BLACK);

            ImGui::SetCursorScreenPos(
                ImVec2(start_position.x + height + height / 2 + style.FramePadding.x, start_position.y));
    ...
    ImGui::GetWindowDrawList()->AddRectFilled(p, ImVec2(p.x + height + height / 2, p.y + height),
                                              ImGuiPSWrap::to_ImU32(extruders_colors[selection_idx]));
...
void GLGizmoMmuSegmentation::on_render_input_window(float x, float y, float bottom_limit)
{
    ...
    if (m_popup_render_count == 0)
    {
        // First pass: render far off-screen to measure (ensure it's not visible)
        ImGuiPureWrap::set_next_window_pos(-10000.0f, -10000.0f, ImGuiCond_Always);
        // Request extra frame immediately
    ...
    else
    {
        ...
        ImGuiPureWrap::set_next_window_pos(menu_x, menu_y, ImGuiCond_Always, 1.0f, 0.0f);
    }
    m_popup_render_count++;

    ImGuiPureWrap::begin(get_name(),
    ...
    const float smart_fill_slider_left = ImGuiPureWrap::calc_text_size(m_desc.at("smart_fill_angle")).x +
                                         m_imgui->scaled(1.f);
    const float bucket_fill_slider_left = ImGuiPureWrap::calc_text_size(m_desc.at("bucket_fill_angle")).x +
                                          m_imgui->scaled(1.f);
    const float height_range_slider_left = ImGuiPureWrap::calc_text_size(m_desc.at("height_range_z_range")).x +
                                           m_imgui->scaled(1.f);
    ...
    const float combo_label_width = std::max(ImGuiPureWrap::calc_text_size(m_desc.at("first_color")).x,
                                             ImGuiPureWrap::calc_text_size(m_desc.at("second_color")).x) +
                                    m_imgui->scaled(1.f);

    const float tool_type_radio_brush = ImGuiPureWrap::calc_text_size(m_desc["tool_brush"]).x + m_imgui->scaled(2.5f);
    const float tool_type_radio_bucket_fill = ImGuiPureWrap::calc_text_size(m_desc["tool_bucket_fill"]).x +
                                              m_imgui->scaled(2.5f);
    const float tool_type_radio_smart_fill = ImGuiPureWrap::calc_text_size(m_desc["tool_smart_fill"]).x +
                                             m_imgui->scaled(2.5f);
    const float tool_type_radio_height_range = ImGuiPureWrap::calc_text_size(m_desc["tool_height_range"]).x +
                                               m_imgui->scaled(2.5f);

    const float tool_type_radio_first_line = tool_type_radio_brush + tool_type_radio_bucket_fill +
                                             tool_type_radio_smart_fill;
    ...
    const float split_triangles_checkbox_width = ImGuiPureWrap::calc_text_size(m_desc["split_triangles"]).x +
                                                 m_imgui->scaled(2.5f);
    ...
    const float sliders_left_width = std::max({smart_fill_slider_left, bucket_fill_slider_left, cursor_slider_left,
                                               clipping_slider_left, height_range_slider_left});
    ...
    render_extruders_combo("##first_color_combo", m_original_extruders_names, m_original_extruders_colors,
    ...
    if (ImGui::ColorEdit4(first_label.c_str(), (float *) &first_color,
                          ImGuiColorEditFlags_NoAlpha | ImGuiColorEditFlags_NoInputs | ImGuiColorEditFlags_NoLabel,
    ...
    ImGui::SameLine(combo_label_width);
    ImGui::PushItemWidth(window_width - combo_label_width - color_button_width);
    render_extruders_combo("##second_color_combo", m_original_extruders_names, m_original_extruders_colors,
                           m_second_selected_extruder_idx);
    ImGui::SameLine();
    ...
    if (ImGui::ColorEdit4(second_label.c_str(), (float *) &second_color,
                          ImGuiColorEditFlags_NoAlpha | ImGuiColorEditFlags_NoInputs | ImGuiColorEditFlags_NoLabel,
    ...
    if (ImGui::IsItemHovered())
        ImGuiPureWrap::tooltip(_u8L("Paints neighboring facets whose relative angle is less or equal to set angle."),
                               max_tooltip_width);
    ...
    const float tool_type_second_line_offset = (window_width - tool_type_radio_second_line + m_imgui->scaled(1.5f)) /
                                               2.f;
    ...
    if (m_tool_type == ToolType::BRUSH)
    {
        ...
        float cursor_type_offset = (window_width - cursor_type_radio_sphere - cursor_type_radio_circle -
                                    cursor_type_radio_pointer + m_imgui->scaled(1.5f)) /
                                   2.f;
        ...
        if (ImGui::IsItemHovered())
            ImGuiPureWrap::tooltip(_u8L("Paints all facets inside, regardless of their orientation."),
                                   max_tooltip_width);
        ...
        if (ImGui::IsItemHovered())
            ImGuiPureWrap::tooltip(_u8L("Split bigger facets into smaller ones while the object is painted."),
                                   max_tooltip_width);
    ...
    else if (m_tool_type == ToolType::SMART_FILL || m_tool_type == ToolType::BUCKET_FILL)
    {
        ImGui::AlignTextToFramePadding();
        ImGuiPureWrap::text(
            (m_tool_type == ToolType::SMART_FILL ? m_desc["smart_fill_angle"] : m_desc["bucket_fill_angle"]) + ":");
        std::string format_str_angle = std::string("%.f") +
                                       I18N::translate_utf8("°",
                                                            "Degree sign to use in the respective slider in MMU gizmo,"
        ...
        if (m_imgui->slider_float("##fill_angle", &fill_angle, SmartFillAngleMin, SmartFillAngleMax,
                                  format_str_angle.data(), 1.0f, true, _L("Alt + Mouse wheel")))
        {
            for (auto &triangle_selector : m_triangle_selectors)
            {
                triangle_selector->seed_fill_unselect_all_triangles();
    ...
    else if (m_tool_type == ToolType::HEIGHT_RANGE)
    {
        ...
        std::string format_str_angle =
            std::string("%.2f ") +
            I18N::translate_utf8("mm",
                                 "Millimeter sign to use in the respective slider in multi-material painting gizmo,"
        ...
        if (m_imgui->slider_float("##height_range_z_range", &m_height_range_z_range, HeightRangeZRangeMin,
                                  HeightRangeZRangeMax, format_str_angle.data(), 1.0f, true, _L("Alt + Mouse wheel")))
        {
            for (auto &triangle_selector : m_triangle_selectors)
            {
                triangle_selector->seed_fill_unselect_all_triangles();
...
void GLGizmoMmuSegmentation::init_model_triangle_selectors()
{
    ...
    for (const ModelVolume *mv : mo->volumes)
    {
        ...
        m_triangle_selectors.emplace_back(
            std::make_unique<TriangleSelectorMmGui>(*mesh, m_modified_extruders_colors,
                                                    m_original_extruders_colors[extruder_idx]));
...
wxString GLGizmoMmuSegmentation::handle_snapshot_action_name(bool control_down,
                                                             GLGizmoPainterBase::Button button_down) const
{
    ...
    {
        size_t extruder_id = (button_down == Button::Left ? m_first_selected_extruder_idx
                                                          : m_second_selected_extruder_idx) +
                             1;
