...
void GLGizmoFuzzySkin::on_render_input_window(float x, float y, float bottom_limit)
{
    ...
    for (const std::string t : {"add_fuzzy_skin", "remove_fuzzy_skin", "draw_fuzzy_skin"})
    {
    ...
    if (m_tool_type == ToolType::BRUSH)
    {
        ...
        if (ImGui::IsItemHovered())
            ImGuiPureWrap::tooltip(_u8L("Splits bigger facets into smaller ones while the object is painted."),
                                   max_tooltip_width);
...
wxString GLGizmoFuzzySkin::handle_snapshot_action_name(bool control_down, GLGizmoPainterBase::Button button_down) const
{
    return control_down ? _L("Remove fuzzy skin") : _L("Add fuzzy skin");
}
