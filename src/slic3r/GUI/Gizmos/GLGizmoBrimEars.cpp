...
// These functions provide compatibility with different slicer APIs
namespace
{
// Stub for GizmoImguiSetNextWIndowPos - position is handled by gizmo base
inline void GizmoImguiSetNextWIndowPos(float x, float y, int cond, float pivot_x, float pivot_y)
{
    ImGui::SetNextWindowPos(ImVec2(x, y), (ImGuiCond) cond, ImVec2(pivot_x, pivot_y));
}

// Stub for GizmoImguiBegin
inline bool GizmoImguiBegin(const std::string &name, int flags)
{
...
{
    GLModel::Geometry cylinder_geometry = smooth_cylinder(128, 1.0f, 1.0f); // Was 16, now matches sliced geometry
...
bool GLGizmoBrimEars::on_init()
{
    ...
    m_desc["max_angle"] = _u8L("Max angle");
    m_desc["detection_radius"] = _u8L("Detection radius");
    m_desc["remove_selected"] = _u8L("Remove selected points");
    m_desc["remove_all"] = _u8L("Remove all");
    m_desc["auto_generate"] = _u8L("Auto-generate points");
    m_desc["section_view"] = _u8L("Section view");

    m_desc["left_click_caption"] = _u8L("Left click");
    m_desc["left_click"] = _u8L("Add a brim ear");
    m_desc["right_click_caption"] = _u8L("Right click");
    m_desc["right_click"] = _u8L("Delete a brim ear");
    m_desc["ctrl_mouse_wheel_caption"] = ctrl + _u8L("Mouse wheel");
    m_desc["ctrl_mouse_wheel"] = _u8L("Adjust head diameter");
    m_desc["alt_mouse_wheel_caption"] = alt + _u8L("Mouse wheel");
    m_desc["alt_mouse_wheel"] = _u8L("Adjust section view");
...
void GLGizmoBrimEars::render_points(const Selection &selection, bool use_object_color)
{
    ...
    if (!use_object_color && render_hover_point)
    {
    ...
    for (size_t i = 0; i < cache_size; ++i)
    {
        ...
        if (use_object_color)
        {
            // Use object color when rendering inactive
            render_color = vol->color;
        }
        else if (hover)
        {
...
// Original render_points without color parameter (delegates to overload)
void GLGizmoBrimEars::render_points(const Selection &selection)
{
    render_points(selection, false);
}

void GLGizmoBrimEars::on_render_when_inactive()
{
    // Only render if checkbox is enabled
    if (!m_always_show_brims)
        return;

    // Get selection to access volumes and model
    const Selection &selection = m_parent.get_selection();
    const Model *model = selection.get_model();
    if (!model)
        return;

    // Group volumes by ModelObject that has brim_points
    // Map: ModelObject* -> vector of GLVolume* (one per instance)
    std::map<const ModelObject *, std::vector<const GLVolume *>> objects_with_brims;

    const GLVolumeCollection &volumes = m_parent.get_volumes();
    for (const GLVolume *vol : volumes.volumes)
    {
        if (vol && vol->object_idx() >= 0 && vol->object_idx() < (int) model->objects.size() &&
            vol->volume_idx() >= 0 && // volume_idx >= 0 means it's a model part volume
            vol->is_active)
        { // Only include visible volumes (respects isolation)

            const ModelObject *mo = model->objects[vol->object_idx()];
            // Only include objects that have brim_points
            if (mo && !mo->brim_points.empty())
            {
                objects_with_brims[mo].push_back(vol);
            }
        }
    }

    if (objects_with_brims.empty())
        return;
    ...
    // Render for each object that has brim_points
    for (const auto &obj_pair : objects_with_brims)
    {
        const ModelObject *mo = obj_pair.first;
        const std::vector<const GLVolume *> &object_volumes = obj_pair.second;

        // Render for each instance of this object
        for (const GLVolume *target_volume : object_volumes)
        {
            const Transform3d &instance_scaling_matrix_inverse =
            ...
            {
                // Use object's render color for inactive rendering
                ColorRGBA render_color = target_volume->render_color.is_transparent() ? target_volume->color
                                                                                      : target_volume->render_color;
                m_cylinder.model.set_color(render_color);
                shader->set_uniform("emission_factor", 0.5f);
    ...
    for (auto item : m_mesh_raycaster_map)
    {
        ...
        if (raycaster->unproject_on_mesh(mouse_pos, world_tran, camera, hit, normal, clp_dist != 0. ? clp : nullptr))
        {
            ...
            if (hit_squared_distance < closest_hit_distance)
            {
                ...
                position_on_model =
                    (m_last_hit_volume->get_volume_transformation().get_matrix() * hit.cast<double>()).cast<float>();
...
bool GLGizmoBrimEars::gizmo_event(SLAGizmoEventType action, const Vec2d &mouse_position, bool shift_down, bool alt_down,
                                  bool control_down)
{
    if (action != SLAGizmoEventType::MouseWheelDown || action != SLAGizmoEventType::MouseWheelUp ||
    ...
    if (action == SLAGizmoEventType::Moving)
    {
        ...
        {
            BrimPoint hover_brim(pos_and_normal.first, m_new_point_head_diameter / 2.f);
            hover_brim.overlap_percent = m_new_point_overlap;
            render_hover_point = CacheEntry(hover_brim, false, (inverse_trsf * m_world_normal).cast<float>(), true);
        }
        else
        {
    ...
    else if (action == SLAGizmoEventType::LeftDown && (shift_down || alt_down || control_down))
    {
        ...
        if (m_hover_id == -1)
        {
            ...
            {
                m_selection_rectangle.start_dragging(mouse_position, shift_down
                                                                         ? GLSelectionRectangle::EState::Select
    ...
    if ((action == SLAGizmoEventType::LeftUp || action == SLAGizmoEventType::ShiftUp ||
         action == SLAGizmoEventType::AltUp) &&
        m_selection_rectangle.is_dragging())
    {
        ...
        for (size_t idx : points_idxs)
            points_inside.emplace_back(
                (trafo.get_matrix().cast<float>() * (m_editing_cache[idx].brim_point.pos + m_editing_cache[idx].normal))
                    .cast<float>());
    ...
    if (action == SLAGizmoEventType::MouseWheelUp)
    {
        if (control_down)
        {
            ...
            m_new_point_head_diameter = std::min(30., initial_value + 0.1);
            update_cache_radius();
            return true;
        }
    ...
    if (action == SLAGizmoEventType::MouseWheelUp && alt_down)
    {
        ...
        m_c->object_clipper()->set_position_by_ratio(pos, false);
        return true;
    }
...
void GLGizmoBrimEars::begin_overlap_change(float initial_value)
{
    if (m_old_point_overlap == 0.f)
        m_old_point_overlap = initial_value;
}

void GLGizmoBrimEars::update_cache_overlap()
{
    if (render_hover_point)
        render_hover_point->brim_point.overlap_percent = m_new_point_overlap;

    for (auto &cache_entry : m_editing_cache)
        if (cache_entry.selected)
        {
            cache_entry.brim_point.overlap_percent = m_new_point_overlap;
            find_single();
            update_model_object();
        }
    m_parent.set_as_dirty();
...
void GLGizmoBrimEars::apply_overlap_change()
{
    if (m_old_point_overlap == 0.f)
        return;
...
void GLGizmoBrimEars::on_render_input_window(float x, float y, float bottom_limit)
{
    ...
    int current_width = canvas_size.get_width();
    int current_height = canvas_size.get_height();
    if (m_last_canvas_width != current_width || m_last_canvas_height != current_height)
    {
        m_popup_render_count = 0;
        m_popup_width = 0.0f;
        m_popup_height = 0.0f;
        m_last_canvas_width = current_width;
        m_last_canvas_height = current_height;
    }
    ...
    else
    {
        ...
        // x is the button center, y is below the button
        // Anchor (0.5, 0.0) means center-top of window at (x,y)
        ImGuiPureWrap::set_next_window_pos(x, y, ImGuiCond_Always, 0.5f, 0.0f);
    }

    m_popup_render_count++;

    // Make window invisible on first frame to prevent flicker
    ...
    const float clipping_slider_left =
        std::max(ImGuiPureWrap::calc_text_size(m_desc.at("head_diameter")).x,
                 std::max(ImGuiPureWrap::calc_text_size(m_desc.at("max_angle")).x,
                          std::max(ImGuiPureWrap::calc_text_size(m_desc.at("detection_radius")).x,
                                   ImGuiPureWrap::calc_text_size(m_desc.at("section_view")).x))) +
        m_imgui->scaled(1.5f);
    const float button_width = std::max(ImGuiPureWrap::calc_text_size(m_desc.at("auto_generate")).x,
                                        std::max(ImGuiPureWrap::calc_text_size(m_desc.at("remove_selected")).x,
                                                 ImGuiPureWrap::calc_text_size(m_desc.at("remove_all")).x)) +
                               m_imgui->scaled(1.f);

    // Calculate help text widths
    float caption_max = 0.f;
    float total_text_max = 0.f;
    for (const auto &t : std::array<std::string, 4>{"left_click", "right_click", "ctrl_mouse_wheel", "alt_mouse_wheel"})
    {
    ...
    float window_width = minimal_slider_width + clipping_slider_left + slider_icon_width;
    ...
    // HEAD DIAMETER slider
    ImGui::AlignTextToFramePadding();
    ImGuiPureWrap::text(m_desc.at("head_diameter"));
    ImGui::SameLine(clipping_slider_left);
    ImGui::PushItemWidth(window_width - clipping_slider_left - slider_icon_width);

    float initial_value = m_new_point_head_diameter;
    if (m_imgui->slider_float("##head_diameter", &m_new_point_head_diameter, 5.0f, 30.0f, "%.1f"))
    {
        if (m_old_point_head_diameter == 0.f)
        {
            begin_radius_change(initial_value);
    ...
    if (ImGui::IsItemDeactivatedAfterEdit() && m_old_point_head_diameter != 0.f)
    {
        apply_radius_change();
    }

    ImGui::AlignTextToFramePadding();
    ImGuiPureWrap::text(_u8L("Overlap"));
    ImGui::SameLine(clipping_slider_left);
    ImGui::PushItemWidth(window_width - clipping_slider_left - slider_icon_width);

    float initial_overlap = m_new_point_overlap;
    if (m_imgui->slider_float("##overlap", &m_new_point_overlap, -100.0f, 100.0f, "%.0f%%"))
    {
        if (m_old_point_overlap == 0.f)
        {
            begin_overlap_change(initial_overlap);
        }
        update_cache_overlap();
    }
    if (ImGui::IsItemDeactivatedAfterEdit() && m_old_point_overlap != 0.f)
    {
        apply_overlap_change();
    }

    // MAX ANGLE slider
    ImGui::AlignTextToFramePadding();
    ImGuiPureWrap::text(m_desc.at("max_angle"));
    ImGui::SameLine(clipping_slider_left);
    ImGui::PushItemWidth(window_width - clipping_slider_left - slider_icon_width);
    m_imgui->slider_float("##max_angle", &m_max_angle, 0.0f, 180.0f, "%.1f");

    // DETECTION RADIUS slider
    ImGui::AlignTextToFramePadding();
    ImGuiPureWrap::text(m_desc.at("detection_radius"));
    ImGui::SameLine(clipping_slider_left);
    ImGui::PushItemWidth(window_width - clipping_slider_left - slider_icon_width);
    m_imgui->slider_float("##detection_radius", &m_detection_radius, 0.0f, m_detection_radius_max, "%.1f");

    ImGui::Separator();

    // CLIPPING PLANE slider
    ImGui::AlignTextToFramePadding();
    ImGuiPureWrap::text(m_desc.at("section_view"));
    ImGui::SameLine(clipping_slider_left);
    ImGui::PushItemWidth(window_width - clipping_slider_left - slider_icon_width);
    float clp_dist = float(m_c->object_clipper()->get_position());
    if (m_imgui->slider_float("##clp_dist", &clp_dist, 0.f, 1.f, "%.2f", 1.0f, true, _L("Ctrl + Mouse wheel")))
    {
        m_c->object_clipper()->set_position_by_ratio(clp_dist, false);
    }
    ...
    if (ImGuiPureWrap::button(m_desc.at("remove_selected")))
    {
        delete_selected_points();
    }
    ...
    // WARNING if brim type is not painted
    if (obj_cfg.option("brim_type"))
    {
        if (obj_cfg.opt_enum<BrimType>("brim_type") != btPainted)
        {
            ImGuiPureWrap::text_colored(ImGuiPureWrap::COL_ORANGE_LIGHT,
                                        _u8L("Warning: Brim type must be set to 'Painted' for manual placement."));
        }
    }

    // Show invalid brim ears warning
    if (!m_single_brim.empty())
    {
        ImGuiPureWrap::text_colored(
            ImGuiPureWrap::COL_ORANGE_LIGHT,
            (boost::format(_u8L("Warning: %1% invalid brim ears")) % m_single_brim.size()).str());
    }

    // Pop the alpha style if window was made invisible
    if (m_popup_render_count == 1 && m_popup_height <= 0.0f)
    {
        ImGui::PopStyleVar();
    }

    // Track window size for next frame positioning
    m_popup_width = ImGui::GetWindowWidth();
    m_popup_height = ImGui::GetWindowHeight();
...
{
    return false; // Don't show in the right-side gizmo toolbar
}
...
void GLGizmoBrimEars::on_load(cereal::BinaryInputArchive &ar)
{
    try
    {
        ar(m_new_point_head_diameter, m_editing_cache, m_selection_empty, m_always_show_brims);
    }
    catch (...)
    {
        // Backward compatibility: older saves don't have m_always_show_brims
        ar(m_new_point_head_diameter, m_editing_cache, m_selection_empty);
        m_always_show_brims = false;
    }
}

void GLGizmoBrimEars::on_save(cereal::BinaryOutputArchive &ar) const
{
    ar(m_new_point_head_diameter, m_editing_cache, m_selection_empty, m_always_show_brims);
}
...
void GLGizmoBrimEars::select_point(int i)
{
    if (i == AllPoints || i == NoPoints)
    {
        ...
        if (i == AllPoints)
        {
            ...
            m_new_point_overlap = m_editing_cache[0].brim_point.overlap_percent;
        }
    }
    else
    {
        ...
        m_new_point_overlap = m_editing_cache[i].brim_point.overlap_percent;
    }
}
...
Points GLGizmoBrimEars::generate_points(Polygon &obj_polygon, float ear_detection_length, float brim_ears_max_angle,
                                        bool is_outer)
{
    ...
    if (ear_detection_length > 0)
    {
        ...
        points = douglas_peucker(points, detect_length);
        if (points.size() > 4)
        {
    ...
    append(pt_ears,
           is_outer ? obj_polygon.convex_points(angle_threshold) : obj_polygon.concave_points(angle_threshold));
    return pt_ears;
}
...
void GLGizmoBrimEars::auto_generate()
{
    ...
    Transform3d trsf = volume->get_instance_transformation().get_matrix();
    Vec3f normal =
        (volume->get_instance_transformation().get_matrix_no_offset().inverse() * m_world_normal).cast<float>();
...
void GLGizmoBrimEars::get_detection_radius_max()
{
    ...
    for (const ExPolygon &ex_poly : m_first_layer)
    {
        ...
        min_points_num = douglas_peucker(out_points, 0).size();
        int repeat = 0;
        int loop_protect = 0;
        for (;;)
        {
            loop_protect++;
            tolerance += 10;
            int num = douglas_peucker(out_points, tolerance / SCALING_FACTOR).size();
...
bool GLGizmoBrimEars::add_point_to_cache(Vec3f pos, float head_radius, bool selected, Vec3f normal)
{
    ...
    point.overlap_percent = m_new_point_overlap;
    ...
    if (idxs.size() > 0)
    {
        for (unsigned int idx : idxs)
        {
            ...
            else
            {
                ...
                m_mesh_raycaster_map[v] = std::make_shared<PickRaycaster>(-1, *v->mesh_raycaster,
                                                                          world_tran.get_matrix());
                m_mesh_raycaster_map[v]->set_transform(world_tran.get_matrix());
            }
        }
    }
...
void GLGizmoBrimEars::reset_all_pick()
{
...
void GLGizmoBrimEars::find_single()
{
    ...
    while (cyc)
    {
        ...
        for (auto it = m_single_brim.begin(); it != m_single_brim.end(); ++it)
        {
            ...
            // Check if point_pl overlaps with any polygon in model_pl
            bool has_overlap = false;
            for (const ExPolygon &model_poly : model_pl)
            {
                if (model_poly.overlaps(point_pl))
                {
                    has_overlap = true;
                    break;
                }
