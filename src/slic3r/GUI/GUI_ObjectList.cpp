                         ...
                         wxDV_MULTIPLE)
{
    ...
    // Fix for context menu issue
         ...
         */
             ...
             else
             {
                 ...
                 if (new_selected_item == m_last_selected_item && m_last_selected_column != -1 &&
...
#ifdef __WXMSW__
    GetMainWindow()->Bind(wxEVT_MOTION,
    ...
    GetMainWindow()->Bind(wxEVT_LEFT_DOWN,
                          [this](wxMouseEvent &event)
                          {
                              m_mouse_left_down = true;
                              event.Skip();
                          });
    GetMainWindow()->Bind(wxEVT_LEFT_UP,
                          [this](wxMouseEvent &event)
                          {
...
void ObjectList::set_min_height()
{
    if (m_items_count == size_t(-1))
        m_items_count = 7;
    int list_min_height = lround(2.25 * (m_items_count + 1) *
                                 wxGetApp().em_unit()); // +1 is for height of control header
...
void ObjectList::get_selected_item_indexes(int &obj_idx, int &vol_idx,
                                           const wxDataViewItem &input_item /* = wxDataViewItem(nullptr)*/)
{
    ...
    obj_idx = type & itObject   ? m_objects_model->GetIdByItem(item)
              : type & itVolume ? m_objects_model->GetIdByItem(m_objects_model->GetTopParent(item))
                                : -1;
...
MeshErrorsInfo ObjectList::get_mesh_errors_info(const int obj_idx, const int vol_idx /*= -1*/,
                                                wxString *sidebar_info /*= nullptr*/) const
{
    if (obj_idx < 0)
        return {{}, {}}; // hide tooltip
    ...
    const TriangleMeshStats &stats = vol_idx == -1 ? ModelProcessing::get_object_mesh_stats((*m_objects)[obj_idx])
    ...
    if (stats.repaired())
    {
        ...
        auto_repaired_info = format_wxstr(_L_PLURAL("Auto-repaired %1$d error", "Auto-repaired %1$d errors", errors),
                                          errors);
        ...
        if (repaired.degenerate_facets > 0)
            tooltip += "\t" +
                       format_wxstr(_L_PLURAL("%1$d degenerate facet", "%1$d degenerate facets",
                                              repaired.degenerate_facets),
                                    repaired.degenerate_facets) +
                       "\n";
        if (repaired.edges_fixed > 0)
            tooltip += "\t" +
                       format_wxstr(_L_PLURAL("%1$d edge fixed", "%1$d edges fixed", repaired.edges_fixed),
                                    repaired.edges_fixed) +
                       "\n";
        if (repaired.facets_removed > 0)
            tooltip += "\t" +
                       format_wxstr(_L_PLURAL("%1$d facet removed", "%1$d facets removed", repaired.facets_removed),
                                    repaired.facets_removed) +
                       "\n";
        if (repaired.facets_reversed > 0)
            tooltip += "\t" +
                       format_wxstr(_L_PLURAL("%1$d facet reversed", "%1$d facets reversed", repaired.facets_reversed),
                                    repaired.facets_reversed) +
                       "\n";
        if (repaired.backwards_edges > 0)
            tooltip += "\t" +
                       format_wxstr(_L_PLURAL("%1$d backward edge", "%1$d backward edges", repaired.backwards_edges),
                                    repaired.backwards_edges) +
                       "\n";
    }
    if (!stats.manifold())
    {
        remaining_info = format_wxstr(_L_PLURAL("%1$d open edge", "%1$d open edges", stats.open_edges),
                                      stats.open_edges);
    ...
    if (sidebar_info)
        *sidebar_info = stats.manifold() ? auto_repaired_info
                                         : (remaining_info + (stats.repaired() ? ("\n" + auto_repaired_info) : ""));
...
MeshErrorsInfo ObjectList::get_mesh_errors_info(wxString *sidebar_info /*= nullptr*/)
{
    ...
    if (obj_idx < 0)
    { // child of ObjectItem is selected
        if (sidebar_info)
...
void ObjectList::selection_changed()
{
    ...
    if (const wxDataViewItem item = GetSelection())
    {
        ...
        {
            if (printer_technology() == ptSLA)
                wxGetApp().plater()->canvas3D()->set_sla_view_type(scene_selection().get_first_volume()->composite_id,
                                                                   GLCanvas3D::ESLAViewType::Original);
        }
    }
...
void ObjectList::paste_volumes_into_list(int obj_idx, const ModelVolumePtrs &volumes)
{
    ...
    wxDataViewItemArray items = reorder_volumes_and_get_selection(
        obj_idx, [volumes](const ModelVolume *volume)
...
void ObjectList::extruder_editing()
{
    ...
    m_extruder_editor->Bind(wxEVT_COMBOBOX,
                            [set_extruder](wxCommandEvent &evt)
                            {
                                set_extruder();
                                evt.StopPropagation();
                            });
...
bool ObjectList::can_drop(const wxDataViewItem &item) const
{
    ...
    if (m_dragged_data.type() & itVolume)
    {
        if (m_dragged_data.obj_idx() != m_objects_model->GetObjectIdByItem(item))
            return false;
        wxDataViewItem dragged_item = m_objects_model->GetItemByVolumeId(m_dragged_data.obj_idx(),
                                                                         m_dragged_data.sub_obj_idx());
        if (!dragged_item)
            return false;
        ...
        if (dragged_item_v_type == item_v_type && dragged_item_v_type != ModelVolumeType::MODEL_PART)
            return true;
        if ((wxGetApp().app_config->get_bool("order_volumes") &&
             dragged_item_v_type != item_v_type) ||          // we can't reorder volumes outside of types
        ...
        if ((dragged_item_v_type == ModelVolumeType::NEGATIVE_VOLUME ||
             dragged_item_v_type == ModelVolumeType::PARAMETER_MODIFIER))
        {
            if (only_one_solid_part)
                return false;
            return m_objects_model->GetVolumeIdByItem(item) != 0;
...
void ObjectList::OnDrop(wxDataViewEvent &event)
{
    ...
    **/
    ...
    if (m_dragged_data.type() & itVolume)
    {
        ...
        select_item(
            m_objects_model->ReorganizeChildren(from_volume_id, to_volume_id, m_objects_model->GetParent(item)));
    }
...
void ObjectList::add_category_to_settings_from_selection(
    const std::vector<std::pair<std::string, bool>> &category_options, wxDataViewItem item)
{
...
void ObjectList::load_from_files(const wxArrayString &input_files, ModelObject &model_object,
                                 std::vector<ModelVolume *> &added_volumes, ModelVolumeType type, bool from_galery)
{
...
void ObjectList::load_generic_subobject(const std::string &type_name, const ModelVolumeType type)
{
    ...
    if (type_name == "Slab")
    {
        ...
        offset = Vec3d(inst_center.x(), inst_center.y(),
                       0.5 * mesh_bb.size().z() + instance_bb.min.z() - v->get_instance_offset().z());
    }
    else
    {
        // Translate the new modifier to be pickable: move to the left front corner of the instance's bounding box, lift to print bed.
    ...
    select_item(
        [this, obj_idx, new_volume]()
        {
            wxDataViewItem sel_item;

            wxDataViewItemArray items = reorder_volumes_and_get_selection(obj_idx,
...
void ObjectList::load_mesh_object(const TriangleMesh &mesh, const std::string &name, bool center)
{
    ...
    new_object->instances[0]->set_offset(center
                                             ? to_3d(wxGetApp().plater()->build_volume().bounding_volume2d().center(),
...
bool ObjectList::del_from_cut_object(bool is_cut_connector, bool is_model_part /* = false*/,
                                     bool is_negative_volume /* = false*/)
{
    ...
    if (type == itVolume)
    {
        ...
        if (object->volumes.size() == 1)
        {
            ...
            if (!last_volume->config.empty())
            {
                ...
                if (obj_item)
                {
                    wxString extruder = object->config.has("extruder")
                                            ? wxString::Format("%d", object->config.extruder())
                                            : _L("default");
...
void ObjectList::split()
{
    ...
    const auto nozzle_dmrs_cnt = (nozzle_dmtrs_opt == nullptr)
                                     ? size_t(1)
                                     : dynamic_cast<const ConfigOptionFloats *>(nozzle_dmtrs_opt)->values.size();
    ...
    if (to_multipart_object)
    {
        auto get_object_idxs = [this](std::vector<int> &obj_idxs, wxDataViewItemArray &sels)
        {
            ...
            for (auto map_item : sel_map)
            {
                ...
                { // we should split to separate instances last object
        ...
        for (int obj_idx : obj_idxs)
        {
            ...
            const Transform3d new_inst_trafo = new_object->instances[0]->get_matrix().inverse() *
                                               object->instances[0]->get_matrix();
...
bool ObjectList::has_selected_cut_object() const
{
    ...
    {
        const int obj_idx = m_objects_model->GetObjectIdByItem(item);
        // ys_FIXME: The obj_idx<size condition is a workaround for object list crash,
...
void ObjectList::part_selection_changed()
{
    ...
    if (item && m_objects_model->GetItemType(item) == itInfo &&
        m_objects_model->GetInfoItemType(item) == InfoItemType::CutConnectors)
    {
        ...
        if (item)
        {
                ...
                if (type == itInfo)
                {
                    ...
                    case InfoItemType::FuzzySkin:
                    {
                        GLGizmosManager::EType gizmo_type = info_type == InfoItemType::CustomSupports
                                                            ...
                                                            : info_type == InfoItemType::CustomSeam
            ...
            else
            {
                ...
                else if (type & itVolume)
                {
                    ...
                    enable_manipulation = !(object->is_cut() &&
                                            (volume->is_cut_connector() || volume->is_model_part()));
                }
                else if (type & itInstance)
                {
...
wxDataViewItemArray ObjectList::add_volumes_to_object_in_list(
    size_t obj_idx, std::function<bool(const ModelVolume *)> add_to_selection /* = nullptr*/)
{
    ...
    if (can_add_volumes_to_object(object))
    {
        ...
        for (const ModelVolume *volume : object->volumes)
        {
            ...
            const wxDataViewItem &vol_item = m_objects_model->AddVolumeChild(
                object_item, get_item_name(volume->name, volume->is_text()), volume_idx, volume->type(),
...
void ObjectList::add_object_to_list(size_t obj_idx, bool call_selection_changed)
{
    ...
    const auto item = m_objects_model->AddObject(
        item_name, extruder2str(model_object->config.has("extruder") ? model_object->config.extruder() : 0),
        get_warning_icon_name(model_object->mesh().stats()), model_object->is_cut());
    ...
    if (model_object->instances.size() > 1)
    {
        ...
        m_objects_model->SetPrintableState(model_object->instances[0]->printable ? piPrintable : piUnprintable,
                                           obj_idx);
...
{
    select_item([this, obj_idx, vol_idx]()
                { return m_objects_model->Delete(m_objects_model->GetItemByVolumeId(obj_idx, vol_idx)); });
}

void ObjectList::delete_instance_from_list(const size_t obj_idx, const size_t inst_idx)
{
    select_item([this, obj_idx, inst_idx]()
                { return m_objects_model->Delete(m_objects_model->GetItemByInstanceId(obj_idx, inst_idx)); });
}
...
bool ObjectList::delete_from_model_and_list(const std::vector<ItemForDelete> &items_for_delete)
{
    ...
    for (std::vector<ItemForDelete>::const_reverse_iterator item = items_for_delete.rbegin();
         item != items_for_delete.rend(); ++item)
    {
        ...
        else
        {
            ...
            if (item->type & itVolume)
            {
                ...
                if (obj->volumes.size() == 1)
                {
                    ...
                    // If last volume item with warning was deleted, unmark object item
                    m_objects_model->UpdateWarningIcon(parent, get_warning_icon_name(
                                                                   ModelProcessing::get_object_mesh_stats(obj)));
                }
...
{
    select_item([this, obj_idx, num]()
                { return m_objects_model->AddInstanceChild(m_objects_model->GetItemById(obj_idx), num); });
    selection_changed();
}
...
{
    select_item([this, obj_idx, num]()
                { return m_objects_model->DeleteLastInstance(m_objects_model->GetItemById(obj_idx), num); });
}
...
void ObjectList::select_object_item(bool is_msr_gizmo)
{
    if (wxDataViewItem item = GetSelection())
    {
        ...
        bool is_volume_item = type == itVolume ||
                              (type == itSettings &&
                               m_objects_model->GetItemType(m_objects_model->GetParent(item)) == itVolume);
...
void ObjectList::remove()
{
    ...
    else
    {
        ...
        for (auto &item : sels)
        {
            if (m_objects_model->InvalidItem(
                    item)) // item can be deleted for this moment (like last 2 Instances or Volumes)
                continue;
...
void ObjectList::add_layer_range_after_current(const t_layer_height_range current_range)
{
    ...
    else if (const std::pair<coordf_t, coordf_t> &next_range = it_next_range->first;
             current_range.second <= next_range.first)
    {
            ...
            if (current_range.second == next_range.first)
            {
                ...
                if (delta >= old_min_layer_height + new_min_layer_height - EPSILON)
                {
                    const coordf_t middle_layer_z = (new_min_layer_height > 0.5 * delta)
                                                        ...
                                                        : next_range.first +
                                                              std::max(old_min_layer_height, 0.5 * delta);
...
wxString ObjectList::can_add_new_range_after_current(const t_layer_height_range current_range)
{
    ...
    if (const std::pair<coordf_t, coordf_t> &next_range = it_next_range->first;
        current_range.second <= next_range.first)
    {
        if (current_range.second == next_range.first)
        {
            if (next_range.second - next_range.first <
                get_min_layer_height(it_next_range->second.opt_int("extruder")) + get_min_layer_height(0) - EPSILON)
...
void ObjectList::add_layer_item(const t_layer_height_range &range, const wxDataViewItem layers_item,
                                const int layer_idx /* = -1*/)
{
    ...
    const auto layer_item = m_objects_model->AddLayersChild(layers_item, range,
                                                            extruder2str(config.opt_int("extruder")), layer_idx);
...
bool ObjectList::is_connectors_item_selected() const
{
    const wxDataViewItem &item = GetSelection();
    if (item)
        return m_objects_model->GetItemType(item) == itInfo &&
               m_objects_model->GetInfoItemType(item) == InfoItemType::CutConnectors;
...
{
    for (auto item : sels)
        if (m_objects_model->GetItemType(item) == itInfo &&
            m_objects_model->GetInfoItemType(item) == InfoItemType::CutConnectors)
            return true;
...
void ObjectList::update_selections()
{
    ...
    if (GetSelectedItemsCount() == 1 &&
        m_objects_model->GetItemType(GetSelection()) & (itSettings | itLayerRoot | itLayer))
    {
        const auto item = GetSelection();
        if (selection.is_single_full_object())
        {
        ...
        // Fix for object list update issue
        else
        {
        ...
        if (!selection.is_multiple_full_object() && (m_selection_mode & (smSettings | smLayer | smLayerRoot)))
        {
            ...
            if (m_selection_mode & smSettings)
            {
                if (m_selected_layers_range_idx < 0)
                    ...
                    sels.Add(m_objects_model->GetSettingsItem(
                        m_objects_model->GetItemByLayerId(obj_idx, m_selected_layers_range_idx)));
            }
    ...
    else if (selection.is_any_volume() || selection.is_any_modifier())
    {
        if (m_selection_mode & smSettings)
        {
            ...
            if (gl_vol->volume_idx() >= 0)
            {
                ...
                wxDataViewItem vol_item = m_objects_model->GetItemByVolumeId(gl_vol->object_idx(),
                                                                             gl_vol->volume_idx());
            ...
            for (auto idx : selection.get_volume_idxs())
            {
                ...
                if (gl_vol->volume_idx() >= 0)
                {
                    ...
                    if (object(obj_idx)->volumes[vol_idx]->is_cut_connector())
                        sels.Add(m_objects_model->GetInfoItemByType(m_objects_model->GetItemById(obj_idx),
...
void ObjectList::update_selections_on_canvas()
{
    ...
    auto add_to_selection = [this, &volume_idxs, &single_selection](const wxDataViewItem &item,
                                                                    const Selection &selection, int instance_idx,
                                                                    Selection::EMode &mode)
    {
        const ItemType &type = m_objects_model->GetItemType(item);
        const int obj_idx = m_objects_model->GetObjectIdByItem(item);
        ...
        if (type == itVolume)
        {
            ...
            std::vector<unsigned int> idxs = selection.get_volume_idxs_from_volume(obj_idx, std::max(instance_idx, 0),
                                                                                   vol_idx);
                ...
                for (unsigned int vol_idx = 0; vol_idx < obj->volumes.size(); vol_idx++)
                    if (obj->volumes[vol_idx]->is_cut_connector())
                    {
                        std::vector<unsigned int> idxs =
                            selection.get_volume_idxs_from_volume(obj_idx, std::max(instance_idx, 0), vol_idx);
                        volume_idxs.insert(volume_idxs.end(), idxs.begin(), idxs.end());
                    }
            }
            else
...
bool ObjectList::check_last_selection(wxString &msg_str)
{
    ...
    if (impossible_multi_selection(itVolume, smVolume) || impossible_multi_selection(itLayer, smLayer) ||
        type & itSettings || (type & itVolume && !(m_selection_mode & smVolume)) ||
        (type & itLayer && !(m_selection_mode & smLayer)) || (type & itInstance && !(m_selection_mode & smInstance)))
    {
...
void ObjectList::fix_multiselection_conflicts()
{
    ...
    if (m_selection_mode & (smVolume | smLayer))
    {
        ...
        const wxDataViewItem &parent = m_objects_model->GetParent(m_last_selected_item == sels.front() ? sels.back()
                                                                                                       : sels.front());
...
void ObjectList::change_part_type()
{
    ...
    wxDataViewItemArray sel = reorder_volumes_and_get_selection(obj_idx, [volume](const ModelVolume *vol)
                                                                { return vol == volume; });
    if (!sel.IsEmpty())
...
void ObjectList::fix_through_winsdk()
{
    ...
    auto fix_and_update_progress =
        [this, plater, model_names](const int obj_idx, const int vol_idx, int model_idx, wxProgressDialog &progress_dlg,
                                    std::vector<std::string> &succes_models,
                                    std::vector<std::pair<std::string, std::string>> &failed_models)
    {
        ...
        plater->clear_before_change_mesh(obj_idx,
                                         _u8L("Custom supports, seams, fuzzy skin and multimaterial painting were "
    ...
    plater->get_notification_manager()->push_notification(
        NotificationType::RepairFinished, NotificationManager::NotificationLevel::PrintInfoShortNotificationLevel,
        into_u8(msg));
}
...
wxDataViewItemArray ObjectList::reorder_volumes_and_get_selection(
    size_t obj_idx, std::function<bool(const ModelVolume *)> add_to_selection /* = nullptr*/)
{
