...
LoadStepDialog::LoadStepDialog(wxWindow *parent, const std::string &filename, double linear_precision,
                               double angle_precision, bool multiple_loading)
    : DPIDialog(parent, wxID_ANY, format_wxstr(_L("STEP import quality (%1%)"), filename), wxDefaultPosition,
...
void LoadStepDialog::add_params(wxSizer *sizer)
{
    ...
    for (const auto &[name, params] : default_step_import_params)
    {
        ...
        radio_def->Bind(wxEVT_RADIOBUTTON,
                        [params_copy = params, this](wxEvent &)
                        {
    ...
    radio_custom->Bind(wxEVT_RADIOBUTTON,
                       [&, this](wxEvent &)
                       {
                           enable_customs(true);
#ifndef _WIN32
                           ...
                           m_params.angle = string_to_double_decimal_point(
                               m_angle_precision_val->GetValue().ToStdString());
                       });
    ...
    wxBoxSizer *labels_sizer = new wxBoxSizer(wxHORIZONTAL);
    {
        ...
        const int left_text_gap = std::max(GetTextExtent(_L("Linear precision")).x,
                                           GetTextExtent(_L("Angle precision")).x) +
                                  4 * hgap - GetTextExtent(left_text).x * 0.5;
        const wxString right_text = _L("Higher quality");
        const int right_text_gap = GetTextExtent(_L("mm")).x + def_editor_size.x + 4 * hgap -
                                   GetTextExtent(right_text).x * 0.5;
    ...
    auto high_vals = std::find_if(default_step_import_params.begin(), default_step_import_params.end(),
                                  [](const std::pair<std::string, PrecisionParams> &val)
                                  { return val.first == "High"; });
    ...
    auto process_value_change = [](double &precision, wxTextCtrl *text_ctrl, wxSlider *slider,
                                   const SliderHelper &sl_helper) -> void
    {
    ...
    auto tooltip = [](const SliderHelper &sl_helper) -> wxString
    {
        ...
        return format_wxstr(_L("Set value from the range [%1%; %2%] with %3% step"), sl_helper.min_val,
                            sl_helper.max_val, sl_helper.val_step);
    };

    // Add "Linear precision"

    m_linear_precision_slider = new wxSlider(this, wxID_ANY, m_linear_precision_sl.get_pos(m_params.linear),
                                             m_linear_precision_sl.beg_sl_pos, m_linear_precision_sl.end_sl_pos,
                                             wxDefaultPosition, def_slider_size, slyder_style);
    m_linear_precision_slider->SetTickFreq(1);
    m_linear_precision_slider->Bind(wxEVT_SLIDER,
                                    [this](wxCommandEvent e)
                                    {
                                        m_params.linear = m_linear_precision_sl.get_value(
                                            m_linear_precision_slider->GetValue());
                                        m_linear_precision_val->SetValue(format_wxstr("%1%", m_params.linear));
                                    });

    m_linear_precision_val = new wxTextCtrl(this, wxID_ANY,
                                            format_wxstr("%1%",
                                                         m_linear_precision_sl.adjust_to_region(m_params.linear)),
                                            wxDefaultPosition, def_editor_size, text_ctrl_style);
    ...
    m_linear_precision_val->Bind(wxEVT_KILL_FOCUS,
                                 [process_value_change, this](wxFocusEvent &e)
                                 {
                                     process_value_change(m_params.linear, m_linear_precision_val,
                                                          m_linear_precision_slider, m_linear_precision_sl);
                                     e.Skip();
    ...
    m_angle_precision_slider = new wxSlider(this, wxID_ANY, m_angle_precision_sl.get_pos(m_params.angle),
                                            m_angle_precision_sl.beg_sl_pos, m_angle_precision_sl.end_sl_pos,
                                            wxDefaultPosition, def_slider_size, slyder_style);
    m_angle_precision_slider->SetTickFreq(5);
    m_angle_precision_slider->Bind(wxEVT_SLIDER,
                                   [this](wxCommandEvent e)
                                   {
                                       m_params.angle = m_angle_precision_sl.get_value(
                                           m_angle_precision_slider->GetValue());
                                       m_angle_precision_val->SetValue(format_wxstr("%1%", m_params.angle));
                                   });

    m_angle_precision_val = new wxTextCtrl(this, wxID_ANY,
                                           format_wxstr("%1%", m_angle_precision_sl.adjust_to_region(m_params.angle)),
                                           wxDefaultPosition, def_editor_size, text_ctrl_style);
    ...
    m_angle_precision_val->Bind(wxEVT_KILL_FOCUS,
                                [process_value_change, this](wxFocusEvent &e)
                                {
                                    process_value_change(m_params.angle, m_angle_precision_val,
                                                         m_angle_precision_slider, m_angle_precision_sl);
