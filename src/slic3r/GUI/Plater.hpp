...
wxDECLARE_EVENT(EVT_FORCE_INVALIDATE_SLICE, SimpleEvent);
...
public:
    ...
    std::vector<size_t> load_files(const std::vector<boost::filesystem::path> &input_files, bool load_model = true,
                                   bool load_config = true, bool imperial_units = false);
    // To be called when providing a list of files to the GUI slic3r on command line.
    std::vector<size_t> load_files(const std::vector<std::string> &input_files, bool load_model = true,
                                   bool load_config = true, bool imperial_units = false);
    ...
    void with_mocked_fff_background_process(Print &print, GCodeProcessorResult &result, const int bed_index,
    ...
    void reslice_SLA_until_step(int step, const ModelObject &object, bool postpone_error_messages = false);
    ...
    std::vector<std::string> get_extruder_color_strings_from_plater_config(
        const GCodeProcessorResult *const result = nullptr) const;
    ...
    void set_bed_shape(const Pointfs &shape, const double max_print_height, const std::string &custom_texture,
                       const std::string &custom_model, bool force_as_custom = false) const;
    ...
    public:
        ...
        TakeSnapshot(Plater *plater, const wxString &snapshot_name, UndoRedo::SnapshotType snapshot_type)
            : m_plater(plater)
        {
