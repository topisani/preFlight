...
public:
    ...
    {
        return m_options.size() == 1 && m_options[0].opt_id == opt_key;
    }
    void clear();
...
class CheckBox; // Forward declaration for sidebar checkbox
...
public:
    ...
    static void change_opt_value(DynamicPrintConfig &config, const t_config_option_key &opt_key,
...
public:
    ...
    ConfigOptionsGroup(wxWindow *parent, const wxString &title, ModelConfig *config, bool is_tab_opt = false,
    ...
    Line create_single_option_line(const std::string &title, const std::string &path = std::string(),
                                   int idx = -1) /*const*/
    {
    ...
    std::pair<OG_CustomCtrl *, bool *> get_custom_ctrl_with_blinking_ptr(const t_config_option_key &opt_key,
                                                                         int opt_index /* = -1*/);
