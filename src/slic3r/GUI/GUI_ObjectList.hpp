...
private:
    ...
    bool m_prevent_update_extruder_in_config =
        false; // We use this flag to avoid updating of the extruder value in config
...
public:
    ...
    MeshErrorsInfo get_mesh_errors_info(const int obj_idx, const int vol_idx = -1,
    ...
    void add_category_to_settings_from_selection(const std::vector<std::pair<std::string, bool>> &category_options,
                                                 wxDataViewItem item);
    ...
    void load_from_files(const wxArrayString &input_files, ModelObject &model_object,
                         std::vector<ModelVolume *> &added_volumes, ModelVolumeType type, bool from_galery = false);
    ...
    wxDataViewItemArray add_volumes_to_object_in_list(
