...
std::vector<float> contour_distance(const EdgeGrid::Grid &grid, const size_t idx_contour, const Slic3r::Points &contour,
                                    const std::vector<ResampledPoint> &resampled_point_parameters, double search_radius)
{
    ...
    if (contour.size() > 2)
    {
        ...
        struct Visitor
        {
            Visitor(const EdgeGrid::Grid &grid, const size_t idx_contour,
                    const std::vector<ResampledPoint> &resampled_point_parameters, double dist_same_contour_reject)
                : grid(grid)
            ...
            bool operator()(coord_t iy, coord_t ix)
            {
                ...
                bool valid = true;
                for (auto it_contour_and_segment = cell_data_range.first;
                     it_contour_and_segment != cell_data_range.second; ++it_contour_and_segment)
                {
                        ...
                        if (std::abs(denom) >= EPSILON)
                        {
                            ...
                            if (it_contour_and_segment->first == idx_contour)
                            {
                                ...
                                double param_end = resampled_point_parameters.back().curve_parameter;
                                {
                                    ...
                                    auto it = std::lower_bound(resampled_point_parameters.begin(),
                                                               resampled_point_parameters.end(), key, lower);
                                    assert(it != resampled_point_parameters.end() && it->idx_src == ipt &&
                                           !it->interpolated);
...
#ifdef CONTOUR_DISTANCE_DEBUG_SVG
            SVG svg(debug_out_path("contour_distance_raycasted-%d-%d.svg", iRun, &pt_next - contour.data()).c_str(),
                    bbox);
...
std::vector<float> contour_distance2(const EdgeGrid::Grid &grid, const size_t idx_contour,
                                     const Slic3r::Points &contour,
                                     const std::vector<ResampledPoint> &resampled_point_parameters, double compensation,
                                     double search_radius)
{
    ...
    if (contour.size() > 2)
    {
        ...
        {
            Visitor(const EdgeGrid::Grid &grid, const size_t idx_contour,
                    const std::vector<ResampledPoint> &resampled_point_parameters, double dist_same_contour_accept,
                        ...
                        if (it_contour_and_segment->first == idx_contour)
                        {
                            ...
                            else if (dist < dist_same_contour_reject + SCALED_EPSILON)
                            {
                                ...
                                bool inside = (t == 0.)   ? this->inside_corner(contour, ipt, this->point)
                                              : (t == 1.) ? this->inside_corner(contour, contour.segment_idx_next(ipt),
                                                                                this->point)
...
//#ifdef CONTOUR_DISTANCE_DEBUG_SVG
        ...
        if (out.back() < search_radius)
        {
            ...
            for (size_t i = 0; i < contour.size(); ++i)
                svg.draw(contour[i], out[i] < float(search_radius - SCALED_EPSILON) ? "red" : "green",
                         coord_t(scale_(0.1)));
        }
#endif /* CONTOUR_DISTANCE_DEBUG_SVG */
    }
...
static inline void smooth_compensation_banded(const Points &contour, float band, std::vector<float> &compensation,
                                              float strength, size_t num_iterations)
{
...
ExPolygon elephant_foot_compensation(const ExPolygon &input_expoly, double min_contour_width, const double compensation)
{
    ...
    else
    {
        ...
        for (size_t idx_contour = 0; idx_contour <= simplified.holes.size(); ++idx_contour)
        {
            ...
            std::vector<float> dists = contour_distance2(grid, idx_contour, poly.points, resampled_point_parameters,
                                                         scaled_compensation, search_radius);
            for (float &d : dists)
            {
        ...
        else
        {
            ...
            if (out_vec.size() > 1)
            {
                ...
                SVG::export_expolygons(
                    debug_out_path("elephant_foot_compensation-many_contours-%d.svg", iRun++).c_str(),
                    {{{input_expoly},
