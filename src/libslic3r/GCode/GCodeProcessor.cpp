...
static const float DEFAULT_ACCELERATION = 1500.0f;         // Default firmware values
static const float DEFAULT_RETRACT_ACCELERATION = 1500.0f; // Default firmware values
...
// Default extruder colors for multi-material printing
static const std::vector<std::string> DEFAULT_EXTRUDER_COLORS = {"#FF8000", "#DB5182", "#3EC0FF", "#FF4F4F", "#FBEB7D"};

namespace Slic3r
{

const std::vector<std::string> GCodeProcessor::Reserved_Tags = {"TYPE:",
                                                                "WIPE_START",
                                                                "WIPE_END",
                                                                "HEIGHT:",
                                                                "WIDTH:",
...
static float get_option_value(const ConfigOptionFloats &option, size_t id)
{
    return option.values.empty() ? 0.0f
                                 : ((id < option.values.size()) ? static_cast<float>(option.values[id])
                                                                : static_cast<float>(option.values.back()));
}
...
static float intersection_distance(float initial_rate, float final_rate, float acceleration, float distance)
{
    return (acceleration == 0.0f)
               ? 0.0f
               : (2.0f * acceleration * distance - sqr(initial_rate) + sqr(final_rate)) / (4.0f * acceleration);
}
...
static float calculate_corner_speed_from_jerk(float jerk_x, float jerk_y, float prev_dir_x, float prev_dir_y,
...
// Calculates maximum corner speed using junction deviation algorithm
// Used when machine_max_junction_deviation > 0 (Marlin M205 J)
//
// Formula: v = sqrt(junction_deviation * acceleration * (1/sin(half_angle) - 1))
//
// Parameters:
...
//
// Returns: Maximum corner speed (mm/s), or very large value if no limit needed
static float calculate_corner_speed_from_junction_deviation(float junction_deviation, float acceleration,
                                                            ...
                                                            float curr_dir_y)
{
    ...
    // Calculate the dot product of direction vectors to find the angle
    const float dot = prev_dir_x * curr_dir_x + prev_dir_y * curr_dir_y;

    // Clamp dot product to valid range [-1, 1] to avoid numerical issues with acos
    const float clamped_dot = std::max(-1.0f, std::min(1.0f, dot));

    // Calculate the angle between moves
    const float angle = std::acos(clamped_dot);

    // Nearly straight line - no junction speed limit needed
    if (angle < 0.01f) // ~0.5 degrees
        return std::numeric_limits<float>::max();
    ...
    // Avoid division by zero for very sharp corners
    if (sin_half_angle < 0.001f)
        return 0.0f; // Very sharp corner - full stop

    // Junction deviation formula: v = sqrt(jd * accel * (1/sin(θ/2) - 1))
    const float factor = (1.0f / sin_half_angle) - 1.0f;
    if (factor <= 0.0f)
        return std::numeric_limits<float>::max();

    return std::sqrt(junction_deviation * acceleration * factor);
}

static float acceleration_time_from_distance(float initial_feedrate, float distance, float acceleration)
{
    return (acceleration != 0.0f)
               ? (speed_from_distance(initial_feedrate, distance, acceleration) - initial_feedrate) / acceleration
               : 0.0f;
}
...
{
    float accelerate_distance = std::max(0.0f, estimated_acceleration_distance(feedrate_profile.entry,
                                                                               feedrate_profile.cruise, acceleration));
    const float decelerate_distance = std::max(0.0f,
                                               estimated_acceleration_distance(feedrate_profile.cruise,
                                                                               feedrate_profile.exit, -acceleration));
    ...
    {
        accelerate_distance = std::clamp(intersection_distance(feedrate_profile.entry, feedrate_profile.exit,
...
{
    //
    // Reference: Marlin firmware planner.cpp
    // Line 954
    //
    ...
    if (!prev.flags.nominal_length && prev.feedrate_profile.entry < curr.feedrate_profile.entry)
    {
        ...
        const float new_entry_speed = max_allowable_speed(-prev.acceleration, prev.feedrate_profile.entry,
                                                          prev.distance);
    ...
    if (curr.feedrate_profile.entry != max_entry_speed || next.flags.recalculate)
    {
        ...
        const float new_entry_speed = curr.flags.nominal_length
                                          ...
                                          : std::min(max_entry_speed,
                                                     max_allowable_speed(-curr.acceleration,
                                                                         next.feedrate_profile.entry, curr.distance));
                                                 ...
                                                 float time_compensation, std::function<void(int)> progress_callback)
{
    ...
    // NOTE: Block processing is INSTANT - the real bottleneck is move insertion later
    // Don't report progress here, only in move insertion loop

    for (size_t i = 0; i < n_blocks_process; ++i)
    {
        // Block processing - no progress reporting here (too fast and runs multiple times for time modes)
        ...
        // Negative compensation (e.g., -10%) increases time estimate by 10% (block_time *= 1.10)
        // Positive compensation (e.g., +10%) decreases time estimate by 10% (block_time *= 0.90)
        // Default is 0% since 16ms min segment time handles most real-world timing effects
        if (std::abs(time_compensation) > 0.001f)
            block_time *= (1.0f - time_compensation);
        ...
        if (mode == PrintEstimatedStatistics::ETimeMode::Normal)
        {
            ...
            if (curr_move.type == EMoveType::Extrude || curr_move.type == EMoveType::Travel ||
                curr_move.type == EMoveType::Wipe)
            {
                ...
                // Interlocking uses deliberate width transitions that should be instant, not tapered
                const bool interlock_transition = (prev_move.extrusion_role ==
                                                       GCodeExtrusionRole::InterlockingPerimeter ||
                                                   curr_move.extrusion_role ==
                                                       GCodeExtrusionRole::InterlockingPerimeter);
                ...
                if (EPSILON < block.trapezoid.accelerate_until &&
                    block.trapezoid.accelerate_until < block.distance - EPSILON)
                {
                    ...
                    {
                        const float delta_extruder = interpolate
                                                         ? lerp(prev_move.delta_extruder, curr_move.delta_extruder, t)
                                                         : curr_move.delta_extruder;
                        const float feedrate = interpolate ? lerp(prev_move.feedrate, curr_move.feedrate, t)
                                                           : curr_move.feedrate;
                        const float width = (interpolate && !interlock_transition)
                                                ? lerp(prev_move.width, curr_move.width, t)
                                                : curr_move.width;
                        const float height = interpolate ? lerp(prev_move.height, curr_move.height, t)
                                                         : curr_move.height;
                        const float mm3_per_mm = interpolate ? lerp(prev_move.mm3_per_mm, curr_move.mm3_per_mm, t)
                                                             : curr_move.mm3_per_mm;
                        const float fan_speed = interpolate ? lerp(prev_move.fan_speed, curr_move.fan_speed, t)
                                                            : curr_move.fan_speed;
                        const float temperature = interpolate ? lerp(prev_move.temperature, curr_move.temperature, t)
                                                              : curr_move.temperature;
                        actual_speed_moves.push_back({block.move_id, position, block.trapezoid.cruise_feedrate,
                                                      delta_extruder, feedrate, width, height, mm3_per_mm, fan_speed,
                                                      temperature});
                    }
                }
                ...
                actual_speed_moves.push_back({block.move_id, std::nullopt,
                                              (is_cruise_only || !has_deceleration) ? block.trapezoid.cruise_feedrate
...
void GCodeProcessor::UsedFilaments::increase_caches(double extruded_volume, unsigned char extruder_id,
                                                    double parking_volume, double extra_loading_volume)
{
...
void GCodeProcessor::UsedFilaments::process_role_cache(const GCodeProcessor *processor)
{
    if (role_cache != 0.0)
    {
        ...
        const double filament_diameter = processor->m_result.filament_diameters.empty()
                                             ? 1.75
                                             : ((static_cast<size_t>(processor->m_extruder_id) <
                                                 processor->m_result.filament_diameters.size())
                                                    ? processor->m_result.filament_diameters[processor->m_extruder_id]
                                                    : processor->m_result.filament_diameters.back());
        const double filament_density = processor->m_result.filament_densities.empty()
                                            ? 1.25
                                            : ((static_cast<size_t>(processor->m_extruder_id) <
                                                processor->m_result.filament_densities.size())
                                                   ? processor->m_result.filament_densities[processor->m_extruder_id]
                                                   : processor->m_result.filament_densities.back());

        const double s = PI * sqr(0.5 * filament_diameter);
        filament.first = role_cache / s * 0.001;
        filament.second = role_cache * filament_density * 0.001;
...
GCodeProcessorResult::~GCodeProcessorResult()
{
    // Clean up virtual files if owned
    if (virtual_gcode_file && owns_virtual_file)
    {
        delete virtual_gcode_file;
    }

    if (output_virtual_file && owns_output_virtual_file)
    {
        delete output_virtual_file;
    }
}

GCodeProcessorResult::GCodeProcessorResult(GCodeProcessorResult &&other) noexcept
    : is_binary_file(other.is_binary_file)
    , moves(std::move(other.moves))
    , lines_ends(std::move(other.lines_ends))
    , bed_shape(std::move(other.bed_shape))
    ...
    , sequential_collision_detected(std::move(other.sequential_collision_detected))
    , role_metrics(other.role_metrics)
    , overall_metrics(other.overall_metrics)
    , filename(std::move(other.filename))
    , id(other.id)
    , print_statistics(std::move(other.print_statistics))
    , virtual_gcode_file(other.virtual_gcode_file)
    , output_virtual_file(other.output_virtual_file)
    , owns_virtual_file(other.owns_virtual_file)
    , owns_output_virtual_file(other.owns_output_virtual_file)
{
    // Clear the other's pointers to prevent double-delete
    other.virtual_gcode_file = nullptr;
    other.output_virtual_file = nullptr;
    other.owns_virtual_file = false;
    other.owns_output_virtual_file = false;
}

GCodeProcessorResult &GCodeProcessorResult::operator=(GCodeProcessorResult &&other) noexcept
{
    if (this != &other)
    {
        // First clean up our current resources
        reset();
        ...
        // Transfer virtual file ownership
        virtual_gcode_file = other.virtual_gcode_file;
        output_virtual_file = other.output_virtual_file;
        owns_virtual_file = other.owns_virtual_file;
        owns_output_virtual_file = other.owns_output_virtual_file;
...
void GCodeProcessorResult::reset()
{
    ...
    // DEPRECATED: final_gcode_array has been replaced by output_virtual_file

    // The viewer can access virtual files after reset is called
    // To prevent crashes, we'll leak the memory rather than delete while viewer is active
    // TODO: Implement proper reference counting or viewer notification
    if (virtual_gcode_file && owns_virtual_file)
    {
        // delete virtual_gcode_file;
        virtual_gcode_file = nullptr;
    }
    ...
    {
        // delete output_virtual_file;
        output_virtual_file = nullptr;
    }
    owns_output_virtual_file = false;

#ifdef _WIN32
    _heapmin();                                                              // Compact the heap
    SetProcessWorkingSetSize(GetCurrentProcess(), (SIZE_T) -1, (SIZE_T) -1); // Trim working set
#endif
}

const std::vector<std::pair<GCodeProcessor::EProducer, std::string>> GCodeProcessor::Producers = {
    {EProducer::preFlight, "generated by preFlight"},
    {EProducer::Slic3rPE, "generated by Slic3r PE"},
...
bool GCodeProcessor::contains_reserved_tag(const std::string &gcode, std::string &found_tag)
{
    ...
    parser.parse_buffer(gcode,
                        [&ret, &found_tag](GCodeReader &parser, const GCodeReader::GCodeLine &line)
                        {
...
bool GCodeProcessor::contains_reserved_tags(const std::string &gcode, unsigned int max_count,
                                            std::vector<std::string> &found_tag)
{
    ...
    GCodeReader parser;
    parser.parse_buffer(gcode,
                        [&ret, &found_tag, max_count](GCodeReader &parser, const GCodeReader::GCodeLine &line)
                        {
                            std::string comment = line.raw();
                            if (comment.length() > 2 && comment.front() == ';')
                            {
...
{
    reset();
    m_time_processor.machines[static_cast<size_t>(PrintEstimatedStatistics::ETimeMode::Normal)].line_m73_main_mask =
        "M73 P%s R%s\n";
    m_time_processor.machines[static_cast<size_t>(PrintEstimatedStatistics::ETimeMode::Normal)].line_m73_stop_mask =
        "M73 C%s\n";
    m_time_processor.machines[static_cast<size_t>(PrintEstimatedStatistics::ETimeMode::Stealth)].line_m73_main_mask =
        "M73 Q%s S%s\n";
    m_time_processor.machines[static_cast<size_t>(PrintEstimatedStatistics::ETimeMode::Stealth)].line_m73_stop_mask =
        "M73 D%s\n";
}
...
void GCodeProcessor::apply_config(const PrintConfig &config)
{
    ...
    m_producer = EProducer::preFlight;
    m_flavor = config.gcode_flavor;
    // Default is P0 (true) - set in reset(). Will be updated when M566 is parsed.

    m_time_compensation = static_cast<float>(config.machine_time_compensation.value) / 100.0f;
    ...
    // Use slicer's retraction settings as defaults for firmware retraction
    // These will be overridden by M207 commands if present in gcode
    if (!config.retract_length.values.empty())
    {
        m_firmware_retraction.retract_length = static_cast<float>(config.retract_length.get_at(0));
    }
    if (!config.retract_speed.values.empty())
    {
        m_firmware_retraction.retract_feedrate = static_cast<float>(config.retract_speed.get_at(0));
    }
    if (!config.deretract_speed.values.empty())
    {
        float deretract = static_cast<float>(config.deretract_speed.get_at(0));
        // deretract_speed of 0 means use retract_speed
    ...
    // With MM setups like MMU2, the filaments may be expected to be parked at the beginning.
    ...
    for (size_t i = 0; i < static_cast<size_t>(PrintEstimatedStatistics::ETimeMode::Count); ++i)
    {
        float max_acceleration = get_option_value(m_time_processor.machine_limits.machine_max_acceleration_extruding,
                                                  i);
        m_time_processor.machines[i].max_acceleration = max_acceleration;
        m_time_processor.machines[i].acceleration = (max_acceleration > 0.0f) ? max_acceleration : DEFAULT_ACCELERATION;
        float max_retract_acceleration =
            get_option_value(m_time_processor.machine_limits.machine_max_acceleration_retracting, i);
        m_time_processor.machines[i].max_retract_acceleration = max_retract_acceleration;
        m_time_processor.machines[i].retract_acceleration = (max_retract_acceleration > 0.0f)
        ...
        m_time_processor.machines[i].travel_acceleration = (max_travel_acceleration > 0.0f)
    ...
    const ConfigOptionFloatOrPercent *first_layer_height = config.option<ConfigOptionFloatOrPercent>(
        "first_layer_height");
...
void GCodeProcessor::apply_config(const DynamicPrintConfig &config)
{
    ...
    // With MM setups like MMU2, the filaments may be expected to be parked at the beginning.
    // Remember the parking position so the initial load is not included in filament estimate.
    const ConfigOptionBool *single_extruder_multi_material = config.option<ConfigOptionBool>(
    ...
    m_single_extruder_multi_material = single_extruder_multi_material != nullptr &&
                                       single_extruder_multi_material->value;

    if (m_single_extruder_multi_material && wipe_tower != nullptr && parking_pos_retraction != nullptr &&
        extra_loading_move != nullptr)
    {
    ...
    const ConfigOptionEnum<MachineLimitsUsage> *machine_limits_usage =
        config.option<ConfigOptionEnum<MachineLimitsUsage>>("machine_limits_usage");
    ...
    if (use_machine_limits && (m_flavor == gcfMarlinLegacy || m_flavor == gcfMarlinFirmware ||
                               m_flavor == gcfRepRapFirmware || m_flavor == gcfRapid || m_flavor == gcfKlipper))
    {
        const ConfigOptionFloats *machine_max_acceleration_x = config.option<ConfigOptionFloats>(
            "machine_max_acceleration_x");
        if (machine_max_acceleration_x != nullptr)
            m_time_processor.machine_limits.machine_max_acceleration_x.values = machine_max_acceleration_x->values;

        const ConfigOptionFloats *machine_max_acceleration_y = config.option<ConfigOptionFloats>(
        ...
        const ConfigOptionFloats *machine_max_acceleration_z = config.option<ConfigOptionFloats>(
        ...
        const ConfigOptionFloats *machine_max_acceleration_e = config.option<ConfigOptionFloats>(
        ...
        if (machine_max_acceleration_extruding != nullptr)
            m_time_processor.machine_limits.machine_max_acceleration_extruding.values =
                machine_max_acceleration_extruding->values;

        // RepRapFirmware does not have a separate retraction acceleration concept.
        // It uses the E-axis maximum acceleration for all E moves including retractions.
        const ConfigOptionFloats *machine_max_acceleration_retracting = config.option<ConfigOptionFloats>(
            (m_flavor == gcfRepRapFirmware || m_flavor == gcfRapid) ? "machine_max_acceleration_e"
                                                                    : "machine_max_acceleration_retracting");
        if (machine_max_acceleration_retracting != nullptr)
            m_time_processor.machine_limits.machine_max_acceleration_retracting.values =
                machine_max_acceleration_retracting->values;
        ...
        if (machine_max_acceleration_travel != nullptr)
            m_time_processor.machine_limits.machine_max_acceleration_travel.values =
                machine_max_acceleration_travel->values;

        const ConfigOptionFloats *machine_min_extruding_rate = config.option<ConfigOptionFloats>(
        ...
        const ConfigOptionFloats *machine_min_travel_rate = config.option<ConfigOptionFloats>(
        ...
        const ConfigOptionFloats *machine_max_junction_deviation = config.option<ConfigOptionFloats>(
    ...
    const ConfigOptionPercent *time_comp = config.option<ConfigOptionPercent>("machine_time_compensation");
    if (time_comp != nullptr)
        m_time_compensation = static_cast<float>(time_comp->value) / 100.0f;

    if (m_flavor == gcfRepRapFirmware || m_flavor == gcfRapid)
    {
        // Parse individual M-code fields
        const char *mcode_keys[] = {"machine_rrf_m566", "machine_rrf_m201", "machine_rrf_m203", "machine_rrf_m204",
                                    "machine_rrf_m207"};
        for (const char *key : mcode_keys)
        {
            const ConfigOptionString *opt = config.option<ConfigOptionString>(key);
            if (opt != nullptr && !opt->value.empty())
            {
                parse_rrf_mcode_field(opt->value);
            }
        }
    }
}
...
void GCodeProcessor::parse_rrf_mcode_field(const std::string &mcode_line)
{
    // Parse a single M-code line from config
    if (mcode_line.empty())
        return;

    // Trim leading whitespace
    std::string line = mcode_line;
    ...
    // Parse using GCodeReader and route to appropriate handler
    GCodeReader parser;
    parser.parse_line(line,
                      [this](GCodeReader &, const GCodeReader::GCodeLine &gline)
                      {
                          if (gline.cmd_is("M566"))
                          {
                              this->process_M566(gline);
                          }
                          else if (gline.cmd_is("M201"))
                          {
                              this->process_M201(gline);
                          }
                          else if (gline.cmd_is("M203"))
                          {
                              this->process_M203(gline);
                          }
                          else if (gline.cmd_is("M204"))
                          {
                              this->process_M204(gline);
                          }
                          else if (gline.cmd_is("M207"))
                          {
                              this->process_M207(gline);
                          }
                      });
}
...
void GCodeProcessor::parse_rrf_limits_from_config(const PrintConfig &config)
{
    ...
    if (!config.machine_rrf_m566.value.empty())
        parse_rrf_mcode_field(config.machine_rrf_m566.value);
    if (!config.machine_rrf_m201.value.empty())
        parse_rrf_mcode_field(config.machine_rrf_m201.value);
    if (!config.machine_rrf_m203.value.empty())
        parse_rrf_mcode_field(config.machine_rrf_m203.value);
    if (!config.machine_rrf_m204.value.empty())
        parse_rrf_mcode_field(config.machine_rrf_m204.value);
    if (!config.machine_rrf_m207.value.empty())
...
void GCodeProcessor::process_M207(const GCodeReader::GCodeLine &line)
{
    ...
    // M207 S<length> R<extra_restart> F<retract_feedrate> T<unretract_feedrate> Z<zlift>
    // All feedrates in mm/min for RRF, lengths in mm

    float value;
    if (line.has_value('S', value))
        m_firmware_retraction.retract_length = value;

    if (line.has_value('R', value))
        m_firmware_retraction.retract_restart_extra = value;

    if (line.has_value('F', value))
        m_firmware_retraction.retract_feedrate = value * MMMIN_TO_MMSEC; // Convert mm/min to mm/s
...
void GCodeProcessor::reset()
{
    ...
    // We need to reconstruct it to ensure it's in a valid state
    m_result = GCodeProcessorResult(); // Reconstruct to valid empty state
    ...
    // Prevent dangling pointers when slicing multiple times
    m_virtual_file = nullptr;
    m_use_virtual_file = false;
}

GCodeProcessorResult &&GCodeProcessor::extract_result()
{
    // Transfer ownership of virtual files if they exist
    if (m_use_virtual_file && m_virtual_file)
    {
        m_result.virtual_gcode_file = m_virtual_file;
        m_result.owns_virtual_file = true;
        m_virtual_file = nullptr;
        m_use_virtual_file = false;
    }

    // Note: output_virtual_file is already set in post_process_virtual_file

    return std::move(m_result);
}

void GCodeProcessor::post_process_virtual_file()
{
    if (!m_virtual_file)
    {
        printf("ERROR: post_process_virtual_file called with no virtual file\n");
        return;
    }

    // Create output virtual file
    VirtualGCodeFile *output_vf = new VirtualGCodeFile();
    output_vf->reserve_lines(m_virtual_file->line_count() * 1.1); // +10% for M73

    // Initialize moves for rendering - CRITICAL!
    initialize_result_moves();
    m_line_id = 0; // Will be incremented to 1 on first line
    size_t g1_lines_counter = 0;

    // Clear and initialize lines_ends for virtual file
    m_result.lines_ends.clear();
    m_result.lines_ends.emplace_back(std::vector<size_t>());

    // The machine limits (from RRF M203, M201, etc.) and acceleration values (from M204)
    // were set during apply_config(). TimeProcessor::reset() would wipe them with defaults.
    // We must preserve them for accurate time estimation.

    // Save TimeProcessor-level settings
    MachineEnvelopeConfig saved_machine_limits = m_time_processor.machine_limits;
    bool saved_envelope_processing_enabled = m_time_processor.machine_envelope_processing_enabled;
    std::vector<float> saved_filament_load_times = m_time_processor.filament_load_times;
    std::vector<float> saved_filament_unload_times = m_time_processor.filament_unload_times;

    // Save per-machine acceleration values (from M204)
    struct SavedMachineAccel
    {
        float acceleration;
    ...
    std::array<SavedMachineAccel, static_cast<size_t>(PrintEstimatedStatistics::ETimeMode::Count)> saved_machine_accels;
    for (size_t i = 0; i < static_cast<size_t>(PrintEstimatedStatistics::ETimeMode::Count); ++i)
    {
        saved_machine_accels[i] = {m_time_processor.machines[i].acceleration,
                                   m_time_processor.machines[i].max_acceleration,
                                   m_time_processor.machines[i].retract_acceleration,
                                   m_time_processor.machines[i].max_retract_acceleration,
                                   m_time_processor.machines[i].travel_acceleration,
                                   m_time_processor.machines[i].max_travel_acceleration};
    }

    // Initialize time estimation
    m_time_processor.reset();

    // Restore the machine envelope settings that were configured for this print
    m_time_processor.machine_limits = saved_machine_limits;
    m_time_processor.machine_envelope_processing_enabled = saved_envelope_processing_enabled;
    m_time_processor.filament_load_times = saved_filament_load_times;
    m_time_processor.filament_unload_times = saved_filament_unload_times;

    // Restore per-machine acceleration values
    for (size_t i = 0; i < static_cast<size_t>(PrintEstimatedStatistics::ETimeMode::Count); ++i)
    {
        m_time_processor.machines[i].acceleration = saved_machine_accels[i].acceleration;
        m_time_processor.machines[i].max_acceleration = saved_machine_accels[i].max_acceleration;
        m_time_processor.machines[i].retract_acceleration = saved_machine_accels[i].retract_acceleration;
        m_time_processor.machines[i].max_retract_acceleration = saved_machine_accels[i].max_retract_acceleration;
        m_time_processor.machines[i].travel_acceleration = saved_machine_accels[i].travel_acceleration;
        m_time_processor.machines[i].max_travel_acceleration = saved_machine_accels[i].max_travel_acceleration;
    }
    // Note: simulate_st_synchronize behavior is handled in the processor, not individual machines
    ...
    // Lambda functions for time formatting and M73 lines
    ...
    // Lambda to insert M73 progress
    auto insert_m73 = [&](size_t g1_counter)
    {
        if (m_time_processor.export_remaining_time_enabled)
        {
            for (size_t i = 0; i < static_cast<size_t>(PrintEstimatedStatistics::ETimeMode::Count); ++i)
            {
                ...
                if (machine.enabled)
                {
                    // Skip to current position in cache
                    auto &it = g1_times_cache_it[i];
                    while (it != machine.g1_times_cache.end() && it->id < g1_counter)
                        ++it;
                    if (it != machine.g1_times_cache.end() && it->id == g1_counter)
                    {
                        ...
                        {
                            const std::string &line_M73 = format_line_M73_main(machine.line_m73_main_mask.c_str(),
                                                                               to_export_main.first,
                                                                               to_export_main.second);
                            output_vf->write_line(line_M73);
    ...
    // CRITICAL: Parse G-code to generate moves!
    GCodeReader parser;

    // Parse G-code to generate moves

    for (size_t line_idx = 0; line_idx < m_virtual_file->line_count(); ++line_idx)
    {
        std::string line = m_virtual_file->get_line(line_idx);
        ...
        // Set m_line_id to line_idx (0-based) so that after process_gcode_line
        // increments it with ++m_line_id, it becomes the correct 1-based line number.
        // This ensures store_move_vertex gets the correct gcode_id for the G1 command.
        m_line_id = line_idx;

        // Parse line to generate moves
        parser.parse_line(line,
                          [&](GCodeReader &reader, const GCodeReader::GCodeLine &gline)
                          {
                              // Save current line ID AFTER increment happens inside process_gcode_line
                              // We need to save/restore to prevent the outer loop from seeing the increment

                              // CRITICAL: This generates the move vertices!
                              this->process_gcode_line(gline, true);

                              // After process_gcode_line, m_line_id has been incremented to line_idx + 1
                              // We don't need to restore it since the outer loop will set it fresh next iteration

                              // Track G1/G0 for M73 insertion
                              if (gline.cmd_is("G1") || gline.cmd_is("G0"))
                              {
                                  // Check if M73 needed after this move
                                  insert_m73(g1_lines_counter);
                                  g1_lines_counter++;
                              }
                          });

        // Check for placeholders and replace them with actual values
        // Placeholder format in G-code: ";_GP_ESTIMATED_PRINTING_TIME_PLACEHOLDER"
        // We need to skip the leading semicolon when comparing
        bool placeholder_processed = false;
        std::string_view line_view(line);
        if (line_view.length() > 1 && line_view[0] == ';')
        {
            line_view = line_view.substr(1); // Skip the semicolon
        }

        if (line_view == reserved_tag(ETags::Estimated_Printing_Time_Placeholder))
        {
            // Replace placeholder with actual estimated printing time
            for (size_t i = 0; i < static_cast<size_t>(PrintEstimatedStatistics::ETimeMode::Count); ++i)
            {
                const TimeMachine &machine = m_time_processor.machines[i];
                ...
                if (mode == PrintEstimatedStatistics::ETimeMode::Normal || machine.enabled)
                {
                    ...
                    output_vf->write_line(buf);
                }
            }
            // Also write first layer time
            for (size_t i = 0; i < static_cast<size_t>(PrintEstimatedStatistics::ETimeMode::Count); ++i)
            {
                const TimeMachine &machine = m_time_processor.machines[i];
                PrintEstimatedStatistics::ETimeMode mode = static_cast<PrintEstimatedStatistics::ETimeMode>(i);
        ...
        // Write original line to output (with newline) if not a placeholder
        if (!placeholder_processed)
        {
            output_vf->write_line(line + "\n");
        }
    }

    // Moves generated successfully

    // Store output in result
    m_result.output_virtual_file = output_vf;
    m_result.owns_output_virtual_file = true;

    // Populate lines_ends for compatibility with libvgcode
    // Each entry is the cumulative character position after each line
    size_t cumulative_pos = 0;
    for (size_t i = 0; i < output_vf->line_count(); ++i)
    {
        std::string line = output_vf->get_line(i);
        cumulative_pos += line.length();
        m_result.lines_ends.front().push_back(cumulative_pos);
    }

    // lines_ends populated successfully

    // Must call finalize() to process accumulated filament data into print statistics
    // Without this, legend shows 0.00 for all values because used_filaments_per_role is empty
    this->finalize(false);
}
                                        ...
                                        std::function<void(void)> cancel_callback)
{
    ...
    {
        m_parser.parse_file_raw(filename,
                                [this](GCodeReader &reader, const char *begin, const char *end)
                                {
        ...
        // if the gcode was produced by preFlight,
        // extract the config from it
        if (m_producer == EProducer::preFlight || m_producer == EProducer::Slic3rPE || m_producer == EProducer::Slic3r)
        {
            DynamicPrintConfig config;
            config.apply(FullPrintConfig::defaults());
            // Silently substitute unknown values by new ones for loading configurations from preFlight's own G-code.
...
static void update_lines_ends_and_out_file_pos(const std::string &out_string, std::vector<size_t> &lines_ends,
                                               size_t *out_file_pos)
{
                                         ...
                                         std::function<void()> cancel_callback)
{
    ...
    if ((EBlockType) block_header.type == EBlockType::FileMetadata)
    {
        ...
        auto producer_it = std::find_if(file_metadata_block.raw_data.begin(), file_metadata_block.raw_data.end(),
                                        ...
                                        { return item.first == "Producer"; });
        if (producer_it != file_metadata_block.raw_data.end() &&
            boost::starts_with(producer_it->second, std::string(SLIC3R_APP_NAME)))
            m_producer = EProducer::preFlight;
        else
            m_producer = EProducer::Unknown;
        res = read_next_block_header(*file.f, file_header, block_header, cs_buffer.data(), cs_buffer.size());
...
{
    // Smooth progress will be reported from calculate_time() based on moves processed
    if (m_print && perform_post_process)
    {
        m_print->set_status(50, _u8L("Processing G-code"));
    }
    ...
    // Uses the accurate time values computed by the motion planner simulation.
    // For each move, scans forward 1 second and counts segments.
    {
        struct RoleStats
        {
            size_t max_commands = 0;
            size_t max_at_move = 0;
            double max_window_time = 0.0;
            unsigned int max_layer = 0;
        };

        constexpr size_t NUM_ROLES = static_cast<size_t>(GCodeExtrusionRole::Count);
        std::array<RoleStats, NUM_ROLES> role_stats{};
        RoleStats overall_stats{};

        const size_t num_moves = m_result.moves.size();
        constexpr size_t NORMAL_MODE = static_cast<size_t>(PrintEstimatedStatistics::ETimeMode::Normal);

        for (size_t i = 0; i < num_moves; i++)
        {
            const auto &start_move = m_result.moves[i];

            // Skip moves with no time (non-motion commands)
            if (start_move.time[NORMAL_MODE] <= 0.0f)
                continue;

            unsigned int start_layer = start_move.layer_id;

            // === OVERALL STATS: Count ALL commands in 1-second window ===
            // Note: 0-time segments are still commands the motion controller must process
            {
                double accumulated_time = 0.0;
                size_t command_count = 0;

                for (size_t j = i; j < num_moves && accumulated_time < 1.0; j++)
                {
                    const auto &move = m_result.moves[j];

                    if (move.layer_id != start_layer)
                        break;

                    // Accumulate time for window calculation
                    float move_time = move.time[NORMAL_MODE];
                    if (move_time > 0.0f)
                        accumulated_time += move_time;

                    // Count ALL commands (including 0-time segments)
                    command_count++;
                }

                if (command_count > overall_stats.max_commands)
                {
                    overall_stats.max_commands = command_count;
                    overall_stats.max_at_move = i;
                    overall_stats.max_window_time = accumulated_time;
                    overall_stats.max_layer = start_layer;
                }
            }

            // === PER-ROLE STATS: Only count commands of SAME role ===
            if (start_move.type == EMoveType::Extrude)
            {
                double accumulated_time = 0.0;
                size_t command_count = 0;
                GCodeExtrusionRole start_role = start_move.extrusion_role;
                ...
                for (size_t j = i; j < num_moves && accumulated_time < 1.0; j++)
                {
                    ...
                    // For per-role, count time from ALL moves but only count segments of same role
                    float move_time = move.time[NORMAL_MODE];
                    if (move_time > 0.0f)
                        accumulated_time += move_time;

                    // Only count if same role
                    if (move.type == EMoveType::Extrude && move.extrusion_role == start_role)
                        command_count++;
                }

                size_t role_idx = static_cast<size_t>(start_role);
                if (role_idx < NUM_ROLES && command_count > role_stats[role_idx].max_commands)
                {
                    role_stats[role_idx].max_commands = command_count;
                    role_stats[role_idx].max_at_move = i;
                    role_stats[role_idx].max_window_time = accumulated_time;
                    role_stats[role_idx].max_layer = start_layer;
                }
            }
        }

        // Store metrics in result for GUI display
        double overall_cps = (overall_stats.max_window_time > 0)
                                 ? overall_stats.max_commands / overall_stats.max_window_time
                                 : 0.0;
        m_result.overall_metrics.max_commands_per_sec = static_cast<unsigned int>(std::ceil(overall_cps));
        m_result.overall_metrics.max_layer = overall_stats.max_layer;

        for (size_t idx = 0; idx < NUM_ROLES; ++idx)
        {
            if (role_stats[idx].max_commands > 0 && role_stats[idx].max_window_time > 0)
            {
                double cps = role_stats[idx].max_commands / role_stats[idx].max_window_time;
                m_result.role_metrics[idx].max_commands_per_sec = static_cast<unsigned int>(std::ceil(cps));
                m_result.role_metrics[idx].max_layer = role_stats[idx].max_layer;
            }
        }
    }

    // process the time blocks (instant)
    for (size_t i = 0; i < static_cast<size_t>(PrintEstimatedStatistics::ETimeMode::Count); ++i)
    {
    ...
    // process filament caches (instant)
...
float GCodeProcessor::get_time(PrintEstimatedStatistics::ETimeMode mode) const
{
    return (mode < PrintEstimatedStatistics::ETimeMode::Count)
               ? float(m_time_processor.machines[static_cast<size_t>(mode)].time)
               : 0.0f;
}
                                                     ...
                                                     ForwardCompatibilitySubstitutionRule compatibility_rule)
{
    ...
    if (key_value_pairs < 80)
        throw Slic3r::RuntimeError(format("Suspiciously low number of configuration values extracted from %1%: %2%",
                                          filename, key_value_pairs));
...
void GCodeProcessor::apply_config_kissslicer(const std::string &filename)
{
    ...
    m_parser.parse_file_raw(filename,
                            [this, &found_counter](GCodeReader &reader, const char *begin, const char *end)
                            {
                                ...
                                auto detect_printer = [this](const std::string_view comment)
                                {
                                    ...
                                    if (pos != comment.npos)
                                    {
                                        ...
                                        if (elements.size() == 2)
                                        {
                                            ...
                                            if (boost::contains(elements[1], "MK2.5") ||
                                                boost::contains(elements[1], "MK3"))
...
{
    return (mode < PrintEstimatedStatistics::ETimeMode::Count)
               ? m_time_processor.machines[static_cast<size_t>(mode)].first_layer_time
               : 0.0f;
}
...
void GCodeProcessor::apply_config_simplify3d(const std::string &filename)
{
    ...
    m_parser.parse_file_raw(
        filename,
        [this, &bed_size, &producer_detected](GCodeReader &reader, const char *begin, const char *end)
        {
    ...
    if (m_result.extruders_count == 0)
        m_result.extruders_count = std::max<size_t>(1, std::min(m_result.filament_diameters.size(),
                                                                std::min(m_result.filament_densities.size(),
                                                                         m_result.filament_cost.size())));

    if (bed_size.is_defined())
    {
        m_result.bed_shape = {{0.0, 0.0}, {bed_size.x, 0.0}, {bed_size.x, bed_size.y}, {0.0, bed_size.y}};
    }
}
    ...
    if (cmd.length() > 1)
    {
        ...
        case 'G':
            ...
            case 2:
                ...
                case '2':
                {
                    process_G2_G3(line, true);
                    break;
                } // CW Arc Move
                case '3':
                {
                    process_G2_G3(line, false);
                    break;
                } // CCW Arc Move
                default:
                    break;
...
void GCodeProcessor::process_tags(const std::string_view comment, bool producers_enabled)
{
    ...
    if (!producers_enabled || m_producer == EProducer::preFlight)
    {
        // height tag
    ...
    if (boost::starts_with(comment, reserved_tag(ETags::Color_Change)))
    {
            ...
            if (tokens[1][0] == 'T')
            {
                ...
                if (!parse_number(tokens[1].substr(1), eid) || eid < 0 || eid > 255)
                {
                    BOOST_LOG_TRIVIAL(error)
                        << "GCodeProcessor encountered an invalid value for Color_Change (" << comment << ").";
                    return;
                }
        ...
        if (extruder_id < m_extruder_colors.size())
            m_extruder_colors[extruder_id] = static_cast<unsigned char>(m_extruder_offsets.size()) +
        ...
        if (m_extruder_id == extruder_id)
        {
            ...
            CustomGCode::Item item = {static_cast<double>(m_end_position[2]), CustomGCode::ColorChange, extruder_id + 1,
                                      color, ""};
    ...
    // layer change tag
    // preFlight: Use starts_with since LAYER_CHANGE now includes layer number (e.g., "LAYER_CHANGE: 123")
    if (boost::starts_with(comment, reserved_tag(ETags::Layer_Change)))
    {
        ++m_layer_id;
        return;
    }
...
bool GCodeProcessor::process_producers_tags(const std::string_view comment)
{
    ...
    case EProducer::preFlight:
    {
        return process_slicer_tags(comment);
    }
    case EProducer::Cura:
    {
    ...
    case EProducer::BambuStudio:
    {
        return process_bambustudio_tags(comment);
    }
    default:
    {
        return false;
...
bool GCodeProcessor::process_slicer_tags(const std::string_view comment)
{
    return false;
}
...
bool GCodeProcessor::process_simplify3d_tags(const std::string_view comment)
{
    ...
    if (pos == 0)
    {
        ...
        {
            if (!parse_number(data.substr(h_start + 1, (h_end != data.npos) ? h_end - h_start - 1 : h_end),
                              m_forced_height))
                BOOST_LOG_TRIVIAL(error) << "GCodeProcessor encountered an invalid value for Height (" << comment
                                         << ").";
        }
        ...
        {
            if (!parse_number(data.substr(w_start + 1, (w_end != data.npos) ? w_end - w_start - 1 : w_end),
                              m_forced_width))
                BOOST_LOG_TRIVIAL(error) << "GCodeProcessor encountered an invalid value for Width (" << comment
                                         << ").";
        }
...
{
    const float filament_diameter = m_result.filament_diameters.empty()
                                        ? 1.75f
                                        : ((static_cast<size_t>(m_extruder_id) < m_result.filament_diameters.size())
    ...
    auto move_type = [this](const AxisCoords &delta_pos)
    {
        if (m_wiping)
            ...
            return (delta_pos[X] != 0.0f || delta_pos[Y] != 0.0f || delta_pos[Z] != 0.0f) ? EMoveType::Travel
                                                                                          : EMoveType::Retract;
        else if (delta_pos[E] > 0.0f)
        {
    ...
    auto extract_absolute_position_on_axis =
        [&](Axis axis, std::optional<double> value, double area_filament_cross_section)
    {
        if (value.has_value())
        {
    ...
    if (type == EMoveType::Extrude)
    {
        ...
        if (m_forced_width > 0.0f)
            ...
            m_width = filament_diameter * std::sqrt(delta_pos[E] / delta_xyz);
        else
    ...
    // Direction is only meaningful for XY moves; Z-only or E-only moves don't contribute to junction angles
    const float xy_length = std::sqrt(sqr(delta_pos[X]) + sqr(delta_pos[Y]));
    float move_dir_x = 0.0f;
    float move_dir_y = 0.0f;
    bool move_has_direction = false;
    if (xy_length > EPSILON)
    {
        move_dir_x = static_cast<float>(delta_pos[X]) / xy_length;
        move_dir_y = static_cast<float>(delta_pos[Y]) / xy_length;
        move_has_direction = true;
    }
    ...
    for (size_t i = 0; i < static_cast<size_t>(PrintEstimatedStatistics::ETimeMode::Count); ++i)
    {
        ...
        curr.dir_x = move_dir_x;
        curr.dir_y = move_dir_y;
        curr.has_direction = move_has_direction;
        ...
        for (unsigned char a = X; a <= E; ++a)
        {
            ...
            {
                const float axis_max_feedrate =
                    get_axis_max_feedrate(static_cast<PrintEstimatedStatistics::ETimeMode>(i), static_cast<Axis>(a));
        ...
        for (unsigned char a = X; a <= E; ++a)
        {
            const float axis_max_jerk = get_axis_max_jerk(static_cast<PrintEstimatedStatistics::ETimeMode>(i),
                                                          static_cast<Axis>(a));
        ...
        if (!blocks.empty() && prev.feedrate > PREVIOUS_FEEDRATE_THRESHOLD)
        {
            ...
            const float smaller_speed_factor = prev_speed_larger ? (block.feedrate_profile.cruise / prev.feedrate)
            ...
            // For XY moves, calculate corner speed from junction angle and jerk/JD limits.
            // This is pure physics: velocity_change = 2 * V * sin(angle/2) must respect jerk.
            // For Z/E-only moves, fall back to per-axis jerk calculation.
            if (prev.has_direction && curr.has_direction)
            {
                float corner_speed;

                // Junction deviation only applies to Marlin firmware - RRF uses M566/M595 instead
                const float junction_deviation =
                    (m_flavor == gcfMarlinLegacy || m_flavor == gcfMarlinFirmware || m_flavor == gcfKlipper)
                        ? get_option_value(m_time_processor.machine_limits.machine_max_junction_deviation, i)
                        : 0.0f;
                if (junction_deviation > 0.0f)
                {
                    // Use junction deviation algorithm (Marlin M205 J)
                    // Get acceleration for the current move
                    const float acceleration = get_acceleration(static_cast<PrintEstimatedStatistics::ETimeMode>(i));
                    corner_speed = calculate_corner_speed_from_junction_deviation(junction_deviation, acceleration,
                                                                                  prev.dir_x, prev.dir_y, curr.dir_x,
                                                                                  curr.dir_y);
                }
                else
                {
                    // Use classic jerk algorithm (M205 XYZE / M566)
                    const float jerk_x = get_axis_max_jerk(static_cast<PrintEstimatedStatistics::ETimeMode>(i), X);
                    const float jerk_y = get_axis_max_jerk(static_cast<PrintEstimatedStatistics::ETimeMode>(i), Y);
                    corner_speed = calculate_corner_speed_from_jerk(jerk_x, jerk_y, prev.dir_x, prev.dir_y, curr.dir_x,
                                                                    curr.dir_y);
                }

                // Apply corner speed limit
                vmax_junction = std::min(vmax_junction, corner_speed);

                // Also apply Z and E jerk limits if those axes are moving
                float v_factor = 1.0f;
                for (unsigned char a = Z; a <= E; ++a)
                {
                    ...
                    const float jerk = std::abs(v_exit - v_entry);
                    const float axis_max_jerk = get_axis_max_jerk(static_cast<PrintEstimatedStatistics::ETimeMode>(i),
                                                                  static_cast<Axis>(a));
                    if (jerk > axis_max_jerk && jerk > 0.0f)
                        v_factor = std::min(v_factor, axis_max_jerk / jerk);
                }

                vmax_junction *= v_factor;

                // NOTE: We intentionally do NOT apply safe_feedrate limit here.
                // The original code incorrectly treats jerk as a max speed
                // rather than max velocity change. Our corner speed calculation already
                // properly accounts for jerk limits based on direction change, so the
                // safe_feedrate cap would be redundant and overly conservative.
            }
            else
            {
                // Z-only or E-only moves - use classic per-axis jerk calculation
                float v_factor = 1.0f;
                bool limited = false;
                ...
                for (unsigned char a = X; a <= E; ++a)
                {
                    ...
                    const float jerk = (v_exit > v_entry)
                                           ? ((v_entry > 0.0f || v_exit < 0.0f) ? (v_exit - v_entry)
                                                                                : std::max(v_exit, -v_entry))
                                           : ((v_entry < 0.0f || v_exit > 0.0f) ? (v_entry - v_exit)
                                                                                : std::max(-v_exit, v_entry));
        ...
        // P0 (m_rrf_jerk_policy=true): Jerk only between consecutive extrusion OR consecutive travel moves.
        //     Transitions between move types get zero entry speed.
        // P1 (m_rrf_jerk_policy=false): Marlin-like behavior, jerk applied between any moves.
        if (m_rrf_jerk_policy && (m_flavor == gcfRepRapFirmware || m_flavor == gcfRapid))
        {
            // Determine if current and previous moves are "consecutive compatible" for P0
            // Compatible pairs: Extrude<->Extrude, Travel<->Travel, Wipe<->Wipe, Travel<->Wipe
            bool curr_is_extrude = (type == EMoveType::Extrude);
            bool curr_is_travel = (type == EMoveType::Travel || type == EMoveType::Wipe);

            bool prev_is_extrude = false;
            bool prev_is_travel = false;
            if (!blocks.empty())
            {
                EMoveType prev_type = blocks.back().move_type;
                prev_is_extrude = (prev_type == EMoveType::Extrude);
                prev_is_travel = (prev_type == EMoveType::Travel || prev_type == EMoveType::Wipe);
            }

            // Allow jerk only for consecutive same-category moves
            bool consecutive_compatible = (curr_is_extrude && prev_is_extrude) || (curr_is_travel && prev_is_travel);

            if (!consecutive_compatible)
            {
                block.max_entry_speed = 0.0f;
                block.feedrate_profile.entry = 0.0f;
                ++m_rrf_jerk_affected_blocks;
            }
        }
    ...
    if (m_seams_detector.is_active() &&
        (type != EMoveType::Extrude || (m_extrusion_role != GCodeExtrusionRole::ExternalPerimeter &&
...
void GCodeProcessor::process_G2_G3(const GCodeReader::GCodeLine &line, bool clockwise)
{
    ...
    if (m_flavor == gcfMarlinFirmware)
    {
        ...
        // reference:
        // Reference: Marlin firmware G2_G3.cpp - plan_arc()
        ...
        if (segments > 1)
        {
            ...
            for (size_t i = 1; i < segments; ++i)
            {
                ...
                m_start_position =
                    m_end_position; // this is required because we are skipping the call to process_gcode_line()
        ...
        internal_only_g1_line(adjust_target(end_position, prev_target), arc.delta_z() != 0.0,
                              (segments == 1) ? feedrate : std::nullopt, extrusion);
    }
    else
    {
        // calculate arc segments
        // reference:
        // Reference: Legacy firmware motion_control.cpp - mc_arc()
        ...
        const size_t segments = Geometry::ArcWelder::arc_discretization_steps(arc.start_radius(), std::abs(arc.angle),
                                                                              gcode_arc_tolerance);
#endif
...
void GCodeProcessor::create_firmware_retract_time_block(EMoveType type, float distance, float feedrate,
                                                        float acceleration, Axis axis)
{
    if (distance <= 0.0f || feedrate <= 0.0f)
        return;
    ...
    for (size_t i = 0; i < static_cast<size_t>(PrintEstimatedStatistics::ETimeMode::Count); ++i)
    {
        ...
        // Limit feedrate by axis maximum
        float axis_max_feedrate = get_axis_max_feedrate(static_cast<PrintEstimatedStatistics::ETimeMode>(i), axis);
        float limited_feedrate = (axis_max_feedrate > 0.0f) ? std::min(feedrate, axis_max_feedrate) : feedrate;

        // Limit acceleration by axis maximum
        float axis_max_accel = get_axis_max_acceleration(static_cast<PrintEstimatedStatistics::ETimeMode>(i), axis);
        float limited_accel = (axis_max_accel > 0.0f) ? std::min(acceleration, axis_max_accel) : acceleration;

        TimeBlock block;
        block.move_type = type;
        block.role = GCodeExtrusionRole::None;
        block.distance = distance;
        block.g1_line_id = m_g1_line_id;
        block.move_id = static_cast<unsigned int>(m_result.moves.size());
        block.remaining_internal_g1_lines = 0;
        block.layer_id = std::max<unsigned int>(1, m_layer_id);

        // Set feedrate profile for single-axis move starting and ending at rest
        block.feedrate_profile.cruise = limited_feedrate;
        block.feedrate_profile.entry = 0.0f; // Start from rest
        block.feedrate_profile.exit = 0.0f;  // End at rest

        block.acceleration = limited_accel;
        block.max_entry_speed = 0.0f; // Firmware retractions start from rest
        block.safe_feedrate = limited_feedrate;
        block.flags.nominal_length = false;
        block.flags.recalculate = true;

        // Calculate trapezoid for this block
        block.calculate_trapezoid();

        machine.blocks.push_back(block);
    }

    // Check if we need to process accumulated blocks
    if (m_time_processor.machines[0].blocks.size() > TimeProcessor::Planner::refresh_threshold)
        calculate_time(m_result, TimeProcessor::Planner::queue_size);
}
...
void GCodeProcessor::process_G10(const GCodeReader::GCodeLine &line)
{
    if (m_flavor == gcfRepRapFirmware || m_flavor == gcfRapid)
    {
        // Check if this is the temperature-setting variant (G10 P... S...)
        // G10 with P and S parameters sets tool temperature, not retraction
        float new_temp;
    ...
    // This is a firmware retraction command
    // G10 performs: E retract, then optional Z lift

    // Step 1: E-axis retraction
    float retract_accel = get_retract_acceleration(PrintEstimatedStatistics::ETimeMode::Normal);
    if (m_firmware_retraction.retract_length > 0.0f)
    {
        create_firmware_retract_time_block(EMoveType::Retract, m_firmware_retraction.retract_length,
                                           m_firmware_retraction.retract_feedrate, retract_accel, E);
    }

    // Step 2: Z-lift (if configured)
    if (m_firmware_retraction.retract_zlift > 0.0f)
    {
        float z_feedrate = get_axis_max_feedrate(PrintEstimatedStatistics::ETimeMode::Normal, Z);
        float z_accel = get_axis_max_acceleration(PrintEstimatedStatistics::ETimeMode::Normal, Z);

        create_firmware_retract_time_block(EMoveType::Retract, // Z-lift is part of retraction
                                           m_firmware_retraction.retract_zlift, z_feedrate, z_accel, Z);

        m_firmware_retraction.z_before_lift = static_cast<float>(m_end_position[Z]);
        m_firmware_retraction.z_lifted = true;

        // Update Z position (for visualization/tracking)
        m_end_position[Z] += m_firmware_retraction.retract_zlift;
    }

    // Update E position (retraction is negative)
    m_end_position[E] -= m_firmware_retraction.retract_length;

    // Store move vertex (for visualization in GCode viewer)
    store_move_vertex(EMoveType::Retract);
}

void GCodeProcessor::process_G11(const GCodeReader::GCodeLine &line)
{
    // G11 performs: Z lower (if Z was lifted), then E prime

    // Step 1: Z-lower (if Z was lifted during retraction AND we haven't moved elsewhere)
    // If an absolute G1 Z move occurred between G10 and G11, don't apply the Z-lower
    // because the Z position has already been set to the correct absolute value.
    if (m_firmware_retraction.retract_zlift > 0.0f && m_firmware_retraction.z_lifted)
    {
        // Check if we're still at the lifted Z position (within tolerance)
        float expected_lifted_z = m_firmware_retraction.z_before_lift + m_firmware_retraction.retract_zlift;
        if (std::abs(m_end_position[Z] - expected_lifted_z) < 0.001f)
        {
            // We're still at the lifted position, so apply the Z-lower
            float z_feedrate = get_axis_max_feedrate(PrintEstimatedStatistics::ETimeMode::Normal, Z);
            float z_accel = get_axis_max_acceleration(PrintEstimatedStatistics::ETimeMode::Normal, Z);

            create_firmware_retract_time_block(EMoveType::Unretract, // Z-lower is part of unretraction
                                               m_firmware_retraction.retract_zlift, z_feedrate, z_accel, Z);

            // Update Z position
            m_end_position[Z] -= m_firmware_retraction.retract_zlift;
        }
        // else: An absolute Z move occurred, Z is already at correct position, skip Z-lower

        // Clear the lifted state
        m_firmware_retraction.z_lifted = false;
    }

    // Step 2: E-axis prime (retract_length + restart_extra)
    float prime_length = m_firmware_retraction.retract_length + m_firmware_retraction.retract_restart_extra;
    float retract_accel = get_retract_acceleration(PrintEstimatedStatistics::ETimeMode::Normal);

    if (prime_length > 0.0f)
    {
        create_firmware_retract_time_block(EMoveType::Unretract, prime_length, m_firmware_retraction.unretract_feedrate,
                                           retract_accel, E);
    }

    // Update E position
    m_end_position[E] += prime_length;
...
{
    // G22 is Marlin's firmware retraction - same logic as G10
    ...
    if (m_firmware_retraction.retract_zlift > 0.0f)
    {
        float z_feedrate = get_axis_max_feedrate(PrintEstimatedStatistics::ETimeMode::Normal, Z);
        float z_accel = get_axis_max_acceleration(PrintEstimatedStatistics::ETimeMode::Normal, Z);
        create_firmware_retract_time_block(EMoveType::Retract, m_firmware_retraction.retract_zlift, z_feedrate, z_accel,
                                           Z);
        m_end_position[Z] += m_firmware_retraction.retract_zlift;
    }
...
{
    // G23 is Marlin's firmware unretract - same logic as G11

    // Step 1: Z-lower (if Z was lifted)
    if (m_firmware_retraction.retract_zlift > 0.0f)
    {
        float z_feedrate = get_axis_max_feedrate(PrintEstimatedStatistics::ETimeMode::Normal, Z);
        float z_accel = get_axis_max_acceleration(PrintEstimatedStatistics::ETimeMode::Normal, Z);
        create_firmware_retract_time_block(EMoveType::Unretract, m_firmware_retraction.retract_zlift, z_feedrate,
                                           z_accel, Z);
        m_end_position[Z] -= m_firmware_retraction.retract_zlift;
    }
...
void GCodeProcessor::process_G28(const GCodeReader::GCodeLine &line)
{
    ...
    GCodeReader reader;
    reader.parse_line(new_line_raw,
                      [&](GCodeReader &reader, const GCodeReader::GCodeLine &gline) { new_gline = gline; });
...
void GCodeProcessor::process_G92(const GCodeReader::GCodeLine &line)
{
    ...
    {
        // The G92 may be called for axes that preFlight does not recognize,
...
void GCodeProcessor::process_M132(const GCodeReader::GCodeLine &line)
{
    ...
    // Using this command to reset the axis origin to zero
...
void GCodeProcessor::process_M201(const GCodeReader::GCodeLine &line)
{
    ...
    float factor = ((m_flavor != gcfRepRapSprinter && m_flavor != gcfRepRapFirmware && m_flavor != gcfRapid) &&
                    m_units == EUnits::Inches)
                       ? INCHES_TO_MM
                       : 1.0f;
...
void GCodeProcessor::process_M203(const GCodeReader::GCodeLine &line)
{
    ...
    float factor = (m_flavor == gcfMarlinLegacy || m_flavor == gcfMarlinFirmware || m_flavor == gcfSmoothie)
                       ? 1.0f
                       : MMMIN_TO_MMSEC;
    ...
    for (size_t i = 0; i < static_cast<size_t>(PrintEstimatedStatistics::ETimeMode::Count); ++i)
    {
            ...
            m_time_processor.machine_envelope_processing_enabled)
        {
            if (line.has_value('S', value))
            {
                ...
                // It is also generated by preFlight to control acceleration per extrusion type
...
{
    // M566 is only used by RepRapFirmware. Other firmwares use M205 for jerk.
    // Only process M566 for RRF to avoid incorrect unit conversions.
    if (m_flavor != gcfRepRapFirmware && m_flavor != gcfRapid)
        return;

    // RepRapFirmware M566 supports P parameter to control jerk policy:
    // P0 (default): Jerk only between consecutive extrusion or consecutive travel moves
    // P1: Jerk between any moves (Marlin-like behavior)
    float jerk_policy;
    if (line.has_value('P', jerk_policy))
        m_rrf_jerk_policy = (jerk_policy == 0.f);

    // M566 uses mm/min for jerk values, convert to mm/s for internal use
    for (size_t i = 0; i < static_cast<size_t>(PrintEstimatedStatistics::ETimeMode::Count); ++i)
    {
        if (line.has_x())
...
void GCodeProcessor::process_T(const std::string_view command)
{
    if (command.length() > 1)
    {
        ...
        else
        {
            ...
            if (m_extruder_id != id)
            {
                if (((m_producer == EProducer::preFlight || m_producer == EProducer::Slic3rPE ||
                      m_producer == EProducer::Slic3r) &&
                     id >= m_result.extruders_count) ||
                    ((m_producer != EProducer::preFlight && m_producer != EProducer::Slic3rPE &&
                      m_producer != EProducer::Slic3r) &&
                     id >= m_result.extruder_colors.size()))
                    BOOST_LOG_TRIVIAL(error)
                        << "GCodeProcessor encountered an invalid toolchange, maybe from a custom gcode (" << command
                        << ").";
                else
                {
...
void GCodeProcessor::post_process()
{
    ...
    if (in.f == nullptr)
        throw Slic3r::RuntimeError(
            std::string("GCode processor post process export failed.\nCannot open file for reading.\n"));
    ...
    if (m_binarizer.is_enabled())
    {
        ...
        binary_data.print_metadata.raw_data.emplace_back(PrintStatistics::TotalFilamentUsedG,
                                                         stringify({filament_total_g}));
        binary_data.print_metadata.raw_data.emplace_back(PrintStatistics::TotalFilamentCost,
                                                         stringify({filament_total_cost}));
        binary_data.print_metadata.raw_data.emplace_back(PrintStatistics::TotalFilamentUsedWipeTower,
                                                         stringify({total_g_wipe_tower}));

        binary_data.printer_metadata.raw_data.emplace_back(PrintStatistics::FilamentUsedMm,
                                                           stringify(filament_mm)); // duplicated into print metadata
        binary_data.printer_metadata.raw_data.emplace_back(PrintStatistics::FilamentUsedG,
                                                           stringify(filament_g)); // duplicated into print metadata
        binary_data.printer_metadata.raw_data.emplace_back(PrintStatistics::FilamentCost,
                                                           stringify(filament_cost)); // duplicated into print metadata
        binary_data.printer_metadata.raw_data.emplace_back(PrintStatistics::FilamentUsedCm3,
                                                           stringify(filament_cm3)); // duplicated into print metadata
        binary_data.printer_metadata.raw_data.emplace_back(PrintStatistics::TotalFilamentUsedWipeTower,
                                                           stringify(
                                                               {total_g_wipe_tower})); // duplicated into print metadata
        ...
        for (size_t i = 0; i < static_cast<size_t>(PrintEstimatedStatistics::ETimeMode::Count); ++i)
        {
            ...
            if (mode == PrintEstimatedStatistics::ETimeMode::Normal || machine.enabled)
            {
                ...
                binary_data.print_metadata.raw_data.emplace_back("estimated printing time " + std::string(buf),
                                                                 get_time_dhms(machine.time));
                binary_data.print_metadata.raw_data.emplace_back("estimated first layer printing time " +
                                                                     std::string(buf),
                                                                 get_time_dhms(machine.first_layer_time));
                binary_data.printer_metadata.raw_data.emplace_back("estimated printing time " + std::string(buf),
                                                                   get_time_dhms(machine.time));
            }
        }
        ...
        if (res != bgcode::core::EResult::Success)
            throw Slic3r::RuntimeError(
                format("Unable to initialize the gcode binarizer.\nError: %1%", bgcode::core::translate_result(res)));
    }
    ...
    public:
                          ...
                          std::function<std::string(const std::string &)> line_replacer)
        {
            ...
            for (unsigned int i = 0; i < backtrace.steps; ++i)
            {
                ...
                while (rev_it != m_lines.rend() && rev_it->times[Normal] > time_threshold_i && curr_cmd != cmd &&
                       curr_cmd != "G28" && curr_cmd != "G29")
                {
                ...
                if (rev_it != m_lines.rend() && rev_it != start_rev_it && rev_it->times[Normal] != last_time_insertion)
                {
                    ...
                    const auto map_end_it = rev_it_dist <= m_gcode_lines_map.size()
                                                ? m_gcode_lines_map.rbegin() + (rev_it_dist - 1)
                                                : m_gcode_lines_map.rend();
    ...
    private:
        void write_to_file(FilePtr &out, const std::string &out_string, GCodeProcessorResult &result,
    ...
    ExportLines export_lines(m_binarizer,
                             m_result.backtrace_enabled ? ExportLines::EWriteType::ByTime
        ...
        if (line.length() > 1)
        {
            ...
            if (m_time_processor.export_remaining_time_enabled &&
                (line == reserved_tag(ETags::First_Line_M73_Placeholder) ||
                 line == reserved_tag(ETags::Last_Line_M73_Placeholder)))
            {
                for (size_t i = 0; i < static_cast<size_t>(PrintEstimatedStatistics::ETimeMode::Count); ++i)
                {
                    const TimeMachine &machine = m_time_processor.machines[i];
                    if (machine.enabled)
                    {
                        ...
                        export_lines.append_line(
                            format_line_M73_main(machine.line_m73_main_mask.c_str(),
                        ...
                        if (line == reserved_tag(ETags::First_Line_M73_Placeholder) && !machine.stop_times.empty())
                        {
                            ...
                            export_lines.append_line(
                                format_line_M73_stop_int(machine.line_m73_stop_mask.c_str(), to_export_stop));
    ...
    auto process_line_G1 = [this,
                            ...
                            format_line_M73_stop_float, time_in_last_minute,
                            // Caches, to be modified
                            &g1_times_cache_it, &last_exported_main, &last_exported_stop,
                            &export_lines](const size_t g1_lines_counter)
    {
        if (m_time_processor.export_remaining_time_enabled)
        {
            for (size_t i = 0; i < static_cast<size_t>(PrintEstimatedStatistics::ETimeMode::Count); ++i)
            {
                ...
                if (machine.enabled)
                {
                    ...
                    if (it != machine.g1_times_cache.end() && it->id == g1_lines_counter)
                    {
                        ...
                        // export remaining time to next printer stop
                        auto it_stop = std::upper_bound(machine.stop_times.begin(), machine.stop_times.end(),
                                                        it->elapsed_time,
                        ...
                        if (it_stop != machine.stop_times.end())
                        {
                            ...
                            if (last_exported_stop[i] != to_export_stop)
                            {
                                if (to_export_stop > 0)
                                {
                                    ...
                                    {
                                        export_lines.append_line(
                                            format_line_M73_stop_int(machine.line_m73_stop_mask.c_str(),
                                                                     to_export_stop));
                                        last_exported_stop[i] = to_export_stop;
                                    }
                                }
                                else
                                {
                                    ...
                                    if (next_it != machine.g1_times_cache.end())
                                    {
                                        auto next_it_stop = std::upper_bound(
                                            machine.stop_times.begin(), machine.stop_times.end(), next_it->elapsed_time,
                                            [](float value, const TimeMachine::StopTime &t)
                                            { return value < t.elapsed_time; });
                                    ...
                                    if (is_last)
                                    {
                                        if (std::distance(machine.stop_times.begin(), it_stop) ==
                                            ...
                                            export_lines.append_line(format_line_M73_stop_float(
                                                machine.line_m73_stop_mask.c_str(),
                                                time_in_last_minute(it_stop->elapsed_time - it->elapsed_time)));
    ...
    auto process_line_T = [this, &export_lines](const std::string &gcode_line, const size_t g1_lines_counter,
        ...
        if (cmd.size() >= 2)
        {
            ...
            if (tool_number != -1)
            {
                if (tool_number < 0 || (int) m_extruder_temps_config.size() <= tool_number)
                {
                    ...
                    // emit warning
                    std::string warning = _u8L(
                        "GCode Post-Processor encountered an invalid toolchange, maybe from a custom gcode:");
                    warning += "\n> ";
                    warning += gcode_line;
            ...
            export_lines.insert_lines(
                ...
                [this, tool_number](const std::string &line)
                {
                    if (GCodeReader::GCodeLine::cmd_is(line, "M104"))
                    {
                        ...
                        GCodeReader reader;
                        reader.parse_line(line, [&gline](GCodeReader &reader, const GCodeReader::GCodeLine &l)
                                          { gline = l; });
        ...
        for (;;)
        {
            ...
            while (it != it_bufend || (eof && !gcode_line.empty()))
            {
                ...
                if (eol)
                {
                    ++line_id;
                    gcode_line += "\n";
                    const unsigned int internal_g1_lines_counter = export_lines.update(gcode_line, line_id,
                                                                                       g1_lines_counter);
                    ...
                    {
                        if (GCodeReader::GCodeLine::cmd_is(gcode_line, "G0") ||
                        ...
                        else if (GCodeReader::GCodeLine::cmd_is(gcode_line, "G2") ||
    ...
    if (rename_file(out_path, result_filename))
        throw Slic3r::RuntimeError(std::string("Failed to rename the output G-code file from ") + out_path + " to " +
                                   result_filename + '\n' + "Is " + out_path + " locked?" + '\n');
}

void GCodeProcessor::store_move_vertex(EMoveType type, bool internal_only)
{
    m_last_line_id = (type == EMoveType::Color_change || type == EMoveType::Pause_Print ||
                      type == EMoveType::Custom_GCode)
                         ? m_line_id + 1
    ...
    m_result.moves.push_back({m_last_line_id,
                              ...
                              m_extruder_id,
                              m_cp_color.current,
                              Vec3f(m_end_position[X], m_end_position[Y], m_end_position[Z] - m_z_offset) +
...
float GCodeProcessor::minimum_feedrate(PrintEstimatedStatistics::ETimeMode mode, float feedrate) const
{
    ...
    return std::max(feedrate, get_option_value(m_time_processor.machine_limits.machine_min_extruding_rate,
...
float GCodeProcessor::minimum_travel_feedrate(PrintEstimatedStatistics::ETimeMode mode, float feedrate) const
{
    ...
    return std::max(feedrate, get_option_value(m_time_processor.machine_limits.machine_min_travel_rate,
                                               static_cast<size_t>(mode)));
}
...
float GCodeProcessor::get_axis_max_feedrate(PrintEstimatedStatistics::ETimeMode mode, Axis axis) const
{
    ...
    case X:
    {
        return get_option_value(m_time_processor.machine_limits.machine_max_feedrate_x, static_cast<size_t>(mode));
    }
    case Y:
    {
        return get_option_value(m_time_processor.machine_limits.machine_max_feedrate_y, static_cast<size_t>(mode));
    }
    case Z:
    {
        return get_option_value(m_time_processor.machine_limits.machine_max_feedrate_z, static_cast<size_t>(mode));
    }
    case E:
    {
        return get_option_value(m_time_processor.machine_limits.machine_max_feedrate_e, static_cast<size_t>(mode));
    }
    default:
    {
        return 0.0f;
...
float GCodeProcessor::get_axis_max_jerk(PrintEstimatedStatistics::ETimeMode mode, Axis axis) const
{
    ...
    case X:
    {
        return get_option_value(m_time_processor.machine_limits.machine_max_jerk_x, static_cast<size_t>(mode));
    }
    case Y:
    {
        return get_option_value(m_time_processor.machine_limits.machine_max_jerk_y, static_cast<size_t>(mode));
    }
    case Z:
    {
        return get_option_value(m_time_processor.machine_limits.machine_max_jerk_z, static_cast<size_t>(mode));
    }
    case E:
    {
        return get_option_value(m_time_processor.machine_limits.machine_max_jerk_e, static_cast<size_t>(mode));
    }
    default:
    {
        return 0.0f;
...
float GCodeProcessor::get_retract_acceleration(PrintEstimatedStatistics::ETimeMode mode) const
{
    ...
    return (id < m_time_processor.machines.size()) ? m_time_processor.machines[id].retract_acceleration
                                                   : DEFAULT_RETRACT_ACCELERATION;
}
...
void GCodeProcessor::set_retract_acceleration(PrintEstimatedStatistics::ETimeMode mode, float value)
{
    ...
    {
        m_time_processor.machines[id].retract_acceleration =
            (m_time_processor.machines[id].max_retract_acceleration == 0.0f) ? value :
...
void GCodeProcessor::set_acceleration(PrintEstimatedStatistics::ETimeMode mode, float value)
{
    ...
    if (id < m_time_processor.machines.size())
    {
        m_time_processor.machines[id].acceleration = (m_time_processor.machines[id].max_acceleration == 0.0f)
                                                         ...
                                                         // Clamp the acceleration with the maximum.
                                                         std::min(value,
                                                                  m_time_processor.machines[id].max_acceleration);
    }
}
...
{
    size_t id = static_cast<size_t>(mode);
    return (id < m_time_processor.machines.size()) ? m_time_processor.machines[id].travel_acceleration
                                                   : DEFAULT_TRAVEL_ACCELERATION;
}
...
void GCodeProcessor::set_travel_acceleration(PrintEstimatedStatistics::ETimeMode mode, float value)
{
    ...
    if (id < m_time_processor.machines.size())
    {
        m_time_processor.machines[id].travel_acceleration =
            (m_time_processor.machines[id].max_travel_acceleration == 0.0f) ? value :
                                                                            // Clamp the acceleration with the maximum.
...
{
    static size_t s_total_lines = 0;
    static size_t s_last_move_count = 0;
    static int s_last_progress = 50;
    static int s_call_num = 0;

    s_call_num++;

    const size_t current_moves = result.moves.size();

    // Reset on new job (detect when moves decrease, indicating new file)
    if (current_moves < s_last_move_count)
    {
        s_total_lines = 0;
        s_last_progress = 50;
        s_call_num = 1;
    ...
    // Get total lines from virtual file on first call
    if (s_total_lines == 0 && result.virtual_gcode_file)
    {
        s_total_lines = result.virtual_gcode_file->line_count();
    }
    ...
    std::vector<std::pair<size_t, std::vector<GCodeProcessorResult::MoveVertex>>> moves_to_insert = {
        std::make_pair(0, std::vector<GCodeProcessorResult::MoveVertex>{})};
    ...
    for (auto it = actual_speed_moves.begin(); it != actual_speed_moves.end(); ++it)
    {
        ...
        {
            moves_to_insert.back().first = base_id_old +
                                           inserted_count; // Save required position of this range in the NEW vector.
            id_map[base_id_old] = base_id_old + inserted_count; // Remember where the old element will end up.
            inserted_count += moves_to_insert.back()
                                  .second.size(); // Increase the number of moves that are already planned to be added.
    ...
    const size_t original_size = m.size();
    ...
    for (auto it = moves_to_insert.rbegin(); it != moves_to_insert.rend(); ++it)
    {
        ...
        for (int i = last_pos; i >= new_pos + new_moves.size(); --i)
    ...
    if (m_print && line_count > 0)
    {
        ...
        // Note: Uses s_last_progress from top of function (line 5027), which resets between jobs
...
double GCodeProcessor::extract_absolute_position_on_axis(Axis axis, const GCodeReader::GCodeLine &line,
...
// void GCodeProcessor::post_process_array()
// {
#if 0  // Disabled - array mode removed

    // Clear the final array
    m_final_gcode_array.clear();


    m_final_gcode_array.reserve(m_gcode_array->size() * 2); // Reserve space for potential M73 additions

    // Calculate filament statistics (same as file-based version)
    std::vector<double> filament_mm(m_result.extruders_count, 0.0);
    std::vector<double> filament_cm3(m_result.extruders_count, 0.0);
    std::vector<double> filament_g(m_result.extruders_count, 0.0);
    std::vector<double> filament_cost(m_result.extruders_count, 0.0);
    ...
    // Full implementation of ExportLines for array processing with M73 support
    class ArrayExportLines {
    public:
        struct LineData {
            std::string line;
            std::array<float, 2> times{ 0.0f, 0.0f }; // Normal and Stealth times
        };
    ...
    private:
        std::vector<std::string>* m_output_array;
        std::deque<LineData> m_lines;
        const std::array<TimeMachine, static_cast<size_t>(PrintEstimatedStatistics::ETimeMode::Count)>& m_machines;
        std::array<float, 2> m_times{ 0.0f, 0.0f };
        size_t m_times_cache_id{ 0 };
        size_t m_added_lines_counter{ 0 };
        std::vector<std::pair<size_t, size_t>> m_gcode_lines_map;

        enum ETimeMode { Normal = 0, Stealth = 1 };

    public:
        ArrayExportLines(std::vector<std::string>* output_array,
                        const std::array<TimeMachine, static_cast<size_t>(PrintEstimatedStatistics::ETimeMode::Count)>& machines)
            : m_output_array(output_array), m_machines(machines) {}
        ...
        void insert_lines(const Backtrace& backtrace, const std::string& cmd,
            ...
            float last_time_insertion = 0.0f;
            ...
            for (unsigned int i = 0; i < backtrace.steps; ++i) {
                ...
                std::string curr_cmd = GCodeReader::GCodeLine::extract_cmd(rev_it->line);
                while (rev_it != m_lines.rend() && rev_it->times[Normal] > time_threshold_i &&
        ...
        void write(float backtrace_time) {
            ...
            // Write lines older than current time - backtrace_time
            while (!m_lines.empty() && m_lines.front().times[Normal] < m_times[Normal] - backtrace_time) {
                m_output_array->push_back(m_lines.front().line);
                m_lines.pop_front();
            }
        }
    ...
    ArrayExportLines export_lines(&m_final_gcode_array, m_time_processor.machines);
    ...
    auto format_line_M73_stop_float = [](const std::string& mask, float time) {
        char line_M73[64];
        sprintf(line_M73, mask.c_str(), time);
        return std::string(line_M73);
    };
    ...
    // Keep track of last exported M73 values
    std::array<std::pair<int, int>, static_cast<size_t>(PrintEstimatedStatistics::ETimeMode::Count)> last_exported_main;
    for (auto& elem : last_exported_main)
        elem = { -1, -1 };
    std::array<int, static_cast<size_t>(PrintEstimatedStatistics::ETimeMode::Count)> last_exported_stop;
    last_exported_stop.fill(-1);
    ...
    auto is_temporary_decoration = [](const std::string_view gcode_line) {
        assert(!gcode_line.empty());
        assert(gcode_line.back() == '\n');
        return false; // No temporary decorations in current implementation
    };
    ...
    // Process G1 lines and add M73 codes
    auto process_line_G1 = [this, time_in_minutes, format_time_float, format_line_M73_main, format_line_M73_stop_int,
                            format_line_M73_stop_float, time_in_last_minute,
                            &g1_times_cache_it, &last_exported_main, &last_exported_stop, &export_lines]
                                ...
                                if (to_export_stop > 0) {
                                    ...
                                    if (is_last) {
                                        if (std::distance(machine.stop_times.begin(), it_stop) == static_cast<ptrdiff_t>(machine.stop_times.size() - 1))
                                            ...
                                            export_lines.append_line(format_line_M73_stop_float(machine.line_m73_stop_mask.c_str(),
                                                time_in_last_minute(it_stop->elapsed_time - it->elapsed_time)));
    ...
    // Process toolchange lines and add M104
    auto process_line_T = [&](const std::string& gcode_line, const size_t g1_lines_counter, const ArrayExportLines::Backtrace& backtrace) {
        const std::string cmd = GCodeReader::GCodeLine::extract_cmd(gcode_line);
        if (cmd.size() >= 2) {
            std::stringstream ss(cmd.substr(1));
            int tool_number = -1;
    ...
    unsigned int line_id = 0;
    size_t g1_lines_counter = 0;
    static const ArrayExportLines::Backtrace backtrace_T = { 120.0f, 10 };
    float max_backtrace_time = 120.0f;

    // Process each line from the input array
    std::string gcode_line;
    for (const auto& chunk : *m_gcode_array) {
        ...
        for (char c : chunk) {
            ...
            if (c == '\n') {
                ...
                // Add line to output if not consumed
                if (!gcode_line.empty())
                    export_lines.append_line(gcode_line);
    ...
    // Synchronize moves with new line numbers
    export_lines.synchronize_moves(m_result);

    // Clear the input array pointer as it's no longer needed
    m_gcode_array = nullptr;

}
#endif // 0 - array mode removed
