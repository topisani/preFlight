...
{
    return (flavor == gcfRepetier || flavor == gcfMarlinFirmware || flavor == gcfRepRapFirmware || flavor == gcfRapid);
}
...
void GCodeWriter::apply_print_config(const PrintConfig &print_config)
{
    ...
    bool use_mach_limits = print_config.gcode_flavor.value == gcfMarlinLegacy ||
                           ...
                           print_config.gcode_flavor.value == gcfRapid;
    m_max_acceleration = static_cast<unsigned int>(
        std::round((use_mach_limits && print_config.machine_limits_usage.value == MachineLimitsUsage::EmitToGCode)
                       ? print_config.machine_max_acceleration_extruding.values.front()
                       : 0));
    m_max_travel_acceleration = static_cast<unsigned int>(
        std::round((use_mach_limits && print_config.machine_limits_usage.value == MachineLimitsUsage::EmitToGCode &&
                    supports_separate_travel_acceleration(print_config.gcode_flavor.value))
...
std::string GCodeWriter::preamble()
{
    ...
    if (FLAVOR_IS(gcfRepRapSprinter) || FLAVOR_IS(gcfRepRapFirmware) || FLAVOR_IS(gcfRapid) ||
        FLAVOR_IS(gcfMarlinLegacy) || FLAVOR_IS(gcfMarlinFirmware) || FLAVOR_IS(gcfKlipper) || FLAVOR_IS(gcfTeacup) ||
        FLAVOR_IS(gcfRepetier) || FLAVOR_IS(gcfSmoothie))
    {
...
std::string GCodeWriter::set_temperature(unsigned int temperature, bool wait, int tool) const
{
    ...
    if (wait && FLAVOR_IS_NOT(gcfTeacup) && FLAVOR_IS_NOT(gcfRepRapFirmware) && FLAVOR_IS_NOT(gcfRapid))
    {
        code = "M109"sv;
    ...
    else
    {
        ...
        { // M104 is deprecated on RepRapFirmware
            code = "G10"sv;
        }
        else
        {
    ...
    if (tool != -1 && (multiple_tools || FLAVOR_IS(gcfMakerWare) || FLAVOR_IS(gcfSailfish) ||
                       FLAVOR_IS(gcfRepRapFirmware) || FLAVOR_IS(gcfRapid)))
    {
        if (FLAVOR_IS(gcfRepRapFirmware) || FLAVOR_IS(gcfRapid))
        {
            gcode << " P" << tool;
    ...
    if ((FLAVOR_IS(gcfTeacup) || FLAVOR_IS(gcfRepRapFirmware) || FLAVOR_IS(gcfRapid)) && wait)
...
std::string GCodeWriter::set_acceleration_internal(Acceleration type, unsigned int acceleration)
{
    ...
    bool separate_travel = (type == Acceleration::Travel &&
                            supports_separate_travel_acceleration(this->config.gcode_flavor));
    ...
    else if (FLAVOR_IS(gcfRepRapFirmware) || FLAVOR_IS(gcfRapid) || FLAVOR_IS(gcfMarlinFirmware))
...
std::string GCodeWriter::update_progress(unsigned int num, unsigned int tot, bool allow_100) const
{
    ...
    if (!allow_100)
        percent = std::min(percent, (unsigned int) 99);
...
std::string GCodeWriter::toolchange(unsigned int extruder_id)
{
    ...
    auto it_extruder = Slic3r::lower_bound_by_predicate(m_extruders.begin(), m_extruders.end(),
                                                        [extruder_id](const Extruder &e)
                                                        { return e.id() < extruder_id; });
...
std::string GCodeWriter::set_speed(double F, const std::string_view comment,
...
std::string GCodeWriter::travel_to_xy_force(const Vec2d &point, const std::string_view comment)
{
    GCodeG1Formatter w;
    w.emit_xy(point);
    w.emit_f(this->get_travel_speed() * 60.0);
    w.emit_comment(this->config.gcode_comments, comment);
...
std::string GCodeWriter::travel_to_xy_G2G3IJ(const Vec2d &point, const Vec2d &ij, const bool ccw,
...
std::string GCodeWriter::travel_to_xyz(const Vec3d &to, const std::string_view comment)
{
    ...
    else if (std::abs(to.x() - m_pos.x()) < GCodeFormatter::XYZ_EPSILON &&
             std::abs(to.y() - m_pos.y()) < GCodeFormatter::XYZ_EPSILON)
    {
...
std::string GCodeWriter::travel_to_xyz_force(const Vec3d &to, const std::string_view comment)
{
    ...
    double speed = this->get_travel_speed();
...
std::string GCodeWriter::extrude_to_xy_G2G3IJ(const Vec2d &point, const Vec2d &ij, const bool ccw, double dE,
                                              const std::string_view comment)
{
    assert(std::abs(dE) < 1000.0);
    assert(dE != 0);
...
std::string GCodeWriter::retract_for_toolchange(bool before_wipe)
{
    double factor = before_wipe ? m_extruder->retract_before_wipe() : 1.;
    assert(factor >= 0. && factor <= 1. + EPSILON);
    return this->_retract(factor * m_extruder->retract_length_toolchange(),
                          m_extruder->retract_restart_extra_toolchange(), "retract for toolchange");
}
...
std::string GCodeWriter::set_fan(const GCodeFlavor gcode_flavor, bool gcode_comments, unsigned int speed)
{
    ...
    else
    {
        ...
        case gcfMach3:
        case gcfMachinekit:
            gcode << "M106 P" << std::lround(255.0 * speed / 100.0);
            break;
        default:
            gcode << "M106 S" << std::lround(255.0 * speed / 100.0);
            break;
        }
        if (gcode_comments)
            gcode << " ; enable fan";
...
{
    assert(digits <= 9);
    static constexpr const std::array<int, 10> pow_10{1,      10,      100,      1000,      10000,
    ...
    *ptr_err.ptr++ = axis;
    ...
    if (writen_digits < digits)
    {
        ...
        for (char *from_ptr = this->ptr_err.ptr - 1, *to_ptr = from_ptr + remaining_digits;
             from_ptr >= this->ptr_err.ptr - writen_digits; --to_ptr, --from_ptr)
            *to_ptr = *from_ptr;
    ...
    for (char *to_ptr = this->ptr_err.ptr, *from_ptr = to_ptr - 1; from_ptr >= this->ptr_err.ptr - digits;
         --to_ptr, --from_ptr)
        *to_ptr = *from_ptr;
