...
ExtrusionPaths calculate_and_split_overhanging_extrusions(
    const ExtrusionPath &path, const AABBTreeLines::LinesDistancer<Linef> &unscaled_prev_layer,
    const AABBTreeLines::LinesDistancer<CurledLine> &prev_layer_curled_lines)
{
    ...
    std::vector<ExtendedPoint> extended_points = estimate_points_properties<true>(path.polyline.points,
                                                                                  unscaled_prev_layer, config);
    ...
    for (size_t i = 0; i < extended_points.size(); i++)
    {
        ...
        const double dist_limit = 10.0 * path.width();
        {
            ...
            auto line_indices = prev_layer_curled_lines.all_lines_in_radius(Point::new_scale(middle),
                                                                            scale_(dist_limit));
            if (!line_indices.empty())
            {
                ...
                if (len > 8)
                {
                    ...
                    for (size_t idx : line_indices)
                    {
                        ...
                        if (inside.empty())
                            continue;
                        double projected_length = abs(
                            dir.dot(unscaled(Vec2d((inside.back().b - inside.back().a).cast<double>()))));
                ...
                for (size_t idx : line_indices)
                {
                    ...
                    float proximity = (1.0 - (distance_from_curled / dist_limit)) *
...
ExtrusionEntityCollection calculate_and_split_overhanging_extrusions(
    const ExtrusionEntityCollection *ecc, const AABBTreeLines::LinesDistancer<Linef> &unscaled_prev_layer,
    const AABBTreeLines::LinesDistancer<CurledLine> &prev_layer_curled_lines)
{
    ...
    for (const auto *e : ecc->entities)
    {
        ...
        {
            result.append(
                calculate_and_split_overhanging_extrusions(col, unscaled_prev_layer, prev_layer_curled_lines));
        }
        else if (auto *loop = dynamic_cast<const ExtrusionLoop *>(e))
        {
            ...
            {
                auto resulting_paths = calculate_and_split_overhanging_extrusions(p, unscaled_prev_layer,
                                                                                  prev_layer_curled_lines);
        ...
        else if (auto *mp = dynamic_cast<const ExtrusionMultiPath *>(e))
        {
            ...
            {
                auto paths = calculate_and_split_overhanging_extrusions(p, unscaled_prev_layer,
                                                                        prev_layer_curled_lines);
...
static std::map<float, float> calc_fan_speed_sections(const ExtrusionAttributes &attributes,
                                                      const FullPrintConfig &config, const size_t extruder_id)
{
...
OverhangSpeeds calculate_overhang_speed(const ExtrusionAttributes &attributes, const FullPrintConfig &config,
                                        const size_t extruder_id, const float external_perimeter_reference_speed,
                                        const float default_speed, const std::optional<float> &current_fan_speed)
{
    ...
    else if (current_fan_speed.has_value() && (fan_speed < *current_fan_speed) &&
             (*current_fan_speed - fan_speed) <= MIN_FAN_SPEED_NEGATIVE_CHANGE_TO_EMIT)
    {
