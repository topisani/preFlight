...
ObjectLayerPerimeters get_perimeters(SpanOfConstPtrs<PrintObject> objects, const Params &params,
                                     ...
                                     const std::function<void(void)> &throw_if_canceled)
{
    ...
    for (const PrintObject *print_object : objects)
    {
        ...
        const Perimeters::LayerInfos layer_infos{
            Perimeters::get_layer_infos(print_object->layers(), params.perimeter.elephant_foot_compensation)};
        const std::vector<Geometry::BoundedPolygons> projected{
            Geometry::project_to_geometry(extrusions, params.max_distance)};
        Perimeters::LayerPerimeters perimeters{
            Perimeters::create_perimeters(projected, layer_infos, painting, params.perimeter)};
...
ObjectSeams precalculate_seams(const Params &params, ObjectLayerPerimeters &&seam_data,
                               const std::function<void(void)> &throw_if_canceled)
{
    ObjectSeams result;
    ...
    for (auto &[print_object, layer_perimeters] : seam_data)
    {
        ...
        case spAligned:
        {
            ...
            Slic3r::ModelInfo::Visibility points_visibility{transformation, volumes, params.visibility,
                                                            throw_if_canceled};
            throw_if_canceled();
            const Aligned::VisibilityCalculator visibility_calculator{points_visibility,
                                                                      params.convex_visibility_modifier,
        ...
        case spRear:
        {
            result[print_object] = Rear::get_object_seams(std::move(layer_perimeters), params.rear_tolerance,
...
Params Placer::get_params(const DynamicPrintConfig &config)
{
    ...
    // Hardcoded value - removed UI slider as it was not helpful for painted seams
    params.aligned.max_detour = 1.0; // 1mm max deviation between layers
    ...
    // Read value from global storage (set by UI)
    params.perimeter.painting_radius = SeamGlobalParams::getSeamDetectionRadius();
...
void Placer::init(SpanOfConstPtrs<PrintObject> objects, const Params &params,
                  const std::function<void(void)> &throw_if_canceled)
{
...
const SeamPerimeterChoice &choose_closest_seam(const std::vector<SeamPerimeterChoice> &seams,
...
std::pair<std::size_t, Vec2d> project_to_extrusion_loop(const SeamChoice &seam_choice,
                                                        ...
                                                        const AABBTreeLines::LinesDistancer<Linef> &distancer)
{
    ...
    const Vec2d normal{is_at_vertex ? Geometry::get_polygon_normal(perimeter.positions, seam_choice.previous_index, 0.1)
...
SeamChoice to_seam_choice(const Geometry::PointOnLine &point_on_line, const Perimeters::Perimeter &perimeter)
{
    SeamChoice result;

    result.position = point_on_line.point;
    result.previous_index = point_on_line.line_index;
    result.next_index = point_on_line.line_index == perimeter.positions.size() - 1 ? 0 : point_on_line.line_index + 1;
    return result;
}

Geometry::Direction1D get_direction(const bool flipped, const ExPolygon &perimeter_polygon, const ExtrusionLoop &loop)
{
...
{
    const unsigned previous_value{
        Perimeters::get_point_value(perimeter.point_types[seam_choice.previous_index],
                                    perimeter.point_classifications[seam_choice.previous_index])};

    const unsigned next_value{Perimeters::get_point_value(perimeter.point_types[seam_choice.next_index],
                                                          perimeter.point_classifications[seam_choice.next_index])};

    return std::max(previous_value, next_value);
}

boost::variant<Point, Scarf::Scarf> finalize_seam_position(const ExtrusionLoop &loop, const PrintRegion *region,
                                                           SeamChoice seam_choice,
                                                           const Perimeters::Perimeter &perimeter,
                                                           const bool staggered_inner_seams, const bool flipped,
                                                           const bool thick_bridges)
{
    ...
    auto [loop_line_index, loop_point]{project_to_extrusion_loop(seam_choice, perimeter, distancer)};

    const Geometry::Direction1D offset_direction{get_direction(flipped, perimeter_polygon, loop)};

    const auto offset_stop_condition{
        [choice_value = get_seam_choice_value(seam_choice, perimeter)](const Perimeters::Perimeter &perimeter,
                                                                       const std::size_t index)
        {
            const unsigned current_point_value{
                Perimeters::get_point_value(perimeter.point_types[index], perimeter.point_classifications[index])};
            if (current_point_value < choice_value &&
    ...
    if (do_staggering)
    {
        ...
        std::optional<PointOnPerimeter> staggered_point{
            offset_along_perimeter({seam_choice.previous_index, seam_choice.next_index, loop_point}, perimeter,
                                   staggering_offset, offset_direction, offset_stop_condition)};
    ...
    const bool is_smooth{seam_choice.previous_index != seam_choice.next_index ||
    ...
    if (place_scarf_seam)
    {
        ...
        const std::optional<PointOnPerimeter> outter_scarf_start_point{
            offset_along_perimeter(seam_choice, perimeter, offset, offset_direction, offset_stop_condition)};
        ...
        { // Outter perimeter
            const Vec2d start_point_candidate{
                project_to_extrusion_loop(*outter_scarf_start_point, perimeter, distancer).second};
        ...
        {
            PointOnPerimeter inner_scarf_end_point{*outter_scarf_start_point};

            if (region->config().external_perimeters_first.value)
            {
                const auto external_first_offset_direction{offset_direction == Geometry::Direction1D::forward
                                                               ? Geometry::Direction1D::backward
                                                               : Geometry::Direction1D::forward};
                if (auto result{offset_along_perimeter(seam_choice, perimeter, offset, external_first_offset_direction,
            ...
            const std::optional<PointOnPerimeter> inner_scarf_start_point{
                offset_along_perimeter(inner_scarf_end_point, perimeter, offset, offset_direction,
                                       offset_stop_condition)};
            ...
            const Vec2d start_point_candidate{
                project_to_extrusion_loop(*inner_scarf_start_point, perimeter, distancer).second};
            ...
            const auto [end_point_previous_index,
                        end_point]{project_to_extrusion_loop(inner_scarf_end_point, perimeter, distancer)};
...
std::pair<SeamChoice, std::size_t> place_seam_near(const std::vector<Perimeters::BoundedPerimeter> &layer_perimeters,
                                                   const ExtrusionLoop &loop, const Point &position,
...
boost::variant<Point, Scarf::Scarf> Placer::place_seam(const Layer *layer, const PrintRegion *region,
        ...
        if (has_2_or_3_perimeters)
        {
                ...
                seams_on_perimeters[0].perimeter.is_hole != seams_on_perimeters[1].perimeter.is_hole)
            {
                const SeamPerimeterChoice &seam_perimeter_choice{
                    seams_on_perimeters[0].perimeter.is_hole ? seams_on_perimeters[1] : seams_on_perimeters[0]};
                return finalize_seam_position(loop, region, seam_perimeter_choice.choice,
                                              seam_perimeter_choice.perimeter, this->params.staggered_inner_seams,
                                              flipped, thick_bridges);
            }
        }

        const SeamPerimeterChoice &seam_perimeter_choice{
            choose_closest_seam(seams_on_perimeters, Geometry::to_polygon(loop))};
        return finalize_seam_position(loop, region, seam_perimeter_choice.choice, seam_perimeter_choice.perimeter,
                                      this->params.staggered_inner_seams, flipped, thick_bridges);
    }
}
} // namespace Slic3r::Seams
