...
// 0 : 3MF files saved by older versions or the painting gizmo wasn't used. No version definition in them.
...
const std::string LAYER_CONFIG_RANGES_FILE = "Metadata/preFlight_layer_config_ranges.xml";
const std::string SLA_SUPPORT_POINTS_FILE = "Metadata/Slic3r_PE_sla_support_points.txt";
const std::string SLA_DRAIN_HOLES_FILE = "Metadata/Slic3r_PE_sla_drain_holes.txt";
const std::string CUSTOM_GCODE_PER_PRINT_Z_FILE = "Metadata/preFlight_custom_gcode_per_print_z.xml";
const std::string WIPE_TOWER_INFORMATION_FILE = "Metadata/preFlight_wipe_tower_information.xml";
const std::string CUT_INFORMATION_FILE = "Metadata/preFlight_cut_information.xml";
...
const char *VALID_OBJECT_TYPES[] = {"model"};

const char *INVALID_OBJECT_TYPES[] = {"solidsupport", "support", "surface", "other"};
...
const char *get_attribute_value_charptr(const char **attributes, unsigned int attributes_size,
...
class _3MF_Importer : public _3MF_Base
{
    ...
    struct Component
    {
        ...
        explicit Component(PathId object_id) : object_id(object_id), transform(Transform3d::Identity()) {}

        Component(PathId object_id, const Transform3d &transform) : object_id(object_id), transform(transform) {}
    };
    ...
    struct ObjectMetadata
    {
        struct VolumeMetadata
        {
            ...
            VolumeMetadata(unsigned int first_triangle_id, unsigned int last_triangle_id)
                : first_triangle_id(first_triangle_id), last_triangle_id(last_triangle_id)
            {
            }
        };
...
private:
    ...
    void _extract_cut_information_from_archive(mz_zip_archive &archive, const mz_zip_archive_file_stat &stat,
                                               ConfigSubstitutionContext &config_substitutions);
    void _extract_layer_heights_profile_config_from_archive(mz_zip_archive &archive,
    ...
    void _extract_wipe_tower_information_from_archive(mz_zip_archive &archive, const mz_zip_archive_file_stat &stat,
                                                      Model &model);
    void _extract_wipe_tower_information_from_archive_legacy(::mz_zip_archive &archive,
                                                             const mz_zip_archive_file_stat &stat, Model &model);

    void _extract_print_config_from_archive(mz_zip_archive &archive, const mz_zip_archive_file_stat &stat,
                                            DynamicPrintConfig &config, ConfigSubstitutionContext &subs_context,
                                            const std::string &archive_filename);
    bool _extract_model_config_from_archive(mz_zip_archive &archive, const mz_zip_archive_file_stat &stat,
                                            Model &model);
    void _extract_embossed_svg_shape_file(const std::string &filename, mz_zip_archive &archive,
                                          const mz_zip_archive_file_stat &stat);
    ...
    bool _create_object_instance(PathId object_id, const Transform3d &transform, const bool printable,
    ...
    bool _generate_volumes(ModelObject &object, const Geometry &geometry,
                                          ...
                                          ConfigSubstitutionContext &config_substitutions)
{
    ...
    if (m_version == 0)
    {
        // if the 3mf was not produced by preFlight and there is more than one instance,
    ...
    for (const IdToModelObjectMap::value_type &object : m_objects)
    {
        ...
        IdToLayerHeightsProfileMap::iterator obj_layer_heights_profile = m_layer_heights_profiles.find(object.second +
                                                                                                       1);
        ...
        if (obj_metadata != m_objects_metadata.end())
        {
            ...
            for (const Metadata &metadata : obj_metadata->second.metadata)
            {
                ...
                else if (metadata.key == "brim_points")
                {
                    // Parse space-separated values: x y z radius overlap x y z radius overlap...
                    std::istringstream iss(metadata.value);
                    model_object->brim_points.clear();
                    float x, y, z, radius, overlap;
                    while (iss >> x >> y >> z >> radius >> overlap)
                    {
                        BrimPoint pt;
                        pt.pos = Vec3f(x, y, z);
                        pt.head_front_radius = radius;
                        pt.overlap_percent = overlap;
        ...
        if (cut_object_info != m_cut_object_infos.end())
        {
            ...
            for (auto connector : cut_object_info->second.connectors)
            {
                ...
                model_object->volumes[connector.volume_id]->cut_info = ModelVolume::CutInfo(
                    CutConnectorType(connector.type), connector.r_tolerance, connector.h_tolerance, true);
            }
        }
    }
    ...
    if (!m_generator_version.has_value() && model.objects.empty())
    {
        const std::string msg =
            (boost::format(_u8L("The 3MF file does not contain a valid mesh.\n\n\"%1%\"")) % filename).str();
...
bool _3MF_Importer::_extract_relationships_from_archive(mz_zip_archive &archive, const mz_zip_archive_file_stat &stat)
{
    if (stat.m_uncomp_size == 0 ||
        stat.m_uncomp_size >
            10000000 // Prevent overloading by big Relations file(>10MB). there is no reason to be soo big
    )
    {
    ...
    mz_bool res = mz_zip_reader_extract_file_to_mem(&archive, stat.m_filename, parser_buffer,
    ...
    {
        char error_buf[1024];
        ::sprintf(error_buf, "Error (%s) while parsing xml file at line %d",
                  XML_ErrorString(XML_GetErrorCode(m_xml_parser)), (int) XML_GetCurrentLineNumber(m_xml_parser));
        add_error(error_buf);
        return false;
    }
...
bool _3MF_Importer::_extract_model_from_archive(mz_zip_archive &archive, const mz_zip_archive_file_stat &stat)
{
    ...
    XML_SetUserData(m_xml_parser, (void *) this);
    XML_SetElementHandler(m_xml_parser, _3MF_Importer::_handle_start_model_xml_element,
                          _3MF_Importer::_handle_end_model_xml_element);
    ...
    struct CallbackData
    {
        ...
        CallbackData(XML_Parser &parser, _3MF_Importer &importer, const mz_zip_archive_file_stat &stat)
...
void _3MF_Importer::_extract_cut_information_from_archive(mz_zip_archive &archive, const mz_zip_archive_file_stat &stat,
                                                          ConfigSubstitutionContext &config_substitutions)
{
    if (stat.m_uncomp_size > 0)
    {
...
void _3MF_Importer::_extract_print_config_from_archive(mz_zip_archive &archive, const mz_zip_archive_file_stat &stat,
                                                       ...
                                                       const std::string &archive_filename)
{
    if (stat.m_uncomp_size > 0)
    {
        ...
        // parsing 3MFs from older versions (which can have duplicated entries in the INI.
...
void _3MF_Importer::_extract_layer_heights_profile_config_from_archive(mz_zip_archive &archive,
                                                                       const mz_zip_archive_file_stat &stat)
{
    if (stat.m_uncomp_size > 0)
    {
...
void _3MF_Importer::_extract_layer_config_ranges_from_archive(mz_zip_archive &archive,
                                                              const mz_zip_archive_file_stat &stat,
                                                              ConfigSubstitutionContext &config_substitutions)
{
    if (stat.m_uncomp_size > 0)
...
void _3MF_Importer::_extract_sla_support_points_from_archive(mz_zip_archive &archive,
                                                             const mz_zip_archive_file_stat &stat)
{
    if (stat.m_uncomp_size > 0)
    {
            ...
            if (version == 0)
            {
                ...
                for (unsigned int i = 0; i < object_data_points.size(); i += 3)
                    sla_support_points.push_back(
                        sla::SupportPoint{Vec3f(float(std::atof(object_data_points[i + 0].c_str())),
...
void _3MF_Importer::_extract_embossed_svg_shape_file(const std::string &filename, mz_zip_archive &archive,
                                                     const mz_zip_archive_file_stat &stat)
{
    ...
    mz_bool res = mz_zip_reader_extract_to_mem(&archive, stat.m_file_index, (void *) file->data(), stat.m_uncomp_size,
                                               0);
    if (res == 0)
    {
...
bool _3MF_Importer::_extract_model_config_from_archive(mz_zip_archive &archive, const mz_zip_archive_file_stat &stat,
                                                       Model &model)
{
    if (stat.m_uncomp_size == 0)
    {
    ...
    XML_SetUserData(m_xml_parser, (void *) this);
    XML_SetElementHandler(m_xml_parser, _3MF_Importer::_handle_start_config_xml_element,
                          _3MF_Importer::_handle_end_config_xml_element);
    ...
    mz_bool res = mz_zip_reader_extract_to_mem(&archive, stat.m_file_index, parser_buffer, (size_t) stat.m_uncomp_size,
                                               0);
    if (res == 0)
    {
        add_error("Error while reading config data to buffer");
...
void _3MF_Importer::_extract_custom_gcode_per_print_z_from_archive(::mz_zip_archive &archive,
                                                                   const mz_zip_archive_file_stat &stat)
{
    if (stat.m_uncomp_size > 0)
    {
        ...
        for (const auto &bed_block : main_tree)
        {
            ...
            for (const auto &code : code_tree)
            {
                ...
                {
                    // It means that data was saved in an old version format
                    // read old data ...
...
void _3MF_Importer::_extract_wipe_tower_information_from_archive(::mz_zip_archive &archive,
                                                                 const mz_zip_archive_file_stat &stat, Model &model)
{
    if (stat.m_uncomp_size > 0)
    {
...
void _3MF_Importer::_extract_wipe_tower_information_from_archive_legacy(::mz_zip_archive &archive,
                                                                        const mz_zip_archive_file_stat &stat,
                                                                        Model &model)
{
    if (stat.m_uncomp_size > 0)
    {
        ...
        while (iss)
        {
            ...
            if (boost::starts_with(line, "wipe_tower_x") || boost::starts_with(line, "wipe_tower_y") ||
                boost::starts_with(line, "wipe_tower_rotation_angle"))
            {
...
void XMLCALL _3MF_Importer::_handle_start_relationships_element(void *userData, const char *name,
...
bool _3MF_Importer::_handle_end_model()
{
    ...
    if (m_version == 0)
    {
        // if the 3mf was not produced by preFlight and there is only one object,
...
bool _3MF_Importer::_handle_start_triangle(const char **attributes, unsigned int num_attributes)
{
    ...
    // missing values are set equal to ZERO
    m_curr_object.geometry.triangles.emplace_back(get_attribute_value_int(attributes, num_attributes, V1_ATTR),
                                                  get_attribute_value_int(attributes, num_attributes, V2_ATTR),
                                                  get_attribute_value_int(attributes, num_attributes, V3_ATTR));

    m_curr_object.geometry.custom_supports.push_back(
        get_attribute_value_string(attributes, num_attributes, CUSTOM_SUPPORTS_ATTR));
    m_curr_object.geometry.custom_seam.push_back(
    ...
    std::string mm_segmentation_serialized = get_attribute_value_string(attributes, num_attributes,
                                                                        MM_SEGMENTATION_ATTR);
...
bool _3MF_Importer::_handle_start_component(const char **attributes, unsigned int num_attributes)
{
    ...
    Transform3d transform = get_transform_from_3mf_specs_string(
...
inline static void check_painting_version(unsigned int loaded_version, unsigned int highest_supported_version,
...
bool _3MF_Importer::_handle_end_metadata()
{
    if (m_curr_metadata_name == SLIC3RPE_3MF_VERSION)
    {
        ...
        if (m_check_version && (m_version > VERSION_3MF_COMPATIBLE))
        {
            ...
            const std::string msg =
                (boost::format(
                     _u8L("The selected 3mf file has been saved with a newer version of %1% and is not compatible.")) %
    ...
    else if (m_curr_metadata_name == SLIC3RPE_FDM_SUPPORTS_PAINTING_VERSION)
    {
        ...
        check_painting_version(
            m_fdm_supports_painting_version, FDM_SUPPORTS_PAINTING_VERSION,
            _u8L(
                "The selected 3MF contains FDM supports painted object using a newer version of preFlight and is not compatible."));
    }
    else if (m_curr_metadata_name == SLIC3RPE_SEAM_PAINTING_VERSION)
    {
        ...
        check_painting_version(
            ...
            _u8L(
                "The selected 3MF contains seam painted object using a newer version of preFlight and is not compatible."));
    }
    else if (m_curr_metadata_name == SLIC3RPE_MM_PAINTING_VERSION)
    {
        m_mm_painting_version = (unsigned int) atoi(m_curr_characters.c_str());
        check_painting_version(
            m_mm_painting_version, MM_PAINTING_VERSION,
            _u8L(
                "The selected 3MF contains multi-material painted object using a newer version of preFlight and is not compatible."));
    }
...
bool _3MF_Importer::_create_object_instance(PathId object_id, const Transform3d &transform, const bool printable,
                                            unsigned int recur_counter)
{
    ...
    else
    {
        ...
        {
            if (!_create_object_instance(component.object_id, transform * component.transform, printable,
                                         recur_counter + 1))
                return false;
        }
    }
...
bool _3MF_Importer::_handle_start_config_object(const char **attributes, unsigned int num_attributes)
{
    ...
    // Instance count output (currently not used)
...
bool _3MF_Importer::_handle_start_config_volume(const char **attributes, unsigned int num_attributes)
{
    ...
    unsigned int first_triangle_id = (unsigned int) get_attribute_value_int(attributes, num_attributes,
                                                                            FIRST_TRIANGLE_ID_ATTR);
    unsigned int last_triangle_id = (unsigned int) get_attribute_value_int(attributes, num_attributes,
...
bool _3MF_Importer::_generate_volumes(ModelObject &object, const Geometry &geometry,
                                      const ObjectMetadata::VolumeMetadataList &volumes,
                                      ConfigSubstitutionContext &config_substitutions)
{
    ...
    {
        if (geo_tri_count <= volume_data.first_triangle_id || geo_tri_count <= volume_data.last_triangle_id ||
        ...
        its.indices.assign(geometry.triangles.begin() + volume_data.first_triangle_id,
                           geometry.triangles.begin() + volume_data.last_triangle_id + 1);
...

        {
            ...
            for (Vec3i &face : its.indices)
                for (coord_t &tri_id : face)
                    tri_id -= min_id;
        }

        if (m_generator_version && *m_generator_version >= *Semver::parse("2.4.0-alpha1") &&
            *m_generator_version < *Semver::parse("2.4.0-alpha3"))
            // Handle older 3MF files where all vertices were saved for each volume.
        ...
        if (m_version == 0)
        {
            // if the 3mf was not produced by preFlight and there is only one instance,
...
class _3MF_Exporter : public _3MF_Base
{
    struct BuildItem
    {
        ...
        BuildItem(unsigned int id, const Transform3d &transform, const bool printable)
            : id(id), transform(transform), printable(printable)
        {
        }
    };
    ...
    struct Offsets
    {
        ...
        Offsets(unsigned int first_vertex_id)
            : first_vertex_id(first_vertex_id), first_triangle_id(-1), last_triangle_id(-1)
        {
        }
    };
    ...
    struct ObjectData
    {
        ...
        explicit ObjectData(ModelObject *object) : object(object) {}
    };
...
public:
    bool save_model_to_file(const std::string &filename, Model &model, const DynamicPrintConfig *config,
                            bool fullpath_sources, const ThumbnailData *thumbnail_data, bool zip64);
...
private:
    ...
    bool _add_model_file_to_archive(const std::string &filename, mz_zip_archive &archive, const Model &model,
                                    IdToObjectDataMap &objects_data);
    bool _add_object_to_model_stream(mz_zip_writer_staged_context &context, unsigned int &object_id,
                                        ...
                                        const ThumbnailData *thumbnail_data)
{
    ...
    // The content of this file is the same for each preFlight 3mf.
    ...
    // Adds custom gcode per height file ("Metadata/preFlight_custom_gcode_per_print_z.xml").
    ...
    // Adds wipe tower information ("Metadata/preFlight_wipe_tower_information.xml").
...
bool _3MF_Exporter::_add_content_types_file_to_archive(mz_zip_archive &archive)
{
    ...
    if (!mz_zip_writer_add_mem(&archive, CONTENT_TYPES_FILE.c_str(), (const void *) out.data(), out.length(),
...
bool _3MF_Exporter::_add_thumbnail_file_to_archive(mz_zip_archive &archive, const ThumbnailData &thumbnail_data)
{
    ...
    size_t png_size = 0;
    void *png_data = tdefl_write_image_to_png_file_in_memory_ex((const void *) thumbnail_data.pixels.data(),
                                                                thumbnail_data.width, thumbnail_data.height, 4,
                                                                &png_size, MZ_DEFAULT_LEVEL, 1);
    if (png_data != nullptr)
    {
        res = mz_zip_writer_add_mem(&archive, THUMBNAIL_FILE.c_str(), (const void *) png_data, png_size,
                                    MZ_DEFAULT_COMPRESSION);
        mz_free(png_data);
    }
...
bool _3MF_Exporter::_add_relationships_file_to_archive(mz_zip_archive &archive)
{
    ...
    stream
        << " <Relationship Target=\"/" << THUMBNAIL_FILE
        << "\" Id=\"rel-2\" Type=\"http://schemas.openxmlformats.org/package/2006/relationships/metadata/thumbnail\"/>\n";
    stream << "</Relationships>";

    std::string out = stream.str();

    if (!mz_zip_writer_add_mem(&archive, RELATIONSHIPS_FILE.c_str(), (const void *) out.data(), out.length(),
                               MZ_DEFAULT_COMPRESSION))
    {
...
bool _3MF_Exporter::_add_model_file_to_archive(const std::string &filename, mz_zip_archive &archive, const Model &model,
                                               IdToObjectDataMap &objects_data)
{
    mz_zip_writer_staged_context context;
    if (!mz_zip_writer_add_staged_open(
            &archive, &context, MODEL_FILE.c_str(),
            m_zip64
                ?
...

    {
        ...
        if (model.is_fdm_support_painted())
            stream << " <" << METADATA_TAG << " name=\"" << SLIC3RPE_FDM_SUPPORTS_PAINTING_VERSION << "\">"
                   << FDM_SUPPORTS_PAINTING_VERSION << "</" << METADATA_TAG << ">\n";

        if (model.is_seam_painted())
            stream << " <" << METADATA_TAG << " name=\"" << SLIC3RPE_SEAM_PAINTING_VERSION << "\">"
                   << SEAM_PAINTING_VERSION << "</" << METADATA_TAG << ">\n";

        if (model.is_mm_painted())
            stream << " <" << METADATA_TAG << " name=\"" << SLIC3RPE_MM_PAINTING_VERSION << "\">" << MM_PAINTING_VERSION
                   << "</" << METADATA_TAG << ">\n";
...
bool _3MF_Exporter::_add_object_to_model_stream(mz_zip_writer_staged_context &context, unsigned int &object_id,
                                                ModelObject &object, BuildItemsList &build_items,
                                                VolumeToOffsetsMap &volumes_offsets)
{
    std::stringstream stream;
...
bool _3MF_Exporter::_add_mesh_to_object_stream(mz_zip_writer_staged_context &context, ModelObject &object,
                                               VolumeToOffsetsMap &volumes_offsets)
{
...
bool _3MF_Exporter::_add_cut_information_file_to_archive(mz_zip_archive &archive, Model &model)
{
    ...
    if (!out.empty())
    {
        if (!mz_zip_writer_add_mem(&archive, CUT_INFORMATION_FILE.c_str(), (const void *) out.data(), out.length(),
                                   MZ_DEFAULT_COMPRESSION))
        {
...
bool _3MF_Exporter::_add_layer_height_profile_file_to_archive(mz_zip_archive &archive, Model &model)
{
    ...
    if (!out.empty())
    {
        if (!mz_zip_writer_add_mem(&archive, LAYER_HEIGHTS_PROFILE_FILE.c_str(), (const void *) out.data(),
...
bool _3MF_Exporter::_add_sla_support_points_file_to_archive(mz_zip_archive &archive, Model &model)
{
    ...
    for (const ModelObject *object : model.objects)
    {
        ...
        if (!sla_support_points.empty())
        {
            ...
            auto support_point_type_to_float = [](sla::SupportPointType t) -> float
            {
                switch (t)
                {
                case Slic3r::sla::SupportPointType::manual_add:
                    return 2.f;
                case Slic3r::sla::SupportPointType::island:
                    return 1.f;
                case Slic3r::sla::SupportPointType::slope:
                    return 3.f;
                default:
                    assert(false);
                    return 0.f;
            ...
            {
                sprintf(buffer, (i == 0 ? "%f %f %f %f %f" : " %f %f %f %f %f"), sla_support_points[i].pos(0),
                        sla_support_points[i].pos(1), sla_support_points[i].pos(2),
    ...
    if (!out.empty())
    {
        ...
        out = std::string("support_points_format_version=") + std::to_string(support_points_format_version) +
              std::string("\n") + out;
...
bool _3MF_Exporter::_add_sla_drain_holes_file_to_archive(mz_zip_archive &archive, Model &model)
{
        ...
        if (!drain_holes.empty())
        {
            ...
            for (size_t i = 0; i < drain_holes.size(); ++i)
                out += string_printf((i == 0 ? "%f %f %f %f %f %f %f %f" : " %f %f %f %f %f %f %f %f"),
                                     drain_holes[i].pos(0), drain_holes[i].pos(1), drain_holes[i].pos(2),
                                     drain_holes[i].normal(0), drain_holes[i].normal(1), drain_holes[i].normal(2),
                                     drain_holes[i].radius, drain_holes[i].height);
    ...
    {
        // Adds version header at the beginning:
        out = std::string("drain_holes_format_version=") + std::to_string(drain_holes_format_version) +
              std::string("\n") + out;

        if (!mz_zip_writer_add_mem(&archive, SLA_DRAIN_HOLES_FILE.c_str(), static_cast<const void *>(out.data()),
                                   out.length(), mz_uint(MZ_DEFAULT_COMPRESSION)))
        {
            add_error("Unable to add sla support points file to archive");
            return false;
        }
                                                      ...
                                                      const Model &model)
{
    ...
    if (!out.empty())
    {
        if (!mz_zip_writer_add_mem(&archive, PRINT_CONFIG_FILE.c_str(), (const void *) out.data(), out.length(),
                                   MZ_DEFAULT_COMPRESSION))
        {
                                                      ...
                                                      const IdToObjectDataMap &objects_data)
{
    ...
    auto add_metadata = [](std::stringstream &stream, unsigned indent, MetadataType type, const std::string &key,
                           const std::string &value)
    {
        ...
        case MetadataType::volume:
            type_value = VOLUME_TYPE;
            break;
        };
        stream << std::string(indent, ' ') << '<' << METADATA_TAG << " " << TYPE_ATTR << "=\"" << type_value << "\" "
    ...
    for (const IdToObjectDataMap::value_type &obj_metadata : objects_data)
    {
        ...
        if (obj == nullptr)
            continue;
        // Output of instances count (currently not used)
        ...
        // Store brim ear points as space-separated values: x y z radius x y z radius...
        if (!obj->brim_points.empty())
        {
            std::stringstream brim_points_str;
            for (size_t i = 0; i < obj->brim_points.size(); ++i)
            {
                if (i > 0)
                    brim_points_str << " ";
                const BrimPoint &pt = obj->brim_points[i];
                brim_points_str << pt.pos(0) << " " << pt.pos(1) << " " << pt.pos(2) << " " << pt.head_front_radius
                                << " " << pt.overlap_percent;
            }
        ...
        for (const ModelVolume *volume : obj_metadata.second.object->volumes)
        {
            ...
            if (it != offsets.end())
            {
                ...
                // stores volume's source data
                {
                    ...
                    std::string prefix = std::string("   <") + METADATA_TAG + " " + TYPE_ATTR + "=\"" + VOLUME_TYPE +
                                         "\" " + KEY_ATTR + "=\"";
                ...
                {
                    stream << "   <" << METADATA_TAG << " " << TYPE_ATTR << "=\"" << VOLUME_TYPE << "\" " << KEY_ATTR
                           << "=\"" << key << "\" " << VALUE_ATTR << "=\"" << volume->config.opt_serialize(key)
                           << "\"/>\n";
                }
    ...
    if (!mz_zip_writer_add_mem(&archive, MODEL_CONFIG_FILE.c_str(), (const void *) out.data(), out.length(),
                               MZ_DEFAULT_COMPRESSION))
    {
                                                                  ...
                                                                  const DynamicPrintConfig *config)
{
    ...
    if (std::any_of(model.get_custom_gcode_per_print_z_vector().begin(),
                    model.get_custom_gcode_per_print_z_vector().end(),
                    [](const auto &cg) { return !cg.gcodes.empty(); }))
    {
        pt::ptree tree;
        for (size_t bed_idx = 0; bed_idx < model.get_custom_gcode_per_print_z_vector().size(); ++bed_idx)
        {
            ...
            for (const CustomGCode::Item &code : model.get_custom_gcode_per_print_z_vector()[bed_idx].gcodes)
            {
                ...
                // add gcode field data for older version compatibility
            ...
            mode_tree.put("<xmlattr>.value", model.custom_gcode_per_print_z().mode == CustomGCode::Mode::SingleExtruder
                                                 ? CustomGCode::SingleExtruderMode
                                             : model.custom_gcode_per_print_z().mode == CustomGCode::Mode::MultiAsSingle
                                                 ? CustomGCode::MultiAsSingleMode
    ...
    if (!out.empty())
    {
        if (!mz_zip_writer_add_mem(&archive, CUSTOM_GCODE_PER_PRINT_Z_FILE.c_str(), (const void *) out.data(),
                                   out.length(), MZ_DEFAULT_COMPRESSION))
        {
...
bool _3MF_Exporter::_add_wipe_tower_information_file_to_archive(mz_zip_archive &archive, Model &model)
{
    ...
    if (!out.empty())
    {
        if (!mz_zip_writer_add_mem(&archive, WIPE_TOWER_INFORMATION_FILE.c_str(), (const void *) out.data(),
                                   out.length(), MZ_DEFAULT_COMPRESSION))
        {
...
static void handle_legacy_project_loaded(DynamicPrintConfig &config, const boost::optional<Semver> &generator_version)
{
    if (!config.has("brim_separation"))
    {
        if (auto *opt_elephant_foot = config.option<ConfigOptionFloat>("elefant_foot_compensation", false);
            opt_elephant_foot)
        {
            // Conversion from older format which applied brim separation equal to elephant foot compensation.
    ...
    // Handle legacy Arachne parameters that depend
    ...
    if (generator_version && *generator_version >= *Semver::parse("2.5.0-alpha2") &&
        *generator_version <= *Semver::parse("2.5.0-alpha3"))
    {
        if (auto *opt_wall_transition_length = config.option<ConfigOptionFloatOrPercent>("wall_transition_length",
                                                                                         false);
            opt_wall_transition_length && !opt_wall_transition_length->percent &&
            opt_wall_transition_length->value == 0.4)
        {
        ...
        if (mz_zip_reader_file_stat(&archive, i, &stat))
        {
            ...
            if (boost::algorithm::iequals(name, MODEL_FILE))
            {
                if (auto *iter = mz_zip_reader_extract_iter_new(&archive, i, 0))
                {
                    ...
                    if (size_t bytes_read = mz_zip_reader_extract_iter_read(iter, buffer, sizeof(buffer));
                        bytes_read > 0)
                    {
...
bool store_3mf(const char *path, Model *model, const DynamicPrintConfig *config, bool fullpath_sources,
               const ThumbnailData *thumbnail_data, bool zip64)
{
...
const TextConfigurationSerialization::HorizontalAlignToName TextConfigurationSerialization::horizontal_align_to_name =
    boost::assign::list_of<HorizontalAlignToName::relation>(FontProp::HorizontalAlign::left, "left")(
        FontProp::HorizontalAlign::center, "center")(FontProp::HorizontalAlign::right, "right");

const TextConfigurationSerialization::VerticalAlignToName TextConfigurationSerialization::vertical_align_to_name =
    boost::assign::list_of<VerticalAlignToName::relation>(FontProp::VerticalAlign::top,
                                                          "top")(FontProp::VerticalAlign::center,
                                                                 "middle")(FontProp::VerticalAlign::bottom, "bottom");
...
FontProp::HorizontalAlign read_horizontal_align(
    const char **attributes, unsigned int num_attributes,
    const TextConfigurationSerialization::HorizontalAlignToName &horizontal_align_to_name)
{
    ...
    if (horizontal_align_str.length() == 1)
    {
        ...
        if (boost::spirit::qi::parse(horizontal_align_str.c_str(), horizontal_align_str.c_str() + 1,
                                     boost::spirit::qi::int_, horizontal_align_int))
            return static_cast<FontProp::HorizontalAlign>(horizontal_align_int);
    }

    return bimap_cvt(horizontal_align_to_name, std::string_view(horizontal_align_str),
                     FontProp::HorizontalAlign::center);
}

FontProp::VerticalAlign read_vertical_align(
    const char **attributes, unsigned int num_attributes,
    const TextConfigurationSerialization::VerticalAlignToName &vertical_align_to_name)
{
    ...
    if (vertical_align_str.length() == 1)
    {
        ...
        if (boost::spirit::qi::parse(vertical_align_str.c_str(), vertical_align_str.c_str() + 1,
                                     boost::spirit::qi::int_, vertical_align_int))
...
std::optional<TextConfiguration> TextConfigurationSerialization::read(const char **attributes,
...
bool to_xml(std::stringstream &stream, const EmbossShape::SvgFile &svg, const ModelVolume &volume,
            mz_zip_archive &archive)
{
    ...
    return mz_zip_writer_add_mem(&archive, svg.path_in_3mf.c_str(), (const void *) file_data_str.c_str(),
                                 file_data_str.size(), MZ_DEFAULT_COMPRESSION);
}
...
std::optional<EmbossShape> read_emboss_shape(const char **attributes, unsigned int num_attributes)
{
    ...
    return EmbossShape{std::move(shapes),     std::move(final_shape), scale,
                       std::move(projection), std::move(fix_tr_mat),  std::move(svg)};
}
