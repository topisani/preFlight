...
static std::vector<std::string> s_project_options{"colorprint_heights", "wiping_volumes_matrix",
                                                  "wiping_volumes_use_custom_matrix"};

const char *PresetBundle::PREFLIGHT_BUNDLE = "preFlight";
...
PresetBundle::PresetBundle()
    ...
    , sla_prints(Preset::TYPE_SLA_PRINT, Preset::sla_print_options(),
                 static_cast<const SLAPrintObjectConfig &>(SLAFullPrintConfig::defaults()))
    , printers(Preset::TYPE_PRINTER, Preset::printer_options(),
               static_cast<const PrintRegionConfig &>(FullPrintConfig::defaults()), "- default -")
    , physical_printers(PhysicalPrinter::printer_options(), this)
{
    ...
    // preFlight: SLA printing not supported, only FFF default printer
    {
...
void PresetBundle::setup_directories()
{
    ...
    std::initializer_list<boost::filesystem::path> paths = {
        data_dir, data_dir / "vendor", data_dir / "cache", data_dir / "cache" / "vendor", data_dir / "shapes",
#ifdef SLIC3R_PROFILE_USE_PRESETS_SUBDIR
        // Store the print/filament/printer presets into a "presets" directory.
        data_dir / "presets", data_dir / "presets" / "print", data_dir / "presets" / "filament",
        data_dir / "presets" / "sla_print", data_dir / "presets" / "sla_material", data_dir / "presets" / "printer",
        data_dir / "presets" / "physical_printer"
#else
        // Store the print/filament/printer presets at the same location as the upstream Slic3r.
        data_dir / "print", data_dir / "filament", data_dir / "sla_print", data_dir / "sla_material",
        data_dir / "printer", data_dir / "physical_printer"
#endif
    };
    for (const boost::filesystem::path &path : paths)
    {
        ...
        if (!boost::filesystem::is_directory(subdir) && !boost::filesystem::create_directory(subdir))
            throw Slic3r::RuntimeError(std::string("Slic3r was unable to create its data directory at ") +
                                       subdir.string());
    }
}
...
static void copy_dir(const boost::filesystem::path &from_dir, const boost::filesystem::path &to_dir)
{
    ...
    for (auto &dir_entry : boost::filesystem::directory_iterator(from_dir))
    {
        ...
        {
            std::string em;
            CopyFileResult cfr = copy_file(dir_entry.path().string(), (to_dir / dir_entry.path().filename()).string(),
                                           em, false);
            if (cfr != SUCCESS)
            {
                BOOST_LOG_TRIVIAL(error) << "Error when copying files from " << from_dir << " to " << to_dir << ": "
                                         << em;
            }
        }
        else
...
// Import newer configuration from alternate preFlight configuration directory.
...
void PresetBundle::import_newer_configs(const std::string &from)
{
    ...
    std::initializer_list<boost::filesystem::path> from_dirs = {
        from_data_dir / "cache", from_data_dir / "vendor", from_data_dir / "shapes",
#ifdef SLIC3R_PROFILE_USE_PRESETS_SUBDIR
        // Store the print/filament/printer presets into a "presets" directory.
        data_dir / "presets" / "print", data_dir / "presets" / "filament", data_dir / "presets" / "sla_print",
        data_dir / "presets" / "sla_material", data_dir / "presets" / "printer",
        data_dir / "presets" / "physical_printer"
#else
        // Store the print/filament/printer presets at the same location as the upstream Slic3r.
        from_data_dir / "print", from_data_dir / "filament", from_data_dir / "sla_print",
        from_data_dir / "sla_material", from_data_dir / "printer", from_data_dir / "physical_printer"
#endif
    };
    ...
    std::string em;
    CopyFileResult cfr = copy_file((from_data_dir / "ArchiveRepositoryManifest.json").string(),
                                   (data_dir / "ArchiveRepositoryManifest.json").string(), em, false);
    if (cfr != SUCCESS)
    {
        BOOST_LOG_TRIVIAL(error) << "Error when copying files from " << from_data_dir << " to " << data_dir << ": "
                                 << em;
    }
}

PresetsConfigSubstitutions PresetBundle::load_presets(
        ...
        if (Slic3r::is_ini_file(dir_entry))
        {
            ...
            try
            {
                ...
                if (first)
                {
                    ...
                    append(substitutions, this->load_configbundle(dir_entry.path().string(), PresetBundle::LoadSystem,
                                                                  compatibility_rule)
                                              .first);
                    first = false;
                }
                else
                {
                    ...
                    PresetBundle other;
                    append(substitutions, other
                                              .load_configbundle(dir_entry.path().string(), PresetBundle::LoadSystem,
                                                                 compatibility_rule)
                                              .first);
...
std::vector<std::string> PresetBundle::merge_presets(PresetBundle &&other)
{
    ...
    std::vector<std::string> duplicate_sla_prints = this->sla_prints.merge_presets(std::move(other.sla_prints),
                                                                                   this->vendors);
    std::vector<std::string> duplicate_filaments = this->filaments.merge_presets(std::move(other.filaments),
                                                                                 this->vendors);
    std::vector<std::string> duplicate_sla_materials = this->sla_materials.merge_presets(std::move(other.sla_materials),
                                                                                         this->vendors);
    std::vector<std::string> duplicate_printers = this->printers.merge_presets(std::move(other.printers),
                                                                               this->vendors);
...
const std::string &PresetBundle::get_preset_name_by_alias_invisible(const Preset::Type &preset_type,
...
bool PresetBundle::transfer_and_save(Preset::Type type, const std::string &preset_from_name,
                                     const std::string &preset_to_name, const std::string &preset_new_name,
...
void PresetBundle::load_installed_filaments(AppConfig &config)
{
    ...
    {
        // Compatibility with older versions where the filament profiles were not installable yet.
        ...
        for (const Preset &printer : printers)
            ...
            {
                const PresetWithVendorProfile printer_with_vendor_profile = printers.get_preset_with_vendor_profile(
                    printer);
                for (const Preset &filament : filaments)
                    if (filament.is_system &&
                        is_compatible_with_printer(filaments.get_preset_with_vendor_profile(filament),
...
void PresetBundle::load_installed_sla_materials(AppConfig &config)
{
    if (!config.has_section(AppConfig::SECTION_MATERIALS))
    {
        ...
        // Compatibility with older versions where the SLA material profiles were not installable yet.
            ...
            if (printer.is_visible && printer.printer_technology() == ptSLA)
            {
                ...
                for (const Preset &material : sla_materials)
                    if (material.is_system &&
                        is_compatible_with_printer(sla_materials.get_preset_with_vendor_profile(material),
                                                   printer_with_vendor_profile))
                                   ...
                                   const PresetPreferences &preferred_selection /* = PresetPreferences()*/)
{
    ...
    const Preset *preferred_printer = printers.find_system_preset_by_model_and_variant(
        preferred_selection.printer_model_id, preferred_selection.printer_variant);
    ...
    for (unsigned int i = 1; i < 1000; ++i)
    {
        ...
        if (!config.has("presets", name))
            break;
        this->extruders_filaments.emplace_back(
            ExtruderFilaments(&filaments, i, remove_ini_suffix(config.get("presets", name))));
    }
    ...
    if (initial_printer != nullptr && (preferred_printer == nullptr || initial_printer == preferred_printer))
    {
        ...
        {
            const std::string &preferred_preset_name = get_preset_name_by_alias(Preset::Type::TYPE_FILAMENT,
                                                                                preferred_selection.filament, 0);
        ...
        {
            const std::string &preferred_preset_name = get_preset_name_by_alias(Preset::Type::TYPE_SLA_MATERIAL,
                                                                                preferred_selection.sla_material);
...
{
    return (this->printers.get_edited_preset().printer_technology() == ptFFF) ? this->full_fff_config()
...
DynamicPrintConfig PresetBundle::full_fff_config() const
{
    ...
    else
    {
        ...
        for (const auto &extr_filaments : this->extruders_filaments)
            filament_configs.emplace_back(
                &this->filaments.find_preset(extr_filaments.get_selected_preset_name(), true)->config);
    ...
    static const char *keys[] = {"perimeter", "infill", "solid_infill", "support_material",
    ...
    out.option<ConfigOptionString>("physical_printer_settings_id", true)->value =
        this->physical_printers.get_selected_printer_name();
...
DynamicPrintConfig PresetBundle::full_sla_config() const
{
    ...
    out.option<ConfigOptionString>("sla_material_settings_id", true)->value =
        this->sla_materials.get_selected_preset_name();
    out.option<ConfigOptionString>("printer_settings_id", true)->value = this->printers.get_selected_preset_name();
    out.option<ConfigOptionString>("physical_printer_settings_id", true)->value =
        this->physical_printers.get_selected_printer_name();
                                                   ...
                                                   ForwardCompatibilitySubstitutionRule compatibility_rule)
{
    if (is_gcode_file(path))
    {
        ...
        const bool is_binary = bgcode::core::is_valid_binary_gcode(*file, true, cs_buffer.data(), cs_buffer.size()) ==
                               bgcode::core::EResult::Success;
        fclose(file);
    ...
    {
        throw Slic3r::RuntimeError(format("Failed loading the Config Bundle \"%1%\": %2% at line %3%", err.filename(),
    ...
    try
    {
        ...
        case CONFIG_FILE_TYPE_APP_CONFIG:
            throw Slic3r::RuntimeError(std::string("Invalid configuration file: ") + path +
...
void PresetBundle::load_config_file_config(const std::string &name_or_path, bool is_external,
                                           DynamicPrintConfig &&config)
{
    ...
    std::vector<std::string> compatible_printers_condition_values = std::move(
        config.option<ConfigOptionStrings>("compatible_printers_condition_cummulative", true)->values);
    std::vector<std::string> compatible_prints_condition_values = std::move(
        config.option<ConfigOptionStrings>("compatible_prints_condition_cummulative", true)->values);
    std::vector<std::string> inherits_values = std::move(
        config.option<ConfigOptionStrings>("inherits_cummulative", true)->values);
    ...
    auto load_preset = [&config, &inherits, &inherits_values, &compatible_printers_condition,
                        &compatible_printers_condition_values, &compatible_prints_condition,
                        &compatible_prints_condition_values, is_external, &name, &name_or_path,
                        &tmp_installed_presets_ref](PresetCollection &presets, size_t idx, const std::string &key)
    {
        ...
        if (is_external)
        {
            ExternalPreset ext_preset = presets.load_external_preset(name_or_path, name, config.opt_string(key, true),
                                                                     config);
    ...
    case ptFFF:
    {
        ...
        if (num_extruders <= 1)
        {
            ...
            if (is_external)
            {
                ExternalPreset ext_preset = this->filaments.load_external_preset(
                    name_or_path, name, old_filament_profile_names->values.front(), config);
        ...
        else
        {
            ...
            for (int i = 0; i < (int) configs.size(); i++)
            {
                ...
                auto [loaded, modified, installed] = this->filaments.load_external_preset(
                    name_or_path, name,
    ...
    else
    {
        ...
        if (pp != nullptr && std::find(pp->preset_names.begin(), pp->preset_names.end(),
                                       this->printers.get_edited_preset().name) != pp->preset_names.end())
...
ConfigSubstitutions PresetBundle::load_config_file_config_bundle(const std::string &path,
                                                                 ...
                                                                 ForwardCompatibilitySubstitutionRule compatibility_rule)
{
    ...
    ConfigSubstitutions config_substitutions;
    auto load_one = [&path, &bundle_name, &presets_substitutions = presets_substitutions,
                     &config_substitutions](PresetCollection &collection_dst, PresetCollection &collection_src,
                                            const std::string &preset_name_src, bool activate) -> std::string
    {
        // If there are substitutions reported for this preset, move them to config_substitutions.
        if (auto it = std::find_if(presets_substitutions.begin(), presets_substitutions.end(),
                                   [&preset_name_src](const PresetConfigSubstitutions &subs)
    ...
    for (size_t i = 1; i < std::min(tmp_bundle.extruders_filaments.size(), this->extruders_filaments.size()); ++i)
        this->extruders_filaments[i].select_filament(
            load_one(this->filaments, tmp_bundle.filaments,
                     tmp_bundle.extruders_filaments[i].get_selected_preset_name(), false));
...
static void flatten_configbundle_hierarchy(boost::property_tree::ptree &tree, const std::string &group_name,
                                           const std::vector<std::string> &system_profiles)
{
            ...
            for (const Prst &prst : presets)
                if (prst.num_incoming_edges_left)
                    BOOST_LOG_TRIVIAL(error)
                        << "flatten_configbundle_hierarchy: The preset " << prst.name << " has cyclic dependencies";
        }
    }
            ...
            for (auto it = (*it_inherits)->node->begin(); it != (*it_inherits)->node->end(); ++it)
                if (it->first == "renamed_from")
                {
                    ...
                    if (boost::starts_with((*it_inherits)->name, "*"))
                        BOOST_LOG_TRIVIAL(error)
                            << boost::format(
                                   "Nonpublic intermediate preset %1% contains a \"renamed_from\" field, which is ignored") %
                                   (*it_inherits)->name;
                }
                else
                {
    ...
    {
        if (boost::starts_with(it_section->first, group_name_preset) &&
            it_section->first.size() > group_name_preset.size())
...
static void flatten_configbundle_hierarchy(boost::property_tree::ptree &tree, const PresetBundle *preset_bundle)
{
    flatten_configbundle_hierarchy(tree, "print",
                                   preset_bundle ? preset_bundle->prints.system_preset_names()
                                                 : std::vector<std::string>());
    flatten_configbundle_hierarchy(tree, "filament",
                                   preset_bundle ? preset_bundle->filaments.system_preset_names()
                                                 : std::vector<std::string>());
    flatten_configbundle_hierarchy(tree, "sla_print",
                                   preset_bundle ? preset_bundle->sla_prints.system_preset_names()
                                                 : std::vector<std::string>());
    flatten_configbundle_hierarchy(tree, "sla_material",
                                   preset_bundle ? preset_bundle->sla_materials.system_preset_names()
                                                 : std::vector<std::string>());
    flatten_configbundle_hierarchy(tree, "printer",
                                   preset_bundle ? preset_bundle->printers.system_preset_names()
                                                 : std::vector<std::string>());
}
    ...
    const std::string &path, LoadConfigBundleAttributes flags, ForwardCompatibilitySubstitutionRule compatibility_rule)
{
    ...
    for (const auto &section : tree)
    {
        ...
        if (presets != nullptr)
        {
            ...
            try
            {
                auto parse_config_section =
                    [&section, &alias_name, &renamed_from, &substitution_context, &path](DynamicPrintConfig &config)
                {
            ...
            if (!incorrect_keys.empty())
                BOOST_LOG_TRIVIAL(error) << "Error in a Vendor Config Bundle \"" << path << "\": The printer preset \""
                                         << section.first
                                         << "\" contains the following incorrect keys: " << incorrect_keys
                                         << ", which were removed";
            if (flags.has(LoadConfigBundleAttribute::LoadSystem) && presets == &printers)
            {
                ...
                {
                    BOOST_LOG_TRIVIAL(error)
                        << "Error in a Vendor Config Bundle \"" << path << "\": The printer preset \"" << section.first
                ...
                auto it_model = std::find_if(vendor_profile->models.cbegin(), vendor_profile->models.cend(),
                                             [&](const VendorProfile::PrinterModel &m)
                                             { return m.id == printer_model; });
                ...
                if (preset_existing != nullptr)
                {
                    BOOST_LOG_TRIVIAL(error)
                        << "Error in a Vendor Config Bundle \"" << path << "\": The printer preset \"" << section.first
                        << "\" has already been loaded from another Confing Bundle.";
                    continue;
                }
            }
                ...
                if (existing != nullptr)
                {
                    if (existing->is_system)
                    {
                        ...
                        BOOST_LOG_TRIVIAL(error)
                            ...
                            << " preset \"" << existing->name << "\" is a system preset of vendor "
                            << existing->vendor->name << " and it will be ignored.";
                        continue;
                    }
                    else
                    {
                        ...
                        BOOST_LOG_TRIVIAL(trace)
                            << "A " << presets->name() << " preset \"" << existing->name
                            << "\" was overwritten with a preset from user Config Bundle \"" << path << "\"";
                    }
                }
                else
                {
                    BOOST_LOG_TRIVIAL(trace) << "A new " << presets->name() << " preset \"" << preset_name
            ...
            // Store the print/filament/printer presets at the same location as the upstream Slic3r.
#endif
                              / presets->section_name() / file_name)
                                 .make_preferred();
        ...
        if (ph_printers != nullptr)
        {
            ...
            std::string incorrect_keys = Preset::remove_invalid_keys(config, default_config);
            if (!incorrect_keys.empty())
                BOOST_LOG_TRIVIAL(error) << "Error in a Vendor Config Bundle \"" << path
                                         << "\": The physical printer \"" << section.first
                                         << "\" contains the following incorrect keys: " << incorrect_keys
                                         << ", which were removed";
            ...
            // Decide a full path to this .ini file.
            auto file_name = boost::algorithm::iends_with(ph_printer_name, ".ini") ? ph_printer_name
                                                                                   : ph_printer_name + ".ini";
            auto file_path = (boost::filesystem::path(data_dir())
#ifdef SLIC3R_PROFILE_USE_PRESETS_SUBDIR
...
#endif
                              / "physical_printer" / file_name)
                                 .make_preferred();
            // Load the preset into the list of presets, save it to disk.
            ph_printers->load_printer(file_path.string(), ph_printer_name, std::move(config), false,
                                      flags.has(LoadConfigBundleAttribute::SaveImported));
            if (!substitution_context.empty())
                substitutions.push_back({ph_printer_name, Preset::TYPE_PHYSICAL_PRINTER,
                                         ...
                                         std::move(substitution_context.substitutions)});
            ++ph_printers_loaded;
        }
    }
...
void PresetBundle::update_multi_material_filament_presets()
{
    ...
    auto *nozzle_diameter = static_cast<const ConfigOptionFloats *>(
        printers.get_edited_preset().config.option("nozzle_diameter"));
    size_t num_extruders = nozzle_diameter->values.size();
    // Verify validity of the current filament presets.
    for (size_t i = 0; i < std::min(this->extruders_filaments.size(), num_extruders); ++i)
        this->extruders_filaments[i].select_filament(
            this->filaments.find_preset(this->extruders_filaments[i].get_selected_preset_name(), true)->name);
    ...
    if (this->extruders_filaments.size() > num_extruders)
        ...
        for (size_t id = extruders_filaments.size(); id < num_extruders; id++)
            extruders_filaments.emplace_back(
                ExtruderFilaments(&filaments, id,
                                  id == 0 ? filaments.first_visible().name
                                          : extruders_filaments[id - 1].get_selected_preset_name()));
    ...
    if (num_extruders != old_number_of_extruders)
    {
        ...
        const double default_purge = static_cast<const ConfigOptionFloat *>(
                                         this->printers.get_edited_preset().config.option("multimaterial_purging"))
                                         ->value;
        const std::vector<double> filament_purging_multipliers =
            get_config_options_for_current_filaments<ConfigOptionPercents>("filament_purge_multiplier");
                                     ...
                                     PresetSelectCompatibleType select_other_filament_if_incompatible)
{
        ...
        int operator()(const Preset &preset) const
        {
            ...
            if (match_quality < std::numeric_limits<int>::max())
            {
                ...
                if (m_prefered_layer_height > 0. &&
                    std::abs(preset.config.opt_float("layer_height") - m_prefered_layer_height) < 0.0005)
                    match_quality *= 10;
            }
            return match_quality;
        }
    ...
    case ptFFF:
    {
        ...
        this->prints.update_compatible(
            printer_preset_with_vendor_profile, nullptr, select_other_print_if_incompatible,
            PreferedPrintProfileMatch(this->prints.get_selected_idx() == size_t(-1) ? nullptr
    ...
    case ptSLA:
    {
        ...
        this->sla_prints.update_compatible(printer_preset_with_vendor_profile, nullptr,
                                           select_other_print_if_incompatible,
                                           PreferedPrintProfileMatch(this->sla_prints.get_selected_idx() == size_t(-1)
                                                                         ? nullptr
                                                                         : &this->sla_prints.get_edited_preset(),
                                                                     printer_preset.config.opt_string(
                                                                         "default_sla_print_profile")));
        const PresetWithVendorProfile sla_print_preset_with_vendor_profile =
            this->sla_prints.get_edited_preset_with_vendor_profile();
...
void PresetBundle::export_configbundle(
    const std::string &path, bool export_system_settings, bool export_physical_printers /* = false*/,
    std::function<bool(const std::string &, const std::string &, std::string &)> secret_callback)
{
...
void copy_bed_model_and_texture_if_needed(DynamicPrintConfig &config)
{
    ...
    auto do_copy = [&user_dir, &res_dir](ConfigOptionString *cfg, const std::string &type)
    {
        ...
        if (src_dir != user_dir && src_dir.parent_path() != res_dir)
        {
            ...
            {
                BOOST_LOG_TRIVIAL(error) << "Copying from " << cfg->value << " to " << dst_value
                                         << " failed. Unable to set custom bed " << type << ". [" << error << "]";
                cfg->value = "";
            }
        }
    };
