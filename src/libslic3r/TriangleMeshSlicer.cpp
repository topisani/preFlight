...
template<>
struct PolygonsType<AdditionalMeshInfo::None>
{
    using type = Polygons;
};

template<>
struct PolygonsType<AdditionalMeshInfo::Color>
{
    using type = ColorPolygons;
};

template<AdditionalMeshInfo mesh_info>
struct FacetColorFunctor;

template<>
struct FacetColorFunctor<AdditionalMeshInfo::None>
{
    constexpr ColorPolygon::Color operator()(size_t facet_idx) const { return 0; }
};

template<>
struct FacetColorFunctor<AdditionalMeshInfo::Color>
{
...
inline Point v3f_scaled_to_contour_point(const Eigen::MatrixBase<Derived> &v)
{
    static_assert(Derived::IsVectorAtCompileTime && int(Derived::SizeAtCompileTime) >= 2,
                  "v3f_scaled_to_contour_point(): Not a 2D or 3D vector.");
...
inline FacetSliceType slice_facet(
    ...
    const Eigen::Matrix<T, 3, 1, Eigen::DontAlign> *vertices, const stl_triangle_vertex_indices &indices,
    const Vec3i &edge_ids, const int idx_vertex_lowest, const bool horizontal, const ColorPolygon::Color facet_color,
    ...
    { // loop through facet edges
        ...
        if (a->z() == slice_z && b->z() == slice_z)
        {
            ...
            if (horizontal)
            {
                ...
                double normal = cross2((to_2d(v1) - to_2d(v0)).template cast<double>(),
                                       (to_2d(v2) - to_2d(v1)).template cast<double>());
                if (normal < 0)
                {
        ...
        else if ((a->z() < slice_z && b->z() > slice_z) || (b->z() < slice_z && a->z() > slice_z))
        {
            ...
            static_cast<Point &>(point) = t <= 0.   ? v3f_scaled_to_contour_point(*a)
                                          : t >= 1. ? v3f_scaled_to_contour_point(*b)
                                                    : v3f_scaled_to_contour_point(
                                                          a->template head<2>().template cast<double>() * (1. - t) +
                                                          b->template head<2>().template cast<double>() * t +
                                                          Vec2d(0.5, 0.5));
            point.edge_id = edge_id;
            ++num_points;
#endif
...
void slice_facet_at_zs(
    ...
    const std::vector<Vec3f> &mesh_vertices, const TransformVertex &transform_vertex_fn,
    const stl_triangle_vertex_indices &indices, const Vec3i &edge_ids, const ColorPolygon::Color facet_color,
...
{
    stl_vertex vertices[3]{transform_vertex_fn(mesh_vertices[indices(0)]),
                           transform_vertex_fn(mesh_vertices[indices(1)]),
    ...
    for (auto it = min_layer; it != max_layer; ++it)
    {
        ...
        if (min_z != max_z && slice_facet(*it, vertices, indices, edge_ids, idx_vertex_lowest, false, facet_color,
                                          il) == FacetSliceType::Slicing)
        {
                                                              ...
                                                              const ThrowOnCancel throw_on_cancel_fn)
{
    ...
    tbb::parallel_for(tbb::blocked_range<int>(0, int(indices.size())),
                      [&vertices, &transform_vertex_fn, &indices, &face_edge_ids, &facet_color_fn, &zs, &lines,
                       &lines_mutex, throw_on_cancel_fn](const tbb::blocked_range<int> &range)
                      {
                          for (int face_idx = range.begin(); face_idx < range.end(); ++face_idx)
                          {
                              if ((face_idx & 0x0ffff) == 0)
                                  throw_on_cancel_fn();
                              slice_facet_at_zs(vertices, transform_vertex_fn, indices[face_idx],
                                                face_edge_ids[face_idx], facet_color_fn(face_idx), zs, lines,
                                                lines_mutex);
                          }
                      });
...
static inline IntersectionLines slice_make_lines(const std::vector<stl_vertex> &mesh_vertices,
                                                 ...
                                                 const std::vector<Vec3i> &face_edge_ids,
                                                 const FacetColorFunctor<mesh_info> &facet_color_fn,
                                                 const float plane_z, FaceFilter face_filter)
{
    ...
    for (int face_idx = 0; face_idx < int(mesh_faces.size()); ++face_idx)
    {
        if (face_filter(face_idx))
        {
            ...
            IntersectionLine il;
            // Ignore horizontal triangles. Any valid horizontal triangle must have a vertical triangle connected, otherwise the part has zero volume.
            if (min_z != max_z && slice_facet(plane_z, vertices, indices, face_edge_ids[face_idx], idx_vertex_lowest,
                                              false, facet_color_fn(face_idx), il) == FacetSliceType::Slicing)
            {
                assert(il.edge_type != IntersectionLine::FacetEdgeType::Horizontal);
...
void slice_facet_with_slabs(
    // Scaled or unscaled vertices. transform_vertex_fn may scale zs.
    const std::vector<Vec3f> &mesh_vertices, const std::vector<stl_triangle_vertex_indices> &mesh_triangles,
    const size_t facet_idx, const Vec3i &facet_neighbors, const Vec3i &facet_edge_ids,
    ...
    const int num_edges, const std::vector<float> &zs, SlabLines &lines, LinesMutexes &lines_mutex)
{
    ...
    if (min_layer == max_layer || horizontal)
    {
        ...
        assert(horizontal || zs.empty() || max_z < zs.front() || min_z > zs.back() ||
               (min_layer == max_layer && min_layer != zs.end() && min_layer != zs.begin() &&
                *(min_layer - 1) < min_z && *min_layer > max_z));
        if (horizontal && min_layer != zs.end() && *min_layer == min_z)
        {
            ...
            if (size_t line_id = ProjectionFromTop ? slice_id : slice_id + 1;
                ProjectionFromTop || line_id < lines.at_slice.size())
#endif
                for (int iedge = 0; iedge < 3; ++iedge)
                    if (facet_neighbors(iedge) == -1)
                    {
                        ...
                        il.edge_a_id = -1;
                        il.edge_b_id = -1;
                        il.edge_type = ProjectionFromTop ? IntersectionLine::FacetEdgeType::Bottom
        ...
        for (auto it = min_layer; it != max_layer; ++it)
        {
            ...
            else
            {
                if (il.edge_type == IntersectionLine::FacetEdgeType::Top ||
                    il.edge_type == IntersectionLine::FacetEdgeType::Bottom)
                {
                    ...
                    else
                    {
                        ...
                        float z = *it;
                        int num_on_plane = (mesh_vertices[neighbor(0)].z() == z) +
                                           (mesh_vertices[neighbor(1)].z() == z) +
                ...
                for (int iedge = 0; iedge < 3; ++iedge)
                    if (facet_neighbors(iedge) == -1)
                    {
                        ...
                        if (intersects_this && intersects_prev)
                        {
                            ...
                            IntersectionLine il_slab;
                            il_slab.a = l.a;
                            il_slab.b = l.b;
                            il_slab.a_id = -1;
                            il_slab.b_id = -1;
                            il_slab.edge_a_id = edge_id;
                            il_slab.edge_b_id = edge_id + num_edges;
                            il_slab.edge_type = IntersectionLine::FacetEdgeType::Slab;
                            emit_slab_edge(il_slab, slab_id, ProjectionFromTop != edge_up);
                        }
                        else if (intersects_this)
                        {
                            // Intersects just the top plane, may touch the bottom plane.
                            assert((vertices[i].z() > *it && vertices[j].z() < *it) ||
                                   (vertices[i].z() < *it && vertices[j].z() > *it));
                            assert(il.edge_a_id == edge_id || il.edge_b_id == edge_id);
                            IntersectionLine il_slab;
                            il_slab.a = to_2d(edge_up ? vertices[i] : vertices[j]).cast<coord_t>();
                            il_slab.b = il.edge_a_id == edge_id ? il.a : il.b;
                            il_slab.a_id = edge_up ? indices(i) : indices(j);
                            il_slab.b_id = -1;
                            il_slab.edge_a_id = -1;
                            il_slab.edge_b_id = edge_id + num_edges;
                            il_slab.edge_type = IntersectionLine::FacetEdgeType::Slab;
...
#endif // NDEBUG
                            IntersectionLine il_slab;
                            il_slab.a = il_prev.edge_a_id == edge_id ? il_prev.a : il_prev.b;
                            il_slab.b = to_2d(edge_up ? vertices[j] : vertices[i]).cast<coord_t>();
                            il_slab.a_id = -1;
                            il_slab.b_id = edge_up ? indices(j) : indices(i);
                            il_slab.edge_a_id = edge_id;
                            il_slab.edge_b_id = -1;
                            il_slab.edge_type = IntersectionLine::FacetEdgeType::Slab;
                            emit_slab_edge(il_slab, slab_id, ProjectionFromTop != edge_up);
                        ...
                        else if (float zi = vertices[i].z(), zj = vertices[j].z(); zi < *it || zj < *it)
                        {
                            ...
                            if (inside_slab)
                            {
                                assert(ProjectionFromTop ? vertices[i].z() >= zs[slab_id]
                                                         : vertices[i].z() <= zs[slab_id]);
                                assert(ProjectionFromTop ? vertices[j].z() >= zs[slab_id]
                                                         : vertices[j].z() <= zs[slab_id]);
                                IntersectionLine il_slab;
                                il_slab.a = to_2d(vertices[i]).cast<coord_t>();
                                il_slab.b = to_2d(vertices[j]).cast<coord_t>();
                                il_slab.a_id = indices(i);
                                il_slab.b_id = indices(j);
                                il_slab.edge_a_id = -1;
                                il_slab.edge_b_id = -1;
                                il_slab.edge_type = IntersectionLine::FacetEdgeType::Slab;
                                emit_slab_edge(il_slab, slab_id, !ProjectionFromTop);
                            }
                        }
                    }
            }
...
inline std::pair<SlabLines, SlabLines> slice_slabs_make_lines(
    const std::vector<stl_vertex> &vertices, const std::vector<stl_triangle_vertex_indices> &indices,
    const std::vector<Vec3i> &face_neighbors, const std::vector<Vec3i> &face_edge_ids,
    ...
    const int num_edges, const std::vector<FaceOrientation> &face_orientation, const std::vector<float> &zs, bool top,
    bool bottom, const ThrowOnCancel throw_on_cancel_fn)
{
    ...
    tbb::parallel_for(tbb::blocked_range<int>(0, int(indices.size())),
                      [&vertices, &indices, &face_neighbors, &face_edge_ids, num_edges, &face_orientation, &zs, top,
                       bottom, &lines_top, &lines_bottom, &lines_mutex_top, &lines_mutex_bottom,
                       throw_on_cancel_fn](const tbb::blocked_range<int> &range)
                      {
                          for (int face_idx = range.begin(); face_idx < range.end(); ++face_idx)
                          {
                              ...
                              if (top && (fo == FaceOrientation::Up || fo == FaceOrientation::Degenerate))
                              {
                                  ...
                                  slice_facet_with_slabs<true>(vertices, indices, face_idx, neighbors, edge_ids,
                              ...
                              if (bottom && (fo == FaceOrientation::Down || fo == FaceOrientation::Degenerate))
                              {
                                  ...
                                  slice_facet_with_slabs<false>(vertices, indices, face_idx, neighbors, edge_ids,
                                                                num_edges, zs, lines_bottom, lines_mutex_bottom);
                              }
                          }
                      });
    return out;
...
struct OpenPolyline
{
    ...
    OpenPolyline(const IntersectionReference &start, const IntersectionReference &end, Points &&points,
        ...
        , length(Slic3r::length(this->points))
        , consumed(false)
    {
    }
    void reverse()
                                                 ...
                                                 std::vector<OpenPolyline> &open_polylines)
{
    ...
    auto by_edge_lower = [](const IntersectionLine *il1, const IntersectionLine *il2)
    {
        return il1->edge_a_id < il2->edge_a_id;
    };
    auto by_vertex_lower = [](const IntersectionLine *il1, const IntersectionLine *il2)
    {
        return il1->a_id < il2->a_id;
    };
        ...
        for (;;)
        {
            ...
            if (next_line == nullptr)
            {
                ...
                else
                {
                    ...
                    open_polylines.emplace_back(
                        OpenPolyline(IntersectionReference(first_line->a_id, first_line->edge_a_id),
                                     IntersectionReference(last_line->b_id, last_line->edge_b_id), std::move(loop_pts),
                                     std::move(loop_colors)));
                }
                break;
            }
            /*
...
std::vector<OpenPolyline *> open_polylines_sorted(std::vector<OpenPolyline> &open_polylines, bool update_lengths)
{
    ...
    std::sort(out.begin(), out.end(),
              [](const OpenPolyline *lhs, const OpenPolyline *rhs) { return lhs->length > rhs->length; });
    return out;
}
...
static void chain_open_polylines_exact(std::vector<OpenPolyline> &open_polylines,
                                       typename PolygonsType<mesh_info>::type &loops, bool try_connect_reversed)
{
    ...
    struct OpenPolylineEnd
    {
        ...
        bool operator==(const OpenPolylineEnd &rhs) const
        {
            return this->polyline == rhs.polyline && this->start == rhs.start;
        }
    };
    auto by_id_lower = [](const OpenPolylineEnd &ope1, const OpenPolylineEnd &ope2)
    {
        return ope1.id() < ope2.id();
    };
    ...
    auto find_polyline_end = [&by_id, by_id_lower](const OpenPolylineEnd &end) ->
        typename std::vector<OpenPolylineEnd>::iterator
    {
...
template<AdditionalMeshInfo mesh_info>
bool handle_color_at_gap_between_open_polylines(OpenPolyline &opl, const Point &next_polyline_first_pt,
...
static void chain_open_polylines_close_gaps(std::vector<OpenPolyline> &open_polylines,
                                            typename PolygonsType<mesh_info>::type &loops, double max_gap,
                                            bool try_connect_reversed)
{
    ...
    {
        const Point *operator()(const OpenPolylineEnd &pt) const
        {
            return pt.polyline->consumed ? nullptr : &pt.point();
        }
    };
    ...
    for (OpenPolyline *opl : sorted_by_length)
    {
        ...
        for (;;)
        {
            ...
            // Check whether we closed this loop.
            double current_loop_closing_distance2 =
                (opl->points.back() - opl->points.front()).cast<double>().squaredNorm();
            ...
            if (loop_closed)
            {
                ...
                else
                {
                    ...
                    if constexpr (mesh_info == AdditionalMeshInfo::Color)
                    {
                        midpoint_inserted = handle_color_at_gap_between_open_polylines<mesh_info>(*opl,
                                                                                                  opl->points.front(),
                                                                                                  opl->colors.front());
                    }
                }
    ...
    tbb::parallel_for(tbb::blocked_range<size_t>(0, lines.size()),
                      [&lines, &layers, &params, throw_on_cancel](const tbb::blocked_range<size_t> &range)
                      {
                          for (size_t line_idx = range.begin(); line_idx < range.end(); ++line_idx)
                          {
                              ...
                              auto this_mode = line_idx < params.slicing_mode_normal_below_layer ? params.mode_below
                                                                                                 : params.mode;
                              if (!polygons.empty())
                              {
    ...
    int num_edges, ThrowOnCancel throw_on_cancel)
{
    ...
    tbb::parallel_for(tbb::blocked_range<int>(0, int(lines.at_slice.size())),
                    ...
                    // Edges in between slice_below and slice_above.
                                  ...
                                  if (!in.empty())
                                  {
                                      ...
                                      coordf_t stroke_width = scale_(0.02);
                                      {
                                          ...
                                          SVG svg(debug_out_path("make_slab_loops-in-%d-%d-%s.svg", iRun, line_idx,
                                          ...
                                          for (const IntersectionLine &line : in)
                                          {
                                              const char *color = line.source == IntersectionLine::Source::BottomPlane
                                                                      ? "red"
                                                                  : line.source == IntersectionLine::Source::TopPlane
                                                                      ? "blue"
                                                                      : "green";
                                      ...
                                      std::vector<OpenPolyline> open_polylines;
                                      chain_lines_by_triangle_connectivity<AdditionalMeshInfo::None>(in, loops,
                                                                                                     open_polylines);
#ifdef SLIC3R_DEBUG_SLICE_PROCESSING
                                      {
                                          SVG svg(debug_out_path("make_slab_loops-out-%d-%d-%s.svg", iRun, line_idx,
                                                                 ProjectionFromTop ? "top" : "bottom")
                                                      .c_str(),
                                                  bbox_svg);
                                          svg.arrows = true;
                                      ...
                                      {
                                          SVG svg(debug_out_path("make_slab_loops-open-polylines-%d-%d-%s.svg", iRun,
                                                                 line_idx, ProjectionFromTop ? "top" : "bottom")
                                                      .c_str(),
                                                  bbox_svg);
                                          svg.draw(loops, "black");
                                          svg.arrows = true;
                                      ...
                                      if (!open_polylines.empty())
                                          BOOST_LOG_TRIVIAL(trace) << "make_slab_loops - chaining failed. #"
                                                                   << open_polylines.size() << " open polylines";
                                  }
                              }
                          }
                      });
...
static void make_expolygons(const Polygons &loops, const float closing_radius, const float extra_offset,
                            Clipper2Lib::FillRule fill_type, ExPolygons *slices)
{
    /*
    ...
    */
    ...
    {
        result = offset_ex(union_result, offset_out);
    }
    else if (offset_in < 0)
    {
        result = offset_ex(union_result, offset_in);
    }
    else
    {
        result = union_result;
    ...
    expolygons_append(*slices, result);
}
...
static std::vector<stl_vertex> transform_mesh_vertices_for_slicing(
    const typename IndexedTriangleSetType<mesh_info>::type &mesh, const Transform3d &trafo)
{
    ...
    const std::vector<float> &zs, const MeshSlicingParams &params, std::function<void()> throw_on_cancel)
{
...

    {
        ...
        if (zs.size() <= 1)
        {
            ...
            if (is_identity(params.trafo))
            {
                lines = slice_make_lines(
                    mesh.vertices,
                    [](const Vec3f &p) { return Vec3f(scaled<float>(p.x()), scaled<float>(p.y()), p.z()); },
            ...
            else
            {
                ...
                lines = slice_make_lines(
                    mesh.vertices, [tf](const Vec3f &p) { return tf * p; }, mesh.indices, face_edge_ids, facet_color_fn,
                    zs, throw_on_cancel);
            }
        }
        else
        {
            ...
            lines = slice_make_lines(
                transform_mesh_vertices_for_slicing<mesh_info>(mesh, params.trafo), [](const Vec3f &p) { return p; },
                mesh.indices, face_edge_ids, facet_color_fn, zs, throw_on_cancel);
        }
    }
...
std::vector<Polygons> slice_mesh(const indexed_triangle_set &mesh,
                                 // Unscaled Zs
                                 const std::vector<float> &zs, const MeshSlicingParams &params,
                                 std::function<void()> throw_on_cancel)
{
    return slice_mesh<AdditionalMeshInfo::None>(mesh, zs, params, throw_on_cancel);
}

std::vector<ColorPolygons> slice_mesh(const indexed_triangle_set_with_color &mesh,
                                      // Unscaled Zs
                                      const std::vector<float> &zs, const MeshSlicingParams &params,
                                      std::function<void()> throw_on_cancel)
{
...
template<AdditionalMeshInfo mesh_info = AdditionalMeshInfo::None>
typename PolygonsType<mesh_info>::type slice_mesh(const typename IndexedTriangleSetType<mesh_info>::type &mesh,
                                                  // Unscaled Zs
...

    {
        ...
        if (trafo_identity)
        {
            lines.emplace_back(slice_make_lines<mesh_info>(
                ...
                mesh.indices, face_edge_ids, facet_color_fn, plane_z,
        ...
        else
        {
            ...
            lines.emplace_back(slice_make_lines<mesh_info>(
                mesh.vertices, [tf](const Vec3f &p) { return tf * p; }, mesh.indices, face_edge_ids, facet_color_fn,
                plane_z, [&face_mask](int face_idx) { return face_mask[face_idx]; }));
        }
    }
...
ColorPolygons slice_mesh(const indexed_triangle_set_with_color &mesh,
                         // Unscaled Zs
                         const float plane_z, const MeshSlicingParams &params)
{
...
std::vector<ExPolygons> slice_mesh_ex(const indexed_triangle_set &mesh, const std::vector<float> &zs,
                                      const MeshSlicingParamsEx &params, std::function<void()> throw_on_cancel)
{
    ...
    tbb::parallel_for(tbb::blocked_range<size_t>(0, layers_p.size()),
                      [&layers_p, &params, &layers, throw_on_cancel](const tbb::blocked_range<size_t> &range)
                      {
                          ...
                          for (size_t layer_id = range.begin(); layer_id < range.end(); ++layer_id)
                          {
                              ...
                              const auto this_mode = layer_id < params.slicing_mode_normal_below_layer
                                                         ? params.mode_below
                                                         : params.mode;
                              Slic3r::make_expolygons(layers_p[layer_id], params.closing_radius, params.extra_offset,
                                                      this_mode == MeshSlicingParams::SlicingMode::EvenOdd
...
void slice_mesh_slabs(const indexed_triangle_set &mesh,
                      // Unscaled Zs
                      const std::vector<float> &zs, const Transform3d &trafo, std::vector<Polygons> *out_top,
                      std::vector<Polygons> *out_bottom, std::function<void()> throw_on_cancel)
{
    ...
    std::pair<SlabLines, SlabLines> lines = slice_slabs_make_lines(vertices_transformed, mesh.indices, face_neighbors,
                                                                   face_edge_ids, num_edges, face_orientation, zs,
                                                                   out_top != nullptr, out_bottom != nullptr,
                                                                   throw_on_cancel);
                              ...
                              float z, bool triangulate, bool normals_down)
{
    ...
    std::sort(map_vertex_to_index.begin(), map_vertex_to_index.end(),
              [](const std::pair<Vec2f, int> &l, const std::pair<Vec2f, int> &r)
              {
                  return l.first.x() < r.first.x() ||
                         (l.first.x() == r.first.x() &&
                          (l.first.y() < r.first.y() || (l.first.y() == r.first.y() && l.second < r.second)));
              });
    ...
    //    Remove denegerate triangles, if they happen to be created by merging duplicate vertices.
    {
        ...
        for (; i < int(map_vertex_to_index.size());)
        {
            ...
            int j = i;
            for (++j; j < int(map_vertex_to_index.size()) && ipos.x() == map_vertex_to_index[j].first.x() &&
                      ipos.y() == map_vertex_to_index[j].first.y();
                 ++j)
            {
    ...
    if (triangulate)
    {
        ...
        for (size_t i = 0; i < triangles.size();)
        {
            ...
            for (size_t j = 0; j < 3; ++j)
            {
                ...
                auto it = lower_bound_by_predicate(
                    map_vertex_to_index.begin(), map_vertex_to_index.end(), [&v](const std::pair<Vec2f, int> &l)
                    { return l.first.x() < v.x() || (l.first.x() == v.x() && l.first.y() < v.y()); });
                int idx = -1;
...
void project_mesh(const indexed_triangle_set &mesh, const Transform3d &trafo, Polygons *out_top, Polygons *out_bottom,
                  std::function<void()> throw_on_cancel)
{
              ...
              bool triangulate_caps)
{
    ...
    for (int facet_idx = 0; facet_idx < int(mesh.indices.size()); ++facet_idx)
    {
        ...
        if (z > min_z - EPSILON && z < max_z + EPSILON)
        {
            ...
            slice_type = slice_facet(double(z), vertices_scaled, mesh.indices[facet_idx], facets_edge_ids[facet_idx],
                                     idx_vertex_lowest, min_z == max_z, 0, line);
        }
        ...
        else if (min_z < z && max_z > z)
        {
            ...
            int isolated_vertex = (vertices[0].z() > z) == (vertices[1].z() > z)   ? 2
            ...
            assert(facets_edge_ids[facet_idx](iv) == line.edge_a_id ||
                   facets_edge_ids[facet_idx](iv) == line.edge_b_id);
            ...
            auto new_vertex = [upper, lower, &upper_slice_vertices, &lower_slice_vertices](const Vec3f &a, const int ia,
                                                                                           const Vec3f &b, const int ib,
                                                                                           const Vec3f &c)
            {
                int iupper, ilower;
                if (c == a)
