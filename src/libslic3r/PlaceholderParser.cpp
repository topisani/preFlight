...
static inline bool opts_equal(const DynamicConfig &config_old, const DynamicConfig &config_new,
...
struct OptWithPos
{
    ...
    OptWithPos(ConfigOptionConstPtr opt, IteratorRange it_range, bool writable = false)
        : opt(opt), it_range(it_range), writable(writable)
    {
    }
...
struct expr
{
        ...
        if (lhs.numeric_type() && rhs.numeric_type())
        {
            ...
            case '=':
                value = (lhs.type() == TYPE_DOUBLE || rhs.type() == TYPE_DOUBLE)
                            ? (std::abs(lhs.as_d() - rhs.as_d()) < 1e-8)
                            : (lhs.i() == rhs.i());
                break;
            case '<':
                value = (lhs.type() == TYPE_DOUBLE || rhs.type() == TYPE_DOUBLE) ? (lhs.as_d() < rhs.as_d())
                                                                                 : (lhs.i() < rhs.i());
                break;
            case '>':
            default:
        ...
        if (!out.b())
        {
            if (match.type() != TYPE_STRING)
                match.throw_exception(
                    "one_of(): First parameter (the string to match against) has to be a string value");
            ...
            if (RegEx)
            {
                ...
                {
                    out.set_b(
                        SLIC3R_REGEX_NAMESPACE::regex_match(match.s(), SLIC3R_REGEX_NAMESPACE::regex(pattern.s())));
                }
    ...
    static void logical_op(expr &lhs, expr &rhs, char op)
    {
        ...
        {
            boost::throw_exception(qi::expectation_failure<Iterator>(
                lhs.it_range.begin(), rhs.it_range.end(),
    ...
    {
        boost::throw_exception(qi::expectation_failure<Iterator>(this->it_range.begin(), this->it_range.end(),
                                                                 spirit::info(std::string("*") + message)));
    }
...
struct MyContext : public ConfigOptionResolver
{
    ...
    static void block_exit(const MyContext *ctx, const bool condition, bool &not_yet_consumed, std::string &data_in,
                           std::string &data_out)
    {
        if (ctx->skipping())
            --ctx->m_depth_suppressed;
            ...
            if (idx != std::string::npos)
            {
                ...
                if (opt != nullptr)
                {
                    ...
                    if (endptr == nullptr || *endptr != 0)
                        ctx->throw_exception("Invalid vector index",
                                             IteratorRange(opt_key.begin() + idx + 1, opt_key.end()));
                }
            }
        }
        if (opt == nullptr)
    ...
    static void legacy_variable_expansion2(const MyContext *ctx, IteratorRange &opt_key,
                                           IteratorRange &opt_vector_index, std::string &output)
    {
        if (ctx->skipping())
            return;
        ...
        if (vec->empty())
            ctx->throw_exception("Indexing an empty vector variable", opt_key);
        const ConfigOption *opt_index = ctx->resolve_symbol(
            std::string(opt_vector_index.begin(), opt_vector_index.end()));
    ...
    static void store_variable_index(const MyContext *ctx, OptWithPos &opt, int index, Iterator it_end,
                                     OptWithPos &output)
    {
        if (!ctx->skipping())
        {
    ...
    static void scalar_variable_to_expr(const MyContext *ctx, OptWithPos &opt, expr &output)
    {
        ...
        case coFloatOrPercent:
        {
            ...
            else
            {
                ...
                for (;;)
                {
                    const ConfigOption *opt_parent = opt_def->ratio_over.empty()
                                                         ? nullptr
                                                         : ctx->resolve_symbol(opt_def->ratio_over);
                    if (opt_parent == nullptr)
                        ctx->throw_exception(
                            "FloatOrPercent variable failed to resolve the \"ratio_over\" dependencies", opt.it_range);
                    if (boost::ends_with(opt_def->ratio_over, "extrusion_width"))
                    {
                        // Extrusion width supports defaults and a complex graph of dependencies.
                        assert(opt_parent->type() == coFloatOrPercent);
                        v *= Flow::extrusion_width(opt_def->ratio_over,
                                                   static_cast<const ConfigOptionFloatOrPercent *>(opt_parent), *ctx,
                    ...
                    if (opt_parent->type() == coFloat || opt_parent->type() == coFloatOrPercent)
                    {
                        ...
                        if (opt_parent->type() == coFloat ||
                            !static_cast<const ConfigOptionFloatOrPercent *>(opt_parent)->percent)
                            break;
    ...
    static void vector_variable_element_assign_scalar(const MyContext *ctx, OptWithPos &lhs, const expr &rhs)
    {
        ...
        ConfigOptionVectorBase *vec = const_cast<ConfigOptionVectorBase *>(
            static_cast<const ConfigOptionVectorBase *>(lhs.opt));
        if (vec->empty())
    ...
    static void vector_variable_assign_expr_with_count(const MyContext *ctx, OptWithPos &lhs, const expr &rhs_count,
    ...
    static void is_nil_test(const MyContext *ctx, OptWithPos &opt, expr &output)
    {
        ...
        else if (opt.opt->is_vector())
        {
            ...
            if (vec->empty())
                ctx->throw_exception("Indexing an empty vector variable", opt.it_range);
            output.set_b(static_cast<const ConfigOptionVectorBase *>(opt.opt)->is_nil(
                opt.index >= int(vec->size()) ? 0 : size_t(opt.index)));
        }
        else
        {
            assert(opt.opt->is_scalar());
    ...
    static void new_old_variable(const MyContext *ctx, bool global_variable, const IteratorRange &it_range,
                                 NewOldVariable &out)
    {
        if (!ctx->skipping())
        {
    ...
    static void scalar_variable_new_from_scalar_expression(const MyContext *ctx, bool global_variable,
                                                           NewOldVariable &lhs, const expr &rhs)
    {
        if (ctx->skipping())
        {
        }
    ...
    static void vector_variable_new_from_array(const MyContext *ctx, bool global_variable, NewOldVariable &lhs,
                                               const expr &rhs_count, const expr &rhs_value)
    {
        if (ctx->skipping())
        {
        }
    ...
    static void vector_variable_assign_array(const MyContext *ctx, OptWithPos &lhs, const expr &rhs_count,
                                             const expr &rhs_value)
    {
        if (!ctx->skipping())
        {
    ...
    static void fill_vector_from_initializer_list(ConfigOption *opt, const std::vector<expr> &il,
    ...
    static void vector_variable_assign_initializer_list(const MyContext *ctx, OptWithPos &lhs,
    ...
    static void vector_variable_new_from_initializer_list(const MyContext *ctx, bool global_variable,
                                                          NewOldVariable &lhs, const std::vector<expr> &il)
    {
        if (ctx->skipping())
            return;
        ...
        else
        {
            if (il.empty())
                ctx->throw_exception(
                    "Cannot create vector variable from an empty initializer list, because its type cannot be deduced.",
                    lhs.it_range);
    ...
    static void copy_vector_variable_to_vector_variable(const MyContext *ctx, OptWithPos &lhs, const OptWithPos &rhs)
    {
        ...
        if (rhs.opt->is_nil())
            ctx->throw_exception(
                "Some elements of the right hand side vector variable of optional values are undefined (nil)",
                rhs.it_range);
        if (lhs.opt->type() != rhs.opt->type())
        {
            ...
            {
            case coFloats:
                ctx->throw_exception("Left hand side is a float vector, while the right hand side is not.",
                                     lhs.it_range);
            case coInts:
                ctx->throw_exception("Left hand side is an int vector, while the right hand side is not.",
                                     lhs.it_range);
            case coStrings:
                ctx->throw_exception("Left hand side is a string vector, while the right hand side is not.",
                                     lhs.it_range);
            case coBools:
                ctx->throw_exception("Left hand side is a bool vector, while the right hand side is not.",
                                     lhs.it_range);
            default:
    ...
    static bool vector_variable_new_from_copy(const MyContext *ctx, bool global_variable, NewOldVariable &lhs,
                                              const OptWithPos &rhs)
    {
        ...
        else
        {
            ...
            if (one_of(rhs.opt->type(), {coFloats, coInts, coStrings, coBools}))
                ...
                opt_new = std::make_unique<ConfigOptionFloats>(
                    static_cast<const ConfigOptionPercents *>(rhs.opt)->values);
            else
    ...
    static void throw_exception(const std::string &msg, const IteratorRange &it_range)
    {
        ...
        boost::throw_exception(
            qi::expectation_failure(it_range.begin(), it_range.end(), spirit::info(std::string("*") + msg)));
    }

    static void process_error_message(const MyContext *context, const boost::spirit::info &info,
                                      const Iterator &it_begin, const Iterator &it_end, const Iterator &it_error)
    {
...
struct InterpolateTableContext
{
    ...
    static void evaluate(const expr &expr_x, const InterpolateTableContext &table, expr &out)
    {
        ...
        for (size_t i = 1; i < table.table.size(); ++i)
        {
            ...
            if (x0 > x1)
                boost::throw_exception(
                    qi::expectation_failure(table.table[i - 1].it_range_x.begin(), table.table[i].it_range_x.end(),
...
struct macro_processor : qi::grammar<Iterator, std::string(const MyContext *), qi::locals<bool>, skipper>
{
    macro_processor() : macro_processor::base_type(start)
    {
        ...
        if_macros = kw["then"] >
                    *(block(_r1)[_val += _1] |
                      (statement(_r1) > (+lit(';') | &(kw["elsif"] | kw["else"] | kw["endif"])))[_val += _1] |
                      +lit(';'));
        ...
        if_else_output =
            ...
            *((kw["elsif"] > bool_expr_eval(_r1)[px::bind(&MyContext::block_enter, _r1, _1 && _a)] >
            ...
            -(kw["else"] > eps[px::bind(&MyContext::block_enter, _r1, _a)] >
              (if_text_block(_r1) | else_macros(_r1)))[px::bind(&MyContext::block_exit, _r1, _a, _a, _1, _val)] >
            kw["endif"];
...
*/
        ...
        legacy_variable_expansion = (identifier >>
                                     &lit(']'))[px::bind(&MyContext::legacy_variable_expansion, _r1, _1, _val)] |
        ...
        logical_or_expression = logical_and_expression(_r1)[_val = _1] >>
                                *(((kw["or"] | "||") >
                                   logical_and_expression(_r1))[px::bind(&expr::logical_or, _val, _1)]);
        logical_or_expression.name("logical_or_expression");

        logical_and_expression = equality_expression(_r1)[_val = _1] >>
                                 *(((kw["and"] | "&&") >
                                    equality_expression(_r1))[px::bind(&expr::logical_and, _val, _1)]);
        ...
        additive_expression = multiplicative_expression(_r1)[_val = _1] >>
        ...
        assignment_statement =
            ...
            ( // Consumes also '(' conditional_expression ')', that means enclosing an expression into braces makes it a single value vector initializer.
                ...
                | conditional_expression(
                      _r1)[px::bind(&MyContext::scalar_variable_assign_scalar_expression, _r1, _a, _1)] |
        ...
        new_variable_statement =
            (kw["local"][_a = false] | kw["global"][_a = true]) >
            identifier[px::bind(&MyContext::new_old_variable, _r1, _a, _1, _b)] > lit('=') >
            ( // Consumes also '(' conditional_expression ')', that means enclosing an expression into braces makes it a single value vector initializer.
                ...
                eps(px::bind(&MyContext::could_be_vector_variable_reference, _b)) >>
                    variable_reference(
                        _r1)[px::ref(qi::_pass) = px::bind(&MyContext::vector_variable_new_from_copy, _r1, _a, _b, _1)]
                // Would NOT consume '(' conditional_expression ')' because such value was consumed with the expression above.
                | conditional_expression(
                      _r1)[px::bind(&MyContext::scalar_variable_new_from_scalar_expression, _r1, _a, _b, _1)] |
                (kw["repeat"] > "(" > additive_expression(_r1) > "," > conditional_expression(_r1) >
        ...
        unary_expression =
            iter_pos[px::bind(&FactorActions::set_start_pos, _1, _val)] >>
        ...
        interpolate_table = (unary_expression(_r1)[_a = _1] > ',' >
                             interpolate_table_list(_r1,
                                                    _a))[px::bind(&InterpolateTableContext::evaluate, _a, _2, _val)];
        ...
        keywords.add("and")("digits")("zdigits")("empty")("if")("int")("is_nil")("local")
            //("inf")
            ("else")("elsif")("endif")("false")("global")("interpolate_table")("min")("max")("random")("repeat")(
                "round")("not")("one_of")("or")("size")("true");
    ...
    qi::rule<Iterator, std::string(const MyContext *), skipper> block, statement, macros, if_text_block, if_macros,
        else_macros;
    // Legacy variable expansion of the original Slic3r, in the form of [scalar_variable] or [vector_variable_index].
    ...
    qi::rule<Iterator, std::string(const MyContext *), qi::locals<bool, MyContext::NewOldVariable>, skipper>
        new_variable_statement;
