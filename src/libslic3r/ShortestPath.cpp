                                                                                  ...
                                                                                  const PointType *start_near)
{
    ...
    else
    {
        ...
        auto coordinate_fn = [&end_points](size_t idx, size_t dimension) -> double
        {
            return end_points[idx].pos[dimension];
        };
        ...
        {
            size_t idx = find_closest_point(
                kdtree, start_near->template cast<double>(),
        ...
        for (int iter = int(num_segments) - 2;; --iter)
        {
            ...
            else
            {
                ...
                end_point1_other_chain_id = equivalent_chain(
                    end_points[(&end_point1 - &end_points.front()) ^ 1].chain_id);
                end_point2_other_chain_id = equivalent_chain(
                    end_points[(&end_point2 - &end_points.front()) ^ 1].chain_id);
            ...
            if (valid)
            {
                ...
                size_t chain_id = (end_point1_other_chain_id == 0)
                                      ? ((end_point2_other_chain_id == 0) ? equivalent_chain.next()
                                      ...
                                      : ((end_point2_other_chain_id == 0) ? end_point1_other_chain_id
                                         : (end_point1_other_chain_id == end_point2_other_chain_id)
                                             ...
                                             : equivalent_chain.merge(end_point1_other_chain_id,
                                                                      end_point2_other_chain_id));
            ...
            else
            {
                ...
                size_t next_idx = find_closest_point(kdtree, end_point1.pos,
                                                     [&end_points, &equivalent_chain, this_idx](size_t idx)
                                                     {
                                                         ...
                                                         if ((idx ^ this_idx) <= 1 || end_points[idx].chain_id != 0)
                                                             ...
                                                             return false;
                                                         size_t chain1 = equivalent_chain(
                                                             end_points[this_idx ^ 1].chain_id);
        ...
        if (REVERSE_COULD_FAIL)
        {
            ...
            if (failed)
                ...
                out = chain_segments_closest_point<EndPoint, decltype(kdtree), CouldReverseFunc>(
                    end_points, kdtree, could_reverse_func,
                    (initial_point != nullptr) ? *initial_point : end_points.front());
        }
        else
        {
            assert(!failed);
...
void update_end_point_in_queue(QueueType &queue, const KDTreeType &kdtree, ChainsType &chains,
                               std::vector<EndPointType> &end_points, EndPointType &end_point, size_t first_point_idx,
                               const EndPointType *first_point)
{
    ...
    else
    {
        ...
        // Find the closest point to this end_point, which lies on a different extrusion path (filtered by the filter lambda).
        size_t next_idx = find_closest_point(
            kdtree, end_point.pos,
        ...
        assert(next_idx < end_points.size());
        assert(chains.equivalent(end_points[next_idx].chain_id) !=
                   chains.equivalent(end_points[next_idx ^ 1].chain_id) ||
                                                                                   ...
                                                                                   const PointType *start_near)
{
    ...
    else
    {
        ...
        struct EndPoint
        {
            ...
            const EndPoint &opposite(const std::vector<EndPoint> &endpoints) const
            {
                return endpoints[(this - endpoints.data()) ^ 1];
            }
        };
        ...
        for (size_t num_connections_to_end = num_segments - 1; num_iter > 0; --num_iter)
        {
            ...
            if (valid)
            {
                ...
                assert(chain1 == nullptr ||
                       (chain1_flip ? (chain1->begin == end_point1_other || chain1->end == end_point1_other)
                                    : (chain1->begin == end_point1 || chain1->end == end_point1)));
                assert(chain2 == nullptr ||
                       (chain2_flip ? (chain2->begin == end_point2_other || chain2->end == end_point2_other)
                ...
                {
                    Chain chain_dst;
                    chain_dst.begin = (chain1 == nullptr)             ? end_point1_other
                                      : (chain1->begin == end_point1) ? chain1->end
                                                                      : chain1->begin;
                    chain_dst.end = (chain2 == nullptr)             ? end_point2_other
                                    : (chain2->begin == end_point2) ? chain2->end
                                                                    : chain2->begin;
                    chain_dst.cost = (chain1 == 0 ? 0. : chain1->cost) + (chain2 == 0 ? 0. : chain2->cost) +
                                     (end_point2->pos - end_point1->pos).norm();
                    chain_dst.cost_flipped = (chain1 == 0 ? 0. : chain1->cost_flipped) +
                                             (chain2 == 0 ? 0. : chain2->cost_flipped) +
                                             (end_point2_other->pos - end_point1_other->pos).norm();
                    chain_dst.num_segments = (chain1 == 0 ? 1 : chain1->num_segments) +
                                             (chain2 == 0 ? 1 : chain2->num_segments);
                ...
                else
                {
                    ...
                    update_end_point_in_queue(queue, kdtree, chains, end_points, chain.begin->opposite(end_points),
                                              first_point_idx, first_point);
                    update_end_point_in_queue(queue, kdtree, chains, end_points, chain.end->opposite(end_points),
                                              first_point_idx, first_point);
                    if (chain1_flip)
                        update_end_point_in_queue(queue, kdtree, chains, end_points, *chain.begin, first_point_idx,
                                                  first_point);
                    if (chain2_flip)
                        update_end_point_in_queue(queue, kdtree, chains, end_points, *chain.end, first_point_idx,
                                                  first_point);
...
std::vector<std::pair<size_t, bool>> chain_segments_greedy_constrained_reversals(SegmentEndPointFunc end_point_func,
                                                                                 ...
                                                                                 const PointType *start_near)
{
    return chain_segments_greedy_constrained_reversals_<PointType, SegmentEndPointFunc, true, CouldReverseFunc>(
        end_point_func, could_reverse_func, num_segments, start_near);
}

template<typename PointType, typename SegmentEndPointFunc>
std::vector<std::pair<size_t, bool>> chain_segments_greedy(SegmentEndPointFunc end_point_func, size_t num_segments,
                                                           const PointType *start_near)
{
    auto could_reverse_func = [](size_t /* idx */) -> bool
    {
        return true;
    };
    return chain_segments_greedy_constrained_reversals_<PointType, SegmentEndPointFunc, false,
                                                        decltype(could_reverse_func)>(end_point_func,
                                                                                      could_reverse_func, num_segments,
                                                                                      start_near);
}
...
std::vector<std::pair<size_t, bool>> chain_segments_greedy_constrained_reversals2(SegmentEndPointFunc end_point_func,
                                                                                  ...
                                                                                  const PointType *start_near)
{
    return chain_segments_greedy_constrained_reversals2_<PointType, SegmentEndPointFunc, true, CouldReverseFunc>(
        end_point_func, could_reverse_func, num_segments, start_near);
}

template<typename PointType, typename SegmentEndPointFunc>
std::vector<std::pair<size_t, bool>> chain_segments_greedy2(SegmentEndPointFunc end_point_func, size_t num_segments,
                                                            const PointType *start_near)
{
    auto could_reverse_func = [](size_t /* idx */) -> bool
    {
...
{
    auto segment_end_point = [&entities, reversed](size_t idx, bool first_point) -> const Point &
    {
        return first_point == reversed ? entities[idx]->last_point() : entities[idx]->first_point();
    };
    ...
    std::vector<std::pair<size_t, bool>> out =
        chain_segments_greedy_constrained_reversals<Point, decltype(segment_end_point), decltype(could_reverse)>(
...
void reorder_extrusion_entities(std::vector<ExtrusionEntity *> &entities,
...
ExtrusionEntityReferences chain_extrusion_references(const std::vector<ExtrusionEntity *> &entities,
                                                     const Point *start_near, const bool reversed)
{
...
ExtrusionEntityReferences chain_extrusion_references(const ExtrusionEntityCollection &eec, const Point *start_near,
                                                     const bool reversed)
{
    if (eec.no_sort)
    {
...
std::vector<std::pair<size_t, bool>> chain_extrusion_paths(std::vector<ExtrusionPath> &extrusion_paths,
                                                           const Point *start_near)
{
    ...
    return chain_segments_greedy<Point, decltype(segment_end_point)>(segment_end_point, extrusion_paths.size(),
                                                                     start_near);
}

void reorder_extrusion_paths(std::vector<ExtrusionPath> &extrusion_paths,
                             const std::vector<std::pair<size_t, bool>> &chain)
{
...
std::vector<size_t> chain_points(const Points &points, const Point *start_near)
{
    ...
    std::vector<std::pair<size_t, bool>> ordered =
        chain_segments_greedy<Point, decltype(segment_end_point)>(segment_end_point, points.size(), start_near);
...
static inline std::pair<double, size_t> minimum_crossover_cost(
    const std::vector<FlipEdge> &edges, const std::pair<size_t, size_t> &span1, const ConnectionCost &cost1,
    const std::pair<size_t, size_t> &span2, const ConnectionCost &cost2, const std::pair<size_t, size_t> &span3,
    const ConnectionCost &cost3, const double cost_current)
{
    auto connection_cost = [&edges](const std::pair<size_t, size_t> &span1, const ConnectionCost &cost1, bool reversed1,
                                    bool flipped1, const std::pair<size_t, size_t> &span2, const ConnectionCost &cost2,
                                    bool reversed2, bool flipped2, const std::pair<size_t, size_t> &span3,
                                    const ConnectionCost &cost3, bool reversed3, bool flipped3)
    {
        ...
        return simple_span_ignore(span1, reversed1) || simple_span_ignore(span2, reversed2) ||
    ...
    for (size_t i = 0; i < (1 << 6); ++i)
    {
        ...
        double c1 = (i == 0) ? cost_current
                             : connection_cost(span1, cost1, (i & 1) != 0, (i & (1 << 1)) != 0, span2, cost2,
                                               (i & (1 << 2)) != 0, (i & (1 << 3)) != 0, span3, cost3,
                                               (i & (1 << 4)) != 0, (i & (1 << 5)) != 0);
        double c2 = connection_cost(span1, cost1, (i & 1) != 0, (i & (1 << 1)) != 0, span3, cost3, (i & (1 << 2)) != 0,
                                    (i & (1 << 3)) != 0, span2, cost2, (i & (1 << 4)) != 0, (i & (1 << 5)) != 0);
        double c3 = connection_cost(span2, cost2, (i & 1) != 0, (i & (1 << 1)) != 0, span1, cost1, (i & (1 << 2)) != 0,
...
static inline void do_crossover(const std::vector<FlipEdge> &edges_in, std::vector<FlipEdge> &edges_out,
                                const std::pair<size_t, size_t> &span1, const std::pair<size_t, size_t> &span2,
                                const std::pair<size_t, size_t> &span3, size_t i)
{
    ...
    {
    case 0:
        do_it(span1, (i & 1) != 0, (i & (1 << 1)) != 0, span2, (i & (1 << 2)) != 0, (i & (1 << 3)) != 0, span3,
              (i & (1 << 4)) != 0, (i & (1 << 5)) != 0);
        break;
    case 1:
        do_it(span1, (i & 1) != 0, (i & (1 << 1)) != 0, span3, (i & (1 << 2)) != 0, (i & (1 << 3)) != 0, span2,
              (i & (1 << 4)) != 0, (i & (1 << 5)) != 0);
        break;
    default:
        assert((i >> 6) == 2);
        do_it(span2, (i & 1) != 0, (i & (1 << 1)) != 0, span1, (i & (1 << 2)) != 0, (i & (1 << 3)) != 0, span3,
              (i & (1 << 4)) != 0, (i & (1 << 5)) != 0);
    }
    assert(edges_in.size() == edges_out.size());
}
...
static inline void reorder_by_two_exchanges_with_segment_flipping(std::vector<FlipEdge> &edges)
{
    ...
    for (size_t iter = 0; iter < max_iterations; ++iter)
    {
        ...
        std::sort(connection_lengths.begin(), connection_lengths.end(),
                  [](const std::pair<double, size_t> &l, const std::pair<double, size_t> &r)
        ...
        for (const std::pair<double, size_t> &first_crossover_candidate : connection_lengths)
        {
            ...
            for (size_t j = 1; j < connections.size(); ++j)
                if (!connection_tried[j])
                {
                    ...
                    if (a > b)
                        std::swap(a, b);
                    std::pair<double, size_t> cost_and_flip = minimum_crossover_cost(
                        edges, std::make_pair(size_t(0), a), connections[a - 1], std::make_pair(a, b),
                        connections[b - 1] - connections[a], std::make_pair(b, edges.size()),
                        connections.back() - connections[b], connections.back().cost);
        ...
        if (crossover_flip_final > 0)
        {
            ...
            do_crossover(edges, edges_tmp, std::make_pair(size_t(0), crossover1_pos_final),
                         std::make_pair(crossover1_pos_final, crossover2_pos_final),
                         std::make_pair(crossover2_pos_final, edges.size()), crossover_flip_final);
...
public:
    ...
    {
        return flipped ? costs[piece_idx]->cost_flipped : costs[piece_idx]->cost;
    }
...
static inline void improve_ordering_by_two_exchanges_with_segment_flipping(Polylines &polylines, bool fixed_start)
{
    ...
    std::transform(
        polylines.begin(), polylines.end(), std::back_inserter(edges), [&polylines](const Polyline &pl)
...
static inline T chain_path_items(const Points &points, const T &items)
{
    ...
    std::vector<std::pair<size_t, bool>> ordered =
        chain_segments_greedy<Point, decltype(segment_end_point)>(segment_end_point, points.size(), nullptr);
    T out;
...
PolyNodes chain_clipper_polynodes(const Points &points, const PolyNodes &items)
{
...
std::vector<const PrintInstance *> chain_print_object_instances(const Print &print)
{
    ...
    std::vector<std::pair<size_t, bool>> ordered =
        chain_segments_greedy<Point, decltype(segment_end_point)>(segment_end_point, instances.size(), nullptr);
        ...
        if (!line_consumed[&seed - lines.data()])
        {
            ...
            for (size_t round = 0; round < 2; ++round)
            {
                for (;;)
                {
                    ...
                    pl.points.back() =
                        (0.5 * (pl.points.back().cast<double>() + line_end->point().cast<double>())).cast<coord_t>();
...
std::vector<size_t> chain_layer_islands(const std::vector<std::reference_wrapper<const LayerIsland>> &islands,
                                        const Point *start_near)
{
    Points ordering_points;
...
void reorder_layer_islands(std::vector<std::reference_wrapper<const LayerIsland>> &islands,
...
void chain_and_reorder_layer_islands(std::vector<std::reference_wrapper<const LayerIsland>> &islands,
                                     const Point *start_near)
{
