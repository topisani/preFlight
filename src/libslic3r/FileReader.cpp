...
static Model read_model_from_file(const std::string &input_file, LoadAttributes options,
                                  const std::optional<std::pair<double, double>> &step_deflections = std::nullopt,
                                  LoadStats *stats = nullptr)
{
    ...
    {
        std::string step_warning;
        result = load_step(input_file.c_str(), &model, step_deflections, &step_warning);
        if (stats && !step_warning.empty())
            stats->step_import_warning = std::move(step_warning);
    }
    ...
    else if (boost::algorithm::iends_with(input_file, ".3mf") || boost::algorithm::iends_with(input_file, ".zip"))
    {
        ...
        result = load_3mf(input_file.c_str(), temp_config, temp_config_substitutions_context, &model, false,
                          generator_version);
    }
...
static bool looks_like_multipart_object(const Model &model)
{
    ...
    for (const ModelObject *obj : model.objects)
    {
        ...
        BoundingBoxf3 tbb_this = (!obj->instances.empty() ? obj->instances[0]->transform_bounding_box(bb_this)
                                                          : bb_this);
    ...
    for (ModelObject *obj : model.objects)
        if (ModelProcessing::get_object_mesh_stats(obj).volume < ModelProcessing::volume_threshold_inches)
        {
            ...
            for (ModelObject *obj_other : model.objects)
            {
                if (obj_other == obj)
                    continue;
                if (obj_other->cut_id.is_equal(obj->cut_id) &&
                    ModelProcessing::get_object_mesh_stats(obj_other).volume >=
                        ModelProcessing::volume_threshold_inches)
                {
...
Model load_model(const std::string &input_file, LoadAttributes options /* = LoadAttribute::AddDefaultInstances*/,
                 LoadStats *stats /*= nullptr*/,
                 std::optional<std::pair<double, double>> step_deflections /* = std::nullopt*/)
{
    Model model = read_model_from_file(input_file, options, step_deflections, stats);
    ...
    if (stats)
    {
        ...
        bool from_3mf = boost::algorithm::iends_with(input_file, ".3mf") ||
...
Model load_model_with_config(const std::string &input_file, DynamicPrintConfig *config,
                             ConfigSubstitutionContext *config_substitutions,
