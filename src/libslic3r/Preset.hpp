...
public:
    ...
    struct PrinterModel
    {
        ...
        const PrinterVariant *variant(const std::string &name) const
        {
            return const_cast<PrinterModel *>(this)->variant(name);
        }
    };
    ...
    static VendorProfile from_ini(const boost::property_tree::ptree &tree, const boost::filesystem::path &path,
                                  bool load_all = true);
...
public:
    ...
    {
        return cfg.option<ConfigOptionString>("inherits", true)->value;
    }
    ...
    {
        return const_cast<Preset *>(this)->compatible_prints_condition();
    }
    ...
    {
        assert(this->type == TYPE_PRINT || this->type == TYPE_SLA_PRINT || this->type == TYPE_FILAMENT ||
               this->type == TYPE_SLA_MATERIAL);
        return Preset::compatible_printers_condition(this->config);
    }
    const std::string &compatible_printers_condition() const
    {
        return const_cast<Preset *>(this)->compatible_printers_condition();
    }
...
struct ExternalPreset
{
    ...
    ExternalPreset(Preset *preset, bool modified, bool installed = false)
        : preset(preset), is_modified(modified), is_installed(installed)
    {
    }
...
// preFlight and reading the user Print / Filament / Printer profiles.
...
public:
    ...
    PresetCollection(Preset::Type type, const std::vector<std::string> &keys, const Slic3r::StaticPrintConfig &defaults,
    ...
    void add_default_preset(const std::vector<std::string> &keys, const Slic3r::StaticPrintConfig &defaults,
                            const std::string &preset_name);

    // Load ini files of the particular type from the provided directory path.
    void load_presets(const std::string &dir_path, const std::string &subdir, PresetsConfigSubstitutions &substitutions,
                      ForwardCompatibilitySubstitutionRule rule);

    // Load a preset from an already parsed config file, insert it into the sorted sequence of presets
    // and select it, losing previous modifications.
    Preset &load_preset(const std::string &path, const std::string &name, const DynamicPrintConfig &config,
                        bool select = true);
    Preset &load_preset(const std::string &path, const std::string &name, DynamicPrintConfig &&config,
                        bool select = true);
    ...
    {
        return (m_idx_selected == size_t(-1)) ? std::string() : this->get_selected_preset().name;
    }
    ...
    virtual const Preset &default_preset_for(const DynamicPrintConfig & /* config */) const
    {
    ...
    {
        return (idx == m_idx_selected && respect_active_preset) ? m_edited_preset : m_presets[idx];
    }
    const Preset &preset(size_t idx, bool respect_active_preset = true) const
    {
        return const_cast<PresetCollection *>(this)->preset(idx);
    }
    ...
    const Preset *find_preset(const std::string &name, bool first_visible_if_not_found = false,
                              bool respect_active_preset = true) const
    {
        return const_cast<PresetCollection *>(this)->find_preset(name, first_visible_if_not_found,
                                                                 respect_active_preset);
    }
    ...
    template<typename PreferedCondition>
    Preset &first_compatible(PreferedCondition prefered_condition)
    {
    ...
    template<typename PreferedCondition>
    void update_compatible(const PresetWithVendorProfile &active_printer, const PresetWithVendorProfile *active_print,
    ...
    void update_compatible(const PresetWithVendorProfile &active_printer, const PresetWithVendorProfile *active_print,
                           PresetSelectCompatibleType select_other_if_incompatible)
    {
        this->update_compatible(active_printer, active_print, select_other_if_incompatible,
                                [](const Preset &) -> int { return 0; });
    }

    size_t num_visible() const
    {
        return std::count_if(m_presets.begin(), m_presets.end(),
                             [](const Preset &preset) { return preset.is_visible; });
    }
...
private:
    ...
    std::deque<Preset>::iterator find_preset_renamed(const std::string &name)
    {
        ...
        auto it = (it_renamed == m_map_system_profile_renamed.end()) ? m_presets.end()
                                                                     : this->find_preset_internal(it_renamed->second);
        assert((it_renamed == m_map_system_profile_renamed.end()) ||
               (it != m_presets.end() && it->name == it_renamed->second));
        return it;
    }
...
{
public:
    PrinterPresetCollection(Preset::Type type, const std::vector<std::string> &keys,
                            const Slic3r::StaticPrintConfig &defaults, const std::string &default_name = "- default -")
    ...
    const Preset *find_system_preset_by_model_and_variant(const std::string &model_id,
...
public:
    ...
    void load_printers(const std::string &dir_path, const std::string &subdir,
                       PresetsConfigSubstitutions &substitutions, ForwardCompatibilitySubstitutionRule rule);
    void load_printers_from_presets(PrinterPresetCollection &printer_presets);
    // Load printer from the loaded configuration
    void load_printer(const std::string &path, const std::string &name, DynamicPrintConfig &&config, bool select,
                      bool save = false);
    ...
    {
        return (m_idx_selected == size_t(-1)) ? PrinterTechnology::ptAny
                                              : this->get_selected_printer().printer_technology();
    }
    ...
    {
        return (m_idx_selected == size_t(-1)) ? std::string() : m_selected_preset;
    }
    ...
    const PhysicalPrinter &printer(size_t idx) const
    {
        return const_cast<PhysicalPrinterCollection *>(this)->printer(idx);
    }
...
private:
    ...
    std::deque<PhysicalPrinter>::const_iterator find_printer_internal(const std::string &name,
                                                                      bool case_sensitive_search = true) const
    {
...
public:
    ExtruderFilaments(PresetCollection *filaments_collection, size_t extruder_id = 0,
    ...
    std::string get_selected_preset_name() const
    {
        return m_idx_selected == size_t(-1) ? std::string() : m_extr_filaments[m_idx_selected].preset->name;
    }
    const Preset *get_selected_preset() const
    {
        return m_idx_selected == size_t(-1) ? nullptr : m_extr_filaments[m_idx_selected].preset;
    }
    const Filament *get_selected_filament() const
    {
        return m_idx_selected == size_t(-1) ? nullptr : &m_extr_filaments[m_idx_selected];
    }
    size_t get_selected_idx() const { return m_idx_selected; }
...
private:
    ...
    {
        return Slic3r::lower_bound_by_predicate(m_extr_filaments.begin(), m_extr_filaments.end(),
                                                [&name](const auto &l) { return l.preset->name < name; });
    }
    ...
    size_t first_compatible_idx() const
    {
        return this->first_compatible_idx([](const /*Filament*/ Preset &) -> int { return 0; });
    }

    template<typename PreferedCondition>
    const Preset *first_compatible(PreferedCondition prefered_condition)
    {
        return m_extr_filaments[this->first_compatible_idx(prefered_condition)].preset;
    }
    ...
    {
        this->update_compatible(active_printer, active_print, select_other_if_incompatible,
                                [](const /*Filament*/ Preset &) -> int { return 0; });
    }
};
