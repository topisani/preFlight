...
private:
    ...
    {
        cereal::load_by_value(ar, wrapped);
    }
    template<class Archive>
    void save(Archive &ar) const
    {
        cereal::save_by_value(ar, wrapped);
    }
    T &wrapped;
};
} // namespace Internal
...
public:
    ...
    {
        return id() == rhs.id() && check_sum() == rhs.check_sum() && connectors_cnt() == rhs.connectors_cnt();
    }
...
struct CutConnector
{
    ...
    CutConnector(Vec3d p, Transform3d rot, float r, float h, float rt, float ht, float za,
    ...
    CutConnector(const CutConnector &rhs)
        : CutConnector(rhs.pos, rhs.rotation_m, rhs.radius, rhs.height, rhs.radius_tolerance, rhs.height_tolerance,
                       rhs.z_angle, rhs.attribs)
    {
    }
...
public:
    ...
    struct Source
    {
        ...
        void serialize(Archive &ar)
        {
            ...
            ar(input_file, object_idx, volume_idx, mesh_offset, transform, is_converted_from_inches,
               is_converted_from_meters, is_from_builtin_objects);
        }
    };
    Source source;
    ...
    {
        return m_type == ModelVolumeType::SUPPORT_BLOCKER || m_type == ModelVolumeType::SUPPORT_ENFORCER;
    }
    ...
    {
        this->scale_geometry_after_creation(Vec3f(scale, scale, scale));
    }
...
private:
    ...
    ModelVolume(ModelObject *object, TriangleMesh &&mesh, TriangleMesh &&convex_hull,
        ...
        , m_convex_hull(new TriangleMesh(std::move(convex_hull)))
        , m_type(type)
        , object(object)
    {
        assert(check());
...
{
    std::sort(model_volumes.begin(), model_volumes.end(),
              [](const ModelVolume *l, const ModelVolume *r) { return l->id() < r->id(); });
}

inline const ModelVolume *model_volume_find_by_id(const ModelVolumePtrs &model_volumes, const ObjectID id)
{
    auto it = lower_bound_by_predicate(model_volumes.begin(), model_volumes.end(),
                                       [id](const ModelVolume *mv) { return mv->id() < id; });
...
private:
    ...
    explicit ModelInstance(ModelObject *object) : print_volume_state(ModelInstancePVS_Inside), object(object)
    {
        assert(this->id().valid());
    }
    // Constructor, which assigns a new unique ID.
...
public:
    ...
    ModelWipeTower &operator=(const ModelWipeTower &rhs)
    {
...
bool model_volume_list_changed(const ModelObject &model_object_old, const ModelObject &model_object_new,
