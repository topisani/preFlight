...
public:
    SupportGridFilter(const PrintObject *po, float voxel_size)
    {
        ...
        origin = min;
        size = max - min;
        cell_count = size.cwiseQuotient(cell_size).cast<coord_t>() + Vec3i::Ones();
    }

    Vec3i to_cell_coords(const Vec3f &position) const
    {
        Vec3i cell_coords = (position - this->origin).cwiseQuotient(this->cell_size).cast<coord_t>();
        return cell_coords;
    }
    ...
    Vec3f get_cell_center(const Vec3i &cell_coords) const
    {
        return origin + cell_coords.cast<float>().cwiseProduct(this->cell_size) +
               this->cell_size.cwiseQuotient(Vec3f(2.0f, 2.0f, 2.0f));
    }
...
SliceConnection estimate_slice_connection(size_t slice_idx, const Layer *layer)
{
    ...
    ExPolygons below{};
    for (const auto &linked_slice_idx_below : linked_slices_below)
    {
...
PrecomputedSliceConnections precompute_slices_connections(const PrintObject *po)
{
    ...
    tbb::parallel_for(tbb::blocked_range<size_t>(0, po->layers().size()),
                      [po, &result](tbb::blocked_range<size_t> r)
                      {
                          for (size_t lidx = r.begin(); lidx < r.end(); lidx++)
                          {
                              ...
                              if (!l)
                                  continue;
                              tbb::parallel_for(tbb::blocked_range<size_t>(0, l->lslices_ex.size()),
                                                [lidx, l, &result](tbb::blocked_range<size_t> r2)
                                                {
                                                    for (size_t slice_idx = r2.begin(); slice_idx < r2.end();
                                                         slice_idx++)
                                                    {
                                                        result[lidx][slice_idx] = estimate_slice_connection(slice_idx,
                                                                                                            l);
                                                    }
                                                });
                          }
...
float estimate_curled_up_height(float distance, float curvature, float layer_height, float flow_width,
                                float prev_line_curled_height, Params params)
{
...
std::vector<ExtrusionLine> check_extrusion_entity_stability(
    const ExtrusionEntity *entity, const LayerRegion *layer_region, const LD &prev_layer_lines,
    const AABBTreeLines::LinesDistancer<Linef> &prev_layer_boundary, const Params &params)
{
    ...
    if (entity->role().is_bridge() && !entity->role().is_perimeter())
    {
        ...
        std::vector<ExtrusionProcessor::ExtendedPoint> annotated_points =
            ExtrusionProcessor::estimate_points_properties<true>(entity->as_polyline().points, prev_layer_boundary,
                                                                 config);
        ...
        for (size_t i = 0; i < annotated_points.size(); ++i)
        {
            ...
            SupportPointCause potential_cause = std::abs(curr_point.curvature) > 0.1
                                                    ? SupportPointCause::FloatingBridgeAnchor
                                                    : SupportPointCause::LongBridge;
            float line_len = (prev_point.position - curr_point.position).norm();
            Vec2d line_dir = line_len > EPSILON ? Vec2d((curr_point.position - prev_point.position) / double(line_len))
                                                : Vec2d::Zero();

            ExtrusionLine line_out{prev_point.position.cast<float>(), curr_point.position.cast<float>(), line_len,
                                   entity};
            ...
            if (!bridging_dir.has_value() && curr_point.distance > flow_width &&
                line_len > params.bridge_distance * 0.6)
            {
                bridging_dir = line_dir;
            }

            if (curr_point.distance > flow_width && potential_cause == SupportPointCause::LongBridge &&
                bridging_dir.has_value() && bridging_dir->dot(line_dir) < 0.8)
            { // skip backward direction of bridge - supported by forward points enough
    ...
    { // single extrusion path, with possible varying parameters
        if (entity->length() < scale_(params.min_distance_to_allow_local_supports))
        {
            return {};
        }
        ...
        config.flow_width = flow_width;
        std::vector<ExtrusionProcessor::ExtendedPoint> annotated_points =
            ExtrusionProcessor::estimate_points_properties<false>(entity->as_polyline().points, prev_layer_lines,
                                                                  config);

        std::vector<ExtrusionLine> lines_out;
        lines_out.reserve(annotated_points.size());
        float bridged_distance = annotated_points.front().position != annotated_points.back().position
                                     ? (params.bridge_distance + 1.0f)
                                     : 0.0f;
        for (size_t i = 0; i < annotated_points.size(); ++i)
        {
            ExtrusionProcessor::ExtendedPoint &curr_point = annotated_points[i];
            ...
            float sign = (prev_layer_boundary.distance_from_lines<true>(curr_point.position) + 0.5f * flow_width) < 0.0f
                             ? -1.0f
                             : 1.0f;
            ...
            line_out.curled_up_height = estimate_curled_up_height(middle_distance,
                                                                  0.5 * (prev_point.curvature + curr_point.curvature),
                                                                  layer_region->layer()->height, flow_width,
...
ObjectPart::ObjectPart(const std::vector<const ExtrusionEntityCollection *> &extrusion_collections,
                       const bool connected_to_bed, const coordf_t print_head_z, const coordf_t layer_height,
                       const std::optional<Polygons> &brim)
{
    ...
    for (const ExtrusionEntityCollection *collection : extrusion_collections)
    {
        ...
        for (const ExtrusionEntity *entity : collection->flatten())
        {
            ...
            else if (const auto *loop = dynamic_cast<const ExtrusionLoop *>(entity); loop != nullptr)
            {
            ...
            else if (const auto *multi_path = dynamic_cast<const ExtrusionMultiPath *>(entity); multi_path != nullptr)
            {
            ...
            this->volume += volume;
            this->volume_centroid_accumulator += to_3d(integrals.x_i, center_z * integrals.area) / integrals.area *
                                                 volume;
...
float ObjectPart::compute_elastic_section_modulus(const Vec2f &line_dir, const Vec3f &extreme_point,
                                                  const Integrals &integrals) const
{
    float second_moment_of_area = compute_second_moment(integrals, Vec2f{-line_dir.y(), line_dir.x()});

    if (second_moment_of_area < EPSILON)
    {
        return 0.0f;
    }

    Vec2f centroid = integrals.x_i / integrals.area;
    float extreme_fiber_dist =
        line_alg::distance_to(Linef(centroid.head<2>().cast<double>(),
...
std::tuple<float, SupportPointCause> ObjectPart::is_stable_while_extruding(const SliceConnection &connection,
                                                                           ...
                                                                           const Vec3f &extreme_point, float layer_z,
                                                                           const Params &params) const
{
    ...
    // section for bed calculations
    {
        ...
        float bed_yield_torque = -compute_elastic_section_modulus(line_dir, extreme_point, integrals) *
        ...
        float bed_weight_dir_xy_variance = compute_second_moment(integrals, {-bed_weight_arm.y(), bed_weight_arm.x()}) /
                                           this->sticking_area;
...
#ifdef DETAILED_DEBUG_LOGS
        BOOST_LOG_TRIVIAL(debug) << "bed_centroid: " << bed_centroid.x() << "  " << bed_centroid.y() << "  "
                                 << bed_centroid.z();
        ...
        if (bed_total_torque > 0)
        {
            return {bed_total_torque / bed_conflict_torque_arm,
                    (this->connected_to_bed ? SupportPointCause::SeparationFromBed
                                            : SupportPointCause::UnstableFloatingPart)};
        }
    }
    ...
    // section for weak connection calculations
    {
        ...
        {
            return {-1.0f, SupportPointCause::WeakObjectPart};
        }
        ...
        float conn_yield_torque = compute_elastic_section_modulus(line_dir, extreme_point, integrals) *
        ...
        {
            conn_weight_arm =
                0.0f; // Given that we do not have very good info about the weight distribution between the connection and current layer,
...
#ifdef DETAILED_DEBUG_LOGS
        BOOST_LOG_TRIVIAL(debug) << "conn_centroid: " << conn_centroid.x() << "  " << conn_centroid.y() << "  "
                                 << conn_centroid.z();
...
Polygons get_brim(const ExPolygon &slice_polygon, const BrimType brim_type, const float brim_width)
{
    ...
    if (brim_type == BrimType::btOuterAndInner || brim_type == BrimType::btOuterOnly)
    {
        ...
        brim_hole.reverse();
        Polygons c = expand(
            slice_polygon.contour,
            scale_(
                brim_width)); // For very small polygons, the expand may result in empty vector, even thought the input is correct.
        if (!c.empty())
        {
...
void reckon_new_support_point(ObjectPart &part, SliceConnection &weakest_conn, SupportPoints &supp_points,
                              SupportGridFilter &supports_presence_grid, const SupportPoint &support_point,
                              bool is_global = false)
{
    ...
    if (weakest_conn.area > EPSILON)
    { // Do not add it to the weakest connection if it is not valid - does not exist
        weakest_conn.area += area;
        weakest_conn.centroid_accumulator += support_point.position * area;
        weakest_conn.second_moment_of_area_accumulator += area * support_point.position.head<2>().cwiseProduct(
                                                                     support_point.position.head<2>());
        weakest_conn.second_moment_of_area_covariance_accumulator += area * support_point.position.x() *
                                                                     support_point.position.y();
    }
}
        ...
        for (const auto &island : slice.islands)
        {
            ...
            for (size_t perimeter_idx : island.perimeters)
            {
                for (const ExtrusionEntity *e :
                     get_flat_entities(perimeter_region->perimeters().entities[perimeter_idx]))
                {
...
LocalSupports compute_local_supports(const std::vector<EnitityToCheck> &entities_to_check,
                                     const std::optional<Linesf> &previous_layer_boundary,
                                     const LD &prev_layer_ext_perim_lines, size_t slices_count, const Params &params)
{
    ...
    AABBTreeLines::LinesDistancer<Linef> prev_layer_boundary_distancer =
        (previous_layer_boundary ? AABBTreeLines::LinesDistancer<Linef>{*previous_layer_boundary}
    ...
    else
    {
        tbb::parallel_for(tbb::blocked_range<size_t>(0, entities_to_check.size()),
                          [&entities_to_check, &prev_layer_ext_perim_lines, &prev_layer_boundary_distancer,
                           &unstable_lines_per_slice, &ext_perim_lines_per_slice, &params](tbb::blocked_range<size_t> r)
                          {
                              for (size_t entity_idx = r.begin(); entity_idx < r.end(); ++entity_idx)
                              {
                                  const auto &e_to_check = entities_to_check[entity_idx];
                                  for (const auto &line :
                                       check_extrusion_entity_stability(e_to_check.e, e_to_check.region,
                                                                        prev_layer_ext_perim_lines,
                                                                        prev_layer_boundary_distancer, params))
                                  {
                                      if (line.support_point_generated.has_value())
                                      {
...
SliceMappings update_active_object_parts(const Layer *layer, const Params &params,
                                         const std::vector<SliceConnection> &precomputed_slice_connections,
                                         const SliceMappings &previous_slice_mappings,
                                         ActiveObjectParts &active_object_parts, PartialObjects &partial_objects)
{
    ...
    for (size_t slice_idx = 0; slice_idx < layer->lslices_ex.size(); ++slice_idx)
    {
        ...
        if (connection_to_below.area < EPSILON)
        { // new object part emerging
        ...
        else
        {
            ...
            // MERGE parts
            {
                ...
                for (const auto &link : slice.overlaps_below)
                {
                    size_t part_id = active_object_parts.get_flat_id(
                        previous_slice_mappings.index_to_object_part_mapping.at(link.slice_idx));
                    parts_ids.insert(part_id);
                    transfered_weakest_connection.add(
                        previous_slice_mappings.index_to_weakest_connection.at(link.slice_idx));
                }
            ...
            auto estimate_conn_strength = [bottom_z](const SliceConnection &conn)
            {
                ...
                { // connection is empty, does not exists. Return max strength so that it is not picked as the
                    // weakest connection.
                    return INFINITY;
                }
...
void reckon_global_supports(const tbb::concurrent_vector<ExtrusionLine> &external_perimeter_lines,
                            const coordf_t layer_bottom_z, const Params &params, ObjectPart &part,
                            SliceConnection &weakest_connection, SupportPoints &supp_points,
                            SupportGridFilter &supports_presence_grid)
{
    ...
    for (const ExtrusionLine &line : external_perimeter_lines)
    {
        ...
        else
        {
            ...
            auto [dist, nidx, nearest_point] = current_slice_lines_distancer.distance_from_lines_extra<false>(
                pivot_site_search_point);
            Vec3f position = to_3d(nearest_point, layer_bottom_z);
            auto [force, cause] = part.is_stable_while_extruding(weakest_connection, line, position, layer_bottom_z,
                                                                 params);
            if (force > 0)
            {
                SupportPoint support_point{cause, position, params.support_points_interface_radius};
                reckon_new_support_point(part, weakest_connection, supp_points, supports_presence_grid, support_point,
                                         true);
            }
        }
    }
...
std::tuple<SupportPoints, PartialObjects> check_stability(
    const PrintObject *po, const PrecomputedSliceConnections &precomputed_slices_connections,
    const PrintTryCancel &cancel_func, const Params &params)
{
    ...
    {
        if (layer_idx >= precomputed_slices_connections.size())
            continue;
        cancel_func();
        ...
        slice_mappings = update_active_object_parts(layer, params, precomputed_slices_connections[layer_idx],
                                                    slice_mappings, active_object_parts, partial_objects);
        ...
        LocalSupports local_supports{compute_local_supports(gather_entities_to_check(layer), prev_layer_boundary,
                                                            prev_layer_ext_perim_lines, layer->lslices_ex.size(),
                                                            params)};
        ...
        for (size_t slice_idx = 0; slice_idx < layer->lslices_ex.size(); ++slice_idx)
        {
            ...
            const tbb::concurrent_vector<ExtrusionLine> &external_perimeter_lines =
                local_supports.ext_perim_lines_per_slice[slice_idx];
            if (layer_idx > 1)
            {
                reckon_global_supports(external_perimeter_lines, bottom_z, params, part, weakest_conn, supp_points,
                                       supports_presence_grid);
            }

            current_layer_ext_perims_lines.insert(current_layer_ext_perims_lines.end(),
                                                  external_perimeter_lines.begin(), external_perimeter_lines.end());
        } // slice iterations
...
void debug_export(const SupportPoints &support_points, const PartialObjects &objects, std::string file_name)
{
    Slic3r::CNumericLocalesSetter locales_setter;
    {
        ...
        for (size_t i = 0; i < objects.size(); ++i)
        {
            ...
            fprintf(fp, "v %f %f %f  %f %f %f\n", objects[i].centroid(0), objects[i].centroid(1),
                    objects[i].centroid(2), color[0], color[1], color[2]);
        }
...
std::tuple<SupportPoints, PartialObjects> full_search(const PrintObject *po, const PrintTryCancel &cancel_func,
                                                      const Params &params)
{
...
void estimate_supports_malformations(SupportLayerPtrs &layers, float flow_width, const Params &params)
{
    ...
    for (SupportLayer *l : layers)
    {
        ...
        for (const ExtrusionEntity *extrusion : l->support_fills.flatten().entities)
        {
            ...
            ExtrusionProcessor::PropertiesEstimationConfig config{};
            config.flow_width = flow_width;
            auto annotated_points = ExtrusionProcessor::estimate_points_properties<false>(pol.points, prev_layer_lines,
                                                                                          config);
            ...
            for (size_t i = 0; i < annotated_points.size(); ++i)
            {
                ...
                line_out.curled_up_height = estimate_curled_up_height(middle_distance * sign,
                                                                      0.5 * (a.curvature + b.curvature), l->height,
        ...
        for (const ExtrusionLine &line : current_layer_lines)
        {
            ...
            {
                l->curled_lines.push_back(
                    CurledLine{Point::new_scale(line.a), Point::new_scale(line.b), line.curled_up_height});
            }
        }
        ...
        for (const ExtrusionLine &line : current_layer_lines)
        {
            if (line.curled_up_height > params.curling_tolerance_limit)
            {
                Vec3f color = value_to_rgbf(-EPSILON, l->height * params.max_curled_height_factor,
                                            line.curled_up_height);
                fprintf(debug_file, "v %f %f %f  %f %f %f\n", line.b[0], line.b[1], l->print_z, color[0], color[1],
                        color[2]);
            }
        }
        for (const ExtrusionLine &line : current_layer_lines)
...
void estimate_malformations(LayerPtrs &layers, const Params &params)
{
    ...
    {
        l->curled_lines.clear();
        std::vector<Linef> boundary_lines = l->lower_layer != nullptr ? to_unscaled_linesf(l->lower_layer->lslices)
                                                                      : std::vector<Linef>();
        ...
        for (const LayerRegion *layer_region : l->regions())
        {
            for (const ExtrusionEntity *extrusion : layer_region->perimeters().flatten().entities)
            {
                ...
                config.add_corners = true;
                config.flow_width = flow_width;
                auto annotated_points = ExtrusionProcessor::estimate_points_properties<false>(extrusion_pts,
                                                                                              prev_layer_lines, config);
                ...
                for (size_t i = 0; i < annotated_points.size(); ++i)
                {
                    ...
                    // correctify the distance sign using slice polygons
                    float sign = (prev_layer_boundary.distance_from_lines<true>(middle.cast<double>()) +
                                  0.5f * flow_width) < 0.0f
                                     ? -1.0f
                                     : 1.0f;

                    line_out.curled_up_height = estimate_curled_up_height(middle_distance * sign,
                                                                          0.5 * (a.curvature + b.curvature), l->height,
                                                                          flow_width, bottom_line.curled_up_height,
                                                                          params);
                                                              ...
                                                              PartialObjects &partial_objects)
{
    ...
    auto coord_fn = [&ext_supp_points](size_t idx, size_t dim)
    {
        return ext_supp_points[idx].position[dim];
    };
